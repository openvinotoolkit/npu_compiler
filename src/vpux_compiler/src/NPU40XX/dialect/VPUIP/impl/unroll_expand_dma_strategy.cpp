...
void UnrollExpandDMAStrategy::addPatterns(mlir::RewritePatternSet& patterns, Logger& log) const {
    ...
    patterns.add<vpux::VPUIP::arch40xx::SingleClusterExpandDMARewriter>(ctx, _dmaPortCount, log);
    patterns.add<vpux::VPUIP::arch40xx::MultiClusterExpandDMARewriter>(ctx, _dmaPortCount, log);
}
