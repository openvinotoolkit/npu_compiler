...
mlir::LogicalResult vpux::IE::ExperimentalDetectronROIFeatureExtractorOp::inferReturnTypeComponents(
        mlir::MLIRContext* ctx, std::optional<mlir::Location> optLoc, mlir::ValueShapeRange operands,
        mlir::DictionaryAttr attrs, mlir::OpaqueProperties prop, mlir::RegionRange,
        SmallVectorImpl<mlir::ShapedTypeComponents>& inferredReturnShapes) {
    const auto loc = optLoc.value_or(mlir::UnknownLoc::get(ctx));

    IE::ExperimentalDetectronROIFeatureExtractorOpAdaptor exp(operands, attrs, prop);
    ...
    inferredReturnShapes.emplace_back(outputShapeFeatures, inType.getElementType());
    inferredReturnShapes.emplace_back(outputShapeROI, inType.getElementType());
