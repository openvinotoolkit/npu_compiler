...
mlir::LogicalResult vpux::IE::FloorModOp::inferReturnTypeComponents(
    ...
    const auto in1Type = mlir::cast<vpux::NDTypeInterface>(floorMod.getInput1().getType());
    const auto in2Type = mlir::cast<vpux::NDTypeInterface>(floorMod.getInput2().getType());

    auto outShapeInfo = inferEltwiseOutputShapeInfo(ShapeInfo::fromNDType(in1Type), ShapeInfo::fromNDType(in2Type),
                                                    floorMod.getAutoBroadcast(), loc);
...
mlir::LogicalResult vpux::IE::FloorModOp::reifyResultShapes(mlir::OpBuilder& builder,
