...
public:
    GenericConverter(mlir::MLIRContext* ctx, Logger log, mlir::PatternBenefit benefit = 1)
...
class SwapMaxPoolWithActivation final : public IE::impl::SwapMaxPoolWithActivationBase<SwapMaxPoolWithActivation> {
public:
...
void vpux::IE::registerSwapMaxpoolWithActivation(RewriterRegistry& registry,
                                                 ArrayRef<mlir::PatternBenefit> benefitLevels, size_t index,
                                                 Logger log) {
    registry.registerRewriter<GenericConverter<IE::ReLUOp>>("generic-converter-relu-op", log, benefitLevels[index]);
}
