...
public:
    FoldConvStrideKernel(mlir::MLIRContext* ctx, Logger log, mlir::PatternBenefit benefit = 1)
...
mlir::LogicalResult FoldConvStrideKernel::matchAndRewrite(IE::ConvolutionOp convOp,
    ...
    if (!IE::isEligibleToFoldStrideKernel(inputType, outputType, kernelX, strideX, strideY, alignedInputChannel,
                                          alignedOutputChannel, padLeft, padRight, _log)) {
        return mlir::failure();
    }
...
public:
    AdjustConvShape(mlir::MLIRContext* ctx, Logger log, mlir::PatternBenefit benefit = 1)
            : mlir::OpRewritePattern<IE::ConvolutionOp>(ctx, benefit), _log(log) {
    }
...
mlir::LogicalResult AdjustConvShape::matchAndRewrite(IE::ConvolutionOp convOp, mlir::PatternRewriter& rewriter) const {
    ...
    if (padNum) {
        ...
        maybePaddedInput =
                rewriter.create<IE::ConcatOp>(appendLoc(convOp.getLoc(), "pad"), valueRange, Dims4D::Act::W.ind())
                        .getOutput();
    }
    auto inputShapeCastOp =
...
public:
    AdjustDWConvShape(mlir::MLIRContext* ctx, Logger log, mlir::PatternBenefit benefit = 1)
...
mlir::LogicalResult AdjustDWConvShape::matchAndRewrite(IE::GroupConvolutionOp origOp,
    ...
    // Reshape input
    auto inShapeCast = rewriter.create<IE::ShapeCastOp>(takeOpLoc(origOp, "in_reshape"), origOp.getInput(),
    ...
    auto outShapeCast = rewriter.create<IE::ShapeCastOp>(takeOpLoc(origOp, "out_reshape"), newGroupConv.getOutput(),
...
void vpux::IE::registerAdjustConvolutionShapeRewriters(RewriterRegistry& registry,
                                                       ArrayRef<mlir::PatternBenefit> benefitLevels, size_t index,
                                                       Logger log) {
    registry.registerRewriterSet("adjust-convolution-shape-set", [&registry, benefitLevels, index, log]() {
        registry.registerRewriter<FoldConvStrideKernel>("fold-conv-stride-kernel", log, benefitLevels[index]);
        registry.registerRewriter<AdjustConvShape>("adjust-conv-shape", log, benefitLevels[index + 1]);
        registry.registerRewriter<AdjustDWConvShape>("adjust-dw-conv-shape", log, benefitLevels[index + 1]);
        // Manually invoking this rewriter despite canonicalizer handling it. This is required for dynamic rewriter
        // implementation
        IE::registerConcatOpRewriters(registry, benefitLevels, index + 1);
    });
}
