...
vpux::VPURT::BarrierPagesSplitHandler::BarrierPagesSplitHandler(mlir::func::FuncOp func, BarrierInfo& barrierInfo,
    ...
    _fifoPerShaveEngineEnabled = config::isFifoPerShaveEngineEnabled(func);
}
...
vpux::VPURT::BarrierPagesSplitHandler::BarrierPagesSplitHandler(
    ...
    // Initialize data for SHV tasks with DMA
    for (auto shvTaskInd : shvTasksWithDma) {
        auto shvQueueIt = llvm::find_if(_taskQueueTypeMap, [&](const auto& item) {
            return llvm::find(item.second, shvTaskInd) != item.second.end();
        });
        VPUX_THROW_WHEN(shvQueueIt == _taskQueueTypeMap.end(), "Can not find task {0} in task queue map", shvTaskInd);
        ...
        _shvTasksWithDmaPerTile[tileIndex].insert(shvTaskInd);
    }
}
...
void vpux::VPURT::BarrierPagesSplitHandler::reconfigureAllowDepsChangeDuringInitialization(bool allowDepsChange) {
    _allowDepsChangeDuringInitialization = allowDepsChange;
}
...
void vpux::VPURT::BarrierPagesSplitHandler::initializePageSplitData() {
    _barrierInfo.buildTaskQueueTypeMap();
...
void vpux::VPURT::BarrierPagesSplitHandler::initializeForLegalization(bool allowDepsChange) {
    _allowDepsChangeDuringInitialization = allowDepsChange;
    initializePageSplitData();
    ensureBoundaryTasksHaveUpdateBarriers();
}

void vpux::VPURT::BarrierPagesSplitHandler::initializeTaskQueueTypeMap(mlir::func::FuncOp func) {
    _taskQueueTypeMap = VPURT::getTaskOpQueues(func, _barrierInfo);
}

void vpux::VPURT::BarrierPagesSplitHandler::findShvTasksWithDpuOrDma(mlir::func::FuncOp func) {
    auto module = func->getParentOfType<mlir::ModuleOp>();
    auto numClusters = config::getTileExecutor(module).getCount();
    _shvSubmitSyncDmaP0DdrByShvTask.clear();

    // Shave submitting DMA tasks on using compiler DMA FIFOs requires WLM enqueue logic
    // to be aware of implicit dependency from SHV task to DMA queue
    const bool isShvToDmaImplicitDep = (config::getArch(module) <= config::ArchKind::NPU50XX);
    ...
    for (auto& [queueType, taskVec] : _taskQueueTypeMap) {
        ...
        for (auto taskInd : taskVec) {
            ...
            if (isDpuShaveKernelType(taskOp)) {
                _shvTasksWithDpuPerTile[tileIndex].push_back(taskInd);
            }

            if (isShvToDmaImplicitDep) {
                auto swKernelOp = mlir::cast<VPUIP::SwKernelOp>(taskOp.getInnerTaskOp());
                if (isIoDmaSwKernel(swKernelOp)) {
                    _shvTasksWithDmaPerTile[tileIndex].insert(taskInd);
                }

                auto logicalIdxAttr = swKernelOp->getAttrOfType<mlir::IntegerAttr>(VPUIP::LOGICAL_TASK_INDEX_ATTR_NAME);
                if (logicalIdxAttr == nullptr) {
                    continue;
                }

                const auto logicalIdx = logicalIdxAttr.getValue().getSExtValue();
                auto syncDmaIt = _shvSubmitSyncDmaP0DdrByLogicalTask.find(logicalIdx);
                if (syncDmaIt != _shvSubmitSyncDmaP0DdrByLogicalTask.end()) {
                    _shvSubmitSyncDmaP0DdrByShvTask[taskInd] = syncDmaIt->second;
                }
            }
        }
    }
...
void vpux::VPURT::BarrierPagesSplitHandler::findShvSubmitSyncDmaTasks() {
    _shvSubmitSyncDmaP0DdrByLogicalTask.clear();
    ...
    for (size_t taskInd = 0; taskInd < _barrierInfo.getNumOfTasks(); taskInd++) {
        auto taskOp = _barrierInfo.getTaskOpAtIndex(taskInd);
        if (taskOp != nullptr && VPUIP::isShvGuardSyncDmaTask(taskOp)) {
            if (_barrierInfo.getTaskQueueType(taskInd) != enqueueDmaQueueType) {
                continue;
            }
            ...
            const auto logicalIdx = logicalIdxAttr.getValue().getSExtValue();
            auto existing = _shvSubmitSyncDmaP0DdrByLogicalTask.find(logicalIdx);
            if (existing == _shvSubmitSyncDmaP0DdrByLogicalTask.end()) {
                _shvSubmitSyncDmaP0DdrByLogicalTask[logicalIdx] = taskInd;
            } else {
                existing->second = std::min(existing->second, taskInd);
            }
        }
    }
}
...
void vpux::VPURT::BarrierPagesSplitHandler::initializeForEnqueue(mlir::func::FuncOp func) {
    initializePageSplitData();
    initializeTaskQueueTypeMap(func);

    findShvSubmitSyncDmaTasks();
    findShvTasksWithDpuOrDma(func);
...
void vpux::VPURT::BarrierPagesSplitHandler::readBarrierPageAssignmentFromIr() {
    ...
    for (size_t barInd = 0; barInd < _barrierInfo.getNumOfBarrierOps(); barInd++) {
        ...
        VPUX_THROW_UNLESS(static_cast<size_t>(pageInd) < _pageCount,
                          "Barrier {0} has invalid page assignment - {1}, page count is {2}", barInd, pageInd,
                          _pageCount);
...
void vpux::VPURT::BarrierPagesSplitHandler::initializeTaskToPageAssignment() {
    ...
    for (auto& [taskQueueType, taskVec] : _taskQueueTypeMap) {
        bool checkPageOfPrevTask =
                (_fifoPerShaveEngineEnabled || taskQueueType.type != config::ExecutorKind::SHAVE_ACT);
...
VPURT::TaskQueueType vpux::VPURT::BarrierPagesSplitHandler::getTaskQueueType(size_t taskInd) {
    ...
    auto taskOp = _barrierInfo.getTaskOpAtIndex(taskInd);
    return VPURT::getTaskQueueType(taskOp, false);
}

// Get Task queue type suitable for dependency analysis under assumption that tasks with same queue type
// are blocking each other as they use same HW FIFO.
// This method creates fake SHV IDs when FIFO per SHV engine is disabled as SHV tasks on same tile
// are not assumed to be dependent on each other and compiler has no control on actual SHV engine they will run on
VPURT::TaskQueueType vpux::VPURT::BarrierPagesSplitHandler::getTaskQueueTypeWithExplicitFifo(size_t taskInd) {
    auto taskQueueType = getTaskQueueType(taskInd);
    if (_fifoPerShaveEngineEnabled || taskQueueType.type != config::ExecutorKind::SHAVE_ACT) {
        return taskQueueType;
    }

    // For SHV queues create a fake SHV ID as compiler has no control on actual SHV engine in given
    // tile SHV will execute thus it cannot assume that SHVTask[n+1] execution is blocked by SHVTask[n]
    // Nevertheless logic expects that each task has some queue type so create a fake SHV ID based on task index
    // Since task index is unique queue related analysis will not assume two SHV tasks are dependent except
    // through barriers
    taskQueueType.id = taskInd;
    return taskQueueType;
}
...
void vpux::VPURT::BarrierPagesSplitHandler::updateBoundaryTasksDataForTask(size_t taskInd) {
    auto taskPage = _taskPageAssignment[taskInd];
    auto queueType = getTaskQueueTypeWithExplicitFifo(taskInd);
...
bool vpux::VPURT::BarrierPagesSplitHandler::verifyBoundaryTaskHasUpdateBarrier(size_t pageInd) {
    for (auto boundaryTask : getLastBoundaryTasksForPage(pageInd)) {
        ...
        if (taskUpdateBarsVec.empty()) {
            _log.trace("Boundary task {0} from page {1} does not have update barrier on next pages", boundaryTask,
                       pageInd);
            return false;
        }
    }
...
void vpux::VPURT::BarrierPagesSplitHandler::ensureBoundaryTasksHaveUpdateBarriers() {
    for (size_t pageInd = 0; pageInd < _pageCount; ++pageInd) {
        if (!_allowDepsChangeDuringInitialization) {
            VPUX_THROW_UNLESS(verifyBoundaryTaskHasUpdateBarrier(pageInd),
                              "Some boundary tasks from page {0} do not have update barrier on any of the following "
                              "pages and dependencies are not allowed to change",
                              pageInd);
        } else {
...
void vpux::VPURT::BarrierPagesSplitHandler::enforceBoundaryTaskHasUpdateBarrier(size_t pageInd) {
    for (auto boundaryTask : getLastBoundaryTasksForPage(pageInd)) {
        ...
        auto currentTaskOpt = getPrevTaskOnSameQueueIfQueueOrderEnabled(boundaryTask);
        while (taskUpdateBarsVec.empty() && currentTaskOpt.has_value() &&
            ...
            currentTaskOpt = getPrevTaskOnSameQueueIfQueueOrderEnabled(currentTaskOpt.value());
        }
...
void vpux::VPURT::BarrierPagesSplitHandler::initializeBoundaryTasksData() {
    ...
    for (size_t pageInd = 0; pageInd < _pageCount - 1; pageInd++) {
        ...
        if (pageBoundaryTasks.empty()) {
            VPUX_THROW_UNLESS(_allowDepsChangeDuringInitialization,
                              "Boundary tasks for page {0} not found and dependencies are not allowed to change",
                              pageInd);
            ...
            _firstAndLastBoundaryTaskForEachPagePerFifo[pageInd][getTaskQueueTypeWithExplicitFifo(lastTaskOnPage)] =
...
void vpux::VPURT::BarrierPagesSplitHandler::updateTaskPageAssignmentForShvInCaseOfNoDedicatedShvFifos() {
    VPUX_THROW_WHEN(_taskQueueTypeMap.empty(), "Task queue type map is empty");

    for (auto& [taskQueueType, taskVec] : _taskQueueTypeMap) {
        if (taskQueueType.type != config::ExecutorKind::SHAVE_ACT) {
            continue;
        }
        for (size_t i = 1; i < taskVec.size(); i++) {
            auto taskInd = taskVec[i];

            auto taskPage = _taskPageAssignment[taskInd];

            // Task page assignment cannot be smaller than previous task on same FIFO
            auto prevTaskInd = taskVec[i - 1];
            auto prevTaskPage = _taskPageAssignment[prevTaskInd];
            if (taskPage >= prevTaskPage) {
                continue;
            }
            _log.trace("Task {0}(page {1}) has smaller page assignment than previous task {2}(page {3}) on same "
                       "cluster {4}:{5}",
                       taskInd, taskPage, prevTaskInd, prevTaskPage, stringifyEnum(taskQueueType.type).data(),
                       taskQueueType.id);

            // Update task page assignment and make it depend on additional wait barrier
            // If 1 wait barrier requirement is not needed then just make taskInd depend also
            // on same waitBars as prevTaskInd
            auto prevTaskWaitBars = _barrierInfo.getWaitBarriers(prevTaskInd);
            auto taskWaitBars = _barrierInfo.getWaitBarriers(taskInd);

            VPUX_THROW_WHEN(prevTaskWaitBars.empty(),
                            "Previous task {0} has no wait barriers to depend on to fix page assignment", prevTaskInd);

            // Make all taskWaitBars producers also update prevTaskWaitBars
            for (auto taskWaitBar : taskWaitBars) {
                auto taskWaitBarProducers = _barrierInfo.getBarrierProducers(taskWaitBar);
                for (auto prevTaskWaitBar : prevTaskWaitBars) {
                    for (auto producer : taskWaitBarProducers) {
                        if (isDepFromTaskToBarrier(producer, prevTaskWaitBar)) {
                            continue;
                        }
                        _barrierInfo.addProducer(prevTaskWaitBar, producer);
                        _log.nest().trace("Add producer {0} to barrier {1}", producer, prevTaskWaitBar);
                    }
                    _barrierInfo.addConsumer(prevTaskWaitBar, taskInd);
                    _log.nest().trace("Add consumer {0} to barrier {1}", taskInd, prevTaskWaitBar);
                }
                _barrierInfo.removeConsumer(taskWaitBar, taskInd);
                _log.nest().trace("Remove consumer {0} from barrier {1}", taskInd, taskWaitBar);
            }

            _taskPageAssignment[taskInd] = prevTaskPage;
            auto taskOp = _barrierInfo.getTaskOpAtIndex(taskInd);
            taskOp.setWlmPage(prevTaskPage);
            _log.trace("Update task {0} page assignment to {1}", taskInd, prevTaskPage);
        }
    }
}
...
void vpux::VPURT::BarrierPagesSplitHandler::legalizeWaitBarrierDependency(size_t taskInd,
    ...
    do {
        ...
        taskWithWaitBarOnSamePageOpt =
                getPrevTaskOnSameQueueWithWaitBarIfQueueOrderSupported(taskWithWaitBarOnSamePageOpt.value());
    ...
    for (auto barProdTask : barProdTasks) {
        ...
        auto barProdQueueType = getTaskQueueTypeWithExplicitFifo(barProdTask);
        ...
        if (taskPage - barPage == 1 && barProdTaskPage == barPage) {
            ...
            if (pageBoundaryTasksOnSameFifoIt !=
                ...
                if (pageBoundaryTaskWaitBars.empty()) {
                    auto prevTaskOpt = getPrevTaskOnSameQueueWithWaitBarIfQueueOrderSupported(pageBoundaryTask);
...
void vpux::VPURT::BarrierPagesSplitHandler::legalizeUpdateBarrierDependency(size_t taskInd,
    ...
    do {
        ...
        pageBoundaryTaskWithWaitBarOpt =
                getPrevTaskOnSameQueueWithWaitBarIfQueueOrderSupported(pageBoundaryTaskWithWaitBarOpt.value());
...
void vpux::VPURT::BarrierPagesSplitHandler::legalizeNonAdjacentPageDependencies(
    ...
    for (auto taskInd : tasksWithDepsToLegalize) {
        size_t taskPage = _taskPageAssignment[taskInd];
        auto taskQueueType = getTaskQueueTypeWithExplicitFifo(taskInd);
...
std::optional<size_t> vpux::VPURT::BarrierPagesSplitHandler::getPrevTaskOnSameQueueIfQueueOrderEnabled(
        size_t taskInd) const {
    auto taskQueueType = _barrierInfo.getTaskQueueType(taskInd);
    if (_barrierInfo.isTaskQueueTypeInitialized(taskQueueType)) {
        return _barrierInfo.getPrevTaskOnQueue(taskInd, taskQueueType);
    }
    return std::nullopt;
}
std::optional<size_t> vpux::VPURT::BarrierPagesSplitHandler::getNextTaskOnSameQueueIfQueueOrderSupported(
        size_t taskInd) const {
    auto taskQueueType = _barrierInfo.getTaskQueueType(taskInd);
    if (_barrierInfo.isTaskQueueTypeInitialized(taskQueueType)) {
        return _barrierInfo.getNextTaskOnQueue(taskInd, taskQueueType);
    }
    return std::nullopt;
}

std::optional<size_t> vpux::VPURT::BarrierPagesSplitHandler::getPrevTaskOnSameQueueWithWaitBarIfQueueOrderSupported(
        size_t taskInd) const {
    auto taskQueueType = _barrierInfo.getTaskQueueType(taskInd);
    if (_barrierInfo.isTaskQueueTypeInitialized(taskQueueType)) {
        return _barrierInfo.getPrevTaskOnQueueWithWaitBar(taskInd, taskQueueType);
    }
    return std::nullopt;
}
...
void vpux::VPURT::BarrierPagesSplitHandler::legalizeDepsForBoundaryTasks(
    ...
    for (auto [taskSrc, taskDst] : boundaryTaskPairsMissingDepInBetween) {
        ...
        if (waitBars.empty()) {
            auto prevTaskOpt = getPrevTaskOnSameQueueWithWaitBarIfQueueOrderSupported(taskDst);
            if (prevTaskOpt.has_value()) {
...
std::optional<size_t> vpux::VPURT::BarrierPagesSplitHandler::getInsertionPointForDmaProgrammingBarriers(
    ...
    barProgDmaQueueType.type = config::ExecutorKind::DMA_NN;
...
BarrierInfo::TaskSet vpux::VPURT::BarrierPagesSplitHandler::getPageStartBarsDependingOnPageEndBars(
    ...
    const VPURT::TaskQueueType dmaP0ChDdrQueueType = {config::ExecutorKind::DMA_NN,
                                                      getDMAQueueIdEncoding(/*port*/ 0, VPUIP::DmaChannelType::DDR)};
...
void vpux::VPURT::BarrierPagesSplitHandler::getPageEndTasksAndBars(size_t pageInd, BarrierInfo::TaskSet& pageEndTasks,
    ...
    for (auto& [taskQueueType, firstLastTaskInd] : _firstAndLastBoundaryTaskForEachPagePerFifo[pageInd]) {
        ...
        if (taskWaitBars.empty()) {
            auto taskWithWaitBarOnSamePageOpt = getPrevTaskOnSameQueueWithWaitBarIfQueueOrderSupported(pageEndTask);
        ...
        while (currentTaskOpt.has_value() && currentTaskOpt.value() <= lastTask) {
            ...
            currentTaskOpt = getNextTaskOnSameQueueIfQueueOrderSupported(currentTaskOpt.value());
        }
    }
}
...
void vpux::VPURT::BarrierPagesSplitHandler::legalizeForDmaProgrammingBarriers() {
    ...
    for (size_t pageInd = barrierFifoDepthInPages - 1; pageInd < _pageCount - 1; pageInd++) {
        ...
        if (pageEndBars.contains(newStartBar)) {
            ...
            // If there are no page end barriers left, that means that only page end barrier was also page start
            // barrier. In that case pick some other barrier from the page to be used as end barrier
            if (pageEndAllBars.empty()) {
                for (size_t barInd = _firstBarrierInPage[pageInd + 1]; barInd-- > _firstBarrierInPage[pageInd];) {
                    if (barInd == newStartBar) {
                        continue;
                    }

                    const auto canUseAsEndBar = llvm::any_of(boundaryTasksToBeLegalized, [&](auto boundaryTask) {
                        return !_barrierInfo.getUpdateBarriers(boundaryTask).contains(barInd) &&
                               !isDepFromTaskToBarrier(boundaryTask, barInd);
                    });
                    if (!canUseAsEndBar) {
                        continue;
                    }

                    pageEndAllBars.insert(barInd);
                    _log.trace("Use barrier {0} from page {1} as end barrier for legalization", barInd, pageInd);
                    break;
                }
            }
            ...
            while (!boundaryTasksToBeLegalized.empty()) {
                ...
                for (auto startBarConsumerWhichCannotConsumeEndBar : startBarConsumersWhichCannotConsumeEndBar) {
                    auto nextTaskOnSameQueueOpt =
                            getNextTaskOnSameQueueIfQueueOrderSupported(startBarConsumerWhichCannotConsumeEndBar);
...
void vpux::VPURT::BarrierPagesSplitHandler::updateTaskPageAssignmentForQueue(size_t startTaskIndex, size_t newPageIndex,
    ...
    for (size_t taskInd = startTaskIndex; taskInd < _taskPageAssignment.size(); taskInd++) {
        if (getTaskQueueTypeWithExplicitFifo(taskInd) != queueType) {
...
std::pair<vpux::BarrierInfo::TaskSet, vpux::BarrierInfo::TaskSet> VPURT::BarrierPagesSplitHandler::getDummyDmaBars(
    ...
    if (dummyDmaProposedWaitBars.empty()) {
        auto taskWithWaitBarOnSamePageOpt = getPrevTaskOnSameQueueWithWaitBarIfQueueOrderSupported(boundaryTask);
        VPUX_THROW_UNLESS(taskWithWaitBarOnSamePageOpt.has_value(), "No wait barriers for task {0}(page {1})",
...
// Find suitable place in IR to insert dummy DMA for given page
size_t VPURT::BarrierPagesSplitHandler::getDummyDmaInsertionPoint(
    ...
    // Insert dummy DMA before last task in given page. It is guaranteed to be correct place
    // based on any wait barrier selected for dummy DMA. Also it will not break control block split restrictions
    // as last task in page can be a control block sync task and any tasks for this page should
    // be placed in IR before it
    auto insertAfter = _firstAndLastTaskPerPage[pageInd].value().second - 1;
    if (firstAndLastDmaOfTypePerPage[pageInd + 1].find(dmaQueueType) !=
        firstAndLastDmaOfTypePerPage[pageInd + 1].end()) {
        // In case there is a DMA of this type from next page insert dummy DMA before it
        insertAfter = std::min(insertAfter, firstAndLastDmaOfTypePerPage[pageInd + 1][dmaQueueType].first - 1);
    }

    return insertAfter;
}

// Get information for each page about first and last DMA of each type
SmallVector<mlir::DenseMap<VPURT::TaskQueueType, std::pair<size_t, size_t>>>
VPURT::BarrierPagesSplitHandler::getFirstAndLastDmaOfTypePerPage() {
    SmallVector<mlir::DenseMap<VPURT::TaskQueueType, std::pair<size_t, size_t>>> firstAndLastDmaOfTypePerPage(
            _pageCount);
    ...
    for (size_t taskInd = 0; taskInd < _taskPageAssignment.size(); taskInd++) {
        ...
        if (firstAndLastDmaOfTypePerPage[pageInd].find(taskQueueType) == firstAndLastDmaOfTypePerPage[pageInd].end()) {
            // First DMA of this type on this page
            firstAndLastDmaOfTypePerPage[pageInd][taskQueueType] = std::make_pair(taskInd, taskInd);
        } else {
            firstAndLastDmaOfTypePerPage[pageInd][taskQueueType].second = taskInd;
        }
    }
    return firstAndLastDmaOfTypePerPage;
}
...
VPURT::BarrierPagesSplitHandler::getAndLegalizeDummyDmaInsertionData() {
    ...
    auto firstAndLastDmaOfTypePerPage = getFirstAndLastDmaOfTypePerPage();
    ...
    for (size_t pageInd = 1; pageInd < _pageCount - 1; pageInd++) {
        ...
        for (auto [dmaQueueType, lastPageInd] : dmaQueueTypesAndLastPageInd) {
            ...
            std::optional<size_t> lastDmaTaskOnSameQueueInPageOpt =
                    (firstAndLastDmaOfTypePerPage[pageInd].find(dmaQueueType) !=
                     firstAndLastDmaOfTypePerPage[pageInd].end())
                            ? std::make_optional<size_t>(firstAndLastDmaOfTypePerPage[pageInd][dmaQueueType].second)
                            : std::nullopt;
            ...
            dummyDmaInsertionData.insertAfter =
                    getDummyDmaInsertionPoint(pageInd, dmaQueueType, firstAndLastDmaOfTypePerPage);
...
SmallVector<size_t> vpux::VPURT::BarrierPagesSplitHandler::getLastTasksOnFifoPerPageWithNoUpdBar() {
    ...
    for (size_t pageInd = 0; pageInd < _pageCount; pageInd++) {
        ...
        for (auto& [queueType, taskInd] : lastTaskPerTypePerPageWithNoUpdateBar[pageInd]) {
            ...
            auto syncPointTaskQueueType = getTaskQueueTypeWithExplicitFifo(syncPointTaskOpt.value());
            ...
            auto nextTaskIndOpt = getNextTaskOnSameQueueIfQueueOrderSupported(taskInd);
...
size_t vpux::VPURT::BarrierPagesSplitHandler::getNumberOfWorkloads(size_t taskInd) {
    ...
    return VPURT::getNumberOfWorkloads(taskOp);
}
...
VPURT::BarrierPagesSplitHandler::EnqueueDmaData vpux::VPURT::BarrierPagesSplitHandler::getDataForNewEnqueueDmaForTask(
    ...
    const VPURT::TaskQueueType dmaP0ChDdrQueueType = {config::ExecutorKind::DMA_NN,
                                                      getDMAQueueIdEncoding(/*port*/ 0, VPUIP::DmaChannelType::DDR)};
    auto taskPage = _taskPageAssignment[taskInd];
    auto lastDmaP0ChDdr = firstAndLastDmaOfTypePerPage[taskPage - 1][dmaP0ChDdrQueueType].second;
    auto waitBars = _barrierInfo.getWaitBarriers(lastDmaP0ChDdr);

    if (waitBars.empty()) {
        auto prevTaskOpt = getPrevTaskOnSameQueueWithWaitBarIfQueueOrderSupported(lastDmaP0ChDdr);
        if (prevTaskOpt.has_value()) {
...
// Check if DPU enqueue needs to be delayed after SHV with DPU task to prevent it from blocking DPU FIFO
// and allow SHV task to complete and submit DPU before DPU task starts executing
bool vpux::VPURT::BarrierPagesSplitHandler::isDpuEnqueueDelayNeededAfterShvWithDpu(
    ...
    auto isDepFromShvToDpu = [&](size_t shvTaskInd, size_t dpuTaskInd, size_t dpuTileInd) {
        if (shvTaskInd > dpuTaskInd) {
            return false;
        }

        if (isDepFromTaskAToTaskB(shvTaskInd, dpuTaskInd)) {
            return true;
        }

        // In case SHV submits DMA there is an implicit dependency from SHV to DMA tasks (Skip/Release descriptors) and
        // those DMA tasks may have dependency to DPU task
        auto shvWithDmaIt = _shvTasksWithDmaPerTile.find(dpuTileInd);
        if (shvWithDmaIt == _shvTasksWithDmaPerTile.end() || !shvWithDmaIt->second.contains(shvTaskInd)) {
            return false;
        }

        // TODO: Until the kernels with SHV submission of DMA support multi DMA engine,
        // the check is limited to first port only - E#213511
        const SmallVector<VPURT::TaskQueueType> dmaQueueTypes{
                {config::ExecutorKind::DMA_NN, getDMAQueueIdEncoding(/*port*/ 0, VPUIP::DmaChannelType::DDR)},
                {config::ExecutorKind::DMA_NN, getDMAQueueIdEncoding(/*port*/ 0, VPUIP::DmaChannelType::CMX)}};

        for (auto dmaQueueType : dmaQueueTypes) {
            auto nextDmaTaskOnQueueOpt = _barrierInfo.getNextTaskOnQueue(shvTaskInd, dmaQueueType);
            if (!nextDmaTaskOnQueueOpt.has_value()) {
                continue;
            }
            auto nextDmaTaskOnQueue = nextDmaTaskOnQueueOpt.value();
            if (isDepFromTaskAToTaskB(nextDmaTaskOnQueue, dpuTaskInd)) {
                _log.nest().trace("SHV task {0} submits DMA task {1} which has dependency to DPU task {2}", shvTaskInd,
                                  nextDmaTaskOnQueue, dpuTaskInd);
                return true;
            }
        }
        return false;
    ...
    if (!isDepFromShvToDpu(shvTaskInd, taskInd, queueType.id)) {
        return false;
    }
    ...
    for (auto dpuEnqBar : lastEnqDmaData.waitBars) {
        _log.nest().trace("Check dependency from SHV task {0} to enqueue DMA barrier {1}", shvTaskInd, dpuEnqBar);
        if (isDepFromTaskToBarrier(shvTaskInd, dpuEnqBar)) {
            // If SHV task is before enqueue DMA barrier then it is safe to enqueue
            _log.nest().trace("SHV task {0} is before enqueue DMA barrier {1} for task {2}", shvTaskInd, dpuEnqBar,
                              taskInd);
    ...
    return !isEnqDmaAfterShv;
}

// Create enqueue DMA for DPU task which needs to be delayed after SHV with DPU task so that
// it does not block DPU FIFO and executes after SHV task completes.
void vpux::VPURT::BarrierPagesSplitHandler::createInitialDelayDpuEnqueueAfterShvWithDpu(
    ...
    _log.nest().trace("Enqueue DMA for task {0} is before SHV task {1}. Need to delay enqueue", taskInd, shvTaskInd);
    ...
    VPUX_THROW_WHEN(shvTaskUpdBars.empty(), "SHV task {0} has no update barriers. Cannot delay enqueue for task {1}",
                    shvTaskInd, taskInd);
    ...
    if (taskWorkloadStartIdx == lastEnqDmaData.startTaskIdx) {
        if (!wasDpuAlreadyDelayed || lastEnqDmaData.pageInd < getBarrierPage(newEnqDmaBar)) {
        ...
        _log.nest().trace("Update last enqueue DMA for task {0} to use barriers {1}", taskInd, lastEnqDmaData.waitBars);
    } else {
        // Remove this task from previous one and create new one
        auto taskWorkloadsCount = taskWorkloadEndIdx - taskWorkloadStartIdx + 1;
...
// After identifying that DPU enqueue DMA needs to be delayed because of SHV task with DPU check
// if it introducing new enqueue DMA would not make previous now depend on SHV with DPU what would
// invalidate previous DPU enqueue. If this is the case update new DPU enqueue position to
// be after closest DMA on this queue which is the closest one before previous DPU. This way adding
// new enqueue DMA would not impact previous DPU dependencies.
void vpux::VPURT::BarrierPagesSplitHandler::updateDelayedDpuEnqPositionToNotImpactEnqOfPrevDpu(
        ...
        std::map<VPURT::TaskQueueType, SmallVector<EnqueueDmaData>>& enqDmaDataVecPerQueue) {
    auto& lastEnqDmaData = enqDmaDataVecPerQueue[queueType].back();
    auto prevTask = previousTaskIndOptPerQueue.lookup(queueType).value();

    auto closestDmaTaskIndOpt = _barrierInfo.getPrevTaskOnQueue(prevTask, enqueueDmaQueueType);
    while (closestDmaTaskIndOpt.has_value() && !isDepFromTaskAToTaskB(closestDmaTaskIndOpt.value(), prevTask)) {
        closestDmaTaskIndOpt = getPrevTaskOnSameQueueIfQueueOrderEnabled(closestDmaTaskIndOpt.value());
    }

    VPUX_THROW_WHEN(!closestDmaTaskIndOpt.has_value(), "Cannot be enqueued safely with dpuFromShave execution");

    auto closestDmaTaskInd = closestDmaTaskIndOpt.value();
    if (lastEnqDmaData.insertBefore <= closestDmaTaskInd) {
        _log.nest().trace("DMA {0} (page {1}) is the closest task for prev DPU {2} (page {3})", closestDmaTaskInd,
                          _taskPageAssignment[closestDmaTaskInd], prevTask, _taskPageAssignment[prevTask]);
        auto newInsertBefore = closestDmaTaskInd + 1;
        _log.nest().trace("Change enqueue insertion position from {0} to {1}", lastEnqDmaData.insertBefore,
                          newInsertBefore);

        lastEnqDmaData.insertBefore = newInsertBefore;

        auto closestDmaTaskPage = _taskPageAssignment[closestDmaTaskInd];
        if (closestDmaTaskPage > lastEnqDmaData.pageInd) {
            _log.nest().trace("Enqueue DMA changed page from {0} to {1}. Identified wait bars "
                              "are no longer valid",
                              lastEnqDmaData.pageInd, closestDmaTaskPage);
            lastEnqDmaData.pageInd = closestDmaTaskPage;

            BarrierInfo::TaskSet newWaitBars;
            // Since now Enqueue DMA is in different page its wait barrier needs to be modified
            // as it can no longer wait on barrier from previous page. For each already
            // identified Enqueue DMA wait barrier check if there is already dependency to new
            // Enqueue DMA position (which is after closestDmaTaskInd)
            bool isWaitBarrierNeeded = false;
            for (auto enqWaitBar : lastEnqDmaData.waitBars) {
                _log.nest().trace("Check if there is dependency from enqueue wait bar {0} to "
                                  "previous DMA {1}",
                                  enqWaitBar, closestDmaTaskInd);
                if (isDepFromBarrierToTask(enqWaitBar, closestDmaTaskInd)) {
                    continue;
                }
                isWaitBarrierNeeded = true;
                _log.nest(2).trace("No dependency. Need to find new wait bars");

                bool foundNewWaitBar = false;
                // Find a suitable barrier in new page that guarantees dependency to previous
                // wait barrier
                auto firstBarrierInNewPage = _firstBarrierInPage[closestDmaTaskPage];
                size_t barSearchLimit = ((closestDmaTaskPage + 1) < _pageCount)
                                                ? _firstBarrierInPage[closestDmaTaskPage + 1]
                                                : _barrierInfo.getNumOfBarrierOps();
                for (auto barInd = firstBarrierInNewPage; barInd < barSearchLimit; barInd++) {
                    _log.nest().trace("Check barrier {0} in new page {1} as candidate for "
                                      "enqueue DMA wait barrier",
                                      barInd, closestDmaTaskPage);

                    if (!isDepFromBarAToBarB(enqWaitBar, barInd)) {
                        _log.nest(2).trace("Barrier {0} is not suitable because there is no "
                                           "dependency from previous wait barrier {1}",
                                           barInd, enqWaitBar);
                        continue;
                    }

                    newWaitBars.insert(barInd);
                    _log.nest(2).trace("Barrier {0} is suitable. Add it to new wait bars", barInd);
                    foundNewWaitBar = true;
                    break;
                }
                VPUX_THROW_WHEN(!foundNewWaitBar,
                                "Cannot find suitable wait barrier in page {0} for previous "
                                "wait barrier {1}",
                                closestDmaTaskPage, enqWaitBar);
            }

            VPUX_THROW_WHEN(isWaitBarrierNeeded && newWaitBars.empty(),
                            "Cannot find suitable wait barrier for enqueue DMA after SHV with "
                            "DPU for task {0}",
                            taskInd);
            // After finding new wait barriers update insertion point
            auto latestProducerPosition = _barrierInfo.getBarriersLatestProducer(newWaitBars);
            if (latestProducerPosition >= lastEnqDmaData.insertBefore) {
                _log.nest().trace("Update enqueue DMA insertion point from {0} to {1} to be "
                                  "after latest producer of new wait barriers",
                                  lastEnqDmaData.insertBefore, latestProducerPosition + 1);
                lastEnqDmaData.insertBefore = latestProducerPosition + 1;
            }

            lastEnqDmaData.waitBars = to_small_vector(newWaitBars);
        }
    }
}
...
void vpux::VPURT::BarrierPagesSplitHandler::updateDpuEnqPositionToAccountForControlBlockSyncTask(
        size_t taskInd, VPURT::TaskQueueType queueType, const VPURT::TaskQueueType& enqueueDmaQueueType,
        std::map<VPURT::TaskQueueType, SmallVector<EnqueueDmaData>>& enqDmaDataVecPerQueue) {
    auto& lastEnqDmaData = enqDmaDataVecPerQueue[queueType].back();
    ...
    auto enqDmaWaitBar = *std::max_element(lastEnqDmaData.waitBars.begin(), lastEnqDmaData.waitBars.end());
    if (enqDmaWaitBar == (_firstBarrierInPage[lastEnqDmaData.pageInd + 1] - 1)) {
        ...
        lastEnqDmaData.waitBars = {*std::min_element(syncPointUpdateBars.begin(), syncPointUpdateBars.end())};
        lastEnqDmaData.insertBefore++;
        lastEnqDmaData.pageInd++;
        _log.nest().trace("Enqueue DMA for task {0} is to be inserted before sync point {1}. Move "
                          "it after barrier {2}(page {3})",
                          taskInd, maybeSyncTask, lastEnqDmaData.waitBars[0], lastEnqDmaData.pageInd);
    }
}

// For SHV to submit both DPU and DMA operations, the DPU FIFO must be empty when SHV runs.
// SyncDMA is the anchor of SHV-submitted DMA sequence, followed by SkipDMA chain.
// Enqueue DMA must be placed after this SyncDMA + SkipDMA chain to ensure DPU FIFO is drained
// before SHV task execution and to maintain correct ordering for legalization.
//
// Delay the enqueue in FIFO when:
//   1) enqueue path depends on SHV task that submits both DPU and DMA
//   2) enqueue insertion point is before or at SyncForShvSubmitDma
// Then move enqueue after SyncForShvSubmitDma + SkipDMA chain by updating insertBefore.
bool vpux::VPURT::BarrierPagesSplitHandler::updateDpuEnqPositionToAccountForShvDependentSyncDmas(
        size_t taskInd, VPURT::TaskQueueType queueType, const VPURT::TaskQueueType& enqueueDmaQueueType,
        EnqueueDmaData& lastEnqDmaData) {
    // This adjustment is only relevant for DPU enqueue, because SHV submit path can feed DPU FIFO
    if (queueType.type != config::ExecutorKind::DPU) {
        _log.nest().trace("Skip SHV submit-sync DMA enqueue adjustment for task {0}: queue {1}:{2} is not DPU", taskInd,
                          stringifyEnum(queueType.type).data(), queueType.id);
        return false;
    }

    auto shvWithDpuIt = _shvTasksWithDpuPerTile.find(queueType.id);
    auto shvWithDmaIt = _shvTasksWithDmaPerTile.find(queueType.id);
    if (shvWithDpuIt == _shvTasksWithDpuPerTile.end() || shvWithDmaIt == _shvTasksWithDmaPerTile.end()) {
        _log.nest().trace("Skip SHV submit-sync DMA enqueue adjustment for task {0}: no SHV DPU+DMA data on tile {1}",
                          taskInd, queueType.id);
        return false;
    }

    const size_t newDpuEnqPos = lastEnqDmaData.insertBefore;
    for (auto shvWithDmaAndDpuTaskIdx : shvWithDpuIt->second) {
        if (!shvWithDmaIt->second.contains(shvWithDmaAndDpuTaskIdx)) {
            // Skip SHV tasks that submit DPU but do not submit DMA: there is no
            // SHV submit SyncDMA anchor to move enqueue behind.
            continue;
        }

        auto syncTaskIt = _shvSubmitSyncDmaP0DdrByShvTask.find(shvWithDmaAndDpuTaskIdx);
        if (syncTaskIt == _shvSubmitSyncDmaP0DdrByShvTask.end()) {
            // Skip if no matching SyncDMA was found for this SHV task.
            // Without a concrete sync anchor there is no valid relocation target.
            continue;
        }
        const size_t shvSyncDmaP0DdrTaskIdx = syncTaskIt->second;
        if (newDpuEnqPos <= shvWithDmaAndDpuTaskIdx || newDpuEnqPos > shvSyncDmaP0DdrTaskIdx) {
            // No need for further checks as DPU enqueue is outside of SHV to SHVSyncDMA range:
            // - newDpuEnqPos <= shvWithDmaAndDpuTaskIdx: enqueue is before or at SHV, so by IR
            //   topological ordering it cannot depend on SHV (no barrier flows upward in IR).
            // - newDpuEnqPos > shvSyncDmaP0DdrTaskIdx: enqueue is already past SyncDMA, no move needed.
            _log.nest().trace("No enqueue move for task {0}: insert point {1} is outside SHV {2} to SyncDMA {3} range",
                              taskInd, newDpuEnqPos, shvWithDmaAndDpuTaskIdx, shvSyncDmaP0DdrTaskIdx);
            continue;
        }

        bool isDepFromShvToDpuEnq = false;
        auto closestDmaP0DdrOpt = _barrierInfo.getPrevTaskOnQueue(newDpuEnqPos, enqueueDmaQueueType);
        if (closestDmaP0DdrOpt.has_value() && closestDmaP0DdrOpt.value() > shvWithDmaAndDpuTaskIdx &&
            isDepFromTaskAToTaskB(shvWithDmaAndDpuTaskIdx, closestDmaP0DdrOpt.value())) {
            isDepFromShvToDpuEnq = true;
        }
        if (!isDepFromShvToDpuEnq) {
            for (auto enqWaitBar : lastEnqDmaData.waitBars) {
                if (isDepFromTaskToBarrier(shvWithDmaAndDpuTaskIdx, enqWaitBar)) {
                    isDepFromShvToDpuEnq = true;
                    break;
                }
            }
        ...
        if (!isDepFromShvToDpuEnq) {
            // Skip unrelated SHV tasks: enqueue movement is only legal/needed when
            // enqueue path depends on this SHV task (directly or via wait barriers).
            continue;
        }

        _log.nest().trace("Delay DPU enqueue for task {0}: insertBefore={1} depends on SHV task {2} and is "
                          "not after SyncForShvSubmitDma {3}. Move enqueue after Sync DMA.",
                          taskInd, lastEnqDmaData.insertBefore, shvWithDmaAndDpuTaskIdx, shvSyncDmaP0DdrTaskIdx);
        lastEnqDmaData.insertBefore = shvSyncDmaP0DdrTaskIdx + 1;
        return true;
    }

    _log.nest().trace("No SHV submit-sync DMA enqueue move for task {0}: no dependent SHV candidate requires it",
                      taskInd);
...
SmallVector<VPURT::BarrierPagesSplitHandler::EnqueueDmaData> vpux::VPURT::BarrierPagesSplitHandler::getEnqueueDmaData(
    ...
    // Get latest start barrier producer and set enqueue DMA after it.
    // getBarrierLatestProducer returns a global task index that may belong to any queue.
    auto startBarLatestProducer = _barrierInfo.getBarrierLatestProducer(_startBarrierIndex.value());
    earliestEnqDma.insertBefore = startBarLatestProducer + 1;
    ...
    for (size_t taskInd = 0; taskInd < _barrierInfo.getNumOfTasks(); taskInd++) {
        ...
        if (dpuEnqCheckForShvPerQueue[queueType]) {
            ...
            for (size_t shvTasksWithDpuVecInd = shvTasksWithDpuVecStartIndPerQueue[queueType];
                ...
                if (!isDpuEnqueueDelayNeededAfterShvWithDpu(taskInd, queueType, shvTasksWithDpuVecStartIndPerQueue,
                                                            shvTasksWithDpuVecInd, enqDmaDataVecPerQueue)) {
                    continue;
                }

                createInitialDelayDpuEnqueueAfterShvWithDpu(taskInd, queueType, isDpuDelayedAfterShv,
                                                            taskWorkloadStartIdx, taskWorkloadEndIdx,
                                                            shvTasksWithDpuVecInd, enqDmaDataVecPerQueue);

                isDpuDelayedAfterShv = true;

                if (previousTaskIndOptPerQueue[queueType].has_value()) {
                    updateDelayedDpuEnqPositionToNotImpactEnqOfPrevDpu(
                            taskInd, queueType, enqueueDmaQueueType, previousTaskIndOptPerQueue, enqDmaDataVecPerQueue);
                }
                updateDpuEnqPositionToAccountForShvDependentSyncDmas(taskInd, queueType, enqueueDmaQueueType,
                                                                     enqDmaDataVecPerQueue[queueType].back());
                updateDpuEnqPositionToAccountForControlBlockSyncTask(taskInd, queueType, enqueueDmaQueueType,
                                                                     enqDmaDataVecPerQueue);
            }
        }
    ...
    // Before returning enqueue data make sure that enqueue data is ordered by following factors:
    // - insertBefore
    // - pageInd
    // - queueType
    // - startTaskIdx
    // Enqueue insertion will follow this order and this way generated enqueue DMAs will have always increasing
    // pageInd and startTaskIdx which is required for correct functioning of subsequenet passes and runtime.
    std::sort(enqDmaDataVec.begin(), enqDmaDataVec.end(), [](const EnqueueDmaData& a, const EnqueueDmaData& b) {
        if (a.insertBefore != b.insertBefore) {
            return a.insertBefore < b.insertBefore;
        }
        if (a.pageInd != b.pageInd) {
            return a.pageInd < b.pageInd;
        }
        if (a.queueType != b.queueType) {
            return a.queueType < b.queueType;
        }
        return a.startTaskIdx < b.startTaskIdx;
    });

    return enqDmaDataVec;
}

// For each task retrieve enqueue barrier index. Take into account page split
// and in each page identify enqueue barrier for tasks in next pages
//
// Algorithm for identifying enqueue barrier in each page:
// - Pick earliest barrier in Page
//   - For Page0 pick next barrier as earliest barrier is start barrier which is used
//     to enqueue Page0 SHV and DPU tasks
// - Make sure this barrier is consumed by all boundary tasks from that Page. This way
//   it will be guaranteed that when consumption event for this barrier occurs all barriers
//   from this Page have also been consumed. AssignPhysicalBarriers pass make sure that this barrier
//   has largest PID index thus runtime will handle interrupt event for this barrier as last one.
//   This way other barriers from this page will be reprogrammed by runtime earlier
//
// Algorithm for assigning enqueue barriers for each task:
// - Page0 DMAs and non-boundary DMAs in Page1 - enqueue at BOOTSTRAP
// - Page0 DPUs/SHVs and non-boundary DPUs/SHVs in Page1 - enqueue at start barrier
// - Boundary tasks in Page1 - enqueue at Page0 enqueue barrier
// - Non-boundary tasks in PageN (N>=2) - enqueue at PageN-2 enqueue barrier
// - Boundary tasks in PageN (N>=2) - enqueue at PageN-1 enqueue barrier
//   Boundary tasks are enqueued at later point because they update barriers from next page:
//   PageN boundary tasks wait on PageN barrier but updates barriers from PageN+1 thus to make sure
//   that all barriers got reprogrammed it can be enqueued at PageN-1 enqueue barrier
//
// Such algorithm for enqueue barrier does not need to analyze if tasks HW FIFOs are not overflowing
// because in each Page all tasks are enqueued at the same barrier and since 2 consecutive pages can be alive
// at the same time there will be max of 2 enqueued descriptors per HW queue at any moment, thus DMA SW FIFO
// of depth 64 will not be overflown. For DPU and SHV HW FIFOs which have depth of 256 overflowing would depend
// on how enqueue for range of tasks has been split based on execution groups (unroll-enqueue-ops).
// Since execution group contains up to 32 tasks there would need to be 32 x 256 = 8192 DPUs/SHVs
// in 2 consecutive pages to overflow SHV/DPU HW FIFO. Such scenario is highly unlikely to happen in practice especially
// that there is always a barrier between consecutive DPUs/SHVs thus page size barrier limit will not allow
// to have so many DPUs/SHVs in 2 consecutive pages.
// TODO: Analyze possible mitigation for this scenario if there would not be a barrier between DPU or SHVs.
// Example would be to limit size of page based on DPU/SHV tasks
SmallVector<std::optional<size_t>> vpux::VPURT::BarrierPagesSplitHandler::getAndLegalizeEnqueueBarrierData() {
    VPUX_THROW_UNLESS(_startBarrierIndex.has_value(), "Start barrier index is not set");

    SmallVector<size_t> enqBarrierPerPageVec;
    enqBarrierPerPageVec.reserve(_pageCount);
    // For each page identify and legalize enqueue barrier for tasks in next pages
    _log.trace("Identify and legalize enqueue barriers per page");
    for (size_t pageInd = 2; pageInd < _pageCount; pageInd++) {
        // Tasks which use barriers from PageN to be enqueued at barrier from PageN-2
        auto barInd = _firstBarrierInPage[pageInd - 2];
        if (pageInd == 2 && barInd == _startBarrierIndex.value()) {
            // Cannot use start barrier as enqueue barrier for Page2 tasks as this barrier
            // is enqueue target for Page0 and Page1 DPU and SHV tasks. If start barrier from Page0
            // would be used as enqueue barrier for Page2 tasks then such barrier would be legalized
            // to be consumed by Page0 boundary tasks breaking assumptions for start barrier and making
            // it impossible to schedule DPU/SHV tasks from Page0 on start barrier
            barInd++;
            VPUX_THROW_WHEN(getBarrierPage(barInd) != 0,
                            "No available barrier to be used as enqueue barrier for Page 2 tasks");
        }
        enqBarrierPerPageVec.push_back(barInd);
        // Make sure this barrier is consumed by all boundary tasks from PageN-2
        auto boundaryTasks = getLastBoundaryTasksForPage(pageInd - 2);
        for (auto boundaryTask : boundaryTasks) {
            _barrierInfo.addConsumer(barInd, boundaryTask);
            _log.nest().trace("Add consumer {0}(page {1}) to enqueue barrier {2}(page {3})", boundaryTask,
                              _taskPageAssignment[boundaryTask], barInd, pageInd - 2);
        }
    }

    SmallVector<std::optional<size_t>> enqueueBarrierPerTaskVec;
    DenseMap<VPURT::TaskQueueType, std::optional<size_t>> prevTaskEnqBarPerQueue;
    for (size_t taskInd = 0; taskInd < _barrierInfo.getNumOfTasks(); taskInd++) {
        auto taskQueueType = getTaskQueueType(taskInd);
        auto taskPage = _taskPageAssignment[taskInd];
        _log.trace("Get enqueue barrier for task {0}(page {1}) on queue {2}:{3}", taskInd, taskPage,
                   stringifyEnum(taskQueueType.type), taskQueueType.id);

        // Check if task is a boundary task by checking if it has update barrier in next page
        bool isBoundaryTask = false;
        for (auto updateBar : _barrierInfo.getUpdateBarriers(taskInd)) {
            auto updateBarPage = getBarrierPage(updateBar);
            if (updateBarPage > taskPage) {
                isBoundaryTask = true;
                break;
            }
        }

        std::optional<size_t> enqBar = std::nullopt;
        if (taskPage == 0 || (taskPage == 1 && !isBoundaryTask)) {
            // Tasks from Page 0 and 1 can be enqueued right after startBarrier or at bootstrap
            // if this is DMA. Page 1 boundary tasks cannot be enqueued at bootstrap as they access
            // Page 2 barriers thus they have to be enqueued after Page0 barriers are reprogrammed
            if (taskQueueType.type == config::ExecutorKind::DMA_NN) {
                _log.nest().trace("DMA task can be enqueued at bootstrap");
            } else {
                _log.nest().trace("Non-DMA task can be enqueued after start barrier {0}", _startBarrierIndex.value());
                enqBar = _startBarrierIndex.value();
            }
        } else {
            // Boundary task should use enqueue barrier from previous page
            if (isBoundaryTask) {
                // Boundary task should use enqueue barrier from previous page
                enqBar = enqBarrierPerPageVec[taskPage - 1];
                _log.nest().trace("Boundary task. Enqueue barrier proposal - {0}(page {1})", enqBar, taskPage - 1);
            } else {
                // Non-boundary task can use enqueue barrier from two pages before
                enqBar = enqBarrierPerPageVec[taskPage - 2];
                _log.nest().trace("Non-boundary task. Enqueue barrier proposal - {0}(page {1})", enqBar, taskPage - 2);
            }
        }
        if (prevTaskEnqBarPerQueue.contains(taskQueueType) && prevTaskEnqBarPerQueue[taskQueueType].has_value()) {
            auto prevEnqBar = prevTaskEnqBarPerQueue[taskQueueType].value();
            if (enqBar.has_value() && enqBar.value() < prevEnqBar) {
                _log.nest().trace("Adjust enqueue barrier from {0} to previous {1} on same queue", enqBar.value(),
                                  prevEnqBar);
                enqBar = prevEnqBar;
            } else if (!enqBar.has_value()) {
                _log.nest().trace("Set enqueue barrier to previous {0} on same queue", prevEnqBar);
                enqBar = prevEnqBar;
            }
        }

        enqueueBarrierPerTaskVec.push_back(enqBar);
        prevTaskEnqBarPerQueue[taskQueueType] = enqBar;
    }

    return enqueueBarrierPerTaskVec;
}
...
// Verify on each queue that task[i] page is never smaller than task[i-1] page
void vpux::VPURT::BarrierPagesSplitHandler::verifyTaskPagesAreNeverDecreasingOnQueue() {
    _log.trace("Verifying task pages are never decreasing on given task queue");
    VPUX_THROW_WHEN(_taskQueueTypeMap.empty(), "Task queue type map is empty");
    ...
    for (auto& [taskQueueType, taskVec] : _taskQueueTypeMap) {
        for (size_t i = 1; i < taskVec.size(); i++) {
            ...
            auto prevTaskPage = _taskPageAssignment[prevTaskInd];
            VPUX_THROW_WHEN(
                    taskPage < prevTaskPage,
                    "Task {0}(page {1}) has smaller page assignment than previous task {2}(page {3}) on same FIFO",
                    taskInd, taskPage, prevTaskInd, prevTaskPage);
        }
    }
}
        ...
        if (pageInd == 0) {
            ...
            for (auto pageBar : barriersPerPage[pageInd]) {
                ...
                auto isDmaProducer = [&](auto task) {
                    return _barrierInfo.getTaskQueueType(task).type == config::ExecutorKind::DMA_NN;
                };
...
void vpux::VPURT::BarrierPagesSplitHandler::verifyEnqueueDmas(mlir::func::FuncOp func) {
    ...
    mlir::DenseMap<std::tuple<config::ExecutorKind, size_t, size_t>, SmallVector<size_t>> taskIndexesPerQueueType;
    func->walk([&](VPURT::TaskOp taskOp) {
    ...
    func->walk([&](VPURT::TaskOp taskOp) {
        ...
        if (lastTaskIndexesEnqueuedPerQueueType.count(execKindAndTileAndList) > 0) {
            VPUX_THROW_WHEN(
                    ...
                    config::stringifyExecutorKind(executorKind), tileIdx, listIdx);
        ...
        for (size_t perQueueTaskInd = startTaskIdx; perQueueTaskInd <= static_cast<size_t>(endTaskIdx);
            ...
            VPUX_THROW_UNLESS(taskIndexesPerQueueType.count(execKindAndTileAndList) > 0,
                              ...
                              config::stringifyExecutorKind(executorKind), pageInd);
...
void vpux::VPURT::BarrierPagesSplitHandler::verifyEnqueueOfDmas(mlir::func::FuncOp func) {
    ...
    for (const auto& [queueType, taskOps] : taskQueues) {
        ...
        _log.trace("Check queue type {0}:{1} with {2} tasks", config::stringifyExecutorKind(queueType.type),
                   queueType.id, taskOps.size());
        ...
        VPUX_THROW_UNLESS(pageInd <= 1, "First task in DMA queue {0}:{1} is not on Page0 or Page1, but on Page{2}.",
                          config::stringifyExecutorKind(queueType.type), queueType.id, pageInd);
