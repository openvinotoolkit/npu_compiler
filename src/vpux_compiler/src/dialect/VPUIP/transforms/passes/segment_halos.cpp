...
void updateNceOps(NCEClusterTaskOp nceOp, DenseMap<NCEClusterTaskOp, NceOpOutputs>& newOutputs, Logger log) {
    ...
    auto updatedNceOp = VPURT::wrapIntoTaskOp<NCEClusterTaskOp>(
            ...
            nceOp.getWeightTableAlpha(), nceOp.getWeightZeroPoints(), nceOp.getSprLookupTable(),
            nceOp.getPalletLookupTable(), nceOp.getParentInput(), nceOp.getParentInputSparsityMap(),
            nceOp.getParentInputStorageElementTable(), output, outSparsityMap, mlir::ValueRange(newOutputItis), output,
            outSparsityMap, nceOp.getProfilingData(), nceOp.getDynamicSequenceLength(), nceOp.getMaxPerXyBuff(),
            nceOp.getMinPerXyBuff(), nceOp.getMinMaxPerTensorBuff(), nceOp.getTaskType(), nceOp.getKernelSizeAttr(),
