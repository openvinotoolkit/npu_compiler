...
void PatchWeightsTablePass::safeRunOnFunc() {
    ...
    funcOp.walk([this](vpux::VPUIP::NCEClusterTaskOp nceOp) {
        // Don't need to run weights table patching if generated by shave at runtime
        if (nceOp->hasAttr(vpux::VPUIP::populateWeightTableWithShave)) {
...
unsigned PatchWeightsTablePass::getWeightTableSize(vpux::VPUIP::NCEClusterTaskOp nceOp,
                                                   SmallVector<uint32_t>& weightsPtrPerCluster, uint32_t baseOffset) {
    if (nceOp->hasAttr(vpux::ConstantFusing::constantsFused)) {
        ...
        //     Base                    Base
        //      Fused Const/WT          Weights
        //
        // We only need to patch these entries, as the rest are all constants
        // When weights are present, the number of WT entries would be base address of
        // weights minus base address of fused constants
        if (weightsPtr > 0) {
