...
mlir::LogicalResult vpux::VPU::LessOp::inferReturnTypes(mlir::MLIRContext* ctx, std::optional<mlir::Location> optLoc,
    ...
    return inferEltwiseReturnTypes(inferredReturnTypes, loc, less.getInput1(), less.getInput2(),
                                   less.getAutoBroadcast(), getBool8Type(ctx));
}
...
bool vpux::VPU::LessOp::isOperationSplitOverHeightCompatible(const vpux::TileInfo& outputTile) {
    return VPU::isEltwiseSWOpSplitOverHeightCompatible(getOperation(), outputTile.shape);
}

bool vpux::VPU::LessOp::isOperationSplitOverWidthCompatible(ShapeRef outputShape, ShapeRef /*offset*/,
                                                            ShapeRef /*axis*/) {
