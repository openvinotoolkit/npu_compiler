...
Logger getLogger() {
    return Logger("name-verifier", Logger::global().level());
}
...
bool isOpFromHostCompilePipeline(mlir::Operation* op) {
    return isOpFromAnyDialect<mlir::arith::ArithDialect, mlir::scf::SCFDialect, mlir::tensor::TensorDialect>(op);
}
...
mlir::LogicalResult verifyLocationsUniquenessImpl(mlir::Operation* op, StringRef passName) {
    ...
    op->walk([&](mlir::Operation* nestedOp) {
        // If we hit an operation from late dialects - likely wrong use of setupLocationVerifierPass
        // If we hit an operation the host comile pipeline - do nothing
        VPUX_THROW_WHEN(!isOpFromHostCompilePipeline(nestedOp) && isOpFromIgnoredDialect(nestedOp),
                        "Op '{0}' is from ignored dialect: {1}", nestedOp->getName(),
                        nestedOp->getDialect()->getNamespace());
...
LocationsVerificationMode vpux::getLocationsVerificationMode(
    ...
    return symbolizeLocationsVerificationMode(locationsVerificationMode.getValue());
}
...
mlir::LogicalResult vpux::verifyLocations(mlir::Operation* op, StringRef passName) {
    ...
    const auto mode = getLocationsVerificationMode(moduleOp);
    ...
    if (mode == LocationsVerificationMode::THOROUGH) {
        return verifyLocationsUniquenessFull(op, passName);
    }
    return verifyLocationsUniquenessFast(op, passName);
}
