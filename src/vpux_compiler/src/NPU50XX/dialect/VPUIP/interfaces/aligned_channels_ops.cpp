...
public:
    mlir::LogicalResult verifyChannels(mlir::Operation* op) const {
        if (!canBeExecutedOnNCE(op) && !isAlignmentBeneficialForSWOp(op)) {
    ...
    int64_t getInputChannelAlignment(mlir::Operation* op) const {
        ...
        if (mlir::isa<IE::ConvolutionOp>(op)) {
            auto canUseIDUAutopad =
                    VPU::inputCompatibleWithAutoPad(inputType) && config::hasAutoPaddingIDU(getModuleOp(op));
            if (canUseIDUAutopad || VPU::NCECompressConvolutionOp::isDeprecated(op)) {
                // In case the operation can use IDU autopad, avoid aligning the channels to 4, as this would lead to
                // NCECompressConvolution being used. Instead, the channels should be aligned to the general DPU
                // requirement (e.g. 16 channels), so that a later pass can decide whether IDU autopad should be enabled
                // for this particular case
                // Note: IDU autopad can support multiple values for input channels, rather than only 4, as
                // NCECompressConvolution does. For this reason, it is preferred to use IDU autopad whenever possible
...
private:
    ...
    static bool isAlignmentBeneficialForSWOp(mlir::Operation* op) {
        if (auto interpolateOp = mlir::dyn_cast<IE::InterpolateOp>(op)) {
            ...
            const auto outputShape = mlir::cast<vpux::NDTypeInterface>(interpolateOp.getOutput().getType()).getShape();
            if (inputShape.size() != 4) {
                return false;
            }

            if (outputShape[Dims4D::Act::H] > experimentalHWThreshold ||
                outputShape[Dims4D::Act::W] > experimentalHWThreshold) {
                return false;
            }

            const auto alignedChannels = static_cast<double>(
                    alignValUp(outputShape[Dims4D::Act::C], VPU::NCEInvariant::VPU_CHANNEL_ALIGNMENT));
            if (alignedChannels / static_cast<double>(outputShape[Dims4D::Act::C]) > experimentalChannelIncreaseRatio) {
                return false;
            }
            return inputShape[Dims4D::Act::N] == outputShape[Dims4D::Act::N] &&
        ...
        return VPU::NCEInvariant::isAlignmentBeneficial(op);
    }
};

}  // namespace

void vpux::VPUIP::arch50xx::registerAlignedChannelsOpInterfaces(mlir::DialectRegistry& registry) {
    registry.addExtension(+[](mlir::MLIRContext* ctx, IE::IEDialect*) {
        ...
        IE::ReduceMeanOp::attachInterface<AlignedChannelsOpModel<IE::ReduceMeanOp>>(*ctx);
