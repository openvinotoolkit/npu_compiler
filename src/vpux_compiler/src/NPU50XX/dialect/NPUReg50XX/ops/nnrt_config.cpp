...
void vpux::NPUReg50XX::NNrtConfigOp::serialize(elf::writer::BinaryDataSection<uint8_t>& binDataSection) {
...
size_t vpux::NPUReg50XX::NNrtConfigOp::getBinarySize(config::ArchKind) {
    return sizeof(npu40xx::nn_public::VpuNNRTConfig);
}

size_t vpux::NPUReg50XX::NNrtConfigOp::getAlignmentRequirements(config::ArchKind) {
    return alignof(nn_public::VpuNNRTConfig);
}

size_t getSymRefOffsetForReloc(NPUReg50XX::NNrtConfigOp op, mlir::SymbolRefAttr ref) {
...
std::vector<ELF::RelocationInfo> vpux::NPUReg50XX::NNrtConfigOp::getRelocationInfo(
        [[maybe_unused]] ELF::SymbolReferenceMap& symRefMap) {
