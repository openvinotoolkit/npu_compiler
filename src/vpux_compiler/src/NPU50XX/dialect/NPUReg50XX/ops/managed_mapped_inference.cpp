...
void vpux::NPUReg50XX::ManagedMappedInferenceOp::serialize(elf::writer::BinaryDataSection<uint8_t>& binDataSection) {
...
size_t vpux::NPUReg50XX::ManagedMappedInferenceOp::getBinarySize(config::ArchKind) {
    return sizeof(npu40xx::nn_public::VpuManagedMappedInference);
}

size_t vpux::NPUReg50XX::ManagedMappedInferenceOp::getAlignmentRequirements(config::ArchKind) {
    return alignof(nn_public::VpuManagedMappedInference);
}

namespace {
size_t getSymRefOffsetForReloc(NPUReg50XX::ManagedMappedInferenceOp op, mlir::SymbolRefAttr ref) {
...
std::vector<ELF::RelocationInfo> NPUReg50XX::ManagedMappedInferenceOp::getRelocationInfo(
...
void NPUReg50XX::ManagedMappedInferenceOp::setVersion(const elf::Version& version) {
    ...
    descriptor.write<NPUReg50XX::Fields::MMI_vpu_nnrt_api_ver>(serializedVersion);
    getProperties().setDescriptor(std::move(descriptor));
}

void vpux::NPUReg50XX::ManagedMappedInferenceOp::build(
