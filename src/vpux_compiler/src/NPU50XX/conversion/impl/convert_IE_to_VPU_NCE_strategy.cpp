...
void ConvertIEToVPUNCEStrategy::addPatterns(mlir::RewritePatternSet& patterns) const {
    ...
    patterns.add<MaxPoolToNCE>(ctx, _arch, _log);
    ...
    patterns.add<EltwiseToNCE<IE::AddOp>>(ctx, VPU::EltwiseType::ADD, _arch, _log);
    patterns.add<EltwiseToNCE<IE::SubtractOp>>(ctx, VPU::EltwiseType::SUBTRACT, _arch, _log);
    patterns.add<EltwiseToNCE<IE::MultiplyOp>>(ctx, VPU::EltwiseType::MULTIPLY, _arch, _log);
