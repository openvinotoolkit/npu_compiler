...
class UnrollDistributedOpsPass final :
        public VPUIP::arch37xx::impl::UnrollDistributedOpsBase<UnrollDistributedOpsPass> {
public:
    explicit UnrollDistributedOpsPass(Logger log) {
...
std::unique_ptr<mlir::Pass> vpux::VPUIP::arch37xx::createUnrollDistributedOpsPass(Logger log) {
    return std::make_unique<UnrollDistributedOpsPass>(log);
}
