...
// Reduce the size of the workload, so that a single output pixel is generated. The input & output channel dimensions
// can also be minimized if desired (e.g. if the invalid data generated by the new workload is discarded)
WorkloadComponents minimizeWorkloadSize(const WorkloadComponents& workload, ArrayRef<int64_t> kernelSize,
