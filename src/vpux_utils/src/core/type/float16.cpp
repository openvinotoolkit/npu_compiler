...
vpux::type::float16::float16(float value) {
...
std::string vpux::type::float16::to_string() const {
    return std::to_string(static_cast<float>(*this));
}

size_t vpux::type::float16::size() const {
    return sizeof(m_value);
}

vpux::type::float16::operator float() const {
    union {
        uint32_t i_val;
        float f_val;
    };
