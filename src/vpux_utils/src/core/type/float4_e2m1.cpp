...
static_assert(sizeof(vpux::type::float4_e2m1) == 1, "type f4e2m1 must be exactly 1 byte");
static_assert(std::is_trivially_constructible<vpux::type::float4_e2m1, vpux::type::float4_e2m1>::value,
              "should be trivially constructible");
static_assert(std::is_trivially_copyable<vpux::type::float4_e2m1>::value, "must be trivially copyable");
static_assert(std::is_trivially_destructible<vpux::type::float4_e2m1>::value, "must be trivially destructible");
...
uint8_t f32_to_f4e2m1_bits(float value) {
    ...
    */
    ...
    // For performance reasons the if else statements are not using direct float comparison
...
vpux::type::float4_e2m1::float4_e2m1(const float value): m_value(f32_to_f4e2m1_bits(value)) {};

vpux::type::float4_e2m1::operator float() const {
    return f4e2m1_to_f32_lut[m_value];
}

uint8_t vpux::type::float4_e2m1::to_bits() const {
    return m_value;
}
