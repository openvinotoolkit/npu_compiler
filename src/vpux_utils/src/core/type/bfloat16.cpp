...
std::vector<float> vpux::type::bfloat16::to_float_vector(const std::vector<vpux::type::bfloat16>& v_bf16) {
    std::vector<float> v_f32(v_bf16.begin(), v_bf16.end());
    return v_f32;
}

std::vector<vpux::type::bfloat16> vpux::type::bfloat16::from_float_vector(const std::vector<float>& v_f32) {
    std::vector<bfloat16> v_bf16;
    v_bf16.reserve(v_f32.size());
    for (float a : v_f32) {
        v_bf16.push_back(static_cast<vpux::type::bfloat16>(a));
    }
    return v_bf16;
}

std::string vpux::type::bfloat16::to_string() const {
    return std::to_string(static_cast<float>(*this));
}

size_t vpux::type::bfloat16::size() const {
...
vpux::type::bfloat16::operator float() const {
    uint32_t tmp = 0;
    uint32_t* ptmp = &tmp;
...
uint16_t vpux::type::bfloat16::to_bits() const {
    return m_value;
}
