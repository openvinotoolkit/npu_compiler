...
struct OptionParser<std::map<K, V>> final {
    static std::map<K, V> parse(std::string_view val) {
        ...
        splitAndApply(val_str, ',', [&](std::string_view item) {
            ...
            VPUX_THROW_UNLESS(kv_delim_pos != std::string::npos,
                              "Value '{0}' is not a valid map option, expected key:value pair", item);
...
struct OptionParser<std::chrono::duration<Rep, Period>> final {
    static std::chrono::duration<Rep, Period> parse(std::string_view val) {
        Rep count{};
        const auto result = std::from_chars(val.data(), val.data() + val.size(), count);
        ...
        VPUX_THROW("Can't parse '{0}' as time duration", val.data());
    }
};
...
std::shared_ptr<OptionValue> validateAndParseFromString(std::string_view val) {
    ...
    try {
        ...
        VPUX_THROW("Failed to parse '{0}' option : {1}", Opt::key().data(), e.what());
    }
}
...
private:
    ...
    Logger _log{"OptionsDesc", Logger::global().level()};
};

template <class Opt>
void OptionsDesc::add(std::optional<std::function<bool(std::string_view)>> customValueCheckerOpt) {
    VPUX_THROW_UNLESS(_impl.count(Opt::key().data()) == 0, "Option '{0}' was already registered", Opt::key().data());
    ...
    for (const auto& deprecatedKey : Opt::deprecatedKeys()) {
        VPUX_THROW_UNLESS(_deprecated.count(deprecatedKey.data()) == 0, "Option '{0}' was already registered",
                          deprecatedKey.data());
...
private:
    Logger _log{"Config", Logger::global().level()};
};
...
typename Opt::ValueType Config::get() const {
    ...
    _log.trace("Get value for the option '{0}'", Opt::key().data());

    const auto it = _impl.find(Opt::key());

    if (it == _impl.end()) {
        const std::optional<ValueType> optional = Opt::defaultValue();
        _log.trace("The option '{0}' was not set by user, try default value", Opt::key().data());
    ...
    VPUX_THROW_UNLESS(it->second != nullptr, "Got NULL OptionValue for : {0}", Opt::key().data());
...
#endif
    VPUX_THROW_UNLESS(optVal != nullptr, "Option '{0}' has wrong parsed type: expected '{1}', got '{2}'",
