...
module @Test attributes {config.compilationMode = #config.compilation_mode<DefaultHW>, config.platform = #config.platform<NPU4000>} {
    ...
    func.func @main(%arg0: memref<1x64x60x60xf32, @DDR>, %arg1: memref<1x64x60x60xf32, @DDR>) -> memref<1x64x60x60xf32, @DDR> {
        %cst = const.Declare memref<64x64x3x3xf16, {order = #NHWC}> = dense<0.000000e+00> : tensor<64x64x3x3xf32>, [#const.CastElemType<f16>, #const.Reorder<#NHWC>]
        %cst_0 = const.Declare memref<64x64x5x5xf16, {order = #NHWC}> = dense<0.000000e+00> : tensor<64x64x5x5xf32>, [#const.CastElemType<f16>, #const.Reorder<#NHWC>]
        %cst_1 = const.Declare memref<64x64x3x3xf16, {order = #NHWC}> = dense<0.000000e+00> : tensor<64x64x3x3xf32>, [#const.CastElemType<f16>, #const.Reorder<#NHWC>]
        ...
        %cst_4 = const.Declare memref<32x64x7x7xf16, {order = #NHWC}> = dense<0.000000e+00> : tensor<64x64x7x7xf32>, [#const.SubView<[0, 0, 0, 0], [32, 64, 7, 7]>, #const.CastElemType<f16>, #const.Reorder<#NHWC>]
        %cst_5 = const.Declare memref<32x64x7x7xf16, {order = #NHWC}> = dense<0.000000e+00> : tensor<64x64x7x7xf32>, [#const.SubView<[32, 0, 0, 0], [32, 64, 7, 7]>, #const.CastElemType<f16>, #const.Reorder<#NHWC>]
        %cst_6 = const.Declare memref<32x1x1x4xsi32> = dense<0> : tensor<32x1x1x4xsi32>
        %alloc = memref.alloc() : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>
        %alloc_7 = memref.alloc() : memref<1x64x225x16xf16, [@CMX_NN, 0]>
        %alloc_8 = memref.alloc() : memref<1x16x64x225xf16, {order = #NWCH}, [@CMX_NN, 0]>
        %alloc_9 = memref.alloc() : memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>
        %alloc_10 = memref.alloc() : memref<64x1x1x4xsi32, [@CMX_NN, 0]>
        %alloc_11 = memref.alloc() : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>
        %alloc_12 = memref.alloc() : memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>
        %alloc_13 = memref.alloc() : memref<32x1x1x4xsi32, [@CMX_NN, 0]>
        %alloc_14 = memref.alloc() : memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>
        %alloc_15 = memref.alloc() : memref<64x64x5x5xf16, {order = #NHWC}, [@CMX_NN, 0]>
        %alloc_16 = memref.alloc() : memref<64x1x1x4xsi32, [@CMX_NN, 0]>
        %alloc_17 = memref.alloc() : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>
        %alloc_18 = memref.alloc() : memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>
        ...
        %token_20, %bodyResults_21 = async.execute [%token] (%bodyResults as %arg2: !async.value<memref<1x64x225x16xf16, [@CMX_NN, 0]>>) -> !async.value<memref<1x16x64x225xf16, {order = #NWCH}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, "async-deps-index" = 1 : i64, cycleCost = 1 : i64} {
          %1 = VPUIP.ViewOp %arg2 : memref<1x64x225x16xf16, [@CMX_NN, 0]> to memref<1x16x64x225xf16, {order = #NHWC}, [@CMX_NN, 0]>
          %2 = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 4294967195 : i64, resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_permute_quantize, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, task_type = #VPUIP.nce_task_type<ELTWISE>}> input(%1 : memref<1x16x64x225xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%1 : memref<1x16x64x225xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_input(%1 : memref<1x16x64x225xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%alloc_8 : memref<1x16x64x225xf16, {order = #NWCH}, [@CMX_NN, 0]>) outputs(%alloc_8 : memref<1x16x64x225xf16, {order = #NWCH}, [@CMX_NN, 0]>) -> memref<1x16x64x225xf16, {order = #NWCH}, [@CMX_NN, 0]> variants : {
          ...
          async.yield %2 : memref<1x16x64x225xf16, {order = #NWCH}, [@CMX_NN, 0]>
        }
        %token_22, %bodyResults_23 = async.execute -> !async.value<memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, "async-deps-index" = 2 : i64, cycleCost = 2602 : i64} {
          %1 = VPUIP.NNDMA inputs(%cst : memref<64x64x3x3xf16, {order = #NHWC}>) outputs(%alloc_9 : memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>
          async.yield %1 : memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>
        }
        ...
        %token_26, %bodyResults_27 = async.execute [%token_20, %token_22, %token_24] (%bodyResults_21 as %arg2: !async.value<memref<1x16x64x225xf16, {order = #NWCH}, [@CMX_NN, 0]>>, %bodyResults_23 as %arg3: !async.value<memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>>, %bodyResults_25 as %arg4: !async.value<memref<64x1x1x4xsi32, [@CMX_NN, 0]>>) -> !async.value<memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, "async-deps-index" = 4 : i64, cycleCost = 75051 : i64} {
          %1 = VPUIP.ViewOp %arg2 : memref<1x16x64x225xf16, {order = #NWCH}, [@CMX_NN, 0]> to memref<1x64x225x16xf16, {order = #NHWC}, [@CMX_NN, 0]>
          %2 = VPUIP.ShapeCast {shape = [1, 64, 60, 60]} inputs(%1 : memref<1x64x225x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>
          %3 = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 75051 : i64, resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_zero_offset_weights_table, kernel_padding = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, kernel_size = [3, 3], kernel_strides = [1, 1], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, task_type = #VPUIP.nce_task_type<CONV>}> input(%2 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%arg3 : memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%arg4 : memref<64x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%2 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%alloc_11 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) outputs(%alloc_11 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]> variants : {
          ...
          async.yield %3 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>
        })";

    constexpr llvm::StringLiteral inputIRPart2 = R"(
        %token_28, %bodyResults_29 = async.execute -> !async.value<memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, "async-deps-index" = 5 : i64, cycleCost = 6075 : i64} {
          %1 = VPUIP.NNDMA inputs(%cst_4 : memref<32x64x7x7xf16, {order = #NHWC}>) outputs(%alloc_12 : memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>
          async.yield %1 : memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>
        }
        ...
        %token_32, %bodyResults_33 = async.execute [%token_26, %token_28, %token_30] (%bodyResults_27 as %arg2: !async.value<memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>>, %bodyResults_29 as %arg3: !async.value<memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>>, %bodyResults_31 as %arg4: !async.value<memref<32x1x1x4xsi32, [@CMX_NN, 0]>>) -> !async.value<memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, "async-deps-index" = 7 : i64, cycleCost = 202445 : i64} {
          %1 = VPUIP.SubView %alloc [0, 0, 0, 0] [1, 32, 60, 60] : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]> to memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>
          %2 = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 202445 : i64, resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_zero_offset_weights_table, kernel_padding = #VPU.Padding<left = 3 : i64, right = 3 : i64, top = 3 : i64, bottom = 3 : i64>, kernel_size = [7, 7], kernel_strides = [1, 1], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, task_type = #VPUIP.nce_task_type<CONV>}> input(%arg2 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%arg3 : memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%arg4 : memref<32x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%arg2 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%1 : memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>) outputs(%1 : memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>) -> memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]> variants : {
        ...
        %token_34, %bodyResults_35 = async.execute -> !async.value<memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, "async-deps-index" = 8 : i64, cycleCost = 6075 : i64} {
          %1 = VPUIP.NNDMA inputs(%cst_5 : memref<32x64x7x7xf16, {order = #NHWC}>) outputs(%alloc_14 : memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>
          async.yield %1 : memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>
        }
        %token_36, %bodyResults_37 = async.execute [%token_26, %token_30, %token_34] (%bodyResults_27 as %arg2: !async.value<memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>>, %bodyResults_31 as %arg3: !async.value<memref<32x1x1x4xsi32, [@CMX_NN, 0]>>, %bodyResults_35 as %arg4: !async.value<memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>>) -> !async.value<memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, "async-deps-index" = 9 : i64, cycleCost = 202445 : i64} {
          %1 = VPUIP.SubView %alloc [0, 32, 0, 0] [1, 32, 60, 60] : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]> to memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>
          %2 = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 202445 : i64, resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_zero_offset_weights_table, kernel_padding = #VPU.Padding<left = 3 : i64, right = 3 : i64, top = 3 : i64, bottom = 3 : i64>, kernel_size = [7, 7], kernel_strides = [1, 1], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, task_type = #VPUIP.nce_task_type<CONV>}> input(%arg2 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%arg4 : memref<32x64x7x7xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%arg3 : memref<32x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%arg2 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%1 : memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>) outputs(%1 : memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>) -> memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]> variants : {
            DPUTask {inEnd = [59, 59, 63], inStart = [0, 0, 0], mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [59, 59, 31], outStart = [0, 0, 0], pad = #VPU.Padding<left = 3 : i64, right = 3 : i64, top = 3 : i64, bottom = 3 : i64>}
          } PPE : {
            PPETask {ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.000000e+00 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>}
          }
        ...
        %token_38, %bodyResults_39 = async.execute -> !async.value<memref<64x64x5x5xf16, {order = #NHWC}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, "async-deps-index" = 10 : i64, cycleCost = 6187 : i64} {
          %1 = VPUIP.NNDMA inputs(%cst_0 : memref<64x64x5x5xf16, {order = #NHWC}>) outputs(%alloc_15 : memref<64x64x5x5xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<64x64x5x5xf16, {order = #NHWC}, [@CMX_NN, 0]>
          async.yield %1 : memref<64x64x5x5xf16, {order = #NHWC}, [@CMX_NN, 0]>
        }
        ...
        %token_42, %bodyResults_43 = async.execute [%token_32, %token_36, %token_38, %token_40] (%bodyResults_33 as %arg2: !async.value<memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>>, %bodyResults_37 as %arg3: !async.value<memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>>, %bodyResults_39 as %arg4: !async.value<memref<64x64x5x5xf16, {order = #NHWC}, [@CMX_NN, 0]>>, %bodyResults_41 as %arg5: !async.value<memref<64x1x1x4xsi32, [@CMX_NN, 0]>>) -> !async.value<memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, "async-deps-index" = 12 : i64, cycleCost = 193622 : i64} {
          %1 = VPUIP.ConcatView inputs(%arg2, %arg3 : memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>, memref<1x32x60x60xf16, {order = #NHWC, strides = [230400, 1, 3840, 64]}, [@CMX_NN, 0]>) outputs(%alloc : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>
          %2 = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 193622 : i64, resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_zero_offset_weights_table, kernel_padding = #VPU.Padding<left = 2 : i64, right = 2 : i64, top = 2 : i64, bottom = 2 : i64>, kernel_size = [5, 5], kernel_strides = [1, 1], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, task_type = #VPUIP.nce_task_type<CONV>}> input(%1 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%arg4 : memref<64x64x5x5xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%arg5 : memref<64x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%1 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%alloc_17 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) outputs(%alloc_17 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]> variants : {
          ...
          async.yield %2 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>
        }
        %token_44, %bodyResults_45 = async.execute -> !async.value<memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DMA_NN, "async-deps-index" = 13 : i64, cycleCost = 2602 : i64} {
          %1 = VPUIP.NNDMA inputs(%cst_1 : memref<64x64x3x3xf16, {order = #NHWC}>) outputs(%alloc_18 : memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>
          async.yield %1 : memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>
        }
        %token_46, %bodyResults_47 = async.execute [%token_24, %token_42, %token_44] (%bodyResults_25 as %arg2: !async.value<memref<64x1x1x4xsi32, [@CMX_NN, 0]>>, %bodyResults_43 as %arg3: !async.value<memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>>, %bodyResults_45 as %arg4: !async.value<memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>>) -> !async.value<memref<1x64x60x60xf32, [@CMX_NN, 0]>> attributes {VPUIP.executor = @DPU, "async-deps-index" = 14 : i64, cycleCost = 1 : i64} {
          %1 = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 89942 : i64, resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_superdense, is_zero_offset_weights_table, kernel_padding = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, kernel_size = [3, 3], kernel_strides = [1, 1], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, task_type = #VPUIP.nce_task_type<CONV>}> input(%arg3 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%arg4 : memref<64x64x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%arg2 : memref<64x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%arg3 : memref<1x64x60x60xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%alloc_19 : memref<1x64x60x60xf32, [@CMX_NN, 0]>) outputs(%alloc_19 : memref<1x64x60x60xf32, [@CMX_NN, 0]>) -> memref<1x64x60x60xf32, [@CMX_NN, 0]> variants : {
            DPUTask {inEnd = [59, 59, 63], inStart = [0, 0, 0], mpe_mode = #VPU.mpe_mode<CUBOID_8x16>, outEnd = [59, 59, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>}
          } PPE : {
            PPETask {ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.000000e+00 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>}
          }
