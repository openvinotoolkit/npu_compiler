...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --adapt-LLVM-funcs-for-shave --canonicalize --inline %s | FileCheck %s
...
// Inlining the implementation into the entry wrapper should produce alias
// scopes due to the noalias attributes on the buffer arguments. Loads and
// stores should be decorated with this information.
