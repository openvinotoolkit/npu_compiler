...
bytecode.func_section @function_section {
    bytecode.func @vm_buffer @string_section::@fn_vm_buffer @type_section::@fn_type {
        ...
        %loaded = bytecode.general_register 10
        %stored = bytecode.general_register 11
        %load_idx0 = bytecode.general_register 12
        %load_idx1 = bytecode.general_register 13
        ...
        bytecode.set_imm %stored, 42
        bytecode.set_imm %load_idx0, 1
        bytecode.set_imm %load_idx1, 2

        bytecode.buffer.create %buf, @type_section::@i64_type shape(%dim0, %dim1) strides(%stride0, %stride1)
        bytecode.buffer.store %buf, %stored indices(%load_idx0, %load_idx1)
        bytecode.buffer.load %loaded, %buf indices(%load_idx0, %load_idx1)
        ...
        bytecode.retv %src_dim, %view_dim, %loaded
    }
}
...
bytecode.type_section @type_section {
    ...
    bytecode.type @fn_type #bytecode.function_type<arguments = [], results = [@i64_type, @i64_type, @i64_type]>
}
}

// CHECK: Result[0] (i64): 3
// CHECK: Result[1] (i64): 2
// CHECK: Result[2] (i64): 42
