...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --dma-unrolling %s | FileCheck %s
...
func.func @UnrollS2DepthAndUpsamplingDMA() -> !OutputDistributed {
    ...
    //CHECK:    [[INPUT_BUFFER_0:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>
    //CHECK:    [[INPUT_BUFFER_1:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <2304> -> memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>
    //CHECK:    [[INPUT_BUFFER_2:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <4608> -> memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>
    //CHECK:    [[INPUT_BUFFER_3:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <6912> -> memref<1x4x12x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    //CHECK:    [[OUTPUT_BUFFER_0:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>
    //CHECK:    [[OUTPUT_BUFFER_1:%.+]] = VPURT.DeclareBuffer <CMX_NN> [1] <0> -> memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 1]>
    //CHECK:    [[OUTPUT_BUFFER_2:%.+]] = VPURT.DeclareBuffer <CMX_NN> [2] <0> -> memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 2]>
    //CHECK:    [[OUTPUT_BUFFER_3:%.+]] = VPURT.DeclareBuffer <CMX_NN> [3] <0> -> memref<1x4x12x48x!qElemType, {order = #NHWC}, [@CMX_NN, 3]>
    ...
    //CHECK:                inputs([[INPUT_BUFFER_0]] : memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>)
    //CHECK:                outputs([[OUTPUT_BUFFER_0]] : memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    //CHECK:                inputs([[INPUT_BUFFER_1]] : memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>)
    //CHECK:                outputs([[OUTPUT_BUFFER_1]] : memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 1]>) -> memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 1]>

    //CHECK:    VPURT.Task waits([[BARRIER_0]] : !VPURT.Barrier) updates([[BARRIER_1]] : !VPURT.Barrier) {
    //CHECK:      VPUIP.SpaceToDepthDMA <{block_size = 2 : i64, dma_descriptor = #VPUIP.DMADescriptorAttr<numPlanes = 2 : i64, len = 1536 : i64, srcWidth = 192 : i64, srcStride = 384 : i64, srcPlaneStride = 192 : i64, dstWidth = 8 : i64, dstStride = 16 : i64, dstPlaneStride = 8 : i64>, mode = #IE.space_to_depth_mode<BLOCKS_FIRST>, port = 0 : i64}>
    //CHECK:                inputs([[INPUT_BUFFER_2]] : memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>)
    //CHECK:                outputs([[OUTPUT_BUFFER_2]] : memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 2]>) -> memref<1x4x16x48x!qElemType, {order = #NHWC}, [@CMX_NN, 2]>
    ...
    //CHECK:                inputs([[INPUT_BUFFER_3]] : memref<1x4x12x48x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>)
    //CHECK:                outputs([[OUTPUT_BUFFER_3]] : memref<1x4x12x48x!qElemType, {order = #NHWC}, [@CMX_NN, 3]>) -> memref<1x4x12x48x!qElemType, {order = #NHWC}, [@CMX_NN, 3]>
