...
func.func @PermuteCastLayoutCastOriginFold() -> memref<1x2xf32, {order = #CN}> {
    %cst = const.Declare memref<1x2xf32> = dense<[[1.0, 2.0]]> : memref<1x2xf32>
    %layout_cast = VPUIP.PermuteCast {dst_order = #CN, is_layout_cast, mem_perm = #CN} inputs(%cst : memref<1x2xf32>) -> memref<1x2xf32, {order = #CN}>
    return %layout_cast : memref<1x2xf32, {order = #CN}>
}

// CHECK: func.func @PermuteCastLayoutCastOriginFold() -> memref<1x2xf32, {order = #CN}> {
// CHECK:    [[CST:%.+]] = const.Declare memref<1x2xf32, {order = #CN}> = dense<{{\[\[}}1.000000e+00, 2.000000e+00]]> : memref<1x2xf32>, [#const.LayoutCast<#CN>]
// CHECK:    return [[CST]] : memref<1x2xf32, {order = #CN}>
// CHECK: }
...
// expected-error@+2 {{Compact input memref<1x3x16x16xf32> and non-compact output memref<1x3x16x16xf32, {order = affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, strides = [1536, 512, 16, 1]}> are inconsistent}}
