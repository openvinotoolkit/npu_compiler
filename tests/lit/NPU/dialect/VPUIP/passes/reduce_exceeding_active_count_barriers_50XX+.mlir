...
// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=%arch%" --reduce-exceeding-active-count-barriers="num-barriers=2 max-variant-count=2 share-wait-and-update-barriers=false" %s | FileCheck %s
...
func.func @ParallelUpdateBarriers() -> memref<1x16x1x1xf16, #NHWC, @DDR> {
    ...
    // CHECK: [[BAR4:%.*]] = VPURT.DeclareVirtualBarrier <{isFinalBarrier}> -> !VPURT.Barrier
    ...
    // CHECK: updates([[BAR4]] : !VPURT.Barrier) attributes {idx = 5 : i64}
}
...
func.func @MergeParallelOutputPathsFromSharedBarrier() -> memref<1x16x1x1xf16, #NHWC, @DDR> {
    ...
    // CHECK: VPURT.Task updates([[BAR4]] : !VPURT.Barrier) attributes {idx = 5 : i64}
}
...
func.func @LinearizeBarrierWithMultipleConsumers() -> memref<1x16x1x1xf16, #NHWC, @DDR> {
    ...
    // CHECK: VPURT.Task waits([[BAR0]] : !VPURT.Barrier) updates([[BAR1]] : !VPURT.Barrier) attributes {idx = 1 : i64}
    // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier) attributes {idx = 2 : i64}
    // CHECK: VPURT.Task updates([[BAR2]] : !VPURT.Barrier) attributes {idx = 3 : i64}
    // CHECK: VPURT.Task waits([[BAR2]] : !VPURT.Barrier) attributes {idx = 4 : i64}
    // CHECK: VPURT.Task updates([[BAR3]] : !VPURT.Barrier) attributes {idx = 5 : i64}
    // CHECK: VPURT.Task waits([[BAR3]] : !VPURT.Barrier) attributes {idx = 6 : i64}
    // CHECK: VPURT.Task updates([[BAR4]] : !VPURT.Barrier) attributes {idx = 7 : i64}
}
...
func.func @LinearizeBarrierWithMultipleConsumersAndProducers() -> memref<1x16x1x1xf16, #NHWC, @DDR> {
    ...
    // CHECK: VPURT.Task updates([[BAR3]] : !VPURT.Barrier) attributes {idx = 5 : i64}
    // CHECK: VPURT.Task waits([[BAR3]] : !VPURT.Barrier) attributes {idx = 6 : i64}
    // CHECK: VPURT.Task attributes {idx = 7 : i64}
    // CHECK: VPURT.Task attributes {idx = 8 : i64}
    // CHECK: VPURT.Task updates([[BAR4]] : !VPURT.Barrier) attributes {idx = 9 : i64}
}
...
func.func @LinearizeBarriersWithMultipleConsumersVariousOrder() -> memref<1x16x1x1xf16, #NHWC, @DDR> {
    ...
    // CHECK: VPURT.Task waits([[BAR0]] : !VPURT.Barrier) updates([[BAR1]] : !VPURT.Barrier) attributes {idx = 1 : i64}
    // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier) attributes {idx = 2 : i64}
    // CHECK: VPURT.Task attributes {idx = 3 : i64}
    // CHECK: VPURT.Task updates([[BAR2]] : !VPURT.Barrier) attributes {idx = 4 : i64}
...
func.func @LinearizeBarriersWithMultipleProducers() -> memref<1x16x1x1xf16, #NHWC, @DDR> {
    ...
    // CHECK: VPURT.Task attributes {idx = 0 : i64}

    // CHECK: VPURT.Task updates([[BAR0]] : !VPURT.Barrier) attributes {idx = 1 : i64}
    // CHECK: VPURT.Task waits([[BAR0]] : !VPURT.Barrier) attributes {idx = 2 : i64}
    // CHECK: VPURT.Task updates([[BAR1]] : !VPURT.Barrier) attributes {idx = 3 : i64}
    // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier) attributes {idx = 4 : i64}
    // CHECK: VPURT.Task updates([[BAR2]] : !VPURT.Barrier) attributes {idx = 5 : i64}
...
func.func @ExtremeLinearization() -> memref<1x16x1x1xf16, #NHWC, @DDR> {
    ...
    // CHECK: VPURT.Task attributes {idx = 1 : i64}
    // CHECK: VPURT.Task updates([[BAR0]] : !VPURT.Barrier) attributes {idx = 2 : i64}
...
func.func @ThreeOutputPathsFromSharedBarrier() -> memref<1x16x1x1xf16, #NHWC, @DDR> {
    ...
    // CHECK: VPURT.Task attributes {idx = 6 : i64}

    // CHECK: VPURT.Task updates([[BAR5]] : !VPURT.Barrier) attributes {idx = 7 : i64}
}
...
// CHECK-LABEL: @ReassignFinalBarrierOptimizeRedundantDepsWithoutLinearization
func.func @ReassignFinalBarrierOptimizeRedundantDepsWithoutLinearization() -> memref<1x16x1x1xf16, #NHWC, @DDR> {
    // This test removes redundant barriers before simulation of assigning HW barriers
    // It also reassigns final barrier attribute after optimization is done which can remove such attribute.
    // It also optimizes redundant final barrier dependencies for task on port 1.
    // Final barrier only needs to be updated by final task on port 0.
    ...
    //     bar0
    //      |
    //      1     last task on fifo 1
    //      |   \
    //     bar1 |
    //      |   |
    //      2   | last task on fifo 0
    //      |   /
    //     bar2
