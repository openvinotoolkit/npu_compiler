...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --mlir-print-elementsattrs-with-hex-if-larger=-1 --compute-se-base-ptrs %s | FileCheck %s
...
func.func @SETableMultiCluster(%input_data: !Input_DDR, %input_sm: !InputSM_DDR) -> !OutputDistributed {
    ...
    -> !OutputDistributed variants : {
        DPUTask {cluster_id = 0 : i64, outStart = [0, 0, 0], outEnd = [16, 3, 3], mpe_mode = #VPU.mpe_mode<VECTOR_FP16>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
    } PPE : {
    }
    ...
    %input_sparse_cmx = VPUIP.GroupSparseBuffer (%input_data_cmx, %input_sm_cmx, %input_se_cmx) <{
            seAttr = #VPU.SEInterpolate<mode = <NEAREST>, coordinate_transformation_mode = <ASYMMETRIC>, scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00],
                                 ...
                                 #VPU.SEInterpolate<mode = <NEAREST>, coordinate_transformation_mode = <ASYMMETRIC>, scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00],
                                                    nearest_mode = <FLOOR>, offsets = [0, 0, 0, 0], sizes = [1, 32, 6, 6]>>
    %input = VPUIP.Copy
        inputs(%input_sparse : !VPUIP.SparseBuffer<data = !Input_DDR, sparsity_map = !InputSM_DDR, storage_element_table = !InputSE_DDR, #VPU.SEInterpolate<mode = <NEAREST>, coordinate_transformation_mode = <ASYMMETRIC>, scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00], nearest_mode = <FLOOR>, offsets = [0, 0, 0, 0], sizes = [1, 32, 6, 6]>>)
        outputs(%input_sparse_cmx : !VPUIP.SparseBuffer<data = !InputDistributed, sparsity_map = !InputSMDistributed, storage_element_table = !InputSEDistributed, #VPU.SEInterpolate<mode = <NEAREST>, coordinate_transformation_mode = <ASYMMETRIC>, scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00], nearest_mode = <FLOOR>, offsets = [0, 0, 0, 0], sizes = [1, 32, 6, 6]>>)  -> !VPUIP.SparseBuffer<data = !InputDistributed, sparsity_map = !InputSMDistributed, storage_element_table = !InputSEDistributed, #VPU.SEInterpolate<mode = <NEAREST>, coordinate_transformation_mode = <ASYMMETRIC>, scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00], nearest_mode = <FLOOR>, offsets = [0, 0, 0, 0], sizes = [1, 32, 6, 6]>>
...
func.func @InterpolateOutputOffsets(%input_data: !Input_DDR, %input_sm: !InputSM_DDR) -> !OutputDistributed {
    ...
    %input_sparse_cmx = VPUIP.GroupSparseBuffer (%input_data_cmx, %input_sm_cmx, %input_se_cmx) <{
            seAttr = #VPU.SEInterpolate<mode = <NEAREST>, coordinate_transformation_mode = <ASYMMETRIC>, scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00],
                                 ...
                                 #VPU.SEInterpolate<mode = <NEAREST>, coordinate_transformation_mode = <ASYMMETRIC>, scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00],
                                                    nearest_mode = <FLOOR>, offsets = [0, 0, 1, 0], sizes = [1, 32, 4, 6]>>
    %input = VPUIP.Copy
        inputs(%input_sparse : !VPUIP.SparseBuffer<data = !Input_DDR, sparsity_map = !InputSM_DDR, storage_element_table = !InputSE_DDR, #VPU.SEInterpolate<mode = <NEAREST>, coordinate_transformation_mode = <ASYMMETRIC>, scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00], nearest_mode = <FLOOR>, offsets = [0, 0, 1, 0], sizes = [1, 32, 4, 6]>>)
        outputs(%input_sparse_cmx : !VPUIP.SparseBuffer<data = !InputDistributed, sparsity_map = !InputSMDistributed, storage_element_table = !InputSEDistributed, #VPU.SEInterpolate<mode = <NEAREST>, coordinate_transformation_mode = <ASYMMETRIC>, scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00], nearest_mode = <FLOOR>, offsets = [0, 0, 1, 0], sizes = [1, 32, 4, 6]>>)  -> !VPUIP.SparseBuffer<data = !InputDistributed, sparsity_map = !InputSMDistributed, storage_element_table = !InputSEDistributed, #VPU.SEInterpolate<mode = <NEAREST>, coordinate_transformation_mode = <ASYMMETRIC>, scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00], nearest_mode = <FLOOR>, offsets = [0, 0, 1, 0], sizes = [1, 32, 4, 6]>>
    ...
    %conv_out = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [1, 1], kernel_strides = [1, 1], task_type = #VPUIP.nce_task_type<CONV>}>
        ...
        outputs(%out_cmx : !OutputDistributed)
    -> !OutputDistributed variants : {
        DPUTask {cluster_id = 0 : i64, outStart = [0, 0, 0], outEnd = [16, 4, 6], mpe_mode = #VPU.mpe_mode<VECTOR_FP16>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
    } PPE : {
    }
...
func.func @InterpolateBilinearAlignCornersOutputOffsets(%input_data: !Input_DDR, %input_sm: !InputSM_DDR) -> !OutputDistributed {
    ...
    %input_sparse_cmx = VPUIP.GroupSparseBuffer (%input_data_cmx, %input_sm_cmx, %input_se_cmx) <{
            seAttr = #VPU.SEInterpolate<mode = <BILINEAR>, coordinate_transformation_mode = <ALIGN_CORNERS>, scale = [1.000000e+00, 1.000000e+00, 3.000000e+00, 3.000000e+00],
                                 ...
                                 #VPU.SEInterpolate<mode = <BILINEAR>, coordinate_transformation_mode = <ALIGN_CORNERS>, scale = [1.000000e+00, 1.000000e+00, 3.000000e+00, 3.000000e+00],
                                        offsets = [0, 0, 1, 1], sizes = [1, 16, 5, 5], initial_input_shape = [1, 16, 3, 3], initial_output_shape = [1, 16, 7, 7]>>
    %input = VPUIP.Copy
        inputs(%input_sparse : !VPUIP.SparseBuffer<data = !Input_DDR, sparsity_map = !InputSM_DDR, storage_element_table = !InputSE_DDR, #VPU.SEInterpolate<mode = <BILINEAR>, coordinate_transformation_mode = <ALIGN_CORNERS>, scale = [1.000000e+00, 1.000000e+00, 3.000000e+00, 3.000000e+00], offsets = [0, 0, 1, 1], sizes = [1, 16, 5, 5], initial_input_shape = [1, 16, 3, 3], initial_output_shape = [1, 16, 7, 7]>>)
        outputs(%input_sparse_cmx : !VPUIP.SparseBuffer<data = !InputDistributed, sparsity_map = !InputSMDistributed, storage_element_table = !InputSEDistributed, #VPU.SEInterpolate<mode = <BILINEAR>, coordinate_transformation_mode = <ALIGN_CORNERS>, scale = [1.000000e+00, 1.000000e+00, 3.000000e+00, 3.000000e+00], offsets = [0, 0, 1, 1], sizes = [1, 16, 5, 5], initial_input_shape = [1, 16, 3, 3], initial_output_shape = [1, 16, 7, 7]>>)  -> !VPUIP.SparseBuffer<data = !InputDistributed, sparsity_map = !InputSMDistributed, storage_element_table = !InputSEDistributed, #VPU.SEInterpolate<mode = <BILINEAR>, coordinate_transformation_mode = <ALIGN_CORNERS>, scale = [1.000000e+00, 1.000000e+00, 3.000000e+00, 3.000000e+00], offsets = [0, 0, 1, 1], sizes = [1, 16, 5, 5], initial_input_shape = [1, 16, 3, 3], initial_output_shape = [1, 16, 7, 7]>>
    ...
    %w_data, %w_sm = VPUIP.UngroupSparseBuffer(%weights)  {resultSegmentSizes = array<i32: 1, 1, 0>}
        -> !WeightsDistributed, !WeightsSMDistributed
    %out_cmx = VPURT.AllocDistributed -> !OutputDistributed
    %conv_out = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [3, 3], kernel_strides = [1, 1], task_type = #VPUIP.nce_task_type<CONV>}>
        input(%in_data : !InputDistributed)
        input_storage_element_table(%in_se : !InputSEDistributed)
        weights(%w_data : !WeightsDistributed)
        weights_sparsity_map(%w_sm : !WeightsSMDistributed)
