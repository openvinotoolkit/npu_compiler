...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --async-regions-outlining="async-region-outlining-min-ops-in-block=2" %s | FileCheck %s
...
module @AsyncRegionOutliningThroughDDR {
  ...
  func.func @main(%arg0: memref<1x32x256x256xf16, {order = #NHWC}, @DDR>, %arg1: memref<1x32x256x256xf16, {order = #NHWC}, @DDR>, %arg2: memref<1x32x256x128xf16, {order = #NHWC}, @DDR>) -> memref<1x32x256x128xf16, {order = #NHWC}, @DDR> {
    ...
    %token_8, %bodyResults_9 = async.execute [%token_6] (%bodyResults_7 as %arg3: !async.value<!DistributedBufferType1>) -> !async.value<!DistributedBufferType1> attributes {VPUIP.executor = @DPU, "async-deps-index" = 5 : i64} {
      %6 = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 22748 : i64, resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [1, 1], kernel_strides = [1, 1], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, task_type = #VPUIP.nce_task_type<AVEPOOL>}> input(%arg3 : !DistributedBufferType1) parent_input(%arg3 : !DistributedBufferType1) parent_output(%4 : !DistributedBufferType1) outputs(%4 : !DistributedBufferType1) -> !DistributedBufferType1 variants : {
...
//CHECK:       [[NCEOp:%.+]] = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 13984 : i64} <{eltwise_type = #VPU.eltwise_type<ADD>, is_inplace = true, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>,
...
//CHECK:       [[NCEOp:%.+]] = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 22748 : i64} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [1, 1], kernel_strides = [1, 1], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>,
