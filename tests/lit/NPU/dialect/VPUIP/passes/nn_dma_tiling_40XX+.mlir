...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --nn-dma-tiling %s | FileCheck %s
...
!Output_CMX = memref<1x32x64x64x1xf16, {order = #NCDHW}, [@CMX_NN, 0]>
...
func.func @DoNotSplit5DNNDMAWithLevel3Striding(%input: !Input_DDR, %output: !Output_CMX) -> !Output_CMX {
    ...
    VPURT.Task waits(%bar0 : !VPURT.Barrier) updates(%bar1 : !VPURT.Barrier) attributes {isTrailingSWLayer = false} {
        %2 = VPUIP.NNDMA inputs(%0 : memref<1x32x64x64x1xf16, {order = #NCDHW, strides = [2097152, 65536, 512, 2, 1]}, @DDR>) outputs(%1 : memref<1x32x64x64x1xf16, {order = #NCDHW}, [@CMX_NN, 0]>) -> memref<1x32x64x64x1xf16, {order = #NCDHW}, [@CMX_NN, 0]>
    }
    ...
    // CHECK:    [[OUTPUT_BUF:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <622080> -> memref<1x32x64x64x1xf16, {order = #NCDHW}, [@CMX_NN, 0]>

    // CHECK:      VPURT.Task
    // CHECK:      VPUIP.NNDMA
    // CHECK-SAME:   inputs([[INPUT_DDR_BUF0]] : memref<1x32x64x64x1xf16, {order = #NCDHW, strides = [2097152, 65536, 512, 2, 1]}, @DDR>)
    // CHECK-SAME:   outputs([[OUTPUT_BUF]] : memref<1x32x64x64x1xf16, {order = #NCDHW}, [@CMX_NN, 0]>)

    // CHECK:      return [[OUTPUT_BUF]] : memref<1x32x64x64x1xf16, {order = #NCDHW}, [@CMX_NN, 0]>
}
