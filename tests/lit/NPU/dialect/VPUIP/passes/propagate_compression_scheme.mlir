...
// CHECK:  func.func @SparseConvWeights([[ARG0:%.+]]: memref<1x16x64x64xf16, {order = #NHWC}>, [[ARG1:%.+]]: memref<1x32x64x64xf16, {order = #NHWC}>) -> memref<1x32x64x64xf16, {order = #NHWC}>
func.func @SparseConvWeights(%arg0: memref<1x16x64x64xf16, {order = #NHWC}>, %arg1: memref<1x32x64x64xf16, {order = #NHWC}>) -> memref<1x32x64x64xf16, {order = #NHWC}> {
    %input_cmx = memref.alloc() : memref<1x16x64x64xf16, {order = #NHWC}, @CMX_NN>
    %input = VPUIP.Copy inputs(%arg0 : memref<1x16x64x64xf16, {order = #NHWC}>)
                        outputs(%input_cmx : memref<1x16x64x64xf16, {order = #NHWC}, @CMX_NN>)
        -> memref<1x16x64x64xf16, {order = #NHWC}, @CMX_NN>
    ...
    %cst_weights_sparse = VPUIP.GroupSparseBuffer (%cst_weights, %cst_weights_sm) <{sparsity_compression = #VPUIP.SparsityCompressionAttr<axis = 0 : i64, numElems = dense<1> : tensor<32xi64>, alignment = 16 : i64>, is_weights}>
        -> !VPUIP.SparseBuffer<data=memref<32x16x3x3xf16, {order = #NHWC}>, sparsity_map=memref<32x1x1x256xi1>, is_weights, #VPUIP.SparsityCompressionAttr<axis = 0 : i64, numElems = dense<1> : tensor<32xi64>, alignment = 16 : i64>>
    ...
    %weights_sparse_cmx = VPUIP.GroupSparseBuffer (%weights_data_cmx, %weights_sm_cmx) <{sparsity_compression = #VPUIP.SparsityCompressionAttr<axis = 0 : i64, numElems = dense<1> : tensor<32xi64>, alignment = 16 : i64>, is_weights}>
        -> !VPUIP.SparseBuffer<data=memref<32x16x3x3xf16, {order = #NHWC}, @CMX_NN>, sparsity_map=memref<32x1x1x256xi1, @CMX_NN>, is_weights, #VPUIP.SparsityCompressionAttr<axis = 0 : i64, numElems = dense<1> : tensor<32xi64>, alignment = 16 : i64>>

    %weights = VPUIP.Copy inputs(%cst_weights_sparse : !VPUIP.SparseBuffer<data=memref<32x16x3x3xf16, {order = #NHWC}>, sparsity_map=memref<32x1x1x256xi1>, is_weights, #VPUIP.SparsityCompressionAttr<axis = 0 : i64, numElems = dense<1> : tensor<32xi64>, alignment = 16 : i64>>)
                          outputs(%weights_sparse_cmx : !VPUIP.SparseBuffer<data=memref<32x16x3x3xf16, {order = #NHWC}, @CMX_NN>, sparsity_map=memref<32x1x1x256xi1, @CMX_NN>, is_weights, #VPUIP.SparsityCompressionAttr<axis = 0 : i64, numElems = dense<1> : tensor<32xi64>, alignment = 16 : i64>>)
        -> !VPUIP.SparseBuffer<data=memref<32x16x3x3xf16, {order = #NHWC}, @CMX_NN>, sparsity_map=memref<32x1x1x256xi1, @CMX_NN>, is_weights, #VPUIP.SparsityCompressionAttr<axis = 0 : i64, numElems = dense<1> : tensor<32xi64>, alignment = 16 : i64>>
    ...
    %output_cmx = memref.alloc() : memref<1x32x64x64xf16, {order = #NHWC}, @CMX_NN>
    %conv_out = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{
        ...
        input(%input : memref<1x16x64x64xf16, {order = #NHWC}, @CMX_NN>)
        weights(%weights_data : memref<32x16x3x3xf16, {order = #NHWC}, @CMX_NN>)
        weights_sparsity_map(%weights_sm : memref<32x1x1x256xi1, @CMX_NN>)
        parent_input(%input : memref<1x16x64x64xf16, {order = #NHWC}, @CMX_NN>)
        parent_output(%output_cmx : memref<1x32x64x64xf16, {order = #NHWC}, @CMX_NN>)
        outputs(%output_cmx : memref<1x32x64x64xf16, {order = #NHWC}, @CMX_NN>)
            -> memref<1x32x64x64xf16, {order = #NHWC}, @CMX_NN>
        variants :  {
    ...
    %output = VPUIP.Copy inputs(%conv_out : memref<1x32x64x64xf16, {order = #NHWC}, @CMX_NN>)
                    outputs(%arg1 : memref<1x32x64x64xf16, {order = #NHWC}>)
        -> memref<1x32x64x64xf16, {order = #NHWC}>

    return %output : memref<1x32x64x64xf16, {order = #NHWC}>
    ...
    // CHECK-SAME:      -> memref<32x16x3x3xf16, {order = #NHWC, sparsityCompression = #VPUIP.SparsityCompressionAttr<axis = 0 : i64, numElems = dense<1> : tensor<32xi64>, alignment = 16 : i64>}, @CMX_NN>,
...
!WeightsBufferDDR = memref<32x16x3x3xf16, {order = #NHWC}>
