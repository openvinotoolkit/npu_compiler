...
// RUN: vpux-opt --init-compiler="vpu-arch=%arch% allow-custom-values=true" --split-input-file --add-placeholder-fetch-dmas --canonicalize %s | FileCheck %s
...
module @LegalizeWithMultiTile attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  ...
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 1 : i64, list = 0 : i64, group = 0 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 1 : i64, list = 0 : i64, group = 1 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, group = 0 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, group = 1 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, group = 2 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 1 : i64, list = 0 : i64, group = 2 : i64>)
  // CHECK: }
...
module @PlaceholderForFetchWithMultipleBocks {
  ...
  func.func @main(%arg0: memref<1x3x224x224xf16, @DDR>, %arg1: memref<1x64x56x56xf16, @DDR>) -> memref<1x64x56x56xf16, @DDR> {
    ...
    %6 = VPURT.DeclareBuffer <CMX_NN> [0] <154560> -> memref<1x224x4x114x!qElemType, #NWCH, [@CMX_NN, 0]>
    ...
    %14 = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x224x3x114xf16, #NHWC, [@CMX_NN, 0]>
    %15 = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x224x3x114xf16, #NHWC, [@CMX_NN, 0]>
    %16 = VPURT.DeclareBuffer <CMX_NN> [0] <257600> -> memref<64x16x7x7x!qElemType, #NHWC, [@CMX_NN, 0]>
    ...
    %18 = VPURT.DeclareBuffer <CMX_NN> [0] <154560> -> memref<1x16x114x224x!qElemType2, #NHWC, [@CMX_NN, 0]>
    VPURT.Task updates(%0 : !VPURT.Barrier) {
      %19 = VPUIP.NCEClusterTask {is_permute_quantize, is_superdense, task_type = #VPUIP.nce_task_type<ELTWISE>} input(%15 : memref<1x224x3x114xf16, #NHWC, [@CMX_NN, 0]>) weights(%14 : memref<1x224x3x114xf16, #NHWC, [@CMX_NN, 0]>) parent_input(%13 : !VPUIP.DistributedBuffer<1x224x3x224xf16, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>) parent_output(%5 : !VPUIP.DistributedBuffer<1x224x4x224x!qElemType, #NWCH, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64, equal_memory_and_compute_view}>) outputs(%6 : memref<1x224x4x114x!qElemType, #NWCH, [@CMX_NN, 0]>) -> memref<1x224x4x114x!qElemType, #NWCH, [@CMX_NN, 0]> variants : {
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
    ...
    VPURT.Task waits(%2, %1 : !VPURT.Barrier, !VPURT.Barrier) updates(%3 : !VPURT.Barrier) attributes {"sync-task"} {
      %19 = VPUIP.NCEClusterTask {cm_sp_pattern = 7 : i64, input_channels_compression, kernel_padding = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>, kernel_size = [7, 7], kernel_strides = [2, 2], task_type = #VPUIP.nce_task_type<CONV>} input(%18 : memref<1x16x114x224x!qElemType2, #NHWC, [@CMX_NN, 0]>) weights(%16 : memref<64x16x7x7x!qElemType, #NHWC, [@CMX_NN, 0]>) parent_input(%17 : !VPUIP.DistributedBuffer<1x16x224x224x!qElemType2, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>) parent_output(%8 : !VPUIP.DistributedBuffer<1x64x112x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) outputs(%10 : memref<1x64x56x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>) -> memref<1x64x56x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]> variants : {
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [111, 55, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [111, 55, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [111, 55, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [111, 55, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>}
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, group = 3 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, group = 4 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, group = 5 : i64>)
  // CHECK: }
}
