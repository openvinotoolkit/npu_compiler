...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% compilation-mode=DefaultHW" --optimize-tile-op-as-nndma %s | FileCheck %s
...
func.func @Fuse1x1SwKernelTileIntoClusterCopy(%arg0: memref<1x96x1x1xf16, @DDR>)
    ...
    %0 = VPUIP.PermuteCast {dst_order = #NHWC, mem_perm = #NHWC} inputs(%arg0 : memref<1x96x1x1xf16, @DDR>) -> memref<1x96x1x1xf16, {order = #NHWC}, @DDR>
    %alloc = memref.alloc() : memref<1x96x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %1 = VPUIP.Copy inputs(%0 : memref<1x96x1x1xf16, {order = #NHWC}, @DDR>)
                    outputs(%alloc : memref<1x96x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>)
                        -> memref<1x96x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %alloc_0 = memref.alloc() : memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %results = VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_Tile
                    inputs(%1 as %arg3: memref<1x96x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>)
                    outputs(%alloc_0 as %arg4: memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>) on tile 0
                        -> memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>{
        VPUIP.SW.Kernel.run {attrs = [4, [1, 1, 64, 128]]}(%arg3, %arg4) : memref<1x96x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>, memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>
    }

    %alloc_1 = memref.alloc() : memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %results_3 = VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_Tile
                    inputs(%1 as %arg3: memref<1x96x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>)
                    outputs(%alloc_1 as %arg4: memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>) on tile 0
                        -> memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>{
        VPUIP.SW.Kernel.run {attrs = [4, [1, 1, 64, 128]]}(%arg3, %arg4) : memref<1x96x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>, memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>
    }

    %alloc_2 = memref.alloc() : memref<1x96x128x128xf16, {order = #NHWC}, @DDR>
    %2 = VPUIP.SubView %alloc_2 [0, 0, 0, 0] [1, 96, 64, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x64x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    %3 = VPUIP.Copy inputs(%results : memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>)
    ...
    %4 = VPUIP.SubView %alloc_2 [0, 0, 64, 0] [1, 96, 64, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x64x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    %5 = VPUIP.Copy inputs(%results_3 : memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>)
    ...
    %6 = VPUIP.ConcatView
                    ...
                    outputs(%alloc_2 : memref<1x96x128x128xf16, {order = #NHWC}, @DDR>)
                        -> memref<1x96x128x128xf16, {order = #NHWC}, @DDR>
    %7 = VPUIP.SubView %6 [0, 0, 0, 0] [1, 96, 43, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x43x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    ...
    %9 = VPUIP.Copy  inputs(%7 : memref<1x96x43x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>)
                    outputs(%8 : !OutputDistributed1)
                        -> !OutputDistributed1
    %10 = VPUIP.SubView %6 [0, 0, 43, 0] [1, 96, 43, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x43x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    ...
    %12 = VPUIP.Copy  inputs(%10 : memref<1x96x43x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>)
                    outputs(%11 : !OutputDistributed1)
                        -> !OutputDistributed1
    %13 = VPUIP.SubView %6 [0, 0, 86, 0] [1, 96, 42, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x42x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    ...
    // CHECK:       [[PERMUTECAST:%.+]] = VPUIP.PermuteCast {dst_order = #NHWC, mem_perm = #NHWC} inputs([[INPUT]] : memref<1x96x1x1xf16, @DDR>) -> memref<1x96x1x1xf16, {order = #NHWC}, @DDR>

    // CHECK:       [[ALLOC:%.+]] = memref.alloc() : memref<1x96x43x1xf16, {order = #NHWC}, @DDR>
    // CHECK:       [[PERAXISTILEDMA:%.+]] = VPUIP.PerAxisTileDMA <{axis = 2 : i64, tiles = 43 : i64}>
    // CHECK-SAME:          inputs([[PERMUTECAST]] : memref<1x96x1x1xf16, {order = #NHWC}, @DDR>)
    // CHECK-SAME:          outputs([[ALLOC]] : memref<1x96x43x1xf16, {order = #NHWC}, @DDR>) -> memref<1x96x43x1xf16, {order = #NHWC}, @DDR>

    // CHECK:       [[ALLOC_0:%.+]] = memref.alloc() : memref<1x96x43x1xf16, {order = #NHWC}, @DDR>
    // CHECK:       [[PERAXISTILEDMA_0:%.+]] = VPUIP.PerAxisTileDMA <{axis = 2 : i64, tiles = 43 : i64}>
    // CHECK-SAME:          inputs([[PERMUTECAST]] : memref<1x96x1x1xf16, {order = #NHWC}, @DDR>)
    // CHECK-SAME:          outputs([[ALLOC_0]] : memref<1x96x43x1xf16, {order = #NHWC}, @DDR>) -> memref<1x96x43x1xf16, {order = #NHWC}, @DDR>

    // CHECK:       [[ALLOC_1:%.+]] = memref.alloc() : memref<1x96x42x1xf16, {order = #NHWC}, @DDR>
    // CHECK:       [[PERAXISTILEDMA_1:%.+]] = VPUIP.PerAxisTileDMA <{axis = 2 : i64, tiles = 42 : i64}>
    // CHECK-SAME:          inputs([[PERMUTECAST]] : memref<1x96x1x1xf16, {order = #NHWC}, @DDR>)
    // CHECK-SAME:          outputs([[ALLOC_1]] : memref<1x96x42x1xf16, {order = #NHWC}, @DDR>) -> memref<1x96x42x1xf16, {order = #NHWC}, @DDR>
    ...
    // CHECK-SAME:          inputs([[PERAXISTILEDMA]] : memref<1x96x43x1xf16, {order = #NHWC}, @DDR>)
    ...
    // CHECK-SAME:          inputs([[PERAXISTILEDMA_0]] : memref<1x96x43x1xf16, {order = #NHWC}, @DDR>)
    ...
    // CHECK-SAME:          inputs([[PERAXISTILEDMA_1]] : memref<1x96x42x1xf16, {order = #NHWC}, @DDR>)
...
func.func @Fuse2x2SwKernelTileIntoClusterCopy(%arg0: memref<1x96x2x2xf16, @DDR>)
    ...
    %alloc = memref.alloc() : memref<1x96x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %1 = VPUIP.Copy inputs(%0 : memref<1x96x2x2xf16, {order = #NHWC}, @DDR>)
                    outputs(%alloc : memref<1x96x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>)
                        -> memref<1x96x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %alloc_0 = memref.alloc() : memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %results = VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_Tile
                    inputs(%1 as %arg3: memref<1x96x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>)
                    outputs(%alloc_0 as %arg4: memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>) on tile 0
                        -> memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>{
        VPUIP.SW.Kernel.run {attrs = [4, [1, 1, 32, 64]]}(%arg3, %arg4) : memref<1x96x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>, memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>
    }
    ...
    %6 = VPUIP.ConcatView
                    ...
                    outputs(%alloc_2 : memref<1x96x128x128xf16, {order = #NHWC}, @DDR>)
                        -> memref<1x96x128x128xf16, {order = #NHWC}, @DDR>
    %7 = VPUIP.SubView %6 [0, 0, 0, 0] [1, 96, 42, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x42x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    ...
    %9 = VPUIP.Copy  inputs(%7 : memref<1x96x42x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>)
                    outputs(%8 : !OutputDistributed)
                        -> !OutputDistributed
    %10 = VPUIP.SubView %6 [0, 0, 42, 0] [1, 96, 42, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x42x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    ...
    // CHECK:       [[PERMUTECAST:%.+]] = VPUIP.PermuteCast {dst_order = #NHWC, mem_perm = #NHWC} inputs([[INPUT]] : memref<1x96x2x2xf16, @DDR>) -> memref<1x96x2x2xf16, {order = #NHWC}, @DDR>

    // CHECK:       [[ALLOC:%.+]] = memref.alloc() : memref<1x96x42x2xf16, {order = #NHWC}, @DDR>
    // CHECK:       [[PERAXISTILEDMA:%.+]] = VPUIP.PerAxisTileDMA <{axis = 2 : i64, tiles = 21 : i64}>
    // CHECK-SAME:          inputs([[PERMUTECAST]] : memref<1x96x2x2xf16, {order = #NHWC}, @DDR>)
    // CHECK-SAME:          outputs([[ALLOC]] : memref<1x96x42x2xf16, {order = #NHWC}, @DDR>) -> memref<1x96x42x2xf16, {order = #NHWC}, @DDR>

    // CHECK:       [[ALLOC_0:%.+]] = memref.alloc() : memref<1x96x42x2xf16, {order = #NHWC}, @DDR>
    // CHECK:       [[PERAXISTILEDMA_0:%.+]] = VPUIP.PerAxisTileDMA <{axis = 2 : i64, tiles = 21 : i64}>
    // CHECK-SAME:          inputs([[PERMUTECAST]] : memref<1x96x2x2xf16, {order = #NHWC}, @DDR>)
    // CHECK-SAME:          outputs([[ALLOC_0]] : memref<1x96x42x2xf16, {order = #NHWC}, @DDR>) -> memref<1x96x42x2xf16, {order = #NHWC}, @DDR>

    // CHECK:       [[ALLOC_1:%.+]] = memref.alloc() : memref<1x96x42x2xf16, {order = #NHWC}, @DDR>
    // CHECK:       [[PERAXISTILEDMA_1:%.+]] = VPUIP.PerAxisTileDMA <{axis = 2 : i64, tiles = 21 : i64}>
    // CHECK-SAME:          inputs([[PERMUTECAST]] : memref<1x96x2x2xf16, {order = #NHWC}, @DDR>)
    // CHECK-SAME:          outputs([[ALLOC_1]] : memref<1x96x42x2xf16, {order = #NHWC}, @DDR>) -> memref<1x96x42x2xf16, {order = #NHWC}, @DDR>
    ...
    // CHECK-SAME:          inputs([[ARG0:%[^:]+]] : memref<1x96x42x2xf16, {order = #NHWC}, @DDR>)
    ...
    // CHECK-SAME:          inputs([[PERAXISTILEDMA_0]] : memref<1x96x42x2xf16, {order = #NHWC}, @DDR>)
    ...
    // CHECK-SAME:          inputs([[PERAXISTILEDMA_1]] : memref<1x96x42x2xf16, {order = #NHWC}, @DDR>)
    // CHECK-SAME:          outputs([[CMXBUF_1]] : !VPUIP.DistributedBuffer<1x96x42x128xf16, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) -> !VPUIP.DistributedBuffer<1x96x42x128xf16, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>
...
func.func @NotFuseSwKernelTileIntoClusterCopyForIncompatibleOutput(%arg0: memref<1x96x2x2xf16, @DDR>)
    ...
    // CHECK:       [[ALLOC:%.+]] = memref.alloc() : memref<1x96x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>
    // CHECK:       [[COPYIN:%.+]] = VPUIP.Copy
    // CHECK-SAME:          inputs([[PERMUTECAST]] : memref<1x96x2x2xf16, {order = #NHWC}, @DDR>)
    // CHECK-SAME:          outputs([[ALLOC]] : memref<1x96x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x96x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>
    // CHECK:       [[ALLOC_0:%.+]] = memref.alloc() : memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>
    // CHECK:       [[TILE_0:%.+]] = VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_Tile
    // CHECK-SAME:          inputs([[COPYIN]] as [[ARG0:%[^:]+]]: memref<1x96x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>)
    // CHECK-SAME:          outputs([[ALLOC_0]] as [[ARG1:%[^:]+]]: memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>) on tile 0 -> memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>{
    // CHECK:       [[ALLOC_1:%.+]] = memref.alloc() : memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>
    // CHECK:       [[TILE_1:%.+]] = VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_Tile
    // CHECK-SAME:          inputs([[COPYIN]] as [[ARG2:%[^:]+]]: memref<1x96x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>)
    // CHECK-SAME:          outputs([[ALLOC_1]] as [[ARG3:%[^:]+]]: memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>) on tile 0 -> memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>{

    // CHECK:       [[CONCATBUF:%.+]] = memref.alloc() : memref<1x96x128x128xf16, {order = #NHWC}, @DDR>
    // CHECK:       [[SUBVIEW_0:%.+]] = VPUIP.SubView [[CONCATBUF]] [0, 0, 0, 0] [1, 96, 64, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x64x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    // CHECK:       [[COPY_0:%.+]] = VPUIP.Copy
    // CHECK-SAME:          inputs([[TILE_0]] : memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>)
    ...
    // CHECK:       [[SUBVIEW_1:%.+]] = VPUIP.SubView [[CONCATBUF]] [0, 0, 64, 0] [1, 96, 64, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x64x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    // CHECK:       [[COPY_1:%.+]] = VPUIP.Copy
    // CHECK-SAME:          inputs([[TILE_1]] : memref<1x96x64x128xf16, {order = #NHWC}, [@CMX_NN, 0]>)
    ...
    // CHECK-SAME:          outputs([[CONCATBUF]] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR>) -> memref<1x96x128x128xf16, {order = #NHWC}, @DDR>

    // CHECK:       [[SUBVIEW_2:%.+]] = VPUIP.SubView [[CONCAT]] [0, 0, 0, 0] [1, 96, 43, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x43x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    ...
    // CHECK:       [[SUBVIEW_3:%.+]] = VPUIP.SubView [[CONCAT]] [0, 0, 43, 0] [1, 96, 43, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x43x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
    ...
    // CHECK:       [[SUBVIEW_4:%.+]] = VPUIP.SubView [[CONCAT]] [0, 0, 86, 0] [1, 96, 42, 128] : memref<1x96x128x128xf16, {order = #NHWC}, @DDR> to memref<1x96x42x128xf16, {order = #NHWC, strides = [1572864, 1, 12288, 96]}, @DDR>
