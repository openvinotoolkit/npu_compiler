...
// CHECK-SAME: ([[ARG_0:%.+]]: memref<1x16x24x24xf16, @DDR>
func.func @WrapPermuteAsDMAWithDistributedOp(%arg0: memref<1x16x24x24xf16, @DDR>)
    ...
    //CHECK:   [[VAR1:%.+]] = VPUIP.PermuteDMA <{mem_perm = #NHWC}> inputs([[ARG_0]] : memref<1x16x24x24xf16, @DDR>) outputs([[VAR0]] : !VPUIP.DistributedBuffer<1x16x24x24xf16, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) -> !VPUIP.DistributedBuffer<1x16x24x24xf16, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>
...
// CHECK-SAME: ([[ARG_0:%.+]]: memref<1x3x224x224x!qElemType>
func.func @WrapExpandAndPermuteWithDistributedOp(%arg0: memref<1x3x224x224x!qElemType>) -> !OutputDistributedType {
  ...
  // CHECK-SAME:     inputs([[ARG_0]] : memref<1x3x224x224x!qElemType>)
...
// CHECK-SAME: ([[ARG_0:%.+]]: memref<1x3x224x224xf16>
func.func @CannotWrapExpandAndPermuteWithDistributedOpFP16(%arg0: memref<1x3x224x224xf16>) -> !OutputDistributedType {
  ...
  //CHECK:  [[EXPAND:%.+]] = VPUIP.Expand {pads_begin = [0, 0, 0, 0], pads_end = [0, 13, 0, 0]} inputs([[ARG_0]] : memref<1x3x224x224xf16>) outputs([[VAR0]] : memref<1x16x224x224xf16>) -> memref<1x16x224x224xf16>
...
func.func @FuseMemPermuteWithPermuteCastAndPropagateDistributedType()
    ...
    //CHECK:   [[OUT_BUFF:%.+]] = VPURT.AllocDistributed -> !VPUIP.DistributedBuffer<1x1x40x784xf16
    //CHECK-SAME:     , #NCHW, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>
...
!SubtractDistributedType = !VPUIP.DistributedBuffer<
...
!AddDistributedType = !VPUIP.DistributedBuffer<
    1x16x1x1xf16, #NHWC, @CMX_NN, {
    mode = "DUPLICATED",
    num_clusters = 2 : i64,
    uniform_distributed_segments,
...
!SubViewType = !VPUIP.DistributedBuffer<
    1x1x1x1xf16, {order = #NHWC, strides = [16, 1, 16, 16]}, @CMX_NN, {
    mode = "DUPLICATED",
    num_clusters = 2 : i64,
    uniform_distributed_segments,
...
func.func @WrapExpandMultiUsersAndSharedUsers(%arg0: memref<1x1x1x1xf16, {order = #NHWC}>, %arg1: memref<1x16x1x1xf16, {order = #NHWC}>) -> !SubViewType {
    ...
    %5 = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 137 : i64, resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{eltwise_type = #VPU.eltwise_type<SUBTRACT>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, task_type = #VPUIP.nce_task_type<ELTWISE>}>
        ...
        DPUTask {cluster_id = 1 : i64, inEnd = [0, 0, 15], inStart = [0, 0, 0], mpe_mode = #VPU.mpe_mode<CUBOID_8x16>, outEnd = [0, 0, 15], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
    } PPE : {
    ...
    %8 = VPUIP.SubView %7 [0, 0, 0, 0] [1, 1, 1, 1] : !AddDistributedType to !SubViewType
    return %8 : !SubViewType
    ...
    // CHECK:               -> !VPUIP.DistributedBuffer<1x16x1x1xf16, #NHWC, @CMX_NN,
    // CHECK:               {mode = "DUPLICATED", num_clusters = 2 : i64, uniform_distributed_segments,
    // CHECK-SAME{LITERAL}: compute_shapes = [[1, 16, 1, 1], [1, 16, 1, 1]], compute_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]],
    // CHECK-SAME{LITERAL}: memory_shapes = [[1, 16, 1, 1], [1, 16, 1, 1]], memory_offsets = [[0, 0, 0, 0], [0, 0, 0, 0]]}>
