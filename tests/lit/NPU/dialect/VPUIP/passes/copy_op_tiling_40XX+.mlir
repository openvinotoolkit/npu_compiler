...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --tile-copies %s | FileCheck %s
// REQUIRES: platform-NPU4000 || platform-NPU5010

#NCHW = affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>

// CHECK-LABEL: @LegalizeCopy
// CHECK-SAME:  [[ARG_0:%[^:]+]]: memref<1x256x2048x8192xf16>
// CHECK-SAME:  [[ARG_1:%[^:]+]]: memref<1x256x2048x8192xf16>
func.func @LegalizeCopy(
        %arg0: memref<1x256x2048x8192xf16>,
        %arg1: memref<1x256x2048x8192xf16>)
        -> memref<1x256x2048x8192xf16> {
    %0 = VPUIP.Copy inputs(%arg0 : memref<1x256x2048x8192xf16>)
                   outputs(%arg1 : memref<1x256x2048x8192xf16>)
                   -> memref<1x256x2048x8192xf16>

    return %0 : memref<1x256x2048x8192xf16>

    // Currently, large Copy nodes are tiled C-wise

    // CHECK: [[SUBVIEW_SRC_0:%.+]] = VPUIP.SubView [[ARG_0]] [0, 0, 0, 0] [1, 127, 2048, 8192] :
    ...
    // CHECK: [[SUBVIEW_DST_0:%.+]] = VPUIP.SubView [[ARG_1]] [0, 0, 0, 0] [1, 127, 2048, 8192] :
    // CHECK-SAME:      memref<1x256x2048x8192xf16>
    // CHECK-SAME:   to memref<1x127x2048x8192xf16, {order = #NCHW, strides = [4294967296, 16777216, 8192, 1]}>
    ...
    // CHECK: [[SUBVIEW_SRC_1:%.+]] = VPUIP.SubView [[ARG_0]] [0, 127, 0, 0] [1, 127, 2048, 8192] :
    // CHECK-SAME:      memref<1x256x2048x8192xf16>
    // CHECK-SAME:   to memref<1x127x2048x8192xf16, {order = #NCHW, strides = [4294967296, 16777216, 8192, 1]}>
    // CHECK: [[SUBVIEW_DST_1:%.+]] = VPUIP.SubView [[ARG_1]] [0, 127, 0, 0] [1, 127, 2048, 8192] :
    // CHECK-SAME:      memref<1x256x2048x8192xf16>
    // CHECK-SAME:   to memref<1x127x2048x8192xf16, {order = #NCHW, strides = [4294967296, 16777216, 8192, 1]}>
    ...
    // CHECK: [[SUBVIEW_SRC_2:%.+]] = VPUIP.SubView [[ARG_0]] [0, 254, 0, 0] [1, 1, 2048, 8192] :
    // CHECK-SAME:      memref<1x256x2048x8192xf16>
    // CHECK-SAME:   to memref<1x1x2048x8192xf16, {order = #NCHW, strides = [4294967296, 16777216, 8192, 1]}>
    // CHECK: [[SUBVIEW_DST_2:%.+]] = VPUIP.SubView [[ARG_1]] [0, 254, 0, 0] [1, 1, 2048, 8192] :
    // CHECK-SAME:      memref<1x256x2048x8192xf16>
    // CHECK-SAME:   to memref<1x1x2048x8192xf16, {order = #NCHW, strides = [4294967296, 16777216, 8192, 1]}>
    ...
    // CHECK: [[SUBVIEW_SRC_3:%.+]] = VPUIP.SubView [[ARG_0]] [0, 255, 0, 0] [1, 1, 2048, 8192] :
    // CHECK-SAME:      memref<1x256x2048x8192xf16>
    // CHECK-SAME:   to memref<1x1x2048x8192xf16, {order = #NCHW, strides = [4294967296, 16777216, 8192, 1]}>
    // CHECK: [[SUBVIEW_DST_3:%.+]] = VPUIP.SubView [[ARG_1]] [0, 255, 0, 0] [1, 1, 2048, 8192] :
    // CHECK-SAME:      memref<1x256x2048x8192xf16>
    // CHECK-SAME:   to memref<1x1x2048x8192xf16, {order = #NCHW, strides = [4294967296, 16777216, 8192, 1]}>
    ...
    // CHECK-SAME:        memref<1x1x2048x8192xf16, {order = #NCHW, strides = [4294967296, 16777216, 8192, 1]}>
    // CHECK-SAME:        memref<1x1x2048x8192xf16, {order = #NCHW, strides = [4294967296, 16777216, 8192, 1]}>)
    // CHECK-SAME:      outputs([[ARG_1]] : memref<1x256x2048x8192xf16>)
    // CHECK-SAME:        -> memref<1x256x2048x8192xf16>
...
// CHECK-LABEL: @LegalizeStridedCopy
// CHECK-SAME:  [[ARG_0:%[^:]+]]: memref<1x256x2048x8192xf16, {order = #NCHW, strides = [8589934592, 16777216, 8192, 1]}>
// CHECK-SAME:  [[ARG_1:%[^:]+]]: memref<1x256x2048x8192xf16>
...
// CHECK-LABEL: @DoNotLegalizeCopy
// CHECK-SAME:  [[ARG_0:%[^:]+]]: memref<1x127x2048x8192xf16>
// CHECK-SAME:  [[ARG_1:%[^:]+]]: memref<1x127x2048x8192xf16>
func.func @DoNotLegalizeCopy(
        %arg0: memref<1x127x2048x8192xf16>,
        %arg1: memref<1x127x2048x8192xf16>)
        -> memref<1x127x2048x8192xf16> {
    %0 = VPUIP.Copy inputs(%arg0 : memref<1x127x2048x8192xf16>)
                   outputs(%arg1 : memref<1x127x2048x8192xf16>)
                   -> memref<1x127x2048x8192xf16>

    return %0 : memref<1x127x2048x8192xf16>

    // Small enough Copy nodes should not be affected by the pass

    // CHECK: [[COPY_0:%.+]] = VPUIP.Copy
    // CHECK-SAME:      inputs([[ARG_0]] : memref<1x127x2048x8192xf16>)
    // CHECK-SAME:      outputs([[ARG_1]] : memref<1x127x2048x8192xf16>)
...
// CHECK-LABEL: @SplitByHeight3D
// CHECK-SAME:  [[ARG_0:%[^:]+]]: memref<1x128x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>
// CHECK-SAME:  [[ARG_1:%[^:]+]]: memref<1x128x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>
func.func @SplitByHeight3D(
    ...
    // CHECK: [[ARG_0_TILE_0:%.+]] = VPUIP.SubView [[ARG_0]] [0, 0, 0] [1, 64, 16777216] :
    ...
    // CHECK: [[ARG_1_TILE_0:%.+]] = VPUIP.SubView [[ARG_1]] [0, 0, 0] [1, 64, 16777216] :
    // CHECK-SAME:              memref<1x128x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>
    // CHECK-SAME:           to memref<1x64x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>
    ...
    // CHECK: [[ARG_0_TILE_1:%.+]] = VPUIP.SubView [[ARG_0]] [0, 64, 0] [1, 64, 16777216] :
    // CHECK-SAME:              memref<1x128x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>
    // CHECK-SAME:           to memref<1x64x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>

    // CHECK: [[ARG_1_TILE_1:%.+]] = VPUIP.SubView [[ARG_1]] [0, 64, 0] [1, 64, 16777216] :
    // CHECK-SAME:              memref<1x128x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>
    // CHECK-SAME:           to memref<1x64x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>
    ...
    // CHECK-SAME:      memref<1x64x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>,
    // CHECK-SAME:      memref<1x64x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>)
    // CHECK-SAME:  outputs([[ARG_1]] : memref<1x128x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>)
    // CHECK-SAME:  -> memref<1x128x16777216xf16, {order = #CHW, strides = [8589934592, 33554432, 1]}>
...
// CHECK-LABEL: @SplitByDepth5D
// CHECK-SAME:  [[ARG_0:%[^:]+]]: memref<1x1x4096x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>
// CHECK-SAME:  [[ARG_1:%[^:]+]]: memref<1x1x4096x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>
func.func @SplitByDepth5D(
    ...
    // CHECK: [[ARG_0_TILE_0:%.+]] = VPUIP.SubView [[ARG_0]] [0, 0, 0, 0, 0] [1, 1, 2048, 256, 2048] :
    ...
    // CHECK: [[ARG_1_TILE_0:%.+]] = VPUIP.SubView [[ARG_1]] [0, 0, 0, 0, 0] [1, 1, 2048, 256, 2048] :
    // CHECK-SAME:              memref<1x1x4096x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>
    // CHECK-SAME:           to memref<1x1x2048x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>
    ...
    // CHECK: [[ARG_0_TILE_1:%.+]] = VPUIP.SubView [[ARG_0]] [0, 0, 2048, 0, 0] [1, 1, 2048, 256, 2048] :
    // CHECK-SAME:              memref<1x1x4096x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>
    // CHECK-SAME:           to memref<1x1x2048x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>

    // CHECK: [[ARG_1_TILE_1:%.+]] = VPUIP.SubView [[ARG_1]] [0, 0, 2048, 0, 0] [1, 1, 2048, 256, 2048] :
    // CHECK-SAME:              memref<1x1x4096x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>
    // CHECK-SAME:           to memref<1x1x2048x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>
    ...
    // CHECK-SAME:      memref<1x1x2048x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>,
    // CHECK-SAME:      memref<1x1x2048x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>)
    // CHECK-SAME:  outputs([[ARG_1]] : memref<1x1x4096x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>)
    // CHECK-SAME:  -> memref<1x1x4096x256x2048xf16, {order = #NCDHW, strides = [4294967296, 4294967296, 1048576, 4096, 1]}>
...
// CHECK-LABEL: @DoNotSplit5DCopyWithLevel3Striding
// CHECK-SAME:  [[ARG_0:%[^:]+]]: memref<1x1x4095x256x2048xf16, {order = #NCDHW, strides = [17175674880, 17175674880, 2097152, 4096, 1]}>
// CHECK-SAME:  [[ARG_1:%[^:]+]]: memref<1x1x4095x256x2048xf16, {order = #NCDHW, strides = [17175674880, 17175674880, 2097152, 4096, 1]}>
func.func @DoNotSplit5DCopyWithLevel3Striding(
    ...
    // CHECK: [[COPY_0:%.+]] = VPUIP.Copy
    // CHECK-SAME:      inputs([[ARG_0]] : memref<1x1x4095x256x2048xf16, {order = #NCDHW, strides = [17175674880, 17175674880, 2097152, 4096, 1]}>)
    // CHECK-SAME:      outputs([[ARG_1]] : memref<1x1x4095x256x2048xf16, {order = #NCDHW, strides = [17175674880, 17175674880, 2097152, 4096, 1]}>)
...
!InputType = memref<3x512x2048x8192x!quant.uniform<u8:f16, 1.2608227898092832:124>, {order = #NHWC}, @DDR>
...
// CHECK-SAME:  [[ARG_0:%[^:]+]]: memref<3x512x2048x8192x!qElemType, {order = #NHWC}, @DDR>
// CHECK-SAME:  [[ARG_1:%[^:]+]]: memref<3x512x2048x8192x!qElemType, {order = #NHWC, strides = [34359738368, 1, 16777216, 512]}, @DDR>
func.func @RecursiveSplitLargeCopy(
    ...
    // CHECK:       [[SUBVIEW_0:%.+]] = VPUIP.SubView [[ARG_0]] [0, 0, 0, 0] [1, 512, 2048, 8192]
    // CHECK-SAME:         memref<3x512x2048x8192x!qElemType, {order = #NHWC}, @DDR>
    // CHECK-SAME:      to memref<1x512x2048x8192x!qElemType, {order = #NHWC}, @DDR>
    // CHECK:       [[SUBVIEW_1:%.+]] = VPUIP.SubView [[ARG_1]] [0, 0, 0, 0] [1, 512, 2048, 8192]
    ...
    // CHECK-SAME:         memref<1x512x2048x8192x!qElemType, {order = #NHWC}, @DDR>
    ...
    // CHECK:       [[SUBVIEW_10:%.+]] = VPUIP.SubView [[ARG_0]] [1, 0, 0, 0] [1, 512, 2048, 8192]
    // CHECK-SAME:         memref<3x512x2048x8192x!qElemType, {order = #NHWC}, @DDR>
    // CHECK-SAME:      to memref<1x512x2048x8192x!qElemType, {order = #NHWC}, @DDR>
    // CHECK:       [[SUBVIEW_11:%.+]] = VPUIP.SubView [[ARG_1]] [1, 0, 0, 0] [1, 512, 2048, 8192]
    // CHECK-SAME:         memref<3x512x2048x8192x!qElemType, {order = #NHWC, strides = [34359738368, 1, 16777216, 512]}, @DDR>
    // CHECK-SAME:      to memref<1x512x2048x8192x!qElemType, {order = #NHWC, strides = [34359738368, 1, 16777216, 512]}, @DDR>
    ...
    // CHECK:       [[SUBVIEW_20:%.+]] = VPUIP.SubView [[ARG_0]] [2, 0, 0, 0] [1, 512, 2048, 8192]
    // CHECK-SAME:         memref<3x512x2048x8192x!qElemType, {order = #NHWC}, @DDR>
    // CHECK-SAME:      to memref<1x512x2048x8192x!qElemType, {order = #NHWC}, @DDR>
    // CHECK:       [[SUBVIEW_21:%.+]] = VPUIP.SubView [[ARG_1]] [2, 0, 0, 0] [1, 512, 2048, 8192]
    // CHECK-SAME:         memref<3x512x2048x8192x!qElemType, {order = #NHWC, strides = [34359738368, 1, 16777216, 512]}, @DDR>
    // CHECK-SAME:      to memref<1x512x2048x8192x!qElemType, {order = #NHWC, strides = [34359738368, 1, 16777216, 512]}, @DDR>
    ...
    // CHECK-SAME:         outputs([[ARG_1]] : memref<3x512x2048x8192x!qElemType, {order = #NHWC, strides = [34359738368, 1, 16777216, 512]}, @DDR>)
    // CHECK-SAME:          -> memref<3x512x2048x8192x!qElemType, {order = #NHWC, strides = [34359738368, 1, 16777216, 512]}, @DDR>
