...
func.func @PatchFusedConstantWithSpill() ->  memref<1x16x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]> {
    %in = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x16x14x14xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %out = VPURT.DeclareBuffer <CMX_NN> [0] <556416> -> memref<1x16x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    %5 = VPUIP.SubView %3 [0, 0, 0, 256] [1, 1, 1, 512] : memref<1x1x1x784xui8, [@CMX_NN, 0]> to memref<1x1x1x512xui8, {order = #NCHW, strides = [784, 784, 784, 1]}, [@CMX_NN, 0]>
    ...
    %8 = VPUIP.ViewOp %5 : memref<1x1x1x512xui8, {order = #NCHW, strides = [784, 784, 784, 1]}, [@CMX_NN, 0]> to memref<16x16x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    %10 = VPUIP.NCEClusterTask {constantsFused = true, resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{
        ...
        input(%in : memref<1x16x14x14xf16, {order = #NHWC}, [@CMX_NN, 0]>)

        weights(%8 : memref<16x16x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        weight_table(%7 : memref<16x1x1x4xsi32, [@CMX_NN, 0]>)

        parent_input(%in : memref<1x16x14x14xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        parent_output(%out : memref<1x16x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        outputs(%out : memref<1x16x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x16x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]> variants :  {
    ...
    return %10 : memref<1x16x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>

    // CHECK:   [[INPUT:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x16x14x14xf16, {order = #NHWC}, [@CMX_NN, 0]>
    // CHECK:   [[OUTPUT:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <556416> -> memref<1x16x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>
...
!IpOp_Stub = memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>
...
func.func @PatchFusedConstantWithSpillAsyncConstruct() -> !IpOp_Stub {
    ...
    %in = VPURT.DeclareBuffer <CMX_NN> [0] <512> -> memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %out = VPURT.DeclareBuffer <CMX_NN> [0] <173184> -> memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    %t3, %r3 = async.execute (%r2 as %arg7: !async.value<memref<1x1x1x5120xui8, [@CMX_NN, 0]>>) -> !async.value<!IpOp_Stub> attributes
        ...
        %7 = VPUIP.ViewOp %5 : memref<1x1x1x4096xui8, {order = #NCHW, strides = [5120, 5120, 5120, 1]}, [@CMX_NN, 0]> to memref<64x64x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>
        ...
        input(%in : memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        weights(%7 : memref<64x64x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        weight_table(%6 : memref<64x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%in : memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        parent_output(%out : memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        outputs(%out : memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>
        variants : {
    ...
    async.yield %out : memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>
    }

    %5 = async.await %r3 : !async.value<memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>>
    return %5 : memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    // CHECK:   [[INPUT:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <512> -> memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>
    // CHECK:   [[OUTPUT:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <173184> -> memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    // CHECK-SAME:          -> !async.value<memref<1x64x52x52xf16, {order = #NHWC}, [@CMX_NN, 0]>>
    // CHECK:       [[SUBVIEW_1:%.+]] = VPUIP.SubView
    // CHECK:       [[SUBVIEW_2:%.+]] = VPUIP.SubView
    // CHECK:       [[VIEW_1:%.+]] = VPUIP.ViewOp
    // CHECK:       [[VIEW_2:%.+]] = VPUIP.ViewOp
