...
func.func @PatchWeightTableWeightsOnlyAutopad() -> memref<16x1x1x4xsi32, [@CMX_NN, 0]> {
    ...
    %weights = VPURT.DeclareBuffer <CMX_NN> [0] <1024> -> memref<3x4x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %weight_table_const = const.Declare memref<16x1x1x4xsi32> = dense<1> : tensor<16x1x1x4xsi32>

    %in = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x4x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %out = VPURT.DeclareBuffer <CMX_NN> [0] <428608> -> memref<1x3x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    %5 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [1, 1], kernel_strides = [1, 1], task_type = #VPUIP.nce_task_type<CONV>}> input(%in : memref<1x4x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%weights : memref<3x4x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%weight_table : memref<16x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%in : memref<1x4x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%out : memref<1x3x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) outputs(%out : memref<1x3x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x3x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]> variants : {
    ...
    // CHECK:       [[WEIGHTS_BUF:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <[[WEIGHTS_ADDR:[^>]+]]> -> memref<3x4x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    // CHECK-SAME:  weights([[WEIGHTS_BUF]] : memref<3x4x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>)
