...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% allow-custom-values=true" --assign-physical-barriers="num-barriers=4 workload-management-mode=FWLM_V1_PAGES" %s | FileCheck %s
...
func.func @DmaAndDpuGraph() -> memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]> {
               ...
               updates(%bar2, %bar3: !VPURT.Barrier, !VPURT.Barrier)
    {
        VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{
            ...
            input(%buf0: memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%buf1: memref<16x16x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_input(%buf0: memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%buf3: memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]>) outputs(%buf3: memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]>
