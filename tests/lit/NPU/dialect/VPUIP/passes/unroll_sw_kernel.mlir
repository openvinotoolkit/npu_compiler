...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% compilation-mode=DefaultHW" --unroll-sw-kernel="enable-sw-kernel-fifo-per-shave-engine=false" %s | FileCheck %s
...
func.func @DistributedBufferUnrollSwKernel() -> (!DistributedT, !DistributedT) {
    ...
    // CHECK:   VPURT.Task waits([[BAR0]] : !VPURT.Barrier) updates([[BAR1]] : !VPURT.Barrier) {
    // CHECK:           VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_MVN inputs([[TILE0]] as [[ARG_0:%[^:]+]]: !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>) outputs([[OUTPUT0]] as [[ARG_1:%[^:]+]]: !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>) outputStrides({{\[\[}}131072, 1, 64, 2048], [131072, 1, 64, 2048]]) on tile 0
    // CHECK-SAME: -> !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>{
    // CHECK:                    VPUIP.SW.Kernel.run {attrs = [false, true, 6.0892105102539063E-4]}([[ARG_0]], [[ARG_1]]) : !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>, !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>
    // CHECK:           }
    // CHECK:   }
    // CHECK:   VPURT.Task waits([[BAR0]] : !VPURT.Barrier) updates([[BAR1]] : !VPURT.Barrier) {
    // CHECK:           VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_MVN inputs([[TILE1]] as [[ARG_0:%[^:]+]]: !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>) outputs([[OUTPUT1]] as [[ARG_1:%[^:]+]]: !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>) outputStrides({{\[\[}}131072, 1, 64, 2048], [131072, 1, 64, 2048]]) on tile 0 -> !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>{
    // CHECK:                    VPUIP.SW.Kernel.run {attrs = [false, true, 6.0892105102539063E-4]}([[ARG_0]], [[ARG_1]]) : !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>, !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>
    // CHECK:           }
    // CHECK:   }
...
func.func @DistributedBufferUnrollSwKernelWithProfiling() -> (!DistributedT, !DistributedT) {
    ...
    // CHECK-SAME:          inputs([[TILE0]] as [[ARG_0:%[^:]+]]: !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>)
    // CHECK-SAME:          outputs([[OUTPUT0]] as [[ARG_1:%[^:]+]]: !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>)
    ...
    // CHECK-SAME:          outputStrides({{\[\[}}131072, 1, 64, 2048], [131072, 1, 64, 2048]]) on tile 0
    // CHECK-SAME:      -> (!VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>,
    ...
    // CHECK-SAME:          resultSegmentSizes = array<i32: 1, 0, 1>}
    // CHECK-SAME:          @VPU.SW::@builtin_MVN
    // CHECK-SAME:          inputs([[TILE1]] as [[ARG_0:%[^:]+]]: !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>)
    // CHECK-SAME:          outputs([[OUTPUT1]] as [[ARG_1:%[^:]+]]: !VPUIP.DistributedBuffer<1x64x64x32xf16, #NWHC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>)
...
func.func @UnrollDynamicSwKernel() -> (memref<1x1x35x128xf16, [@CMX_NN, 0]>, memref<1x1x1x128xf16, [@CMX_NN, 0]>, memref<1x1x1x128xf16, [@CMX_NN, 0]>) {
    ...
    VPURT.Task waits(%10, %11, %12, %13, %14 : !VPURT.Barrier, !VPURT.Barrier, !VPURT.Barrier, !VPURT.Barrier, !VPURT.Barrier) updates(%15 : !VPURT.Barrier) {
      %results:6, %dynamicOutputShapes = VPUIP.SW.Kernel {
        ...
        VPUIP.SW.Kernel.run {attrs = [[-9223372036854775808, 1023], 0, 0]}(%arg8, %arg9, %arg10, %arg11, %arg12, %arg18, %arg19, %arg20) :
            ...
            memref<1x1x1x128xf16, [@CMX_NN, 0]>,
            memref<1x1x1x128xf16, [@CMX_NN, 0]>
        VPUIP.SW.Kernel.run {attrs = [[-9223372036854775808, 1023], 0, 0]}(%arg13, %arg14, %arg15, %arg16, %arg17, %arg21, %arg22, %arg23) :
            memref<1x1x35x512xf16, [@CMX_NN, 0]>,
            memref<1x1x1x128xf16, [@CMX_NN, 0]>,
            memref<1x1x1x128xf16, [@CMX_NN, 0]>,
            memref<1x4x128x128xf16, {order = #NWHC}, [@CMX_NN, 0]>,
    ...
    // CHECK:       outputs(
    // CHECK:         [[BUFF_6]] as [[ARG_5:%[^:]+]]: memref<1x1x35x128xf16, [@CMX_NN, 0]>,
    // CHECK:         [[BUFF_8]] as [[ARG_6:%[^:]+]]: memref<1x1x1x128xf16, [@CMX_NN, 0]>,
    // CHECK:         [[BUFF_9]] as [[ARG_7:%[^:]+]]: memref<1x1x1x128xf16, [@CMX_NN, 0]>
    // CHECK:       )
...
!Input0Distributed = !VPUIP.DistributedBuffer<1x1x5x13xf16, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>
!Input1Distributed = !VPUIP.DistributedBuffer<1x1x5x12xf16, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>
...
!OutValues0Distributed = !VPUIP.DistributedBuffer<1x1x1x13xf16, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>
!OutValues1Distributed = !VPUIP.DistributedBuffer<1x1x1x12xf16, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>
!TargetShape0Distributed = !VPUIP.DistributedBuffer<1x1x1x13xsi32, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>
...
func.func @UnrollKernelSharedInputOutputBuffers() -> (!OutValues0Distributed, !OutValues1Distributed) {
    %input_0 = VPURT.DeclareBuffer <CMX_NN> <0> -> !Input0Distributed
    %input_1 = VPURT.DeclareBuffer <CMX_NN> <1> -> !Input1Distributed
    %aux_0 = VPURT.DeclareBuffer <CMX_NN> <2> -> !AuxDistributed
    %aux_1 = VPURT.DeclareBuffer <CMX_NN> <3> -> !AuxDistributed
    %out_values_0 = VPURT.DeclareBuffer <CMX_NN> <4> -> !OutValues0Distributed
    %out_values_1 = VPURT.DeclareBuffer <CMX_NN> <5> -> !OutValues1Distributed
    %target_shape_0 = VPURT.DeclareBuffer <CMX_NN> <6> -> !TargetShape0Distributed
    %target_shape_1 = VPURT.DeclareBuffer <CMX_NN> <7> -> !TargetShape1Distributed

    VPURT.Task {
      %results:6 = VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 6, 0, 0>} @VPU.SW::@builtin_TopK
        inputs(%input_0 as %inner_input_0: !Input0Distributed,
               %aux_0 as %inner_aux_0: !AuxDistributed,
               %input_1 as %inner_input_1: !Input1Distributed,
               %aux_1 as %inner_aux_1: !AuxDistributed)
        outputs(%out_values_0 as %inner_out_values_0: !OutValues0Distributed,
                %target_shape_0 as %inner_target_shape_0: !TargetShape0Distributed,
                %aux_0 as %inner_aux_0_alias: !AuxDistributed,
                %out_values_1 as %inner_out_values_1: !OutValues1Distributed,
                %target_shape_1 as %inner_target_shape_1: !TargetShape1Distributed,
                %aux_1 as %inner_aux_1_alias: !AuxDistributed)
        on tile 0 -> (!OutValues0Distributed, !TargetShape0Distributed, !AuxDistributed, !OutValues1Distributed, !TargetShape1Distributed, !AuxDistributed){
        VPUIP.SW.Kernel.run {attrs = [1, 1, 2, 1]}(%inner_input_0, %inner_aux_0, %inner_out_values_0, %inner_target_shape_0, %inner_aux_0_alias) : !Input0Distributed, !AuxDistributed, !OutValues0Distributed, !TargetShape0Distributed, !AuxDistributed
        VPUIP.SW.Kernel.run {attrs = [1, 1, 2, 1]}(%inner_input_1, %inner_aux_1, %inner_out_values_1, %inner_target_shape_1, %inner_aux_1_alias) : !Input1Distributed, !AuxDistributed, !OutValues1Distributed, !TargetShape1Distributed, !AuxDistributed
      }
    }

    return %out_values_0, %out_values_1 : !OutValues0Distributed, !OutValues1Distributed

    // CHECK:      [[INPUT_0:%.+]] = VPURT.DeclareBuffer <CMX_NN> <0>
    // CHECK:      [[INPUT_1:%.+]] = VPURT.DeclareBuffer <CMX_NN> <1>
    // CHECK:      [[AUX_0:%.+]] = VPURT.DeclareBuffer <CMX_NN> <2>
    // CHECK:      [[AUX_1:%.+]] = VPURT.DeclareBuffer <CMX_NN> <3>
    // CHECK:      [[OUT_VALUES_0:%.+]] = VPURT.DeclareBuffer <CMX_NN> <4>
    // CHECK:      [[OUT_VALUES_1:%.+]] = VPURT.DeclareBuffer <CMX_NN> <5>
    // CHECK:      [[TARGET_SHAPE_0:%.+]] = VPURT.DeclareBuffer <CMX_NN> <6>
    // CHECK:      [[TARGET_SHAPE_1:%.+]] = VPURT.DeclareBuffer <CMX_NN> <7>
