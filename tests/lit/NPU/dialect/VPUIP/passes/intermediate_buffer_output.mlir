...
func.func @TestBufferOutput(%arg0: memref<1x3x224x224xf16, @DDR>, %arg1: memref<1x64x56x56xf16, @DDR>) -> memref<1x64x56x56xf16, @DDR> {
// CHECK: func.func @TestBufferOutput([[ARG0:%.+]]: memref<1x3x224x224xf16, @DDR>, [[ARG1:%.+]]: memref<1x224x4x115x!qElemType, #NWCH, @DDR>) -> memref<1x224x4x115x!qElemType, #NWCH, @DDR> {
  ...
  // CHECK-DAG:   [[TARGET_BUFFER:%.*]] = VPURT.DeclareBuffer <CMX_NN> [1] <154560> -> memref<1x224x4x115x!qElemType, #NWCH, [@CMX_NN, 1]>

  // Target TaskOp
  // CHECK:       VPURT.Task waits([[BAR0]] : !VPURT.Barrier) updates([[BAR1]] : !VPURT.Barrier) attributes {opIndex = 5 : i64}
  ...
  // Updated return
  // CHECK:   return [[ARG1]] : memref<1x224x4x115x!qElemType, #NWCH, @DDR>
}
