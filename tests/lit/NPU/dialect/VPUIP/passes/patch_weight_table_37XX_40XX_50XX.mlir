...
func.func @PatchWeightTable() ->  memref<1008x1x1x4xsi32, [@CMX_NN, 0]> {
    ...
    %weights = VPURT.DeclareBuffer <CMX_NN> [0] <395136> -> memref<1008x64x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %weight_table_const = const.Declare memref<1008x1x1x4xsi32> = dense<1> : tensor<1008x1x1x4xsi32>

    %in = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x1008x14x14xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %out = VPURT.DeclareBuffer <CMX_NN> [0] <556416> -> memref<1x1008x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    %5 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [7, 7], kernel_strides = [7, 7], task_type = #VPUIP.nce_task_type<DWCONV>}> input(%in : memref<1x1008x14x14xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%weights : memref<1008x64x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%weight_table : memref<1008x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%in : memref<1x1008x14x14xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%out : memref<1x1008x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>) outputs(%out : memref<1x1008x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x1008x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]> variants :  {
    ...
    // CHECK:       [[WEIGHTS_BUF:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <[[WEIGHTS_ADDR:[^>]+]]> -> memref<1008x64x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    // CHECK-SAME:  weights([[WEIGHTS_BUF]] : memref<1008x64x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>)
...
func.func @PatchWeightTableWeightsOnly() -> memref<256x1x1x1xsi32, [@CMX_NN, 0]> {
    ...
    %weights = VPURT.DeclareBuffer <CMX_NN> [0] <4096> -> memref<64x16x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %weight_table_const = const.Declare memref<256x1x1x1xsi32> = dense<1> : tensor<256x1x1x1xsi32>

    %in = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x16x59x227xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %out = VPURT.DeclareBuffer <CMX_NN> [0] <428608> -> memref<1x64x29x113xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    %5 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [3, 3], kernel_strides = [2, 2], task_type = #VPUIP.nce_task_type<CONV>}> input(%in : memref<1x16x59x227xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%weights : memref<64x16x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%weight_table : memref<256x1x1x1xsi32, [@CMX_NN, 0]>) parent_input(%in : memref<1x16x59x227xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%out : memref<1x64x29x113xf16, {order = #NHWC}, [@CMX_NN, 0]>) outputs(%out : memref<1x64x29x113xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x64x29x113xf16, {order = #NHWC}, [@CMX_NN, 0]> variants : {
    ...
    // CHECK:       [[WEIGHTS_BUF:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <[[WEIGHTS_ADDR:[^>]+]]> -> memref<64x16x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    // CHECK:       [[NCE_CLUST_TASK_OP:.+]] = VPUIP.NCEClusterTask
    // CHECK-SAME:  weights([[WEIGHTS_BUF]] : memref<64x16x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>)
...
func.func @PatchWeightTableWithSpill() ->  memref<1x1008x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]> {
    ...
    %5 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [7, 7], kernel_strides = [7, 7], task_type = #VPUIP.nce_task_type<DWCONV>}> input(%in : memref<1x1008x14x14xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%weights : memref<1008x64x1x1xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%weight_table_2 : memref<1008x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%in : memref<1x1008x14x14xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%out : memref<1x1008x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>) outputs(%out : memref<1x1008x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x1008x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]> variants :  {
        DPUTask {outEnd = [1, 0, 1007], mpe_mode = #VPU.mpe_mode<VECTOR_FP16>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, outStart = [0, 0, 0]}
    } PPE :  {
    }

    return %5 : memref<1x1008x2x2xf16, {order = #NHWC}, [@CMX_NN, 0]>
...
!WeightsTable_DDR = memref<64x1x1x4xsi32, {order = #NHWC}, @DDR>
!Output_DDR = memref<1x64x16x16xf16, {order = #NHWC}, @DDR>

!InputStub_CMX = memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>
!WeightsStub_CMX = memref<64x32x3x3xf16, {order = #NHWC}, @CMX_NN>
!WeightsTableStub_CMX = memref<64x1x1x4xsi32, {order = #NHWC}, @CMX_NN>
!OutputStub_CMX = memref<1x64x16x16xf16, {order = #NHWC}, @CMX_NN>
...
func.func @PatchWeightTableWithDistributedBuffers(%arg0: !Input_DDR) -> !Output_DDR {
    ...
    // CHECK:       [[OUTPUT_BUF_DDR:%.+]] = VPURT.DeclareBuffer <DDR> <0> -> memref<1x64x16x16xf16, {order = #NHWC}, @DDR>

    // CHECK-DAG:       [[CONST_W:%.+]] = const.Declare memref<64x32x3x3xf16, {order = #NHWC}, @DDR>
    // CHECK-DAG:       [[CONST_WT:%.+]] = const.Declare memref<64x1x1x4xsi32, {order = #NHWC}, @DDR> = dense<1> : tensor<64x1x1x4xsi32>, [#const.Reorder<#NHWC>, #const.RelocateWeightsTable<weightsPtr=[[[WEIGHTS_ADDR]], [[WEIGHTS_ADDR]], [[WEIGHTS_ADDR]], [[WEIGHTS_ADDR]]], sparsityPtr=16777215 : i64, offsets=[0, 0, 0, 0], weightsTableSize=1024 : i64, weightsElemBitSize=16 : i64, channelOffset=0 : i64, originalOC=0 : i64>]
...
func.func @PatchWeightTableWithDistributedBuffersWithSpilling(%arg0: !Input_DDR) -> !Output_DDR {
    ...
    // CHECK:       [[WEIGHT_TABLE_BUF_DDR:%.+]] = VPURT.DeclareBuffer <DDR> <0> -> memref<64x1x1x4xsi32, {order = #NHWC}, @DDR>
...
!WeightsTable_Stub = memref<32x1x1x4xsi32>
...
func.func @PatchWeightTableI4WeightsOnly() -> memref<256x1x1x1xsi32, [@CMX_NN, 0]> {
    %weight_table = VPURT.DeclareBuffer <CMX_NN> [0] <1024> -> memref<256x1x1x1xsi32, [@CMX_NN, 0]>
    %weights = VPURT.DeclareBuffer <CMX_NN> [0] <4096> -> memref<64x16x3x3x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>
    %weight_table_const = const.Declare memref<256x1x1x1xsi32> = dense<1> : tensor<256x1x1x1xsi32>
    ...
    %5 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [3, 3], kernel_strides = [2, 2], task_type = #VPUIP.nce_task_type<CONV>}>
        input(%in : memref<1x16x59x227xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        weights(%weights : memref<64x16x3x3x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>)
        weight_table(%weight_table : memref<256x1x1x1xsi32, [@CMX_NN, 0]>)
        parent_input(%in : memref<1x16x59x227xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        parent_output(%out : memref<1x64x29x113xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        outputs(%out : memref<1x64x29x113xf16, {order = #NHWC}, [@CMX_NN, 0]>)
        -> memref<1x64x29x113xf16, {order = #NHWC}, [@CMX_NN, 0]> variants : {
      DPUTask {outEnd = [112, 4, 63], mpe_mode = #VPU.mpe_mode<VECTOR_FP16>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, outStart = [0, 0, 0]}
    } PPE :  {
    }
    ...
    // CHECK:       [[WEIGHT_TABLE_BUF:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <1024> -> memref<256x1x1x1xsi32, [@CMX_NN, 0]>
    // CHECK:       [[WEIGHTS_BUF:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <[[WEIGHTS_ADDR:[^>]+]]> -> memref<64x16x3x3x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    // CHECK:       [[NDMA_OP:.+]] = VPUIP.NNDMA inputs([[CONST]] : memref<256x1x1x1xsi32>) outputs([[WEIGHT_TABLE_BUF]] : memref<256x1x1x1xsi32, [@CMX_NN, 0]>) -> memref<256x1x1x1xsi32, [@CMX_NN, 0]>
    // CHECK:       [[NCE_CLUST_TASK_OP:.+]] = VPUIP.NCEClusterTask
    // CHECK-SAME:  weights([[WEIGHTS_BUF]] : memref<64x16x3x3x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>)
    // CHECK-SAME:  weight_table([[WEIGHT_TABLE_BUF]] : memref<256x1x1x1xsi32, [@CMX_NN, 0]>)
}

// -----

func.func @PatchWeightTableWithSubViews() -> memref<64x1x1x4xsi32> {
    %cst_weight_table = const.Declare memref<64x1x1x4xsi32> = dense<1> : tensor<64x1x1x4xsi32>

    %weights = VPURT.DeclareBuffer <CMX_NN> <593920> -> !VPUIP.DistributedBuffer<512x448x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, @CMX_NN, {mode = "DUPLICATED", num_tiles = [2, 1, 1, 1], num_clusters = 2 : i64, alignment = [16, 1, 1, 1]}>
    %weight_table = VPURT.DeclareBuffer <CMX_NN> <0> -> !VPUIP.DistributedBuffer<64x1x1x4xsi32, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, @CMX_NN, {mode = "SEGMENTED", num_tiles = [2, 1, 1, 1], num_clusters = 2 : i64, alignment = [16, 1, 1, 1]}>
    %out = VPURT.DeclareBuffer <CMX_NN> <0> -> !VPUIP.DistributedBuffer<1x64x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, @CMX_NN, {mode = "DUPLICATED|SEGMENTED", num_tiles = [1, 2, 1, 1], num_clusters = 2 : i64, alignment = [1, 16, 1, 1]}>
    %in = VPURT.DeclareBuffer <CMX_NN> <0> -> !VPUIP.DistributedBuffer<1x448x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64, alignment = [1, 16, 1, 1]}>

    %dma = VPUIP.NNDMA <{port = 0 : i64}> inputs(%cst_weight_table : memref<64x1x1x4xsi32>) outputs(%weight_table : !VPUIP.DistributedBuffer<64x1x1x4xsi32, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, @CMX_NN, {mode = "SEGMENTED", num_tiles = [2, 1, 1, 1], num_clusters = 2 : i64, alignment = [16, 1, 1, 1]}>) -> !VPUIP.DistributedBuffer<64x1x1x4xsi32, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, @CMX_NN, {mode = "SEGMENTED", num_tiles = [2, 1, 1, 1], num_clusters = 2 : i64, alignment = [16, 1, 1, 1]}>
    ...
    %2 = VPUIP.NCEClusterTask {minimumHardwareExecutionCost = 716 : i64, resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [1, 1], kernel_strides = [1, 1], task_type = #VPUIP.nce_task_type<CONV>}>
        ...
        parent_input(%in : !VPUIP.DistributedBuffer<1x448x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64, alignment = [1, 16, 1, 1]}>)
        parent_output(%out : !VPUIP.DistributedBuffer<1x64x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, @CMX_NN, {mode = "DUPLICATED|SEGMENTED", num_tiles = [1, 2, 1, 1], num_clusters = 2 : i64, alignment = [1, 16, 1, 1]}>)
        outputs(%out : !VPUIP.DistributedBuffer<1x64x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, @CMX_NN, {mode = "DUPLICATED|SEGMENTED", num_tiles = [1, 2, 1, 1], num_clusters = 2 : i64, alignment = [1, 16, 1, 1]}>)
        -> !VPUIP.DistributedBuffer<1x64x1x1xf16, affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>, @CMX_NN, {mode = "DUPLICATED|SEGMENTED", num_tiles = [1, 2, 1, 1], num_clusters = 2 : i64, alignment = [1, 16, 1, 1]}> variants : {
      DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [0, 0, 31], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
    } PPE : {
    }
...
// CHECK-LABEL: @PatchWeightTable5D
func.func @PatchWeightTable5D() ->  memref<4x1008x1x1x4xsi32, [@CMX_NN, 0]> {
    %weight_table = VPURT.DeclareBuffer <CMX_NN> [0] <540288> -> memref<4x1008x1x1x4xsi32, [@CMX_NN, 0]>
    %weights = VPURT.DeclareBuffer <CMX_NN> [0] <395136> -> memref<4x1008x64x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]>
    %weight_table_const = const.Declare memref<4x1008x1x1x4xsi32> = dense<1> : tensor<4x1008x1x1x4xsi32>

    %in = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<4x1x64x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]>
    %out = VPURT.DeclareBuffer <CMX_NN> [0] <556416> -> memref<4x1x1008x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]>
    ...
    %5 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{
        ...
        input(%in : memref<4x1x64x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]>)
        weights(%weights : memref<4x1008x64x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]>)
        weight_table(%weight_table : memref<4x1008x1x1x4xsi32, [@CMX_NN, 0]>)
        parent_input(%in : memref<4x1x64x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]>)
        parent_output(%out : memref<4x1x1008x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]>)
        outputs(%out : memref<4x1x1008x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]>)
            -> memref<4x1x1008x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]> variants :  {
        DPUTask {outEnd = [1, 0, 1007], mpe_mode = #VPU.mpe_mode<VECTOR_FP16>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, outStart = [0, 0, 0]}
    } PPE :  {
    }
    ...
    // CHECK:       [[WEIGHTS_BUF:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <[[WEIGHTS_ADDR:[^>]+]]> -> memref<4x1008x64x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]>
    ...
    // CHECK:       [[NCE_CLUST_TASK_OP:.+]] = VPUIP.NCEClusterTask
    // CHECK-SAME:  weights([[WEIGHTS_BUF]] : memref<4x1008x64x1x1xf16, {order = #GNHWC}, [@CMX_NN, 0]>)
