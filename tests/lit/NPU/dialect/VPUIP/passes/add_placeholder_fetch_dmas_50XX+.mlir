...
// RUN: vpux-opt --init-compiler="platform=%platform% allow-custom-values=true" --split-input-file --add-placeholder-fetch-dmas --canonicalize %s | FileCheck %s
...
module @PlaceholderForFetchWithMultipleBocks {
  ...
  config.PipelineOptions @Options {
    config.Option @config.MetadataMaxVariantCount : 16 : ui64
  ...
  // CHECK: VPURT.Task waits([[BAR0]], [[BAR2]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR3]] : !VPURT.Barrier) attributes {"sync-task"}

  // CHECK: VPURT.Task waits([[BAR3]] : !VPURT.Barrier) updates([[BAR4]], [[BAR5]] : !VPURT.Barrier, !VPURT.Barrier)
  ...
  // CHECK: VPURT.Task waits([[BAR3]], [[BAR6]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR4]] : !VPURT.Barrier)

  // CHECK: VPURT.Task waits([[BAR3]] : !VPURT.Barrier) updates([[BAR4]], [[BAR7]] : !VPURT.Barrier, !VPURT.Barrier)
  ...
  // CHECK: VPURT.Task waits([[BAR4]], [[BAR3]], [[BAR8]] : !VPURT.Barrier, !VPURT.Barrier, !VPURT.Barrier) updates([[BAR9]] : !VPURT.Barrier) attributes {"sync-task"}

  // CHECK: VPURT.Task waits([[BAR9]] : !VPURT.Barrier) updates([[BAR10]], [[BAR11]] : !VPURT.Barrier, !VPURT.Barrier)

  // Group 5
  // CHECK: VPURT.Task waits([[BAR11]] : !VPURT.Barrier) updates([[BAR12]] : !VPURT.Barrier) {
  // CHECK: VPUIP.FetchDMA <{port = 0 : i64}>
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  ...
  // CHECK: VPURT.Task waits([[BAR9]], [[BAR12]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR10]] : !VPURT.Barrier)
...
module @LegalizeWithMultiTileParent attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  ...
  // CHECK: VPURT.Task waits([[BAR2]] : !VPURT.Barrier) updates([[BAR3]], [[BAR5]] : !VPURT.Barrier, !VPURT.Barrier)
  ...
  // Group 2 Tile 1
  // CHECK: VPURT.Task waits([[BAR3]], [[BAR7]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR8]] : !VPURT.Barrier)
  ...
  func.func @main() -> memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]> {
    ...
    VPURT.Task waits(%bar1: !VPURT.Barrier) updates(%bar2: !VPURT.Barrier)
    {
          VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_relu
              ...
              outputs(%buf0 as %output: memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]>) on tile 0 list 0 -> memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]> {
