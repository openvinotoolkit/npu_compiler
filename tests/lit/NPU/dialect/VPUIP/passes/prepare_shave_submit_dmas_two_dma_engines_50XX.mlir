...
module @Activation attributes {config.compilationMode = #config.compilation_mode<DefaultHW>, config.elf_version = #config.version<2 : 0 : 0>, config.platform = #config.platform<NPU5010>, config.revisionID = #config.revision_id<REVISION_NONE>} {
  ...
  // CHECK-DAG: VPUIP.FetchDMA <{port = 1 : i64}> inputs(%6 : memref<0x0x0x0xi32, @DDR>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 0 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 2 : i64>) -> memref<0x0x0x0xi32, @DDR>
  ...
  // CHECK-DAG: VPUIP.FetchDMA <{port = 1 : i64}> inputs(%6 : memref<0x0x0x0xi32, @DDR>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 0 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 3 : i64>) -> memref<0x0x0x0xi32, @DDR>
  ...
  // CHECK-DAG: VPUIP.FetchDMA <{port = 1 : i64}> inputs(%6 : memref<0x0x0x0xi32, @DDR>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 1 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 6 : i64>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK-DAG: VPUIP.FetchDMA <{port = 0 : i64}> inputs(%5 : memref<0x0x0x0xi32, @DDR>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 1 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 4 : i64>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK-DAG: VPUIP.FetchDMA <{port = 1 : i64}> inputs(%6 : memref<0x0x0x0xi32, @DDR>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 1 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 7 : i64>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK-DAG: VPUIP.FetchDMA <{port = 0 : i64}> inputs(%5 : memref<0x0x0x0xi32, @DDR>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 1 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 5 : i64>) -> memref<0x0x0x0xi32, @DDR>
  ...
  // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*logical_task = 0 : i64.*port = 0 : i64.*}} inputs(%7 : memref<0x0x0x0xi32, [@CMX_NN, 0]>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*logical_task = 0 : i64.*port = 1 : i64.*}} inputs(%6 : memref<0x0x0x0xi32, @DDR>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*logical_task = 0 : i64.*port = 1 : i64.*}} inputs(%8 : memref<0x0x0x0xi32, [@CMX_NN, 0]>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  ...
  // CHECK: VPURT.Task waits([[BAR3]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*port = 0 : i64.*}} inputs(%7 : memref<0x0x0x0xi32, [@CMX_NN, 0]>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK: VPURT.Task waits([[BAR3]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*port = 1 : i64.*}} inputs(%6 : memref<0x0x0x0xi32, @DDR>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK: VPURT.Task waits([[BAR3]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*port = 1 : i64.*}} inputs(%8 : memref<0x0x0x0xi32, [@CMX_NN, 0]>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
}
