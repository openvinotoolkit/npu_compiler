...
// RUN: vpux-opt --split-input-file --platform=%platform% --prepare-shave-submit-dmas="num-dma-engines=2" %s | FileCheck %s
...
module @Activation attributes {config.compilationMode = #config.compilation_mode<DefaultHW>, config.elf_version = #config.version<2 : 0 : 0>, config.platform = #config.platform<NPU5010>, config.revisionID = #config.revision_id<REVISION_NONE>} {
  ...
  // CHECK-DAG: VPUIP.FetchDMA <{port = 0 : i64}> inputs([[DDR_IN0:%.+]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT:%.+]] : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 0 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 0 : i64>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK-DAG: VPUIP.FetchDMA <{port = 0 : i64}> inputs([[DDR_IN0]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 0 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 1 : i64>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK-DAG: VPUIP.FetchDMA <{port = 1 : i64}> inputs([[DDR_IN1:%.+]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 0 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 2 : i64>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK-DAG: VPUIP.FetchDMA <{port = 1 : i64}> inputs([[DDR_IN1]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 0 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 3 : i64>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK-DAG: VPUIP.FetchDMA <{port = 0 : i64}> inputs([[DDR_IN0]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 1 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 4 : i64>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK-DAG: VPUIP.FetchDMA <{port = 0 : i64}> inputs([[DDR_IN0]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 1 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 5 : i64>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK-DAG: VPUIP.FetchDMA <{port = 1 : i64}> inputs([[DDR_IN1]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 1 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 6 : i64>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK-DAG: VPUIP.FetchDMA <{port = 1 : i64}> inputs([[DDR_IN1]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) fetch_dma(<<DMA_NN>, tile = 0 : i64, list = 1 : i64, fetchType = <SingleDescriptor>, logicalTaskIdx = 0 : i64, descId = 7 : i64>) -> memref<0x0x0x0xi32, @DDR>

  // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier) updates([[BAR2]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*port = 0 : i64.*}} inputs([[DDR_IN0]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier) updates([[BAR2]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*port = 0 : i64.*}} inputs([[CMX_IN0:%.+]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier) updates([[BAR2]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*port = 1 : i64.*}} inputs([[DDR_IN1]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier) updates([[BAR2]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*port = 1 : i64.*}} inputs([[CMX_IN1:%.+]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>

  // CHECK: VPURT.Task waits([[BAR0]], [[BAR2]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR3]], [[BAR4]] : !VPURT.Barrier, !VPURT.Barrier)
  // CHECK: VPUIP.SW.Kernel {{.*}} logical_task = 0 : i64{{.*}}skipDescIds = [0, 1, 2, 3]{{.*}} on tile 0 list 0
  // CHECK: VPURT.Task waits([[BAR0]], [[BAR2]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR3]], [[BAR4]] : !VPURT.Barrier, !VPURT.Barrier)
  // CHECK: VPUIP.SW.Kernel {{.*}} logical_task = 0 : i64{{.*}}skipDescIds = [4, 5, 6, 7]{{.*}} on tile 0 list 1

  // Release
  // CHECK: VPURT.Task waits([[BAR4]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*logical_task = 0 : i64.*port = 0 : i64.*}} inputs([[DDR_IN0]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK: VPURT.Task waits([[BAR4]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*logical_task = 0 : i64.*port = 0 : i64.*}} inputs([[CMX_IN0]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK: VPURT.Task waits([[BAR4]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*logical_task = 0 : i64.*port = 1 : i64.*}} inputs([[DDR_IN1]] : memref<0x0x0x0xi32, @DDR>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
  // CHECK: VPURT.Task waits([[BAR4]] : !VPURT.Barrier)
  // CHECK: VPUIP.SyncDMA {{.*logical_task = 0 : i64.*port = 1 : i64.*}} inputs([[CMX_IN1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>) outputs([[DUMMY_OUT]] : memref<0x0x0x0xi32, @DDR>) -> memref<0x0x0x0xi32, @DDR>
}
