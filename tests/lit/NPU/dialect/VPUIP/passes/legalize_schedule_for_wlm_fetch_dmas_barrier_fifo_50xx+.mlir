...
// RUN: vpux-opt --init-compiler="vpu-arch=%arch% allow-custom-values=true" --split-input-file --legalize-schedule-for-partial-wlm --canonicalize %s | FileCheck %s
...
module @LegalizeGroupThreeWithSharedBarrierMultiTile attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  ...
  // End DMA
  // CHECK: VPURT.Task waits(%5 : !VPURT.Barrier) updates(%6 : !VPURT.Barrier)
...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
#NWCH = affine_map<(d0, d1, d2, d3) -> (d0, d3, d1, d2)>
module @LegalizeGroupThreeWithSharedBarrierMultiTileAsymmetry attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  config.PipelineOptions @Options {
    config.Option @config.MetadataMaxVariantCount : 8
    config.Option @config.MetadataMaxInvariantCount : 4
    config.Option @config.MetadataMaxKernelInvocationCount : 4
  ...
  // End DMA
  // CHECK: VPURT.Task waits(%6 : !VPURT.Barrier) updates(%7 : !VPURT.Barrier)
