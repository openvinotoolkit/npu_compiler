...
// RUN: vpux-opt --init-compiler="platform=%platform% allow-custom-values=true" --split-input-file --add-placeholder-fetch-dmas-pwlm --canonicalize %s | FileCheck %s
...
module @LegalizeWithMultiTile attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  ...
  func.func @main(%arg0: memref<1x3x224x224xf16, @DDR>, %arg1: memref<1x64x56x56xf16, @DDR>) -> memref<1x64x56x56xf16, @DDR> {
    ...
    %122 = VPURT.DeclareBuffer <CMX_NN> [1] <154560> -> memref<1x224x4x114x!qElemType, {order = #NWCH}, [@CMX_NN, 1]>
    ...
    VPURT.Task waits(%0 : !VPURT.Barrier) updates(%1 : !VPURT.Barrier) {
      %27 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_permute_quantize, is_superdense, task_type = #VPUIP.nce_task_type<ELTWISE>}> input(%23 : memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%22 : memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_input(%21 : !VPUIP.DistributedBuffer<1x224x3x224xf16, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>) parent_output(%11 : !VPUIP.DistributedBuffer<1x224x4x224x!qElemType, #NWCH, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64, equal_memory_and_compute_view}>) outputs(%122 : memref<1x224x4x114x!qElemType, {order = #NWCH}, [@CMX_NN, 1]>) -> memref<1x224x4x114x!qElemType, {order = #NWCH}, [@CMX_NN, 1]> variants : {
  ...
  // CHECK: [[BUF1:%.+]] = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<0x0x0x0xi32, [@CMX_NN, 0]>

  // CHECK: VPURT.Task {
  // CHECK: VPUIP.FetchDMA <{port = 0 : i64}>
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 1 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 0 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 1 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 1 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 0 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 1 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 2 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 1 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 2 : i64>)
  // CHECK: }

  // Group 1 Tile 0
  // CHECK: VPURT.Task waits([[BAR1]], [[BAR4]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR6]] : !VPURT.Barrier)
  // Group 1 Tile 1
  // CHECK: VPURT.Task waits([[BAR1]], [[BAR5]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR6]] : !VPURT.Barrier)
...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
#NWCH = affine_map<(d0, d1, d2, d3) -> (d0, d3, d1, d2)>
module @LegalizeMultiTileAsymmetry attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  config.PipelineOptions @Options {
    config.Option @config.MetadataMaxVariantCount : 8
    config.Option @config.MetadataMaxInvariantCount : 4
    config.Option @config.MetadataMaxKernelInvocationCount : 4
  ...
  // CHECK: VPURT.Task waits([[BAR0]] : !VPURT.Barrier) updates([[BAR1]], [[BAR2]] : !VPURT.Barrier, !VPURT.Barrier)
  // Group 0 Tile 1
  // CHECK: VPURT.Task waits([[BAR0]] : !VPURT.Barrier) updates([[BAR3]], [[BAR4]] : !VPURT.Barrier, !VPURT.Barrier)
  ...
  // Group 1 Tile 0
  // CHECK: VPURT.Task waits([[BAR1]], [[BAR5]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR7]] : !VPURT.Barrier)
  // Group 1 Tile 1
  // CHECK: VPURT.Task waits([[BAR3]], [[BAR6]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR7]] : !VPURT.Barrier)
...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
#NWCH = affine_map<(d0, d1, d2, d3) -> (d0, d3, d1, d2)>
module @PlaceholderForFetchWithMultipleBocks {
  ...
  config.ExecutorResource 1 of @M2I
  config.ExecutorResource 2 of @DMA_NN
  config.MemoryResource 67108864000 bytes of @DDR {VPU.bandwidth = 64 : i64, VPU.derateFactor = 6.000000e-01 : f64}
  config.PipelineOptions @Options {
    config.Option @config.MetadataMaxVariantCount : 8 : ui64
    config.Option @config.MetadataMaxInvariantCount : 4 : ui64
  }
  func.func @main(%arg0: memref<1x3x224x224xf16, @DDR>, %arg1: memref<1x64x56x56xf16, @DDR>) -> memref<1x64x56x56xf16, @DDR> {
    %0 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    %1 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    ...
    %3 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    %4 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    %5 = VPURT.DeclareBuffer <CMX_NN> <154560> -> !VPUIP.DistributedBuffer<1x224x4x224x!qElemType, #NWCH, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64, equal_memory_and_compute_view}>
    %6 = VPURT.DeclareBuffer <CMX_NN> [0] <154560> -> memref<1x224x4x114x!qElemType, {order = #NWCH}, [@CMX_NN, 0]>
    %8 = VPURT.DeclareBuffer <CMX_NN> <278528> {swizzlingKey = 5 : i64} -> !VPUIP.DistributedBuffer<1x64x112x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>
    %9 = VPURT.DeclareBuffer <CMX_NN> [0] <278528> {swizzlingKey = 5 : i64} -> memref<1x64x56x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>
    %10 = VPURT.DeclareBuffer <CMX_NN> [0] <278528> {swizzlingKey = 5 : i64} -> memref<1x64x56x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>
    %11 = VPURT.DeclareBuffer <CMX_NN> <0> {swizzlingKey = 5 : i64} -> !VPUIP.DistributedBuffer<1x64x56x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>
    %12 = VPURT.DeclareBuffer <CMX_NN> [0] <0> {swizzlingKey = 5 : i64} -> memref<1x64x28x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>
    %13 = VPURT.DeclareBuffer <CMX_NN> <0> -> !VPUIP.DistributedBuffer<1x224x3x224xf16, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>
    %14 = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %15 = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %16 = VPURT.DeclareBuffer <CMX_NN> [0] <257600> -> memref<64x16x7x7x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>
    %17 = VPURT.DeclareBuffer <CMX_NN> <154560> -> !VPUIP.DistributedBuffer<1x16x224x224x!qElemType2, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>
    %18 = VPURT.DeclareBuffer <CMX_NN> [0] <154560> -> memref<1x16x114x224x!qElemType2, {order = #NHWC}, [@CMX_NN, 0]>
    VPURT.Task updates(%0 : !VPURT.Barrier) {
      %19 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_permute_quantize, is_superdense, task_type = #VPUIP.nce_task_type<ELTWISE>}> input(%15 : memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%14 : memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_input(%13 : !VPUIP.DistributedBuffer<1x224x3x224xf16, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>) parent_output(%5 : !VPUIP.DistributedBuffer<1x224x4x224x!qElemType, #NWCH, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64, equal_memory_and_compute_view}>) outputs(%6 : memref<1x224x4x114x!qElemType, {order = #NWCH}, [@CMX_NN, 0]>) -> memref<1x224x4x114x!qElemType, {order = #NWCH}, [@CMX_NN, 0]> variants : {
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
    ...
    VPURT.Task waits(%0 : !VPURT.Barrier) updates(%1 : !VPURT.Barrier) attributes {"sync-task"} {
      %19 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_permute_quantize, is_superdense, task_type = #VPUIP.nce_task_type<ELTWISE>}> input(%15 : memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%14 : memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_input(%13 : !VPUIP.DistributedBuffer<1x224x3x224xf16, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>) parent_output(%5 : !VPUIP.DistributedBuffer<1x224x4x224x!qElemType, #NWCH, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64, equal_memory_and_compute_view}>) outputs(%6 : memref<1x224x4x114x!qElemType, {order = #NWCH}, [@CMX_NN, 0]>) -> memref<1x224x4x114x!qElemType, {order = #NWCH}, [@CMX_NN, 0]> variants : {
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [113, 2, 223], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
    ...
    VPURT.Task waits(%2, %1 : !VPURT.Barrier, !VPURT.Barrier) updates(%3 : !VPURT.Barrier) attributes {"sync-task"} {
      %19 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{cm_sp_pattern = 7 : i64, input_channels_compression, kernel_padding = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>, kernel_size = [7, 7], kernel_strides = [2, 2], task_type = #VPUIP.nce_task_type<CONV>}> input(%18 : memref<1x16x114x224x!qElemType2, {order = #NHWC}, [@CMX_NN, 0]>) weights(%16 : memref<64x16x7x7x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>) parent_input(%17 : !VPUIP.DistributedBuffer<1x16x224x224x!qElemType2, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>) parent_output(%8 : !VPUIP.DistributedBuffer<1x64x112x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) outputs(%10 : memref<1x64x56x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>) -> memref<1x64x56x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]> variants : {
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [111, 55, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [111, 55, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [111, 55, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [111, 55, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>}
    ...
    VPURT.Task waits(%3 : !VPURT.Barrier) updates(%4 : !VPURT.Barrier) {
      %19 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_segmented, kernel_padding = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>, kernel_size = [3, 3], kernel_strides = [2, 2], task_type = #VPUIP.nce_task_type<MAXPOOL>}> input(%9 : memref<1x64x56x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>) parent_input(%8 : !VPUIP.DistributedBuffer<1x64x112x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) parent_output(%11 : !VPUIP.DistributedBuffer<1x64x56x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) outputs(%12 : memref<1x64x28x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>) -> memref<1x64x28x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]> variants : {
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [55, 27, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [55, 27, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [55, 27, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [55, 27, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>}
  ...
  // CHECK: VPURT.Task waits([[BAR0]], [[BAR2]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR3]], [[BAR4]] : !VPURT.Barrier, !VPURT.Barrier) attributes {"sync-task"}

  // CHECK: VPURT.Task waits([[BAR4]] : !VPURT.Barrier) updates([[BAR5]] : !VPURT.Barrier) {
  // CHECK: VPUIP.FetchDMA <{port = 0 : i64}>
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 3 : i64>)
  // CHECK: }

  // Group 2 Tile 0
  // CHECK: VPURT.Task waits([[BAR3]], [[BAR5]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR6]], [[BAR7]] : !VPURT.Barrier, !VPURT.Barrier)

  // CHECK: VPURT.Task waits([[BAR7]] : !VPURT.Barrier) updates([[BAR8]] : !VPURT.Barrier)  {
  // CHECK: VPUIP.FetchDMA <{port = 0 : i64}>
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 4 : i64>)
  // CHECK: }

  // Group 2 Tile 0
  // CHECK: VPURT.Task waits([[BAR6]], [[BAR3]], [[BAR8]] : !VPURT.Barrier, !VPURT.Barrier, !VPURT.Barrier) updates([[BAR9]], [[BAR10]] : !VPURT.Barrier, !VPURT.Barrier) attributes {"sync-task"}

  // CHECK: VPURT.Task waits([[BAR10]] : !VPURT.Barrier) updates([[BAR11]] : !VPURT.Barrier)  {
  // CHECK: VPUIP.FetchDMA <{port = 0 : i64}>
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 0 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 5 : i64>)
  // CHECK: }
}
...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
#NWCH = affine_map<(d0, d1, d2, d3) -> (d0, d3, d1, d2)>
module @LegalizeWithMultiTileParent attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  config.PipelineOptions @Options {
    config.Option @config.MetadataMaxVariantCount : 8
    config.Option @config.MetadataMaxInvariantCount : 4
    config.Option @config.MetadataMaxKernelInvocationCount : 4
  ...
  func.func @main(%arg0: memref<1x3x224x224xf16, @DDR>, %arg1: memref<1x64x56x56xf16, @DDR>) -> memref<1x64x56x56xf16, @DDR> {
    ...
    %3 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    %4 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    %51 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    %52 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    %5 = VPURT.DeclareVirtualBarrier -> !VPURT.Barrier
    ...
    VPURT.Task waits(%4 : !VPURT.Barrier) updates(%51 : !VPURT.Barrier) {
      %27 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_segmented, kernel_padding = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>, kernel_size = [3, 3], kernel_strides = [2, 2], task_type = #VPUIP.nce_task_type<MAXPOOL>}> input(%15 : memref<1x64x56x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>) parent_input(%14 : !VPUIP.DistributedBuffer<1x64x112x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) parent_output(%17 : !VPUIP.DistributedBuffer<1x64x56x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) outputs(%18 : memref<1x64x28x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>) -> memref<1x64x28x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]> variants : {
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [55, 27, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [55, 27, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>}
      } PPE : {
    ...
    VPURT.Task waits(%51 : !VPURT.Barrier) updates(%52 : !VPURT.Barrier) {
      %27 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_segmented, kernel_padding = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>, kernel_size = [3, 3], kernel_strides = [2, 2], task_type = #VPUIP.nce_task_type<MAXPOOL>}> input(%15 : memref<1x64x56x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>) parent_input(%14 : !VPUIP.DistributedBuffer<1x64x112x112x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) parent_output(%17 : !VPUIP.DistributedBuffer<1x64x56x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) outputs(%18 : memref<1x64x28x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>) -> memref<1x64x28x56x!qElemType1, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]> variants : {
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [55, 27, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>}
        DPUTask {cluster_id = 0 : i64, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [55, 27, 63], outStart = [0, 0, 0], pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>}
      } PPE : {
    ...
    VPURT.Task waits(%52 : !VPURT.Barrier) updates(%5 : !VPURT.Barrier) {
      %27 = VPUIP.NNDMA {port = 0 : i64} inputs(%20 : memref<1x64x28x56xf16, [@CMX_NN, 0]>) outputs(%8 : memref<1x64x28x56xf16, {order = #NCHW, strides = [200704, 3136, 56, 1]}, @DDR>) -> memref<1x64x28x56xf16, {order = #NCHW, strides = [200704, 3136, 56, 1]}, @DDR>
    }
    return %arg1 : memref<1x64x56x56xf16, @DDR>
  }
  ...
  // Group 0 Tile 0 first task
  // CHECK: VPURT.Task waits([[BAR0]] : !VPURT.Barrier) updates([[BAR1]] : !VPURT.Barrier)
  // CHECK-NEXT: VPUIP.NCEClusterTask

  // Group 0 Tile 1 first task
  // CHECK: VPURT.Task waits([[BAR0]] : !VPURT.Barrier) updates([[BAR1]] : !VPURT.Barrier)
  // CHECK-NEXT: VPUIP.NCEClusterTask

  // Group 0 Tile 0 last task
  // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier) updates([[BAR2]], [[BAR3]] : !VPURT.Barrier, !VPURT.Barrier)
  // CHECK-NEXT: VPUIP.NCEClusterTask

  // Group 0 Tile 1 last task
  // CHECK: VPURT.Task waits([[BAR1]] : !VPURT.Barrier) updates([[BAR2]], [[BAR4]] : !VPURT.Barrier, !VPURT.Barrier)
  // CHECK-NEXT: VPUIP.NCEClusterTask
  ...
  // Group 1 Tile 0 first task
  // CHECK: VPURT.Task waits([[BAR2]] : !VPURT.Barrier) updates([[BAR6]] : !VPURT.Barrier)
  // CHECK-NEXT: VPUIP.NCEClusterTask

  // Group 1 Tile 1 first task
  // CHECK: VPURT.Task waits([[BAR2]] : !VPURT.Barrier) updates([[BAR6]] : !VPURT.Barrier)
  // CHECK-NEXT: VPUIP.NCEClusterTask

  // CHECK: VPURT.Task waits([[BAR4]] : !VPURT.Barrier) updates([[BAR7]] : !VPURT.Barrier)
  // CHECK: VPUIP.FetchDMA <{port = 0 : i64}>
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<DPU>, tile = 1 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 2 : i64>)
  ...
  // Group 1 Tile 0 last task
  // CHECK: VPURT.Task waits([[BAR6]], [[BAR5]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR8]] : !VPURT.Barrier)
  // CHECK-NEXT: VPUIP.NCEClusterTask

  // Group 1 Tile 1 last task
  // CHECK: VPURT.Task waits([[BAR6]], [[BAR7]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR8]] : !VPURT.Barrier)
  // CHECK-NEXT: VPUIP.NCEClusterTask

  // Group 2 Tile 0 first task
  // CHECK: VPURT.Task waits([[BAR8]] : !VPURT.Barrier) updates([[BAR9]] : !VPURT.Barrier)
  // CHECK-NEXT: VPUIP.NCEClusterTask

  // Group 2 Tile 1 first task
  // CHECK: VPURT.Task waits([[BAR8]] : !VPURT.Barrier) updates([[BAR9]] : !VPURT.Barrier)
  // CHECK-NEXT: VPUIP.NCEClusterTask
...
module @DmaAndShvGraphFetchReuseBars attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  config.PipelineOptions @Options {
  ...
  func.func @main() -> memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]> {
    ...
    %bar9 = VPURT.DeclareVirtualBarrier <{isFinalBarrier}> -> !VPURT.Barrier
    ...
    VPURT.Task waits(%bar7: !VPURT.Barrier) updates(%bar8: !VPURT.Barrier)
    {
          VPUIP.SW.Kernel {resultSegmentSizes = array<i32: 1, 0, 0>} @VPU.SW::@builtin_relu
              inputs(%buf0 as %input: memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]>)
              outputs(%buf0 as %output: memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]>) on tile 0 -> memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]> {
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<SHAVE_ACT>, tile = 0 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 0 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<SHAVE_ACT>, tile = 0 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 1 : i64>)
  // CHECK: }
  ...
  // CHECK-SAME: inputs([[BUF0]] : memref<0x0x0x0xi32, @DDR>)
  // CHECK-SAME: outputs([[BUF1]] : memref<0x0x0x0xi32, [@CMX_NN, 0]>)
  // CHECK-SAME: fetch_dma(<<SHAVE_ACT>, tile = 0 : i64, list = 0 : i64, fetchType = <DescriptorGroup>, group = 2 : i64>)
  // CHECK: }
