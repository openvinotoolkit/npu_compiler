...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% allow-custom-values=true enable-sw-kernel-fifo-per-shave-engine=false" --add-sw-kernel-instruction-prefetch="minimum-shave-init-time=5000 minimum-shave-idle-time-for-prefetch=0" %s | FileCheck %s
...
module @prefetchSingleKernel attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
    ...
    module @VPU.SW {
      ...
      func.func nested @cache_prefetch() attributes {VPU.task_type = @CACHE_PREFETCH}
    ...
    func.func @main(%arg0: memref<1x3x62x62xui8, @DDR>) -> memref<1x3x62x62xui8, @DDR> {
      ...
      // CHECK: [[BAR_0:%.+]] = VPURT.DeclareVirtualBarrier
      // CHECK: [[BAR_1:%.+]] = VPURT.DeclareVirtualBarrier
      // CHECK: [[BAR_2:%.+]] = VPURT.DeclareVirtualBarrier
      // CHECK: [[BAR_3:%.+]] = VPURT.DeclareVirtualBarrier
...
module @prefetchMultipleKernels attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
    ...
    module @VPU.SW {
      ...
      func.func nested @builtin_Convert(memref<*xf32, [@CMX_NN, 0]>, memref<*xf16, [@CMX_NN, 0]>) attributes {VPU.kernel_code = "convert.cpp", VPU.kernel_entry = "convert", VPU.task_type = @COMPUTE}
...
module @dontPrefetchKernelsIfThereIsNoTimeAtStart attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
    ...
    module @VPU.SW {
      func.func nested @builtin_SoftMax(memref<*xf16, @CMX_NN>, memref<*xf16, @CMX_NN>, i64, i64) attributes {VPU.kernel_code = "softmax.cpp", VPU.kernel_entry = "softmax", VPU.task_type = @COMPUTE}
      func.func nested @runtime() attributes {VPU.kernel_code = "nnActEntry"}
      // CHECK-NOT: func.func nested @cache_prefetch() attributes {VPU.task_type = @CACHE_PREFETCH}
    }
    config.Resources 2 of @NCE at 1.700000e+03 MHz {
      builtin.module @ReservedMemory {
        module @DmaProfilingReservedMemory {
