...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% compilation-mode=DefaultHW allow-custom-values=true" --strategy-manager %s | FileCheck %s
...
func.func @ConvAssignedSOH(%arg0 : tensor<1x64x28x28xf16, {order = #NHWC}>)->tensor<1x80x28x28xf16, {order = #NHWC}> {
    ...
    // CHECK:        [[VAL0:%.+]] = VPU.NCE.Convolution([[ARG_0]], [[WEIGHTS]], [[WEIGHTSTABLE]])
...
func.func @ConvAssignedSOK(%arg0 : tensor<1x64x1x1xf16, {order = #NHWC}>)->tensor<1x48x1x1xf16, {order = #NHWC}> {
    ...
    %0 = VPU.NCE.Convolution(%arg0, %cst_0, %cst) rawFilterShape [48, 64, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        ppe = #VPU.PPEStub<>,
        pad = #VPU.Padding< left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64 >,
         strides = [ 1, 1 ]
...
func.func @DepthConvAssignedSOK(%arg0: tensor<1x32x112x112xf16, {order = #NHWC}>) -> tensor<1x32x112x112xf16, {order = #NHWC}> {
    %cst_0 = const.Declare tensor<32x16x1x1xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<32x1x1x3x3xf16>, [
    ...
    // CHECK:        [[VAL0:%.+]] = VPU.NCE.DepthConvolution([[ARG_0]], [[WEIGHTS]])
...
func.func @DepthConvAssignedSOK(%arg0: tensor<1x128x1x1xf16, {order = #NHWC}>) -> tensor<1x128x1x1xf16, {order = #NHWC}> {
    %cst_0 = const.Declare tensor<128x16x1x1xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<128x1x1x3x3xf16>,
      [
    ...
    %0 = VPU.NCE.DepthConvolution(%arg0, %cst_0) rawFilterShape [128, 1, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        ppe = #VPU.PPEStub<>,
        pad = #VPU.Padding< left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64 >,
         strides = [ 1, 1 ]
...
// CHECK-SAME: ([[ARG_0:%[^:]+]]: tensor<1x32x80x80xf16, {order = #NHWC}>, [[ARG_1:%[^:]+]]: tensor<1x32x80x80xf16, {order = #NHWC}>)
func.func @EltwiseAddAssignedSOH(%arg0
                                 : tensor<1x32x80x80xf16, {order = #NHWC}>, %arg1
                                 : tensor<1x32x80x80xf16, {order = #NHWC}>)
        ->tensor<1x32x80x80xf16, {order = #NHWC}> {
    %0 = VPU.NCE.Eltwise(%arg0, %arg1){resultSegmentSizes = array<i32: 1, 0, 0, 0>, op_type = #VPU.eltwise_type<ADD>,ppe = #VPU.PPEStub<>} : tensor<1x32x80x80xf16, {order = #NHWC}>,
      tensor<1x32x80x80xf16, {order = #NHWC}>->tensor<1x32x80x80xf16, {order = #NHWC}>
return %0 : tensor<1x32x80x80xf16, {order = #NHWC}>

    // CHECK:      [[VAL0:%.+]] = VPU.NCE.Eltwise([[ARG_0]], [[ARG_1]])
    // CHECK-SAME: {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>
...
func.func @TanhAssignedClustering(%arg0: tensor<1x4x1x512xf16, {order = #NCHW}>)->tensor<1x4x1x512xf16, {order = #NCHW}> {
    ...
    // CHECK:   [[ResultTANH:%.+]] = VPU.Tanh([[ARG_0]])
...
func.func @SoftMaxAssignedClustering(%arg0 : tensor<1x4x1x512xf16, {order = #NCHW}>)->tensor<1x4x1x512xf16, {order = #NCHW}> {
    ...
    // CHECK:   [[ResultSoftMax:%.+]] = VPU.SoftMax([[ARG_0]])
    // CHECK-SAME: multiClusterStrategy = #VPU.multi_cluster_strategy<Clustering>
...
func.func @InterpolateHalfPixelAssignedSOHOverlapped(%arg0: tensor<1x1x96x160xf16>) -> tensor<1x1x192x320xf16> {
    ...
    // CHECK:   [[INTERPOLATE:%.+]] = VPU.Interpolate([[ARG_0]])
...
func.func @CompressConvolutionAssignedSOHOverlapped(%arg0 : tensor<1x3x224x224xf16, {order = #NHWC}>)->tensor<1x64x112x112xf16, {order = #NHWC}> {
    ...
    // CHECK:        [[EXPAND:%.+]] = VPU.Expand([[ARG_0]]) {pads_begin = [0, 0, 0, 0], pads_end = [0, 1, 0, 0]} :
...
// CHECK-SAME: ([[ARG_0:%[^:]+]]: tensor<1x1x1x44xf16, {order = #NHWC}>, [[ARG_1:%[^:]+]]: tensor<1x1x1x1xf16, {order = #NHWC}>)
func.func @MultiplyAssignedClustering(%arg0 : tensor<1x1x1x44xf16, {order = #NHWC}>,
    ...
    // CHECK:      [[MULTIPLY:%.+]] = VPU.Multiply([[ARG_0]], [[ARG_1]])
    // CHECK-SAME: multiClusterStrategy = #VPU.multi_cluster_strategy<Clustering>
...
func.func @HSwishAssignedSplitOverKernel(%arg0
    ...
    // CHECK:   [[ResultHSwish:%.+]] = VPU.HSwish([[ARG_0]])
    // CHECK-SAME: multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>}
...
func.func @ConvertOpAssignedMCStratF32To16(%arg0: tensor<1x48x160x80xf32>) -> tensor<1x48x160x80xf16> {
    ...
    // CHECK:       [[CONVERT:%.+]] = VPU.Convert([[ARG_0]]) {dstElemType = f16, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>} : tensor<1x48x160x80xf32> -> tensor<1x48x160x80xf16>
...
func.func @ConvertOpAssignedMCStratF16To32(%arg0: tensor<1x48x160x80xf16>) -> tensor<1x48x160x80xf32> {
    ...
    // CHECK:       [[CONVERT:%.+]] = VPU.Convert([[ARG_0]]) {dstElemType = f32, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>} : tensor<1x48x160x80xf16> -> tensor<1x48x160x80xf32>
...
func.func @ConvertOpAssignedMCStratClustering(%arg0: tensor<2x48x160x80xf16>) -> tensor<2x48x160x80xf32> {
    ...
    // CHECK:       [[CONVERT:%.+]] = VPU.Convert([[ARG_0]]) {dstElemType = f32,
    // CHECK-SAME:  multiClusterStrategy = #VPU.multi_cluster_strategy<Clustering>
...
func.func @SmallConvertOpAssignedMCStratClustering(%arg0: tensor<1x48x3x3xf16>) -> tensor<1x48x3x3xf32> {
    ...
    // CHECK:       [[CONVERT:%.+]] = VPU.Convert([[ARG_0]]) {dstElemType = f32, multiClusterStrategy = #VPU.multi_cluster_strategy<Clustering>} : tensor<1x48x3x3xf16> -> tensor<1x48x3x3xf32>
...
// CHECK-SAME: ([[ARG_0:%[^:]+]]: tensor<1x480x3x3xf16>)
...
func.func @AssignSOHForLayerWithTopK(%arg0: tensor<1x151x513x513xf16>) -> tensor<1x1x513x513xsi32> {
    ...
    // CHECK:        [[OUTPUT_VALUES:%.+]], [[TARGET_SHAPE:%.+]] = VPU.TopK([[ARG_0]], [[AUX]])
...
func.func @LastOpExceptionCase(%arg0: tensor<1x32x60x60x!qElemType, {order = #NHWC}>) -> tensor<1x32x15x15x!qElemType, {order = #NHWC}>  {
    ...
    %cst_3 = const.Declare tensor<64x16x1x1xf16, {order = #NHWC}> = dense<10.0> : tensor<1x64x1x1xf32>, [#const.CastElemType<f16>, #const.Reshape<[64, 1, 1, 1]>, #const.Reorder<#NHWC>, #const.Reorder<#NCHW>, #const.Reshape<[64, 1, 1, 1]>, #const.PadWithZero<[0, 0, 0, 0], [0, 15, 0, 0]>, #const.Reorder<#NHWC>]
    %cst_4 = const.Declare tensor<64x1x1x4xsi32> = dense<10> : tensor<64x1x1x4xsi32>
    %cst_5 = const.Declare tensor<16x1x1x4xsi32> = dense<10> : tensor<16x1x1x4xsi32>

    %0 = VPU.NCE.Convolution(%arg0, %cst, %cst_5) rawFilterShape [16, 32, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,  strides = [2, 2]} : tensor<1x32x60x60x!qElemType, {order = #NHWC}>, tensor<16x32x3x3x!qElemType, {order = #NHWC}>, tensor<16x1x1x4xsi32> -> tensor<1x16x30x30x!qElemType, {order = #NHWC}>
    %1 = VPU.NCE.Convolution(%0, %cst_0, %cst_4) rawFilterShape [64, 16, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, ppe = #VPU.PPEStub<>,  strides = [1, 1]} : tensor<1x16x30x30x!qElemType, {order = #NHWC}>, tensor<64x16x3x3x!qElemType, {order = #NHWC}>, tensor<64x1x1x4xsi32> -> tensor<1x64x30x30xf16, {order = #NHWC}>
    %2 = VPU.NCE.DepthConvolution(%1, %cst_3) rawFilterShape [64, 1, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,  strides = [1, 1]} -> tensor<1x64x30x30x!qElemType, {order = #NHWC}>
    %3 = VPU.NCE.Convolution(%2, %cst_1, %cst_2) rawFilterShape [32, 64, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,  strides = [2, 2]} : tensor<1x64x30x30x!qElemType, {order = #NHWC}>, tensor<32x64x3x3x!qElemType, {order = #NHWC}>, tensor<32x1x1x4xsi32> -> tensor<1x32x15x15x!qElemType, {order = #NHWC}>

    return %3 : tensor<1x32x15x15x!qElemType, {order = #NHWC}>
    //CHECK:            [[VAL0:%.+]] = VPU.NCE.Convolution([[INPUT]],
