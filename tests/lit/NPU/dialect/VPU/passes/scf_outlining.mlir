...
// CHECK:    func.func private @main_func0([[INPUT0:%.+]]: tensor<1x16x256x?xf16, {bounds = #const.OpaqueI64Elements<[1, 16, 256, 480]> : tensor<4xsi64>, order = #NHWC}>, [[INPUT1:%.+]]: tensor<1x16x256x?xf16, {bounds = #const.OpaqueI64Elements<[1, 16, 256, 480]> : tensor<4xsi64>, order = #NHWC}>) -> tensor<1x16x256x?xf16, {order = #NHWC}> {
...
// CHECK:      return [[OUTPUT]] : tensor<1x16x256x?xf16, {order = #NHWC}>
// CHECK:    }
...
// CHECK:          [[ELTWISE:%.+]] = func.call @main_func0([[SLICE0]], [[SLICE1]]) : (tensor<1x16x256x?xf16, {bounds = #const.OpaqueI64Elements<[1, 16, 256, 480]> : tensor<4xsi64>, order = #NHWC}>, tensor<1x16x256x?xf16, {bounds = #const.OpaqueI64Elements<[1, 16, 256, 480]> : tensor<4xsi64>, order = #NHWC}>) -> tensor<1x16x256x?xf16, {order = #NHWC}>
// CHECK:          [[INSERTED_SLICE:%.+]] = tensor.insert_slice [[ELTWISE]] into [[ARG3]][0, 0, 0, [[ARG2]]] [1, 16, 256, [[MIN]]] [1, 1, 1, 1] : tensor<1x16x256x?xf16, {order = #NHWC}> into tensor<1x16x256x?xf16, {bounds = #const.OpaqueI64Elements<[1, 16, 256, 480]> : tensor<4xsi64>, order = #NHWC}>
