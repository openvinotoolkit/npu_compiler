...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --sparsify-weights %s | FileCheck %s
...
func.func @DoNotSparsifyFullyDense(%input: tensor<1x32x16x16xf16, {order = #NHWC}>, %weight_table: tensor<32x1x1x4xsi32>) -> tensor<1x32x16x16xf16, {order = #NHWC}> {
    ...
    %nce = VPU.NCE.Convolution(%input, %weights, %weight_table) rawFilterShape [32, 32, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
...
func.func @SparsifyWithMultiUsers(%arg0: tensor<1x32x16x16xf16, {order = #NHWC}>, %arg1: tensor<32x1x1x4xsi32>,
    ...
    %1 = VPU.NCE.Convolution(%arg0, %weights, %arg1) rawFilterShape [32, 32, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,

            strides = [1, 1]
        } : tensor<1x32x16x16xf16, {order = #NHWC}>, tensor<32x32x1x1xf16, {order = #NHWC}>, tensor<32x1x1x4xsi32> -> tensor<1x32x16x16xf16, {order = #NHWC}>
    %2 = VPU.NCE.Convolution(%arg2, %weights, %arg3) rawFilterShape [32, 32, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
...
func.func @DoNotSparsifyForFragmentation(%arg0:tensor<1x128x32x1xf16, {order = #NHWC}>)-> tensor<1x128x8x4xf16, {order = #NHWC}> {
    ...
    %1 = VPU.NCE.Convolution(%0, %cst_1, %cst_0) rawFilterShape [128, 128, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>,
        pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    ...
    // CHECK:    [[CONV:%.+]] = VPU.NCE.Convolution([[RESHAPE]], [[CST_0]], [[CST]]) rawFilterShape [128, 128, 1, 1] {
