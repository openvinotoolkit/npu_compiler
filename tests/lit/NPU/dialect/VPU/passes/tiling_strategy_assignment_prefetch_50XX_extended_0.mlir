...
module @executors {
    ...
    func.func @SplitNCEConvOverOH(%arg0: tensor<1x32x64x48xf16, {order = #NHWC}>) -> tensor<1x256x64x48xf16, {order = #NHWC}> {
        ...
        // CHECK:        [[OUTPUT:%.+]] = VPU.NCE.Convolution([[INPUT]], [[FILTER]]) rawFilterShape [256, 32, 3, 3] {
    // CHECK-SAME:          pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
        // CHECK-SAME:           ppe = #VPU.PPEStub<>, {{.*}}strides = [1, 1], tilingStrategy = [1, 1, 4, 1]}
...
module @executors {
    ...
    func.func @SplitI4QuantNCEConvOverOC(%arg0: tensor<1x128x256x4xf16, {order = #NHWC}>) -> tensor<1x6320x256x4xf16, {order = #NHWC}> {
        ...
        %weights_table = const.Declare tensor<6320x1x1x4xsi32> = dense<1> : tensor<6320x1x1x4xsi32>
        %0 = VPU.NCE.Convolution(%arg0, %weights, %weights_table) rawFilterShape [6320, 128, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ...
            ppe = #VPU.PPEStub<>,
             strides = [1, 1]
        } : tensor<1x128x256x4xf16, {order = #NHWC}>, tensor<6320x128x1x1x!qElemType, {order = #NHWC}>, tensor<6320x1x1x4xsi32> -> tensor<1x6320x256x4xf16, {order = #NHWC}>
        ...
        // CHECK-DAG:       [[WEIGHTS_TABLE:%.+]] = const.Declare tensor<6320x1x1x4xsi32> = dense<1> : tensor<6320x1x1x4xsi32>
...
module @executors {
    ...
    func.func @TileOverCWithBigC(
        ...
        %0 = VPU.NCE.Convolution(%arg0, %weights) rawFilterShape [8016, 1024, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
            ppe = #VPU.PPEStub<>,
    ...
    // CHECK:        [[OUTPUT:%.+]] = VPU.NCE.Convolution([[INPUT]], [[FILTER]]) rawFilterShape [8016, 1024, 1, 1] {
    // CHECK-SAME:          {{.*}}pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    // CHECK-SAME:          {{.*}}ppe = #VPU.PPEStub<>,
    // CHECK-SAME:          {{.*}}strides = [1, 1], tilingStrategy = [1, 36, 1, 1]}
...
module @executors {
    ...
    func.func @TileWithSOK(
        ...
        %0 = VPU.NCE.Convolution(%arg0, %weights) rawFilterShape [768, 32, 7, 7] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        ...
        // CHECK:       [[CONV1:%.+]] = VPU.NCE.Convolution([[INPUT]], [[WEIGHTS]]) rawFilterShape [768, 32, 7, 7] {
...
module @executors {
    ...
    func.func @SplitNCECompressConv(
        ...
        // CHECK-SAME:      strides = [2, 2], tilingStrategy = [1, 1, 4, 1]}
...
module @executors {
    ...
    func.func @SplitSoftMaxBenefitFromPipelining(%arg0: tensor<1x64x320x4xf16, {order = #NHWC}>) -> tensor<1x1280x320x4xf16, {order = #NHWC}> {
        ...
        %0 = VPU.NCE.Convolution(%arg0, %weights) rawFilterShape [1280, 64, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        ...
        // CHECK:       [[CONV:%.+]] = VPU.NCE.Convolution({{.*}}) rawFilterShape [1280, 64, 1, 1] {
