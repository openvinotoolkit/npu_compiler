...
func.func @SplitSparseQuantNCEConvOverH(%arg0: tensor<1x32x80x80x!qElemType, {order = #NHWC}>) -> tensor<1x320x80x80x!qElemType1, {order = #NHWC}> {
    ...
    // CHECK-SAME:          pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>
    // CHECK-SAME:          strides = [1, 1], tilingStrategy = [1, 1, 3, 1]
...
func.func @DontPrefetchToNonComputeParentOp(%arg: tensor<2x1x1024xf16>) -> tensor<1x4096x1x1xf16, {order = #NHWC}>
{
    ...
    %conv = VPU.NCE.Convolution(%slice, %weights, %weights_table) rawFilterShape [4096, 1024, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
...
module @executors {
    ...
    func.func @DWConvTileOverCPipelining(%arg0: tensor<1x160x65x65x!qElemType1, {order = #NHWC}>) -> tensor<1x960x65x65x!qElemType1, {order = #NHWC}> {
        ...
        %conv = VPU.NCE.Convolution(%arg0, %cst_30, %cst_50) rawFilterShape [960, 160, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
            ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
             strides = [1, 1]} : tensor<1x160x65x65x!qElemType1, {order = #NHWC}>, tensor<960x160x1x1x!qElemType, {order = #NHWC}>, tensor<960x1x1x4xsi32> -> tensor<1x960x65x65x!qElemType1, {order = #NHWC}>
        %dwconv = VPU.NCE.DepthConvolution(%conv, %weights, %wt) rawFilterShape [960, 1, 9, 9] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
...
module @executors {
    ...
    func.func @AvoidExcessiveTiling(%arg0: tensor<1x6304x256x4xf16, {order = #NHWC}>, %arg1: tensor<6304x16x1x1xf16, {order = #NHWC}>) -> tensor<1x6304x256x4xf16, {order = #NHWC}>  {
        ...
        %dwconv = VPU.NCE.DepthConvolution(%arg0, %arg1, %cst) rawFilterShape [6304, 1, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
