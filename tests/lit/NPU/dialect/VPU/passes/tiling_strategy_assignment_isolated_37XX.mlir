...
// CHECK-SAME:        ([[ARG_0:%[^:]+]]: tensor<1x80x3840x4xf16, {order = #NHWC}>
func.func @NoSplitDepthConvOverCWithSOK(%arg0: tensor<1x80x3840x4xf16, {order = #NHWC}>) -> tensor<1x80x3840x4xf16, {order = #NHWC}> {
    ...
    %0 = VPU.NCE.DepthConvolution(%arg0, %weights, %wt) rawFilterShape [80, 1, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
    ...
    // CHECK: [[DWConv:%.+]] = VPU.NCE.DepthConvolution([[ARG_0]], [[CST]], [[CST0]])  rawFilterShape [80, 1, 1, 1]
...
func.func @SplitAddSameInputOverC(
    ...
    // CHECK:       return [[AND]] : tensor<1x4096x14x14xf16>
}
...
func.func @MaximumSplitOverH(
    ...
    // CHECK:  [[MAXIMUM0:%.+]] = VPU.Maximum([[INPUT]], [[CST]])
    // CHECK-SAME:  multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>
    // CHECK-SAME:  tilingStrategy = [1, 1, 147, 1]
    // CHECK-SAME:  : tensor<1x32x2048x2048xf16>, tensor<1x1x1x1xf16>
    // CHECK-SAME:  -> tensor<1x32x2048x2048xf16>
...
func.func @MinimumSplitOverH(
    ...
    // CHECK:  [[CST:%.+]] = const.Declare tensor<1x1x1x1xf16> = dense<1.000000e+00> : tensor<1x1x1x1xf16>
    // CHECK:  [[Minimum0:%.+]] = VPU.Minimum([[INPUT]], [[CST]])
    // CHECK-SAME:  multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>
    // CHECK-SAME:  tilingStrategy = [1, 1, 147, 1]
    // CHECK-SAME:  : tensor<1x32x2048x2048xf16>, tensor<1x1x1x1xf16>
    // CHECK-SAME:  -> tensor<1x32x2048x2048xf16>
...
func.func @Yuv2RGBSplit(%arg0: tensor<1x993x982x1xf16>) -> tensor<1x662x982x3xf16> {
    ...
    // CHECK:    [[SLICE0:%.+]] = VPU.Slice [[ARG_0]] [0, 0, 0, 0] [1, 662, 982, 1] : tensor<1x993x982x1xf16> to tensor<1x662x982x1xf16>
    // CHECK:    [[SLICE1:%.+]] = VPU.Slice [[ARG_0]] [0, 662, 0, 0] [1, 331, 982, 1] : tensor<1x993x982x1xf16> to tensor<1x331x982x1xf16>
...
func.func @DepthToSpaceBlocksFirstSplit(%arg0: tensor<1x480x10x120xf32, {order = #NHWC}>) -> tensor<1x30x40x480xf32, {order = #NHWC}> {
    ...
    // CHECK:       [[CONVERT0:%.+]] = VPU.Convert([[ARG_0]]) {dstElemType = f16, tilingStrategy = [1, 2, 1, 1]} : tensor<1x480x10x120xf32, {order = #NHWC}> -> tensor<1x480x10x120xf16, {order = #NHWC}>
...
func.func @SpaceToDepthBlockFirstSplit(%arg0: tensor<1x48x160x80xf32>) -> tensor<1x768x40x20xf32> {
    ...
    // CHECK:       [[CONVERT0:%.+]] = VPU.Convert([[ARG_0]]) {dstElemType = f16, tilingStrategy = [1, 1, 2, 1]} : tensor<1x48x160x80xf32> -> tensor<1x48x160x80xf16>
...
func.func @SplitNCEConvOverOH(%arg0: tensor<1x32x64x64xf16, {order = #NHWC}>) -> tensor<1x256x64x64xf16, {order = #NHWC}> {
    ...
    // CHECK:        [[CONV:%.+]] = VPU.NCE.Convolution([[INPUT]], [[FILTER]], [[WEIGHTS_TABLE]]) rawFilterShape [256, 32, 3, 3]
    // CHECK-SAME:          pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
...
func.func @SplitQuantNCEConvOverOC(%arg0: tensor<1x32x64x64x!qElemType, {order = #NHWC}>) -> tensor<1x512x64x64x!qElemType1, {order = #NHWC}> {
    ...
    // CHECK:        [[CONV:%.+]] = VPU.NCE.Convolution([[INPUT]], [[WEIGHTS]], [[WEIGHTS_TABLE]]) rawFilterShape [512, 32, 3, 3]
    // CHECK-SAME:          pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
    // CHECK-SAME:          strides = [1, 1], tilingStrategy = [1, 1, 2, 1]
...
func.func @SplitNCEEltwiseAddSameInput(%arg0: tensor<1x4096x14x14xf16, {order = #NHWC}>) -> tensor<1x4096x14x14xf16, {order = #NHWC}> {
    ...
    // CHECK:       return [[ELTWISE]] : tensor<1x4096x14x14xf16, {order = #NHWC}>
}
...
func.func @SplitSparseQuantNCEConvOverOH(%arg0: tensor<1x32x80x80x!qElemType, {order = #NHWC}>) -> tensor<1x320x80x80x!qElemType1, {order = #NHWC}> {
    ...
    // CHECK:        [[OUTPUT:%.+]] = VPU.NCE.Convolution([[INPUT]], [[WEIGHTS_SPARSE]], [[WEIGHTS_TABLE]]) rawFilterShape [320, 32, 3, 3]
    // CHECK-SAME:          pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
    // CHECK-SAME:          strides = [1, 1], tilingStrategy = [1, 1, 2, 1]
...
func.func @GridSampleSplit(%arg0: tensor<1x3x272x480xf16>, %arg1: tensor<1x272x480x2xf16>) -> tensor<1x3x272x480xf16> {
    ...
    // CHECK:       [[OUTPUT:%.+]] = VPU.GridSample([[ARG_0]], [[ARG_1]])
    // CHECK-SAME:          tilingStrategy = [1, 1, 1, 2]
...
// CHECK-SAME:        [[ARG_0:%[^:]+]]: tensor<1x16x192x192xf16>
func.func @HardSigmoidSplit(%arg0: tensor<1x16x192x192xf16>) -> tensor<1x16x192x192xf16> {
    ...
    // CHECK:       [[OUTPUT:%.+]] = VPU.HardSigmoid([[ARG_0]])
    // CHECK-SAME:          tilingStrategy = [1, 1, 2, 1]
...
func.func @SplitSparseDepthConvWithBigC(%arg0: tensor<1x4080x40x40xf16, {order = #NHWC}>) -> !VPU.SparseTensor<data=tensor<1x4080x37x37xf16, {order = #NHWC}>, sparsity_map=tensor<1x4080x37x37xi1, {order = #NHWC}>> {
    ...
    %0 = VPU.NCE.DepthConvolution(%arg0, %cst0, %wt) rawFilterShape [4080, 1, 4, 4] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    ...
    // CHECK: [[DWConv:%.+]] = VPU.NCE.DepthConvolution([[ARG_0]], [[INPUT]], [[WT]]) rawFilterShape [4080, 1, 4, 4] {
    // CHECK-SAME:     pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>
    // CHECK-SAME:    strides = [1, 1], tilingStrategy = [1, 14, 1, 1]
...
// CHECK-SAME:        [[ARG_0:%[^:]+]]: tensor<1x4112x40x40xf16, {order = #NHWC}>
func.func @SplitSparseDepthConvWithBigCWithSOK(%arg0: tensor<1x4112x40x40xf16, {order = #NHWC}>) -> !VPU.SparseTensor<data=tensor<1x4112x37x37xf16, {order = #NHWC}>, sparsity_map=tensor<1x4112x37x37xi1, {order = #NHWC}>> {
    ...
    %0 = VPU.NCE.DepthConvolution(%arg0, %cst0, %wt) rawFilterShape [4112, 1, 4, 4] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
            multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    ...
    // CHECK: [[DWConv:%.+]] = VPU.NCE.DepthConvolution([[ARG_0]], [[INPUT]], [[WT]]) rawFilterShape [4112, 1, 4, 4] {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
...
// CHECK-SAME:        [[ARG_0:%[^:]+]]: tensor<1x4112x16x16xf16, {order = #NHWC}>
func.func @SplitSparseNCEMaxPoolWithBigCWithSOK(%arg0: tensor<1x4112x16x16xf16, {order = #NHWC}>) -> tensor<1x4112x16x16xf16, {order = #NHWC}> {
    ...
    // CHECK:       [[VAL0:%.+]] = VPU.Sparsify([[ARG_0]]) : tensor<1x4112x16x16xf16, {order = #NHWC}>
...
// CHECK-LABEL: func.func @SelectSplitAtOuterDim
// CHECK-SAME:      [[INPUT_0:%.+]]: tensor<1x1x1x2048xf16>
// CHECK-SAME:      [[INPUT_1:%.+]]: tensor<1x1x1024x2048xf16>
// CHECK-SAME:      [[INPUT_2:%.+]]: tensor<1x1x1x1xf16>
func.func @SelectSplitAtOuterDim(%arg0: tensor<1x1x1x2048xf16>, %arg1: tensor<1x1x1024x2048xf16>, %arg2: tensor<1x1x1x1xf16>) -> tensor<1x1x1024x2048xf16> {
    %0 = VPU.Select(%arg0, %arg1, %arg2) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>} : tensor<1x1x1x2048xf16>, tensor<1x1x1024x2048xf16>, tensor<1x1x1x1xf16> -> tensor<1x1x1024x2048xf16>
    return %0 : tensor<1x1x1024x2048xf16>

    // CHECK:       [[SELECT:%.+]] = VPU.Select([[INPUT_0]], [[INPUT_1]], [[INPUT_2]]) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>, tilingStrategy = [1, 1, 5, 1]} : tensor<1x1x1x2048xf16>, tensor<1x1x1024x2048xf16>, tensor<1x1x1x1xf16> -> tensor<1x1x1024x2048xf16>
    // CHECK:       return [[SELECT]] : tensor<1x1x1024x2048xf16>
}
...
// CHECK-LABEL: @SplitDepthConvByMinNumTiles
// CHECK-SAME:    ([[ARG0:%.+]]: tensor<1x3x387072x1xf16, {order = #NHWC}>)

func.func @SplitDepthConvByMinNumTiles(%arg0: tensor<1x3x387072x1xf16, {order = #NHWC}>) -> tensor<1x96768x4x3xf16, {order = #NHWC}> {
    %weights = const.Declare tensor<96768x16x1x1xf16, {order = #NHWC}> = dense<1.0> : tensor<96768x1x1x1xf16, {order = #NHWC}>, [#const.PadWithZero<[0, 0, 0, 0], [0, 15, 0, 0]>]
    %weights_table = const.Declare tensor<96768x1x1x4xsi32> = dense<10> : tensor<96768x1x1x4xsi32>
    %shape_cast = VPU.ShapeCast {shape = [1, 96768, 4, 3]} inputs(%arg0: tensor<1x3x387072x1xf16, {order = #NHWC}>) -> tensor<1x96768x4x3xf16, {order = #NHWC}>
    %0 = VPU.NCE.DepthConvolution(%shape_cast, %weights, %weights_table) rawFilterShape [96768, 1, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} -> tensor<1x96768x4x3xf16, {order = #NHWC}>
    return %0 : tensor<1x96768x4x3xf16, {order = #NHWC}>
...
// CHECK-LABEL: @SplitSmallDepthConvByMinNumTiles
// CHECK-SAME:    ([[ARG0:%.+]]: tensor<1x2568x2x2xf16, {order = #NHWC}>)

func.func @SplitSmallDepthConvByMinNumTiles(%arg0: tensor<1x2568x2x2xf16, {order = #NHWC}>) -> tensor<1x10272x1x1xf16, {order = #NHWC}> {
    %weights = const.Declare tensor<10272x16x1x1xf16, {order = #NHWC}> = dense<1.0> : tensor<10272x1x1x1xf16, {order = #NHWC}>, [#const.PadWithZero<[0, 0, 0, 0], [0, 15, 0, 0]>]
    %weights_table = const.Declare tensor<10272x1x1x4xsi32> = dense<10> : tensor<10272x1x1x4xsi32>
    %shape_cast = VPU.ShapeCast {shape = [1, 10272, 1, 1]} inputs(%arg0: tensor<1x2568x2x2xf16, {order = #NHWC}>) -> tensor<1x10272x1x1xf16, {order = #NHWC}>
    %0 = VPU.NCE.DepthConvolution(%shape_cast, %weights, %weights_table) rawFilterShape [10272, 1, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} -> tensor<1x10272x1x1xf16, {order = #NHWC}>
    return %0 : tensor<1x10272x1x1xf16, {order = #NHWC}>

    // CHECK:      tilingStrategy = [1, 2, 1, 1]
}
...
func.func @ConvWithLargeOCTileOverChannelAndHeight(%arg0: tensor<1x1024x256x4xf16, {order = #NHWC}>) -> tensor<1x18944x256x4xf16, {order = #NHWC}> {
    ...
    %0 = VPU.NCE.Convolution(%arg0, %weights, %weights_table) rawFilterShape [18944, 1024, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.000000e+00 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x1024x256x4xf16, {order = #NHWC}>, tensor<18944x1024x1x1xf16, {order = #NHWC}>, tensor<18944x1x1x4xsi32> -> tensor<1x18944x256x4xf16, {order = #NHWC}>
    return %0 : tensor<1x18944x256x4xf16, {order = #NHWC}>

    // CHECK:      VPU.NCE.Convolution
    // CHECK-SAME: tilingStrategy = [1, 11, 37, 1]
}
