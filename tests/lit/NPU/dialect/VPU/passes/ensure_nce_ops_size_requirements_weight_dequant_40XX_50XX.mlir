...
func.func @EnableWeightDeqauntEnsuranceBeforeStrategy(%arg0: tensor<1x1280x256x1xf16, {order = #NHWC}>) -> tensor<1x10240x64x4xf16, {order = #NHWC}> {
    ...
    %2 = VPU.NCE.Convolution(%1, %0, %cst_0) rawFilterShape [10240, 1280, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, prelu_alpha = [1.000000e+00], adder = 0.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x1280x64x4xf16, {order = #NHWC}>, tensor<10240x1280x1x1xf16, {order = #NHWC}>, tensor<10240x1x1x4xsi32> -> tensor<1x10240x64x4xf16, {order = #NHWC}>
  ...
  // CHECK: [[CONV:%.+]] = VPU.NCE.Convolution([[ACTIVATION]], [[WT_DEQUANT]], [[CST0]]) rawFilterShape [10240, 1280, 1, 1] {mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>,
