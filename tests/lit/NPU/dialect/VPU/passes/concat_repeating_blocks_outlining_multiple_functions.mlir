...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --concat-repeating-blocks-outlining="min-seq-length=2 single-function-per-concat=false" --canonicalize %s | FileCheck %s
...
module @OutlineConcat {
    ...
    func.func @main(%input: tensor<1x48x32x32xf16, {order = #NHWC}>) -> tensor<1x96x32x32xf16, {order = #NHWC}> {
        ...
        // CHECK:  func.func nested @main_concat1_input1([[ARG0:%.+]]: tensor<1x48x32x32xf16, {order = #NHWC}>) -> tensor<1x48x32x32xf16, {order = #NHWC}> {
        // CHECK:      [[SOFTMAX:%.+]] = VPU.SoftMax([[ARG0]])
        // CHECK:      [[RELU:%.+]] = VPU.ReLU([[SOFTMAX]])
        // CHECK:      return [[RELU]]
        // CHECK:  }
        // CHECK:  func.func nested @main_concat1_input2([[ARG0:%.+]]: tensor<1x48x32x32xf16, {order = #NHWC}>) -> tensor<1x48x32x32xf16, {order = #NHWC}> {
        // CHECK:      [[SOFTMAX:%.+]] = VPU.SoftMax([[ARG0]])
        // CHECK:      [[RELU:%.+]] = VPU.ReLU([[SOFTMAX]])
        // CHECK:      return [[RELU]]
        // CHECK:  }
        // CHECK:  func.func nested @main_concat2_input1([[ARG0:%.+]]: tensor<1x48x32x32xf16, {order = #NHWC}>, [[ARG1:%.+]]: tensor<1x48x32x32xf16, {order = #NHWC}>) -> tensor<1x96x32x32xf16, {order = #NHWC}> {
