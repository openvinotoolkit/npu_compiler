...
module @executors {
    ...
    func.func @TileConvolutionOnHeightWithRestraintOfMinimalH(%arg0: tensor<1x3840x256x4xf16, {order = #NHWC}>, %arg1: tensor<1536x3840x1x1xf16, {order = #NHWC}>) -> tensor<1x1536x256x4xf16, {order = #NHWC}>  {
        ...
        %conv = VPU.NCE.Convolution(%arg0, %arg1, %cst) rawFilterShape [1536, 3840, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
...
#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
!qElemType1 = !quant.uniform<!QuantileType.quantile<ui4:f16, {0.000000e+00,1.000000e+00,2.000000e+00,3.000000e+00,4.000000e+00,5.000000e+00,6.000000e+00,7.000000e+00,-8.000000e+00,-7.000000e+00,-6.000000e+00,-5.000000e+00,-4.000000e+00,-3.000000e+00,-2.000000e+00,-1.000000e+00}>:f16, 1.000000e+00>
...
module @executors {
  ...
  func.func @multiDimPipelineTilingForSpatialFirst(%arg0: tensor<1x3072x256x4xf16, {order = #NHWC}>, %arg1: tensor<8192x3072x1x1x!qElemType1, {order = #NHWC}>) -> tensor<1x8192x256x4xf16, {order = #NHWC}> {
    ...
    %0 = VPU.NCE.Convolution(%arg0, %arg1, %cst_0) rawFilterShape [8192, 3072, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.000000e+00 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x3072x256x4xf16, {order = #NHWC}>, tensor<8192x3072x1x1x!qElemType1, {order = #NHWC}>, tensor<8192x1x1x4xsi32> -> tensor<1x8192x256x4xf16, {order = #NHWC}>
...
module @executors {
  ...
  func.func @multiDimPipelineTilingForWeightFirst(%arg0: tensor<1x2048x256x4xf16, {order = #NHWC}>, %arg1: tensor<2048x2048x1x1xf16, {order = #NHWC}>) -> tensor<1x2048x256x4xf16, {order = #NHWC}> {
    ...
    %0 = VPU.NCE.Convolution(%arg0, %arg1, %cst_0) rawFilterShape [2048, 2048, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.000000e+00 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x2048x256x4xf16, {order = #NHWC}>, tensor<2048x2048x1x1xf16, {order = #NHWC}>, tensor<2048x1x1x4xsi32> -> tensor<1x2048x256x4xf16, {order = #NHWC}>
...
// CHECK-LABEL: @NCEMatMulSOGAndMultiDimPipelined
  func.func @NCEMatMulSOGAndMultiDimPipelined(%arg0: tensor<32x1x128x1024x1xf16, {order = #GNHWC}>) ->  tensor<32x1x1408x1024x1xf16, {order = #GNHWC}>{
    ...
    %grouped_matmul = VPU.NCE.MatMul(%arg0, %weight, %weight_table) rawFilterShape [32, 1408, 128, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverGroup>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} -> tensor<32x1x1408x1024x1xf16, {order = #GNHWC}>
