...
module {
  func.func @ApplyTilingNCEConv(%arg0: tensor<1x32x64x64xf16, {order = #NHWC}>) -> tensor<1x256x64x64xf16, {order = #NHWC}> {
    ...
    %1 = scf.for %arg1 = %c0 to %c64 step %c32 iter_args(%arg2 = %0) -> (tensor<1x256x64x64xf16, {order = #NHWC}>) {
      ...
      } : tensor<1x32x33x64xf16, {order = #NHWC}> to tensor<1x32x?x66xf16, {order = #NHWC}>
      %7 = VPU.NCE.Convolution(%padded, %cst_0) {pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>, rawFilterShape = [256, 32, 3, 3], strides = [1, 1]} : tensor<1x32x?x66xf16, {order = #NHWC}>, tensor<256x32x3x3xf16, {order = #NHWC}> -> tensor<1x256x?x64xf16, {order = #NHWC}>
      %cast = tensor.cast %7 : tensor<1x256x?x64xf16, {order = #NHWC}> to tensor<1x256x32x64xf16, {order = #NHWC}>
    ...
    //CHECK:                [[CAST0:%.+]] = tensor.cast [[SLICE]] : tensor<1x32x33x64xf16, {order = #NHWC}> to tensor<1x32x?x64xf16, {order = #NHWC}>
    //CHECK:                [[CONV:%.+]] = VPU.NCE.Convolution([[CAST0]], [[WEIGHTS]])
    //CHECK-SAME:           {pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, ppe = #VPU.PPEStub<>, rawFilterShape = [256, 32, 3, 3], strides = [1, 1]} : tensor<1x32x?x64xf16, {order = #NHWC}>, tensor<256x32x3x3xf16, {order = #NHWC}>
    //CHECK-SAME:           -> tensor<1x256x?x64xf16, {order = #NHWC}>
    //CHECK:                [[CAST1:%.+]] = tensor.cast [[CONV]] : tensor<1x256x?x64xf16, {order = #NHWC}> to tensor<1x256x32x64xf16, {order = #NHWC}>
  ...
  func.func @Tiling2DPaddedMaxPool(%arg0: tensor<1x16x512x480xf16, {order = #NHWC}>) -> tensor<1x16x512x480xf16, {order = #NHWC}> {
    ...
    %1 = scf.for %arg1 = %c0 to %c512 step %c256 iter_args(%arg2 = %0) -> (tensor<1x16x512x480xf16, {order = #NHWC}>) {
      %2 = scf.for %arg3 = %c0 to %c480 step %c120 iter_args(%arg4 = %arg2) -> (tensor<1x16x512x480xf16, {order = #NHWC}>) {
        ...
        %extracted_slice = tensor.extract_slice %arg0[0, 0, %3, %8] [1, 16, 257, %13] [1, 1, 1, 1] : tensor<1x16x512x480xf16, {order = #NHWC}> to tensor<1x16x257x?xf16, {order = #NHWC}>
        %padded = tensor.pad %extracted_slice low[0, 0, %5, %10] high[0, 0, %7, %12] {
        ^bb0(%arg5: index, %arg6: index, %arg7: index, %arg8: index):
          tensor.yield %cst : f16
        } : tensor<1x16x257x?xf16, {order = #NHWC}> to tensor<1x16x?x?xf16, {order = #NHWC}>
        %14 = VPU.NCE.MaxPool(%padded) {kernel_size = [3, 3], multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>, strides = [1, 1]} -> tensor<1x16x?x?xf16, {order = #NHWC}>
        %cast = tensor.cast %14 : tensor<1x16x?x?xf16, {order = #NHWC}> to tensor<1x16x256x120xf16, {order = #NHWC}>
    ...
    //CHECK:                                 [[CAST0:%.+]] = tensor.cast [[SLICE]] : tensor<1x16x257x?xf16, {order = #NHWC}> to tensor<1x16x?x?xf16, {order = #NHWC}>
    ...
    //CHECK-SAME:                                           -> tensor<1x16x?x?xf16, {order = #NHWC}>
    //CHECK:                                 [[CAST1:%.+]] = tensor.cast [[POOL]] : tensor<1x16x?x?xf16, {order = #NHWC}> to tensor<1x16x256x120xf16, {order = #NHWC}>
...
module {
  func.func @MergeVFChain3Tiles(%arg0: tensor<1x256x540x120xf16, {order = #NHWC}>) -> tensor<1x128x540x240xf16, {order = #NHWC}> {
    ...
    %2 = scf.for %arg1 = %c0 to %c960 step %c26 iter_args(%arg2 = %1) -> (tensor<1x32x540x960xf16, {order = #NHWC}>) {
      ...
      %extracted_slice = tensor.extract_slice %0[0, 0, 0, %30] [1, 32, 540, %35] [1, 1, 1, 1] : tensor<1x32x540x960xf16, {order = #NHWC}> to tensor<1x32x540x?xf16, {order = #NHWC}>
      %padded = tensor.pad %extracted_slice low[0, 0, 1, %32] high[0, 0, 1, %34] {
      ^bb0(%arg3: index, %arg4: index, %arg5: index, %arg6: index):
        tensor.yield %cst : f16
      } : tensor<1x32x540x?xf16, {order = #NHWC}> to tensor<1x32x542x?xf16, {order = #NHWC}>
      %36 = VPU.NCE.Convolution(%padded, %cst_0) {mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <LRELU>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>, rawFilterShape = [32, 32, 3, 3], strides = [1, 1]} : tensor<1x32x542x?xf16, {order = #NHWC}>, tensor<32x32x3x3xf16, {order = #NHWC}> -> tensor<1x32x540x?xf16, {order = #NHWC}>
      ...
      %inserted_slice = tensor.insert_slice %43 into %arg2[0, 0, 0, %arg1] [1, 32, 540, %4] [1, 1, 1, 1] : tensor<1x32x540x?xf16, {order = #NHWC}> into tensor<1x32x540x960xf16, {order = #NHWC}>
    ...
    //CHECK-SAME:           : tensor<1x32x540x960xf16, {order = #NHWC}> to tensor<1x32x540x?xf16, {order = #NHWC}>
    ...
    //CHECK-SAME:           : tensor<1x32x540x?xf16, {order = #NHWC}> into tensor<1x32x540x960xf16, {order = #NHWC}>
