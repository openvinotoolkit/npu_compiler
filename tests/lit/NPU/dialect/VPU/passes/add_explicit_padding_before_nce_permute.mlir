...
// RUN: vpux-opt --split-input-file  --init-compiler="platform=%platform% allow-custom-values=true" --add-explicit-padding-before-nce-permute %s | FileCheck %s
...
func.func @InsertExplicitPadding(%arg0:  tensor<1x166x1x16xf16>) -> tensor<1x160x1x1xf16, {order = #NHWC}> {
    ...
    %conv = VPU.NCE.Convolution(%slice, %weights) rawFilterShape [160, 176, 1, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
            ppe = #VPU.PPEStub<>,
             strides = [1, 1]}
        : tensor<1x176x1x3xf16, {order = #NHWC}>, tensor<160x176x1x3xf16, {order = #NHWC}> -> tensor<1x160x1x1xf16, {order = #NHWC}>
    ...
    // CHECK:       [[CONV:%.+]] = VPU.NCE.Convolution([[SLICE]], [[WEIGHTS]]) rawFilterShape [160, 176, 1, 3] {
...
func.func @NCEPermuteDepthConvWeightsSparseConv(%arg0: tensor<1x3x257x257xf16>) ->  tensor<1x16x129x129xf16, {order = #NHWC}> {
    ...
    %depth_conv = VPU.NCE.DepthConvolution(%slice, %depth_conv_weights) rawFilterShape [16, 1, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, ppe = #VPU.PPEStub<>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
         strides = [1, 1]}
        -> tensor<1x16x257x257xf16, {order = #NHWC}>

    %conv = VPU.NCE.Convolution(%depth_conv, %weights_sparse_group) rawFilterShape [16, 16, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, ppe = #VPU.PPEStub<>, pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
         strides = [2, 2]}
        : tensor<1x16x257x257xf16, {order = #NHWC}>, !VPU.SparseTensor<data=tensor<16x16x3x3xf16, {order = #NHWC}>, sparsity_map=tensor<16x1x1x256xi1>, is_weights, #VPU.SparsityCompression<axis = 0 : i64, numElems = dense<27> : tensor<16xi64>, alignment = 16 : i64>> -> tensor<1x16x129x129xf16, {order = #NHWC}>
    ...
    // CHECK:       [[DEPTH_CONV:%.+]] = VPU.NCE.DepthConvolution([[SLICE]], [[WEIGHTS_DEPTHCONV]])
    // CHECK-SAME:      -> tensor<1x16x257x257xf16, {order = #NHWC}>

    // CHECK:       [[CONV:%.+]] = VPU.NCE.Convolution([[DEPTH_CONV]], [[SPARSE_GROUP]])
...
func.func @NCEPermuteDepthConvSliceOnChannels(%arg0: tensor<1x5929x3x3xf16>) ->  tensor<1x5929x3x3xf16, {order = #NHWC}> {
    ...
    %depth_conv = VPU.NCE.DepthConvolution(%slice0, %weights) rawFilterShape [5936, 1, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
        ppe = #VPU.PPEStub<>,
         strides = [1, 1]}
    ...
    // CHECK:       [[DEPTH_CONV:%.+]] = VPU.NCE.DepthConvolution([[SLICE0]], [[WEIGHTS]])
...
func.func @NCEPermuteDepthConvAvgPoolSliceOnChannels(%arg0: tensor<1x39x13x3xf16>) ->  tensor<1x39x1x1xf16, {order = #NHWC}> {
    ...
    %depth_conv = VPU.NCE.DepthConvolution(%slice0, %weights) rawFilterShape [48, 1, 7, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 1 : i64>,
        ppe = #VPU.PPEStub<>,
         strides = [7, 1]}
...
func.func @NCEPermuteEltwiseSliceOnChannels(%arg0: tensor<1x12x77x77xf16>) ->  tensor<1x12x77x77xf16, {order = #NHWC}> {
    ...
    %eltwise = VPU.NCE.Eltwise(%slice0, %constant) {resultSegmentSizes = array<i32: 1, 0, 0, 0>, op_type = #VPU.eltwise_type<ADD>, ppe = #VPU.PPEStub<>}
...
func.func @QuantizedNCEPermuteCompressConv(%arg0: tensor<1x4x64x64xf16, {order = #NCHW}>) ->  tensor<1x16x32x32x!qElemType, {order = #NHWC}> {
    ...
    %conv = VPU.NCE.CompressConvolution(%nce_permute, %weights, %weights_table) rawFilterShape [16, 1, 3, 3] {cm_sp_pattern = 1 : i64,
    ...
    // CHECK:       [[CONV:%.+]] = VPU.NCE.CompressConvolution([[NCE_PERMUTE]], [[WEIGHTS]], [[WEIGHTS_TABLE]]) rawFilterShape [16, 1, 3, 3] {cm_sp_pattern = 1 : i64,
...
module @DoNotAddExplicitPadWhenUserProducesSlicedChannels {
    ...
    func.func @main(%input: tensor<1x3x720x1280xf16>,
        ...
        %permute = VPU.NCE.Permute(%input) {
            ...
            ppe = #VPU.PPEStub<>
        } -> tensor<1x16x720x1280xf16, {order = #NHWC}>

        %depth_conv = VPU.NCE.DepthConvolution(%permute, %weights_depth_conv) rawFilterShape [3, 1, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
            ppe = #VPU.PPEStub<>,
            strides = [1, 1]
        } -> tensor<1x3x720x1280xf16, {order = #NHWC}>

        %conv = VPU.NCE.Convolution(%depth_conv, %weights_conv) rawFilterShape [32, 3, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        ...
        // CHECK-SAME:      -> tensor<1x16x720x1280xf16, {order = #NHWC}>
        // CHECK:       [[DEPTH_CONV:%.+]] = VPU.NCE.DepthConvolution([[PERMUTE]], [[WEIGHTS_DEPTH_CONV]])
        // CHECK-SAME:      -> tensor<1x3x720x1280xf16, {order = #NHWC}>
        // CHECK:       [[CONV:%.+]] = VPU.NCE.Convolution([[DEPTH_CONV]], [[WEIGHTS_CONV]])
        // CHECK-SAME:      -> tensor<1x32x360x640xf16, {order = #NHWC}>
        // CHECK:       return [[CONV]]
}
}
