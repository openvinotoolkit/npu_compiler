...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --cmx-stack-frames-reserve-mem --cmx-metadata-reserve-mem --tiling-strategy-assignment="tiling-mode=ISOLATED" %s | FileCheck %s
...
func.func @SplitDepthConvWithBigC(%arg0: tensor<1x5120x64x4xf16, {order = #NHWC}>) -> tensor<1x5120x64x4xf16, {order = #NHWC}> {
    ...
    %0 = VPU.NCE.DepthConvolution(%arg0, %weights, %wt) rawFilterShape [5120, 1, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
    ...
    // CHECK: [[DWConv:%.+]] = VPU.NCE.DepthConvolution([[ARG_0]], [[CST]], [[CST0]]) rawFilterShape [5120, 1, 1, 1]
...
// CHECK-SAME: ([[ARG_0:%[^:]+]]: tensor<1x160x3840x4xf16, {order = #NHWC}>)
func.func @NoSplitDepthConvOverCWithSOK(%arg0: tensor<1x160x3840x4xf16, {order = #NHWC}>) -> tensor<1x160x3840x4xf16, {order = #NHWC}> {
    ...
    %0 = VPU.NCE.DepthConvolution(%arg0, %weights, %wt) rawFilterShape [160, 1, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
    ...
    // CHECK: [[DWConv:%.+]] = VPU.NCE.DepthConvolution([[ARG_0]], [[CST]], [[CST0]]) rawFilterShape [160, 1, 1, 1]
...
func.func @SEPDWConvCTile(%arg0: tensor<1x768x32x32xf16, {order = #NHWC}>) -> tensor<1x768x16x16xf16, {order = #NHWC}> {
  ...
  %4 = VPU.NCE.DepthConvolution(%group_sparse_tensor, %cst_1, %cst_0) rawFilterShape [768, 1, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
...
func.func @NCEMatMulSOGAndGTile(%arg0: tensor<16x1x128x1x4xf16, {order = #GNHWC}>) -> tensor<16x1x4224x1x4xf16, {order = #GNHWC}> {
    ...
    %grouped_matmul = VPU.NCE.MatMul(%arg0, %weights, %weights_table) rawFilterShape [16, 4224, 128, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>,
        multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverGroup>,
        pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
