...
func.func @ConvChannel2DTiling(%arg0: tensor<1x512x64x64xf16, {order = #NHWC}>) -> tensor<1x256x64x64xf16, {order = #NHWC}> {
    ...
    //CHECK:                                  [[SLICE_INPUT:%.+]] = tensor.extract_slice [[INPUT]][0, 0, [[SLICE_OFFSET]], 0] [1, 512, [[INPUT_SIZE]], 64] [1, 1, 1, 1] : tensor<1x512x64x64xf16, {order = #NHWC}> to tensor<1x512x?x64xf16, {order = #NHWC}>
    ...
    //CHECK:                                  tensor<1x512x?x64xf16, {order = #NHWC}> to tensor<1x512x?x66xf16, {order = #NHWC}>

    //CHECK:                                  [[CONV:%.+]] = VPU.NCE.Convolution([[PAD]], [[SLICE_WEIGHTS]], [[SLICE_WEIGHTS_TABLE]])
    //CHECK:                                  [[PADDED_DIM:%.+]] = arith.constant 8 : index
    //CHECK:                                  [[INSERT:%.+]] = tensor.insert_slice [[CONV]] into [[LOOP_OUT]][0, [[LOOP_ITER_C]], [[LOOP_ITER_H]], 0] [1, 128, [[PADDED_DIM]], 64] [1, 1, 1, 1] : tensor<1x128x?x64xf16, {order = #NHWC}> into tensor<1x256x64x64xf16, {order = #NHWC}>
