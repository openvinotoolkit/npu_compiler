...
// CHECK-LABEL: @DecomposedOpsInSdpaSoftmaxLowRatio
module @DecomposedOpsInSdpaSoftmaxLowRatio {
  config.PipelineOptions @Options {
    config.Option @config.AutoPaddingIDU : true
  }
  // CHECK:   ([[ARG0:%.+]]: tensor<1x48x1024x4x!qElemType, {order = #NHWC}>
  // CHECK:   [[ARG1:%.+]]: tensor<80x48x1x1x!qElemType1, {order = #NHWC}>
  // CHECK:   [[ARG2:%.+]]: tensor<48x80x1x1xf16, {order = #NHWC}>
  func.func @main(%arg0: tensor<1x48x1024x4x!qElemType, {order = #NHWC}>, %arg1: tensor<80x48x1x1x!qElemType1, {order = #NHWC}>, %arg2: tensor<48x80x1x1xf16, {order = #NHWC}>) -> tensor<1x48x1024x4xf16, {order = #NHWC}> {
    %209 = VPU.NCE.Convolution(%arg0, %arg1) rawFilterShape [80, 48, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, input_padding = [0, 8, 0, 0], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, output_padding = [0, 0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.200000e-4 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>, strides = [1, 1]} : tensor<1x48x1024x4x!qElemType, {order = #NHWC}>, tensor<80x48x1x1x!qElemType1, {order = #NHWC}> -> tensor<1x80x1024x4xf16, {order = #NHWC}>

    %207 = VPU.SoftMax(%209) {axisInd = 1 : i64, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>} : tensor<1x80x1024x4xf16, {order = #NHWC}> -> tensor<1x80x1024x4xf16, {order = #NHWC}>

    %208 = VPU.NCE.Convolution(%207, %arg2) rawFilterShape [48, 80, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, input_padding = [0, 0, 0, 0], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, output_padding = [0, 8, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.000000e+00 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>, strides = [1, 1]} : tensor<1x80x1024x4xf16, {order = #NHWC}>, tensor<48x80x1x1xf16, {order = #NHWC}> -> tensor<1x48x1024x4xf16, {order = #NHWC}>
...
// CHECK-LABEL: @DecomposedOpsInSdpaSoftmaxTooSmall
module @DecomposedOpsInSdpaSoftmaxTooSmall {
  config.PipelineOptions @Options {
    config.Option @config.AutoPaddingIDU : true
  }
  // CHECK:   ([[ARG0:%.+]]: tensor<1x512x4x1xf16, {order = #NHWC}>
  // CHECK:   [[ARG1:%.+]]: tensor<16x512x1x1xf16, {order = #NHWC}>
  func.func @main(%arg0: tensor<1x512x4x1xf16, {order = #NHWC}>, %arg1: tensor<16x512x1x1xf16, {order = #NHWC}>) -> tensor<1x16x4x1xf16, {order = #NHWC}> {
    %207 = VPU.SoftMax(%arg0) {axisInd = 1 : i64, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>} : tensor<1x512x4x1xf16, {order = #NHWC}> -> tensor<1x512x4x1xf16, {order = #NHWC}>

    %208 = VPU.NCE.Convolution(%207, %arg1) rawFilterShape [16, 512, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, input_padding = [0, 0, 0, 0], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, output_padding = [0, 0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.000000e+00 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>, strides = [1, 1]} : tensor<1x512x4x1xf16, {order = #NHWC}>, tensor<16x512x1x1xf16, {order = #NHWC}> -> tensor<1x16x4x1xf16, {order = #NHWC}>

    //  CHECK: [[SOFTMAX:%.+]] = VPU.SoftMax([[ARG0]])
    //  CHECK: [[CONV:%.+]] = VPU.NCE.Convolution([[SOFTMAX]], [[ARG1]])

    //  CHECK: return [[CONV]] : tensor<1x16x4x1xf16, {order = #NHWC}>

    return %208 : tensor<1x16x4x1xf16, {order = #NHWC}>
  }
}
...
!qElemType = !quant.uniform<f8E4M3FN:f16, 0.012>
!qElemType1 = !quant.uniform<f8E4M3FN:f16, 0.01>
!qElemType2 = !quant.uniform<i8:f16, 0.2:5>

// CHECK-LABEL: @DecomposedOpsInSdpaSoftmaxNonZeroZeroPoint
module @DecomposedOpsInSdpaSoftmaxNonZeroZeroPoint {
  config.PipelineOptions @Options {
    config.Option @config.AutoPaddingIDU : true
  }
  // CHECK:   ([[ARG0:%.+]]: tensor<1x48x1024x4x!qElemType, {order = #NHWC}>
  ...
  func.func @main(%arg0: tensor<1x48x1024x4x!qElemType, {order = #NHWC}>, %arg1: tensor<4096x48x1x1x!qElemType1, {order = #NHWC}>, %arg2: tensor<48x4096x1x1xf16, {order = #NHWC}>) -> tensor<1x48x1024x4x!qElemType2> {
    ...
    //  CHECK: return [[CONV_1]] : tensor<1x48x1024x4x!qElemType2>
...
// CHECK-LABEL: @DecomposedOpsInSdpaSoftmaxRelaxedRatio
module @DecomposedOpsInSdpaSoftmaxRelaxedRatio {
  config.PipelineOptions @Options {
    config.Option @config.AutoPaddingIDU : true
  }
  // CHECK:   ([[ARG0:%.+]]: tensor<1x48x128x4x!qElemType, {order = #NHWC}>
  // CHECK:   [[ARG1:%.+]]: tensor<1024x48x1x1x!qElemType1, {order = #NHWC}>
  // CHECK:   [[ARG2:%.+]]: tensor<48x1024x1x1xf16, {order = #NHWC}>
  func.func @main(%arg0: tensor<1x48x128x4x!qElemType, {order = #NHWC}>, %arg1: tensor<1024x48x1x1x!qElemType1, {order = #NHWC}>, %arg2: tensor<48x1024x1x1xf16, {order = #NHWC}>) -> tensor<1x48x128x4xf16, {order = #NHWC}> {
    %209 = VPU.NCE.Convolution(%arg0, %arg1) rawFilterShape [1024, 48, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, input_padding = [0, 8, 0, 0], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, output_padding = [0, 0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.200000e-4 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>, strides = [1, 1]} : tensor<1x48x128x4x!qElemType, {order = #NHWC}>, tensor<1024x48x1x1x!qElemType1, {order = #NHWC}> -> tensor<1x1024x128x4xf16, {order = #NHWC}>

    %207 = VPU.SoftMax(%209) {axisInd = 1 : i64, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>} : tensor<1x1024x128x4xf16, {order = #NHWC}> -> tensor<1x1024x128x4xf16, {order = #NHWC}>

    %208 = VPU.NCE.Convolution(%207, %arg2) rawFilterShape [48, 1024, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, input_padding = [0, 0, 0, 0], mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, output_padding = [0, 8, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.000000e+00 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>, strides = [1, 1]} : tensor<1x1024x128x4xf16, {order = #NHWC}>, tensor<48x1024x1x1xf16, {order = #NHWC}> -> tensor<1x48x128x4xf16, {order = #NHWC}>
    ...
    //  CHECK: [[CONV_2:%.+]] = VPU.NCE.Convolution([[EXPAND]], [[CST_1]], [[CST_2]])
    //  CHECK: [[ELTWISE:%.+]] = VPU.NCE.Eltwise([[CONV_1]], [[CONV_2]])
    //  CHECK: return [[ELTWISE]] : tensor<1x48x128x4xf16, {order = #NHWC}>

    return %208 : tensor<1x48x128x4xf16, {order = #NHWC}>
  }
}
