...
module @MultipleOpsInVF {
  ...
  func.func @main(%arg0: tensor<1x48x1024x4xf16, {order = #NHWC}>, %arg1: tensor<4096x48x1x1xf16, {order = #NHWC}>, %arg2: tensor<48x4096x1x1xf16, {order = #NHWC}>) -> tensor<1x48x1024x4xf16, {order = #NHWC}> {
    %0 = VPU.VerticalFusion (%arg0 as %arg3: tensor<1x48x1024x4xf16, {order = #NHWC}>, %arg1 as %arg4: tensor<4096x48x1x1xf16, {order = #NHWC}>, %arg2 as %arg6: tensor<48x4096x1x1xf16, {order = #NHWC}>) attributes {tilingStrategy = [1, 1, 19, 1]} -> tensor<1x48x1024x4xf16, {order = #NHWC}> {
      %1 = VPU.NCE.Convolution(%arg3, %arg4) rawFilterShape [4096, 48, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,  strides = [1, 1]} : tensor<1x48x1024x4xf16, {order = #NHWC}>, tensor<4096x48x1x1xf16, {order = #NHWC}> -> tensor<1x4096x1024x4xf16, {order = #NHWC}>
      ...
      %3 = VPU.NCE.Convolution(%2, %arg6) rawFilterShape [48, 4096, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,  strides = [1, 1]} : tensor<1x4096x1024x4xf16, {order = #NHWC}>, tensor<48x4096x1x1xf16, {order = #NHWC}> -> tensor<1x48x1024x4xf16, {order = #NHWC}>
      VPU.Yield %3
    }
...
// CHECK: func.func nested @main_vf1([[ARG0:%.+]]: tensor<1x48x1024x4xf16, {order = #NHWC}>, [[ARG1:%.+]]: tensor<4096x48x1x1xf16, {order = #NHWC}>, [[ARG2:%.+]]: tensor<48x4096x1x1xf16, {order = #NHWC}>) -> tensor<1x48x1024x4xf16, {order = #NHWC}> attributes {pure_vertical_fusion_region} {
...
// CHECK:    [[OP1:%.+]] = VPU.NCE.Convolution({{[^:]+}}, {{[^:]+}}) rawFilterShape [4096, 48, 1, 1] {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,
...
// CHECK:    [[OP3:%.+]] = VPU.NCE.Convolution([[OP2]], {{[^:]+}}) rawFilterShape [48, 4096, 1, 1] {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,
// CHECK-SAME: strides = [1, 1]}
...
module @ParallelConcatInput {
  ...
  func.func @main(%arg0: tensor<1x32x256x256xf16, {order = #NHWC}>) -> (tensor<1x64x256x256xf16, {order = #NHWC}>, tensor<1x64x256x256xf16, {order = #NHWC}>) {
      ...
      %0 = VPU.NCE.Convolution(%arg0, %cst_0) rawFilterShape [32, 32, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
        ppe = #VPU.PPEStub<>,
      ...
      %1 = VPU.NCE.Convolution(%arg0, %cst_0) rawFilterShape [32, 32, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
        ppe = #VPU.PPEStub<>,
        pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
         strides = [1, 1],
        tilingStrategy = [1, 1, 2, 1]}
...
// CHECK: func.func nested @main_vf1([[ARG:%.+]]: tensor<1x32x256x256xf16, {order = #NHWC}>) -> (tensor<1x64x256x256xf16, {order = #NHWC}>, tensor<1x64x256x256xf16, {order = #NHWC}>) {
// CHECK:  [[CST0:%.+]] = const.Declare tensor<32x32x3x3xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<32x32x3x3xf16, {order = #NHWC}>
// CHECK:  [[OP0:%.+]] = VPU.NCE.Convolution([[ARG]], [[CST0]]) rawFilterShape [32, 32, 3, 3] {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, ppe = #VPU.PPEStub<>,
// CHECK-SAME: strides = [1, 1], tilingStrategy = [1, 1, 2, 1]}
// CHECK-SAME:  -> tensor<1x32x256x256xf16, {order = #NHWC}>
// CHECK:  [[OP1:%.+]] = VPU.NCE.Convolution([[ARG]], [[CST0]]) rawFilterShape [32, 32, 3, 3] {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, ppe = #VPU.PPEStub<>,
// CHECK-SAME:  strides = [1, 1], tilingStrategy = [1, 1, 2, 1]}
// CHECK-SAME:  -> tensor<1x32x256x256xf16, {order = #NHWC}>
...
// CHECK: func.func nested @main_vf2({{[^:]+}}: tensor<1x64x256x256xf16, {order = #NHWC}>) -> tensor<1x64x256x256xf16, {order = #NHWC}> attributes {pure_vertical_fusion_region} {
...
// CHECK: func.func nested @main_vf1([[ARG:%.+]]: tensor<1x32x256x256xf16, {order = #NHWC}>) -> (tensor<1x32x256x256xf16, {order = #NHWC}>, tensor<1x16x256x256xf16, {order = #NHWC}>) {
...
// CHECK: func.func nested @main_vf2([[ARG0:%.+]]: tensor<1x32x256x256xf16, {order = #NHWC}>, [[ARG1:%.+]]: tensor<1x16x256x256xf16, {order = #NHWC}>) -> tensor<1x48x256x256xf16, {order = #NHWC}> {
...
// CHECK: func.func nested @main_vf1([[ARG:%.+]]: tensor<1x32x256x256xf16, {order = #NHWC}>) -> tensor<1x48x256x256xf16, {order = #NHWC}> {
// CHECK:  [[VF:%.+]] = VPU.VerticalFusion ([[ARG]] as {{[^:]+}}: tensor<1x32x256x256xf16, {order = #NHWC}>) attributes {tilingStrategy = [1, 1, 4, 1]} -> tensor<1x32x256x256xf16, {order = #NHWC}> {
// CHECK:                [[VFOP:%.+]] = VPU.SoftMax({{[^:]+}}) {axisInd = 1 : i64, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>} : tensor<1x32x256x256xf16, {order = #NHWC}> -> tensor<1x32x256x256xf16, {order = #NHWC}>
// CHECK:                VPU.Yield [[VFOP]]
// CHECK:               }
...
// CHECK: func.func nested @main_vf2([[ARG:%.+]]: tensor<1x48x256x256xf16, {order = #NHWC}>) -> tensor<1x80x256x256xf16, {order = #NHWC}> {
...
module @OutliningWithQuantizeCast {
  ...
  func.func @main(%arg0: tensor<1x48x1024x4x!qElemType1, {order = #NHWC}>, %arg1: tensor<4096x48x1x1x!qElemType, {order = #NHWC}>, %arg2: tensor<48x4096x1x1x!qElemType, {order = #NHWC}>) -> tensor<1x48x1024x4x!qElemType, {order = #NHWC}> {
    ...
    %1 = VPU.VerticalFusion (%0 as %arg3: tensor<1x48x1024x4x!qElemType, {order = #NHWC}>, %arg1 as %arg4: tensor<4096x48x1x1x!qElemType, {order = #NHWC}>, %arg2 as %arg6: tensor<48x4096x1x1x!qElemType, {order = #NHWC}>) attributes {tilingStrategy = [1, 1, 19, 1]} -> tensor<1x48x1024x4x!qElemType, {order = #NHWC}> {
      %1 = VPU.NCE.Convolution(%arg3, %arg4) rawFilterShape [4096, 48, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,  strides = [1, 1]} : tensor<1x48x1024x4x!qElemType, {order = #NHWC}>, tensor<4096x48x1x1x!qElemType, {order = #NHWC}> -> tensor<1x4096x1024x4x!qElemType, {order = #NHWC}>
      %2 = VPU.NCE.Convolution(%1, %arg6) rawFilterShape [48, 4096, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,  strides = [1, 1]} : tensor<1x4096x1024x4x!qElemType, {order = #NHWC}>, tensor<48x4096x1x1x!qElemType, {order = #NHWC}> -> tensor<1x48x1024x4x!qElemType, {order = #NHWC}>
      VPU.Yield %2
    }
...
// CHECK: func.func nested @main_vf1([[ARG0:%.+]]: tensor<1x48x1024x4x!qElemType, {order = #NHWC}>, [[ARG1:%.+]]: tensor<4096x48x1x1x!qElemType1, {order = #NHWC}>, [[ARG2:%.+]]: tensor<48x4096x1x1x!qElemType1, {order = #NHWC}>) -> tensor<1x48x1024x4x!qElemType1, {order = #NHWC}> attributes {pure_vertical_fusion_region} {
...
// CHECK:    [[OP2:%.+]] = VPU.NCE.Convolution([[OP1]], {{[^:]+}}) rawFilterShape [48, 4096, 1, 1] {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,
// CHECK-SAME:    strides = [1, 1]}
...
module @OutliningWithGroupSparseTensor {
  ...
  func.func @main(%arg0: tensor<1x16x1034x2058xf16, {order = #NHWC}>, %arg1: tensor<1x16x1024x2048xf16, {order = #NHWC}>, %arg2: tensor<1x16x1024x2048xf16, {order = #NHWC}>, %arg3: tensor<1x16x1034x2058xf16, {order = #NHWC}>) -> tensor<1x16x1024x2048xf16, {order = #NHWC}> {
    ...
    %2 = VPU.NCE.Convolution(%arg0, %1) rawFilterShape [16, 16, 11, 11] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
      ...
      ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 0.0069205216132104397 : f64>,
       strides = [1, 1], tilingStrategy = [1, 1, 47, 1]
    ...
    %3 = VPU.VerticalFusion (%2 as %arg4: tensor<1x16x1024x2048xf16, {order = #NHWC}>, %arg1 as %arg5: tensor<1x16x1024x2048xf16, {order = #NHWC}>, %arg2 as %arg6: tensor<1x16x1024x2048xf16, {order = #NHWC}>) attributes {scenario = #VPU.vf_scenario<FULL_PREFETCHING>, tilingStrategy = [1, 1, 1, 98]} -> tensor<1x16x1024x2048xf16, {order = #NHWC}> {
      %4 = VPU.NCE.Eltwise(%arg4, %arg5) {resultSegmentSizes = array<i32: 1, 0, 0, 0>, is_inplace = true, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, op_type = #VPU.eltwise_type<ADD>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, quant_scale = [1.000000e+00], fp_prelu_alpha = 1.000000e+00 : f64>} -> tensor<1x16x1024x2048xf16, {order = #NHWC}>
      %5 = VPU.NCE.Eltwise(%4, %arg6) {resultSegmentSizes = array<i32: 1, 0, 0, 0>, is_inplace = true, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, op_type = #VPU.eltwise_type<ADD>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, quant_scale = [1.000000e+00], fp_prelu_alpha = 1.000000e+00 : f64>} -> tensor<1x16x1024x2048xf16, {order = #NHWC}>
      VPU.Yield %5
    }

    %6 = VPU.NCE.Convolution(%arg3, %0) rawFilterShape [16, 16, 11, 11] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
      mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>,
      multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
      pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
      ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 0.0069205216132104397 : f64>,
    ...
    %7 = VPU.NCE.Eltwise(%6, %3) {resultSegmentSizes = array<i32: 1, 0, 0, 0>, is_inplace = true, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, op_type = #VPU.eltwise_type<ADD>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, quant_scale = [1.000000e+00], fp_prelu_alpha = 1.000000e+00 : f64>} -> tensor<1x16x1024x2048xf16, {order = #NHWC}>
...
// CHECK: func.func nested @main_vf1([[ARG0:%.+]]: tensor<1x16x1034x2058xf16, {order = #NHWC}>, [[ARG1:%.+]]: tensor<1x16x1024x2048xf16, {order = #NHWC}>, [[ARG2:%.+]]: tensor<1x16x1024x2048xf16, {order = #NHWC}>, [[ARG3:%.+]]: tensor<1x16x1034x2058xf16, {order = #NHWC}>) -> tensor<1x16x1024x2048xf16, {order = #NHWC}> {
...
// CHECK-NOT:  func.func nested @main_vf2
...
module @NotOutliningPotentiallyOptimizableDDR2DDRCopy {
  net.NetworkInfo entryPoint : @main
    inputsInfo : {
        DataInfo "input1" : tensor<1x5120x256x4xf16>
        DataInfo "input2" : tensor<7168x5120x1x1xui4>
        DataInfo "input3" : tensor<1x17920x1x1xf16>
    }
    outputsInfo : {
        DataInfo "output" : tensor<1x17920x1024x1xf16>
    }

  func.func @main(
    %arg0: tensor<1x5120x256x4xf16, {order = #NHWC}>, %arg1: tensor<7168x5120x1x1x!qElemType, {order = #NHWC}>,
    %arg2: tensor<1x17920x1x1xf16, {order = #NHWC}>)
        -> tensor<1x17920x1024x1xf16, {order = #NHWC}> {

    %conv = VPU.NCE.Convolution(%arg0, %arg1)  rawFilterShape [7168, 5120, 1, 1] {
      mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>,
      multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
      pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
      ppe = #VPU.PPEStub<>,
      resultSegmentSizes = array<i32: 1, 0, 0, 0>,
      strides = [1, 1], tilingStrategy = [1, 7, 32, 1]
    } : tensor<1x5120x256x4xf16, {order = #NHWC}>, tensor<7168x5120x1x1x!qElemType, {order = #NHWC}>
    -> tensor<1x7168x256x4xf16, {order = #NHWC}>


    // Concat (starting point)
    %0 = VPU.Concat(%conv, %conv, %conv, %conv, %conv) {
        static_offsets = [[0, 0, 0, 0], [0, 7168, 0, 0], [0, 14336, 0, 0], [0, 21504, 0, 0], [0, 28672, 0, 0]]
    } : tensor<1x7168x256x4xf16, {order = #NHWC}>,
        tensor<1x7168x256x4xf16, {order = #NHWC}>,
        tensor<1x7168x256x4xf16, {order = #NHWC}>,
        tensor<1x7168x256x4xf16, {order = #NHWC}>,
        tensor<1x7168x256x4xf16, {order = #NHWC}>
          -> tensor<1x35840x256x4xf16, {order = #NHWC}>

    // VF input 1: Slice from Concat
    %1 = VPU.Slice %0 [0, 0, 0, 0] [1, 17920, 256, 4]
      : tensor<1x35840x256x4xf16, {order = #NHWC}> to tensor<1x17920x256x4xf16, {order = #NHWC}>

    // VF block: Swish -> AffineReshape -> Multiply
    %2 = VPU.VerticalFusion (%1 as %arg3: tensor<1x17920x256x4xf16, {order = #NHWC}>, %arg2 as %arg4: tensor<1x17920x1x1xf16, {order = #NHWC}>)
          attributes {scenario = #VPU.vf_scenario<WEIGHTS_PREFETCHING>, tilingStrategy = [1, 19, 1, 1]}
      -> tensor<1x17920x1024x1xf16, {order = #NHWC}> {
        %3 = VPU.Swish(%arg3) {beta_value = 1.000000e+00 : f64, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>}
          : tensor<1x17920x256x4xf16, {order = #NHWC}> -> tensor<1x17920x256x4xf16, {order = #NHWC}>
        %4 = VPU.AffineReshape(%3) {dim_mapping = [[0], [1], [2], [2, 3]], shape_value = [1, 17920, 1024, 1]}
          : tensor<1x17920x256x4xf16, {order = #NHWC}> -> tensor<1x17920x1024x1xf16, {order = #NHWC}>
        %5 = VPU.Multiply(%4, %arg4) {
          auto_broadcast = #IE.auto_broadcast_type<NUMPY>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>
        } : tensor<1x17920x1024x1xf16, {order = #NHWC}>, tensor<1x17920x1x1xf16, {order = #NHWC}>
          -> tensor<1x17920x1024x1xf16, {order = #NHWC}>
        VPU.Yield %5
    }

    return %2: tensor<1x17920x1024x1xf16, {order = #NHWC}>
  }

// Outlining is not splitting the graph due to Concat -> Slice pattern

// CHECK: func.func nested @main_vf1
// CHECK:   [[CONV:%.+]] = VPU.NCE.Convolution
// CHECK:   [[CONCAT:%.+]] = VPU.Concat([[CONV]], [[CONV]], [[CONV]], [[CONV]], [[CONV]])
// CHECK:   [[SLICE:%.+]] = VPU.Slice [[CONCAT]] [0, 0, 0, 0] [1, 17920, 256, 4]
// CHECK:   [[VF:%.+]] = VPU.VerticalFusion ([[SLICE]] as [[INNER_ARG:%.+]]: tensor<1x17920x256x4xf16, {order = #NHWC}>, {{.+}} as [[INNER_ARG1:%.+]]: tensor<1x17920x1x1xf16, {order = #NHWC}>)
// CHECK:     [[SWISH:%.+]] = VPU.Swish([[INNER_ARG]])
// CHECK:     [[RESHAPE:%.+]] = VPU.AffineReshape([[SWISH]])
// CHECK:     [[MULTIPLY:%.+]] = VPU.Multiply([[RESHAPE]], [[INNER_ARG1]])
// CHECK:   return [[VF]]

// CHECK: func.func @main
// CHECK:   [[OUTLINE:%.+]] = call @main_vf1
// CHECK:  return [[OUTLINE]]
}
