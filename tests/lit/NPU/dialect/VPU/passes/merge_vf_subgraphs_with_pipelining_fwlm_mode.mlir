...
func.func @MergeVFWithViewOpAndGenericVFPipelining(
   ...
   %0 = VPU.VerticalFusion (%arg0 as %arg2: tensor<1x48x1024x4x!qElemType0, {order = #NHWC}>,
      ...
      %3 = VPU.NCE.Convolution(%2, %arg3) rawFilterShape [4096, 48, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
      ppe = #VPU.PPEStub<>,
       strides = [1, 1]} : tensor<1x48x1024x4x!qElemType2, {order = #NHWC}>, tensor<4096x48x1x1x!qElemType1, {order = #NHWC}> -> tensor<1x4096x1024x4xf16, {order = #NHWC}>
      VPU.Yield %3
   }
...
// CHECK-LABEL: @DoNotAdjustMVNStrategyFromSOKToSOH
// CHECK-SAME: ([[ARG_0:%[^:]+]]: tensor<1x64x64x48xf16, {order = #NHWC}>)
//
// VF0 wraps MVN with across_channels=false and SplitOverKernel.
// VF1 wraps SoftMax with SplitOverHeight.
//
// The strategies are incompatible (SOK vs SOH). The v2 rewriter calls
// adjustMCStrategyInMergedVF, which calls alignMCStrategy and tries to
// change MVN from SOK to SOH. MVN::checkStrategyCompatibility returns false
// for SOH when across_channels=false (normalization statistics require the
// full H×W of each channel on one cluster). alignMCStrategy finds no valid
// replacement strategy and returns failure, so adjustMCStrategyInMergedVF
// returns false and the merge is rejected.
//
// The two VF blocks must remain separate with their original strategies intact:
// MVN keeps SplitOverKernel and SoftMax keeps SplitOverHeight.
func.func @DoNotAdjustMVNStrategyFromSOKToSOH(
        %arg0: tensor<1x64x64x48xf16, {order = #NHWC}>)
        -> tensor<1x64x64x48xf16, {order = #NHWC}> {

    // VF0: MVN with SplitOverKernel — correct for across_channels=false
    %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x64x64x48xf16, {order = #NHWC}>)
            attributes {tilingStrategy = [1, 1, 1, 1]} -> tensor<1x64x64x48xf16, {order = #NHWC}> {
        %2 = VPU.MVN(%arg1) {
            across_channels = false,
            eps = 9.9999997473787516E-6 : f64,
            multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
            normalize_variance = true
        } : tensor<1x64x64x48xf16, {order = #NHWC}> -> tensor<1x64x64x48xf16, {order = #NHWC}>
        VPU.Yield %2
    }

    // VF1: SoftMax with SplitOverHeight — incompatible with MVN's SOK
    %1 = VPU.VerticalFusion (
            %0 as %arg1: tensor<1x64x64x48xf16, {order = #NHWC}>)
            attributes {tilingStrategy = [1, 1, 1, 1]} -> tensor<1x64x64x48xf16, {order = #NHWC}> {
        %2 = VPU.SoftMax(%arg1) {
            axisInd = 1 : i64,
            multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>
    ...
    return %1 : tensor<1x64x64x48xf16, {order = #NHWC}>

    // VFs must NOT merge. MVN::checkStrategyCompatibility blocks SOH for
    // across_channels=false, so alignMCStrategy returns failure and
    // adjustMCStrategyInMergedVF returns false.
    // CHECK:      [[VF0:%.+]] = VPU.VerticalFusion ([[ARG_0]] as
    // CHECK-SAME:     tilingStrategy = [1, 1, 1, 1]
    // CHECK:          VPU.MVN
    // CHECK-SAME:         multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>
    // CHECK:      [[VF1:%.+]] = VPU.VerticalFusion ([[VF0]] as
    // CHECK-SAME:     tilingStrategy = [1, 1, 1, 1]
    // CHECK:          VPU.SoftMax
...
func.func @MergeSubgraphsWithSoftMaxPermuteCastAffineReshapeConv(
    ...
    //CHECK-SAME: attributes {scenario = #VPU.vf_scenario<VF_PIPELINING>, tilingStrategy = [1, 1, 24, 1]}
