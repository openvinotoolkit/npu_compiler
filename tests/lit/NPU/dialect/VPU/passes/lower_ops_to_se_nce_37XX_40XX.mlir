...
func.func @TransposedConvolutionWithPostOp(%input: tensor<1x32x64x1xf16, {order = #NHWC}>) -> tensor<1x16x128x2xf16, {order = #NHWC}> {
    ...
    // CHECK:       [[OUTPUT:%.+]] = VPU.NCE.Convolution([[INPUT_SPARSE]], [[WEIGHTS]], [[WEIGHTS_TABLE]]) rawFilterShape [16, 32, 3, 2] {
    // CHECK-SAME:      pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
...
func.func @InterpolateBilinearHalfPixelWithEvenScale(%arg0: tensor<1x16x3x3xf16, {order = #NHWC}>) -> tensor<1x16x6x6xf16, {order = #NHWC}> {
    ...
    // CHECK:       [[INPUT_SM:%.+]] = const.Declare tensor<1x16x8x8xi1, {order = #NHWC}> =
    // CHECK-SAME:      dense<1> : tensor<1x16x8x8xi8>, [#const.Reorder<#NHWC>, #const.CastElemType<i1>]
    // CHECK:       [[INPUT_SPARSE:%.+]] = VPU.GroupSparseTensor([[INPUT_DATA]], [[INPUT_SM]], [[INPUT_SE]])
    // CHECK-SAME:      {seAttr = #VPU.SEInterpolate<mode = <BILINEAR>, coordinate_transformation_mode = <HALF_PIXEL>,
    // CHECK-SAME:                scale = [1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00]>}
    // CHECK-SAME:    -> !VPU.SparseTensor<data=tensor<1x16x3x3xf16, {order = #NHWC}>,
    // CHECK-SAME:                         sparsity_map=tensor<1x16x8x8xi1, {order = #NHWC}>,
...
func.func @RollToNCE(%input: tensor<1x16x80x80xf16, {order = #NHWC}>) -> tensor<1x16x80x80xf16, {order = #NHWC}> {
    ...
    // CHECK:       [[OUTPUT:%.+]] = VPU.NCE.Convolution([[INPUT_SPARSE]], [[WEIGHTS]], [[WEIGHTS_TABLE]]) rawFilterShape [16, 16, 1, 1] {
    // CHECK-SAME:      pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    // CHECK-SAME:      strides = [1, 1]
    // CHECK-SAME:  }
...
func.func @LowerReflectPadOpToNCE(%input: tensor<1x64x80x80xf16, {order = #NHWC}>) -> tensor<1x64x83x83xf16, {order = #NHWC}> {
    ...
    // CHECK-DAG:   [[INPUT_SM:%.+]] = const.Declare tensor<1x64x83x83xi1, {order = #NHWC}> = dense<1>
    // CHECK-SAME:          : tensor<1x64x83x83xi8>, [#const.Reorder<#NHWC>, #const.CastElemType<i1>]
    // CHECK:       [[INPUT_SPARSE:%.+]] = VPU.GroupSparseTensor([[INPUT_DATA]], [[INPUT_SM]], [[INPUT_SE]])
    // CHECK-SAME:          seAttr = #VPU.SEPadding<mode = <REFLECT>, padding = [1, 2, 2, 1]>
    // CHECK-SAME:      } -> !VPU.SparseTensor<data=tensor<1x64x80x80xf16, {order = #NHWC}>,
    // CHECK-SAME:                         sparsity_map=tensor<1x64x83x83xi1, {order = #NHWC}>,
    ...
    // CHECK:       [[OUTPUT:%.+]] = VPU.NCE.Convolution([[INPUT_SPARSE]], [[WEIGHTS]], [[WEIGHTS_TABLE]]) rawFilterShape [64, 64, 1, 1] {
    // CHECK-SAME:      pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    // CHECK-SAME:      strides = [1, 1]
    // CHECK-SAME:  }
...
func.func @ReflectPadOpToNCEQuantized(%input: tensor<1x48x80x80x!qElemType, {order = #NHWC}>) -> tensor<1x48x83x83x!qElemType, {order = #NHWC}> {
    ...
    // CHECK-SAME{LITERAL}: = dense<[[[[0, 0, 1073745408, 0]]], [[[48, 0, 1073745408, 0]]], [[[96, 0, 1073745408, 0]]], [[[144, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[192, 0, 1073745408, 0]]], [[[240, 0, 1073745408, 0]]], [[[288, 0, 1073745408, 0]]], [[[336, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[384, 0, 1073745408, 0]]], [[[432, 0, 1073745408, 0]]], [[[480, 0, 1073745408, 0]]], [[[528, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[576, 0, 1073745408, 0]]], [[[624, 0, 1073745408, 0]]], [[[672, 0, 1073745408, 0]]], [[[720, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[768, 0, 1073745408, 0]]], [[[816, 0, 1073745408, 0]]], [[[864, 0, 1073745408, 0]]], [[[912, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[960, 0, 1073745408, 0]]], [[[1008, 0, 1073745408, 0]]], [[[1056, 0, 1073745408, 0]]], [[[1104, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[1152, 0, 1073745408, 0]]], [[[1200, 0, 1073745408, 0]]], [[[1248, 0, 1073745408, 0]]], [[[1296, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[1344, 0, 1073745408, 0]]], [[[1392, 0, 1073745408, 0]]], [[[1440, 0, 1073745408, 0]]], [[[1488, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[1536, 0, 1073745408, 0]]], [[[1584, 0, 1073745408, 0]]], [[[1632, 0, 1073745408, 0]]], [[[1680, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[1728, 0, 1073745408, 0]]], [[[1776, 0, 1073745408, 0]]], [[[1824, 0, 1073745408, 0]]], [[[1872, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[1920, 0, 1073745408, 0]]], [[[1968, 0, 1073745408, 0]]], [[[2016, 0, 1073745408, 0]]], [[[2064, 0, 1073745408, 0]]],
    // CHECK-SAME{LITERAL}:         [[[2112, 0, 1073745408, 0]]], [[[2160, 0, 1073745408, 0]]], [[[2208, 0, 1073745408, 0]]], [[[2256, 0, 1073745408, 0]]]]>
    // CHECK-SAME:            tensor<48x1x1x4xsi32>

    // CHECK:       [[OUTPUT:%.+]] = VPU.NCE.Convolution([[INPUT_SPARSE]], [[WEIGHTS]], [[WEIGHTS_TABLE]]) rawFilterShape [48, 48, 1, 1] {
    // CHECK-SAME:      pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    // CHECK-SAME:      strides = [1, 1]
    // CHECK-SAME:  }
...
func.func @FuseReflectPadOpToConv(%input: tensor<1x48x80x80xf16, {order = #NHWC}>) -> tensor<1x16x81x81xf16, {order = #NHWC}> {
    ...
    %conv = VPU.NCE.Convolution(%pad, %weights, %weights_table) rawFilterShape [16, 48, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
    ...
    // CHECK:       [[OUTPUT:%.+]] = VPU.NCE.Convolution([[INPUT_SPARSE]], [[WEIGHTS]], [[WEIGHTS_TABLE]]) rawFilterShape [16, 48, 3, 3] {
    // CHECK-SAME:          pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
