...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% compilation-mode=DefaultHW allow-custom-values=true" --cmx-stack-frames-reserve-mem --cmx-metadata-reserve-mem --merge-vertical-fusion-subgraphs %s | FileCheck %s
...
// CHECK-LABEL: @VerticalFuseEltwiseWithTwoParents
// CHECK-SAME:      [[INPUT:%.+]]: tensor<1x16x256x256x!qElemType, {order = #NHWC}>
func.func @VerticalFuseEltwiseWithTwoParents(%arg0: tensor<1x16x256x256x!qElemType, {order = #NHWC}>) -> (tensor<1x32x256x256x!qElemType, {order = #NHWC}>, tensor<1x32x256x256x!qElemType, {order = #NHWC}>) {
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x16x256x256x!qElemType, {order = #NHWC}>, %cst_0 as %arg2: tensor<32x16x3x3x!qElemType1, {order = #NHWC}>, %cst_1 as %arg3: tensor<32x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 1, 1]} -> tensor<1x32x256x256x!qElemType, {order = #NHWC}> {
      %4 = VPU.NCE.Convolution(%arg1, %arg2, %arg3) rawFilterShape [32, 16, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
         ppe = #VPU.PPEStub<>,
    ...
    %2 = VPU.VerticalFusion (%0 as %arg1: tensor<1x32x256x256x!qElemType, {order = #NHWC}>, %cst_2 as %arg2: tensor<32x32x3x3x!qElemType1, {order = #NHWC}>, %cst_3 as %arg3: tensor<32x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 2, 1]} -> tensor<1x32x256x256x!qElemType, {order = #NHWC}> {
      %4 = VPU.NCE.Convolution(%arg1, %arg2, %arg3) rawFilterShape [32, 32, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
         ppe = #VPU.PPEStub<>,
         pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
    ...
    //CHECK: [[CST:%.+]] = const.Declare tensor<32x16x3x3x!qElemType1, {order = #NHWC}>
    //CHECK: [[CST_0:%.+]] = const.Declare tensor<32x1x1x4xsi32>
    //CHECK: [[CST_1:%.+]] = const.Declare tensor<32x32x3x3x!qElemType1, {order = #NHWC}>

    //CHECK: [[VERTICAL_FUSION0:%.+]] = VPU.VerticalFusion ([[INPUT]] as [[ARG0:%.+]]: tensor<1x16x256x256x!qElemType, {order = #NHWC}>
    //CHECK-SAME:                                           [[CST]] as [[ARG1:%.+]]: tensor<32x16x3x3x!qElemType1, {order = #NHWC}>
    //CHECK-SAME:                                           [[CST_0]] as [[ARG2:%.+]]: tensor<32x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 1, 1]} -> tensor<1x32x256x256x!qElemType, {order = #NHWC}> {
    //CHECK: [[CONV:%.+]] = VPU.NCE.Convolution([[ARG0]], [[ARG1]], [[ARG2]])
    //CHECK: VPU.Yield [[CONV]]

    //CHECK: [[VERTICAL_FUSION1:%.+]] = VPU.VerticalFusion ([[VERTICAL_FUSION0]] as [[ARG0:%.+]]: tensor<1x16x256x256x!qElemType, {order = #NHWC}>
    //CHECK-SAME:                                           [[CST]] as [[ARG1:%.+]]: tensor<32x16x3x3x!qElemType1, {order = #NHWC}>
    //CHECK-SAME:                                           [[CST_0]] as [[ARG2:%.+]]: tensor<32x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 1, 1]} -> tensor<1x32x256x256x!qElemType, {order = #NHWC}> {
    //CHECK: [[CONV:%.+]] = VPU.NCE.Convolution([[ARG0]], [[ARG1]], [[ARG2]])
    //CHECK: VPU.Yield [[CONV]]

    //CHECK: [[VERTICAL_FUSION2:%.+]] = VPU.VerticalFusion ([[VERTICAL_FUSION0]] as [[ARG0:%.+]]: tensor<1x32x256x256x!qElemType, {order = #NHWC}>
    //CHECK-SAME:                                           [[CST_1]] as [[ARG1:%.+]]: tensor<32x32x3x3x!qElemType1, {order = #NHWC}>
    //CHECK-SAME:                                           [[CST_0]] as [[ARG2:%.+]]: tensor<32x1x1x4xsi32>) attributes {scenario = #VPU.vf_scenario<FULL_PREFETCHING>, tilingStrategy = [1, 1, 2, 1]} -> tensor<1x32x256x256x!qElemType, {order = #NHWC}> {
    //CHECK: [[CONV:%.+]] = VPU.NCE.Convolution([[ARG0]], [[ARG1]], [[ARG2]])
    //CHECK: [[ELTWISE:%.+]] = VPU.NCE.Eltwise([[ARG0]], [[CONV]])
...
func.func @MergeSubgraphsWithCompatibleDistributedTensorType(%arg0: tensor<1x96x176x176xf16, {order = #NHWC}>) -> tensor<1x96x176x176xf16, {order = #NHWC}> {
    ...
    //CHECK-SAME:                        [[CST_0]] as [[ARG_3:%[^:]+]]: tensor<96x1x1x4xsi32>) attributes {scenario = #VPU.vf_scenario<FULL_PREFETCHING>, tilingStrategy = [1, 1, 4, 1]} -> tensor<1x96x176x176xf16, {order = #NHWC}> {
...
func.func @BuildSubgraphCTilingEltwise(%arg0: tensor<1x512x12x512xf16, {order = #NHWC}>) -> tensor<1x512x12x512xf16, {order = #NHWC}> {
  %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x512x12x512xf16, {order = #NHWC}>) attributes {tilingStrategy = [1, 3, 1, 1]} -> tensor<1x512x12x512xf16, {order = #NHWC}> {
      %2 = VPU.NCE.Eltwise(%arg1, %arg1)
         {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, op_type = #VPU.eltwise_type<ADD>, ppe = #VPU.PPEStub<>} -> tensor<1x512x12x512xf16, {order = #NHWC}>
      VPU.Yield %2
  }
...
func.func @BuildSubgraphCTilingToWTiling(%arg0: tensor<1x512x12x512xf16, {order = #NHWC}>) -> tensor<1x512x12x512xf16, {order = #NHWC}> {
  ...
  %1 = VPU.VerticalFusion (%0 as %arg1: tensor<1x512x12x512xf16, {order = #NHWC}>, %cst_0 as %arg2: tensor<512x512x1x1xf16, {order = #NHWC}>, %cst_1 as %arg3: tensor<512x1x1x4xsi32>) attributes {tilingStrategy = [1, 4, 1, 1]} -> tensor<1x512x12x512xf16, {order = #NHWC}> {
      %2 = VPU.NCE.Convolution(%arg1, %arg2, %arg3) rawFilterShape [512, 512, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,  strides = [1, 1]} : tensor<1x512x12x512xf16, {order = #NHWC}>, tensor<512x512x1x1xf16, {order = #NHWC}>, tensor<512x1x1x4xsi32> -> tensor<1x512x12x512xf16, {order = #NHWC}>
      VPU.Yield %2
  }
  return %1 : tensor<1x512x12x512xf16, {order = #NHWC}>
...
func.func @PipeliningWithoutCostCondition(%arg0: tensor<1x768x128x4xf16, {order = #NHWC}>) -> tensor<1x3072x128x4xf16, {order = #NHWC}> {
  ...
  %0 = VPU.VerticalFusion (%arg0 as %arg1: tensor<1x768x128x4xf16, {order = #NHWC}>, %cst_0 as %arg2: tensor<3072x768x1x1xf16, {order = #NHWC}>, %cst_1 as %arg3: tensor<3072x1x1x4xsi32>) attributes {tilingStrategy = [1, 8, 1, 1]} -> tensor<1x3072x128x4xf16, {order = #NHWC}> {
    %2 = VPU.NCE.Convolution(%arg1, %arg2, %arg3) rawFilterShape [3072, 768, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,  strides = [1, 1]} : tensor<1x768x128x4xf16, {order = #NHWC}>, tensor<3072x768x1x1xf16, {order = #NHWC}>, tensor<3072x1x1x4xsi32> -> tensor<1x3072x128x4xf16, {order = #NHWC}>
    VPU.Yield %2
  }
...
// CHECK-LABEL: @BuildSubgraphMinimalRequirements
// CHECK-SAME: ([[ARG_0:%[^:]+]]: tensor<1x768x128x4xf16, {order = #NHWC}>)
func.func @BuildSubgraphMinimalRequirements(%arg0: tensor<1x768x128x4xf16, {order = #NHWC}>) -> tensor<1x3072x128x4xf16, {order = #NHWC}> {
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg2: tensor<1x768x128x4xf16, {order = #NHWC}>, %cst_0 as %arg3: tensor<3072x768x1x1x!qElemType, {order = #NHWC}>, %cst_1 as %arg4: tensor<3072x1x1x4xsi32>) attributes {tilingStrategy = [1, 1, 5, 1]} -> tensor<1x3072x128x4xf16, {order = #NHWC}> {
      %4 = VPU.NCE.Convolution(%arg2, %arg3, %arg4) rawFilterShape [3072, 768, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,  strides = [1, 1]} : tensor<1x768x128x4xf16, {order = #NHWC}>, tensor<3072x768x1x1x!qElemType, {order = #NHWC}>, tensor<3072x1x1x4xsi32> -> tensor<1x3072x128x4xf16, {order = #NHWC}>
      VPU.Yield %4
    }
    ...
    // CHECK-DAG:   [[CST:%.+]] = const.Declare tensor<3072x768x1x1x!qElemType, {order = #NHWC}> = dense<1.000000e+00> : tensor<3072x768x1x1xf16>
    // CHECK-DAG:   [[CST_0:%.+]] = const.Declare tensor<3072x1x1x4xsi32> = dense<1> : tensor<3072x1x1x4xsi32>

    //CHECK: [[VERTICAL_FUSION:%.+]] = VPU.VerticalFusion ([[ARG_0]] as [[ARG_1:%[^:]+]]: tensor<1x768x128x4xf16, {order = #NHWC}>, [[CST]] as [[ARG_2:%[^:]+]]: tensor<3072x768x1x1x!qElemType, {order = #NHWC}>, [[CST_0]] as [[ARG_3:%[^:]+]]: tensor<3072x1x1x4xsi32>)
...
module @executors {
  ...
  func.func @BuildSubgraphWithOtherAxis(%arg0: tensor<1x48x512x128xf16, {order = #NHWC}>, %arg1: tensor<1x12x512x512xf16>) -> tensor<1x12x512x512xf16, {order = #NHCW}> {
    ...
    %0 = VPU.VerticalFusion (%shapecast as %arg3: tensor<1x512x12x512xf16, {order = #NHWC}>,
        ...
        %2 = VPU.NCE.Eltwise(%arg3, %arg4) {resultSegmentSizes = array<i32: 1, 0, 0, 0>, is_inplace = true, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
    ...
    %1 = VPU.VerticalFusion (%0 as %arg3: tensor<1x512x12x512xf16, {order = #NHWC}>, %permutecast0 as %arg4: tensor<1x512x12x512xf16, {order = #NHWC}>) attributes {tilingStrategy = [1, 4, 1, 1]} -> tensor<1x512x12x512xf16, {order = #NWHC}> {
        %2 = VPU.NCE.Eltwise(%arg3, %arg4) {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
            op_type = #VPU.eltwise_type<ADD>, ppe = #VPU.PPEStub<>}
...
// CHECK-SAME:      ([[INPUT_0:%.+]]: tensor<1x64x160x160xf16, {order = #NHWC}>, [[INPUT_1:%.+]]: tensor<1x64x160x160xf16, {order = #NHWC}>) -> tensor<1x64x160x160xf16, {order = #NHWC}> {
func.func @BuildSubgraphWhenSpillIsBetweenVFBlocks(
  ...
  %0 = VPU.VerticalFusion (
      ...
      %2 = VPU.NCE.Convolution(%arg2, %arg3, %arg4) rawFilterShape [64, 64, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
            pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
            ppe = #VPU.PPEStub<>,

            strides = [1, 1]} : tensor<1x64x160x160xf16, {order = #NHWC}>, tensor<64x64x3x3xf16, {order = #NHWC}>, tensor<64x1x1x4xsi32> -> tensor<1x64x160x160xf16, {order = #NHWC}>
      VPU.Yield %2
  }
  ...
  %1 = VPU.VerticalFusion (
      ...
      %2 = VPU.NCE.Eltwise(%arg2, %arg3) {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            is_inplace = true,
            multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
...
func.func @BuildSubgraphTwoConvTileChannelVF(%arg0: tensor<1x128x112x112xf16, {order = #NHWC}>) -> tensor<1x128x112x112xf16, {order = #NHWC}> {
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg2: tensor<1x128x112x112xf16, {order = #NHWC}>,
      ...
      %2 = VPU.NCE.Convolution(%arg2, %arg3, %arg4) rawFilterShape [128, 128, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
              multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
              pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
              ppe = #VPU.PPEStub<>,
               strides = [1, 1]
            } : tensor<1x128x112x112xf16, {order = #NHWC}>, tensor<128x128x1x1xf16, {order = #NHWC}>, tensor<128x1x1x4xsi32> -> tensor<1x128x112x112xf16, {order = #NHWC}>
      VPU.Yield %2
    }
    ...
    %1 = VPU.VerticalFusion (%0 as %arg5: tensor<1x128x112x112xf16, {order = #NHWC}>,
                  ...
                  attributes {tilingStrategy = [1, 3, 1, 1]} -> tensor<1x128x112x112xf16, {order = #NHWC}> {
      %2 = VPU.NCE.Convolution(%arg5, %arg6, %arg7) rawFilterShape [128, 128, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
              multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
              pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
              ppe = #VPU.PPEStub<>,
               strides = [1, 1]
    ...
    //CHECK:      [[NCE0:%.+]] = VPU.NCE.Convolution([[INNER_ARG1]], [[INNER_ARG2]], [[INNER_ARG3]]) rawFilterShape [128, 128, 1, 1] {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>
    //CHECK:      [[NCE1:%.+]] = VPU.NCE.Convolution([[NCE0]], [[INNER_ARG2]], [[INNER_ARG3]]) rawFilterShape [128, 128, 1, 1] {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>
...
func.func @BuildSubgraphWithViewLikeOps(
  ...
  %2 = VPU.VerticalFusion (%1 as %arg2: tensor<1x512x12x512x!qElemType1, {order = #NHWC}>, %arg1 as %arg3: tensor<1x512x12x512x!qElemType3, {order = #NHWC}>) attributes {tilingStrategy = [1, 1, 1, 3]} -> tensor<1x512x12x512xf16, {order = #NHWC}> {
    ...
    %4 = VPU.NCE.Eltwise(%3, %arg3) {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, op_type = #VPU.eltwise_type<ADD>, ppe = #VPU.PPEStub<>} -> tensor<1x512x12x512xf16, {order = #NHWC}>
    VPU.Yield %4
  }
    ...
    //CHECK:      [[ELTWISE:%.+]] = VPU.NCE.Eltwise([[QC]], [[INNER_ARG1]]) {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>, op_type = #VPU.eltwise_type<ADD>, ppe = #VPU.PPEStub<>, resultSegmentSizes = array<i32: 1, 0, 0, 0>} -> tensor<1x512x12x512xf16, {order = #NHWC}>
    //CHECK:      VPU.Yield [[ELTWISE]]
...
func.func @DontMergeSpillingWeightsToVF(%arg0: tensor<1x512x26x26x!qElemType, {order = #NHWC}>) -> tensor<1x1024x13x13x!qElemType, {order = #NHWC}> {
    ...
    %0 = VPU.VerticalFusion (
        ...
        %inner = VPU.NCE.Convolution(%arg1, %arg2, %arg3) rawFilterShape [512, 1024, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
                pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,
                strides = [1, 1]}
                    : tensor<1x1024x13x13x!qElemType, {order = #NHWC}>, tensor<512x1024x1x1x!qElemType, {order = #NHWC}>, tensor<512x1x1x4xsi32> -> tensor<1x512x13x13x!qElemType, {order = #NHWC}>
        VPU.Yield %inner
    }
    ...
    %1 = VPU.VerticalFusion (
        ...
        %inner = VPU.NCE.Convolution(%arg1, %arg2, %arg3) rawFilterShape [1024, 512, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
                pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, ppe = #VPU.PPEStub<>,
                strides = [1, 1]}
                    : tensor<1x512x13x13x!qElemType, {order = #NHWC}>, tensor<1024x512x3x3x!qElemType, {order = #NHWC}>, tensor<1024x1x1x4xsi32> -> tensor<1x1024x13x13x!qElemType, {order = #NHWC}>
        VPU.Yield %inner
    }
...
func.func @MergeVFWithoutTilingC(%arg0: tensor<1x64x1x19284xf16, {order = #NHWC}>, %arg1: tensor<1x64x1x19284xf16, {order = #NHWC}>)
    ...
    //CHECK:	  [[VERTICAL_FUSION:%.+]] = VPU.VerticalFusion ([[INPUT0]] as [[INNER_ARG0:%.+]]: tensor<1x64x1x19284xf16, {order = #NHWC}>, [[INPUT1]] as [[INNER_ARG1:%.+]]: tensor<1x64x1x19284xf16, {order = #NHWC}>)
...
func.func @MergeVFWithTOC(%input: tensor<1x128x16x4xf16, {order = #NHWC}>, %input_1: tensor<1x128x16x4xf16, {order = #NHWC}>,
    ...
    %conv0 = VPU.VerticalFusion (%input as %arg1: tensor<1x128x16x4xf16, {order = #NHWC}>,
      ...
      %inner = VPU.NCE.Convolution(%arg1, %arg2, %arg3) rawFilterShape [6320, 128, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>,
            strides = [1, 1]} : tensor<1x128x16x4xf16, {order = #NHWC}>, tensor<6320x128x1x1x!qElemType, {order = #NHWC}>, tensor<6320x1x1x4xsi32> -> tensor<1x6320x16x4xf16, {order = #NHWC}>
    ...
    %eltwise = VPU.VerticalFusion (%conv1 as %arg1: tensor<1x6320x16x4xf16, {order = #NHWC}>,
            ...
            %inner = VPU.NCE.Eltwise(%arg1, %arg2) {resultSegmentSizes = array<i32: 1, 0, 0, 0>, is_inplace = true, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
...
func.func @BuildSubgraphIncludesNCEAndSWOpsTOC(
    ...
    %0 = VPU.VerticalFusion (%arg0 as %arg9: tensor<1x128x256x4xf16, {order = #NHWC}>,
      ...
      %8 = VPU.NCE.Convolution(%arg9, %arg10, %arg11) rawFilterShape [7584, 128, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
              multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
              pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
              ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64,
              lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,
              strides = [1, 1]
      } : tensor<1x128x256x4xf16, {order = #NHWC}>, tensor<7584x128x1x1x!qElemType, {order = #NHWC}>, tensor<7584x1x1x4xsi32> -> tensor<1x7584x256x4xf16, {order = #NHWC}>
      VPU.Yield %8
    }
    ...
    %4 = VPU.VerticalFusion (%1 as %arg9: tensor<1x7584x256x4xf16, {order = #NHWC}>,
      ...
      %8 = VPU.NCE.Eltwise(%arg9, %arg10) {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
              is_inplace = true,
              multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
              op_type = #VPU.eltwise_type<ADD>,
              ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64,
...
func.func @MergeDequantizeAndConvSOK() -> tensor<1x1280x8x8xf16, {order = #NHWC}> {
    ...
    %2 = VPU.VerticalFusion (%1 as %arg0: tensor<1x1280x8x8xf16, {order = #NHWC}>, %0 as %arg1: tensor<1280x1280x3x3xf16, {order = #NHWC}>, %cst as %arg2: tensor<1280x1x1x4xsi32>) attributes {tilingStrategy = [1, 7, 1, 1]} -> tensor<1x1280x8x8xf16, {order = #NHWC}> {
      %3 = VPU.NCE.Convolution(%arg0, %arg1, %arg2) rawFilterShape [1280, 1280, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, ppe = #VPU.PPEStub<>,  strides = [1, 1]} : tensor<1x1280x8x8xf16, {order = #NHWC}>, tensor<1280x1280x3x3xf16, {order = #NHWC}>, tensor<1280x1x1x4xsi32> -> tensor<1x1280x8x8xf16, {order = #NHWC}>
      VPU.Yield %3
    }
    ...
    // CHECK:   [[CONV:%.+]] = VPU.NCE.Convolution([[INNER_ARG1]], [[DEQUANTIZED_WEIGHT]], [[INNER_ARG2]]) rawFilterShape [1280, 1280, 3, 3] {multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>, ppe = #VPU.PPEStub<>
    // CHECK-SAME: strides = [1, 1]}
    // CHECK-SAME:   -> tensor<1x1280x8x8xf16, {order = #NHWC}>
    // CHECK:   VPU.Yield [[CONV]]
    // CHECK: }
...
func.func @NotMergeVFDueToCost(%input: tensor<1x256x48x72xf16, {order = #NHWC}>,
      ...
      %0 = VPU.VerticalFusion (%input as %arg1: tensor<1x256x48x72x!qElemType, {order = #NHWC}>,
        ...
        %inner = VPU.NCE.Convolution(%arg1, %arg2, %arg3) rawFilterShape [512, 256, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>,
                                                           multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
                                                           pad = #VPU.Padding<left = 1 : i64, right = 0 : i64, top = 1 : i64, bottom = 0 : i64>,
                                                           ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = 0 : i64, clamp_high = 255 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [2, 2]} : tensor<1x256x48x72x!qElemType, {order = #NHWC}>, tensor<512x256x3x3x!qElemType, {order = #NHWC}>, tensor<512x1x1x4xsi32> -> tensor<1x512x24x36x!qElemType, {order = #NHWC}>
        VPU.Yield %inner
      }
      %1 = VPU.VerticalFusion (%0 as %arg1: tensor<1x512x24x36x!qElemType, {order = #NHWC}>,
        ...
        %inner = VPU.NCE.Convolution(%arg1, %arg2, %arg3) rawFilterShape [512, 512, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>,
                                                           multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
                                                           pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
                                                           ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = 0 : i64, clamp_high = 255 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x512x24x36x!qElemType, {order = #NHWC}>, tensor<512x512x3x3x!qElemType, {order = #NHWC}>, tensor<512x1x1x4xsi32> -> tensor<1x512x24x36x!qElemType, {order = #NHWC}>
        VPU.Yield %inner
      }
