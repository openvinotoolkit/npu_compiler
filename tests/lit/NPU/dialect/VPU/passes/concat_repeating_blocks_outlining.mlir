...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --concat-repeating-blocks-outlining="min-seq-length=2" %s | FileCheck %s
...
module @ConcatsRepeatingInputBranches {
    ...
    func.func @main(%input: tensor<1x48x32x32xf16, {order = #NHWC}>) -> tensor<1x192x32x32xf16, {order = #NHWC}> {
        ...
        // CHECK:  func.func nested @main_concat1([[ARG0:%.+]]: tensor<1x48x32x32xf16, {order = #NHWC}>) -> tensor<1x96x32x32xf16, {order = #NHWC}> {
        ...
        // CHECK:      return [[CONCAT]]
        // CHECK:  }
        // CHECK:  func.func nested @main_concat2([[ARG0:%.+]]: tensor<1x96x32x32xf16, {order = #NHWC}>) -> tensor<1x192x32x32xf16, {order = #NHWC}> {
        // CHECK:      [[SOFTMAX1:%.+]] = VPU.SoftMax([[ARG0]])
        // CHECK:      [[RELU1:%.+]] = VPU.ReLU([[SOFTMAX1]])
        // CHECK:      [[SOFTMAX2:%.+]] = VPU.SoftMax([[ARG0]])
        // CHECK:      [[RELU2:%.+]] = VPU.ReLU([[SOFTMAX2]])
...
module @OutlineConcatsWithConstants {
    ...
    func.func @main(%input: tensor<1x48x32x32xf16, {order = #NHWC}>) -> (tensor<1x96x32x32xf16, {order = #NHWC}>, tensor<1x96x32x32xf16, {order = #NHWC}>) {
        ...
        %maxpool1 = VPU.NCE.MaxPool(%input, %maxpool_wt) {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        ...
        %maxpool2 = VPU.NCE.MaxPool(%input, %maxpool_wt) {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
                pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>, strides = [1, 1], kernel_size = [1, 1]
            } -> tensor<1x48x32x32xf16, {order = #NHWC}>
        ...
        %maxpool3 = VPU.NCE.MaxPool(%input, %maxpool_wt) {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
                pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>, strides = [1, 1], kernel_size = [1, 1]
            } -> tensor<1x48x32x32xf16, {order = #NHWC}>
        %relu3 = VPU.ReLU(%maxpool3) : tensor<1x48x32x32xf16, {order = #NHWC}> -> tensor<1x48x32x32xf16, {order = #NHWC}>
        %maxpool4 = VPU.NCE.MaxPool(%input, %maxpool_wt) {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
                pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEStub<>, strides = [1, 1], kernel_size = [1, 1]
            } -> tensor<1x48x32x32xf16, {order = #NHWC}>
        ...
        // CHECK:      return [[CONCAT]]
        // CHECK:  }
        // CHECK:  func.func nested @main_concat2([[ARG0:%.+]]: tensor<1x48x32x32xf16, {order = #NHWC}>) -> tensor<1x96x32x32xf16, {order = #NHWC}> {
        // CHECK:      [[CST:%.+]] = const.Declare tensor<48x1x1x4xsi32>
        // CHECK:      [[MAXPOOL1:%.+]] = VPU.NCE.MaxPool([[ARG0]], [[CST]] )
...
module @ComplexPattern {
    ...
    func.func @main(%input: tensor<1x320x64x64xf16, {order = #NHWC}>) -> tensor<1x128x1024x4xf16, {order = #NHWC}> {
        ...
        %branch1_conv1 = VPU.NCE.Convolution(%input_softmax, %weights) rawFilterShape [64, 320, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x320x64x64xf16, {order = #NHWC}>, tensor<64x320x1x1xf16, {order = #NHWC}> -> tensor<1x64x64x64xf16, {order = #NHWC}>
        ...
        %branch1_conv2 = VPU.NCE.Convolution(%input_softmax, %weights) rawFilterShape [64, 320, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, quant_scale = [0.12656249105930328], fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x320x64x64xf16, {order = #NHWC}>, tensor<64x320x1x1xf16, {order = #NHWC}> -> tensor<1x64x64x64xf16, {order = #NHWC}>
        ...
        %branch1_conv3 = VPU.NCE.Convolution(%branch1_reshape4, %branch1_permcast4) rawFilterShape [4096, 64, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x64x1024x4xf16, {order = #NHWC}>, tensor<4096x64x1x1xf16, {order = #NHWC}> -> tensor<1x4096x1024x4xf16, {order = #NHWC}>
        ...
        %branch1_conv4 = VPU.NCE.Convolution(%input_softmax, %weights) rawFilterShape [64, 320, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x320x64x64xf16, {order = #NHWC}>, tensor<64x320x1x1xf16, {order = #NHWC}> -> tensor<1x64x64x64xf16>
        ...
        %branch1_conv5 = VPU.NCE.Convolution(%branch1_softmax, %branch1_permcast5) rawFilterShape [64, 4096, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x4096x1024x4xf16, {order = #NHWC}>, tensor<64x4096x1x1xf16, {order = #NHWC}> -> tensor<1x64x1024x4xf16, {order = #NHWC}>

        %branch2_conv1 = VPU.NCE.Convolution(%input_softmax, %weights) rawFilterShape [64, 320, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x320x64x64xf16, {order = #NHWC}>, tensor<64x320x1x1xf16, {order = #NHWC}> -> tensor<1x64x64x64xf16, {order = #NHWC}>
        ...
        %branch2_conv2 = VPU.NCE.Convolution(%input_softmax, %weights) rawFilterShape [64, 320, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, quant_scale = [0.12656249105930328], fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x320x64x64xf16, {order = #NHWC}>, tensor<64x320x1x1xf16, {order = #NHWC}> -> tensor<1x64x64x64xf16, {order = #NHWC}>
        ...
        %branch2_conv3 = VPU.NCE.Convolution(%branch2_reshape4, %branch2_permcast4) rawFilterShape [4096, 64, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x64x1024x4xf16, {order = #NHWC}>, tensor<4096x64x1x1xf16, {order = #NHWC}> -> tensor<1x4096x1024x4xf16, {order = #NHWC}>
        ...
        %branch2_conv4 = VPU.NCE.Convolution(%input_softmax, %weights) rawFilterShape [64, 320, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x320x64x64xf16, {order = #NHWC}>, tensor<64x320x1x1xf16, {order = #NHWC}> -> tensor<1x64x64x64xf16>
        ...
        %branch2_conv5 = VPU.NCE.Convolution(%branch2_softmax, %branch2_permcast5) rawFilterShape [64, 4096, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>, mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64, lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,  strides = [1, 1]} : tensor<1x4096x1024x4xf16, {order = #NHWC}>, tensor<64x4096x1x1xf16, {order = #NHWC}> -> tensor<1x64x1024x4xf16, {order = #NHWC}>
        ...
        // CHECK:           func.func nested @main_concat1([[ARG0:%.+]]: tensor<1x320x64x64xf16, {order = #NHWC}>) -> tensor<1x320x64x64xf16, {order = #NHWC}> {
        // CHECK:               [[INPUT_SOFTMAX:%.+]] = VPU.SoftMax([[ARG0]])
        // CHECK:               return [[INPUT_SOFTMAX]]
        // CHECK:           }
        // CHECK:           func.func nested @main_concat2([[ARG0:%.+]]: tensor<1x320x64x64xf16, {order = #NHWC}>) -> tensor<1x128x1024x4xf16, {order = #NHWC}> {
        ...
        // CHECK:               return [[CONCAT]]
        // CHECK:           }
        // CHECK:           func.func nested @main_concat3([[ARG0:%.+]]: tensor<1x128x1024x4xf16, {order = #NHWC}>) -> tensor<1x128x1024x4xf16, {order = #NHWC}> {
