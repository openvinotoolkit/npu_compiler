...
func.func @SplitOverInputChannelWithDequantizeOpAsWeights(%arg0: tensor<1x3072x128x4xf16, {order = #NHWC}>) -> tensor<1x768x128x4xf16, {order = #NHWC}> {
  ...
  %0 = VPU.Dequantize(%cst_0) {dstElemType = f16, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, tilingStrategy = [2, 1, 1, 1]} : tensor<768x3072x1x1x!quant.uniform<u8:f16, 0.0084632095168618599:87>, {order = #NHWC}> -> tensor<768x3072x1x1xf16, {order = #NHWC}> loc(fused<{name = "MatMul_288", type = "MatMul"}>["MatMul_288", "as_convolution", "reorder_in_1"])
  %1 = VPU.NCE.Convolution(%arg0, %0, %cst) {mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, prelu_alpha = [1.000000e+00], adder = 0.000000e+00 : f64>, rawFilterShape = [768, 3072, 1, 1], strides = [1, 1], tilingStrategy = [1, 2, 6, 1]} : tensor<1x3072x128x4xf16, {order = #NHWC}>, tensor<768x3072x1x1xf16, {order = #NHWC}>, tensor<768x1x1x4xsi32> -> tensor<1x768x128x4xf16, {order = #NHWC}>  loc(fused<{name = "MatMul_288", type = "MatMul"}>["MatMul_288", "as_convolution"])
  
  return %1 : tensor<1x768x128x4xf16, {order = #NHWC}>

        // CHECK:       [[CONST0:%.+]] = const.Declare tensor<768x1x1x4xsi32>
        // CHECK:       [[CONST1:%.+]] = const.Declare tensor<768x1536x1x1x!qElemType, {order = #NHWC}> = dense<1> : tensor<3072x768xui8>, [#const.SubView<[1536, 0], [1536, 768]>, #const.Reshape<[1, 1, 1536, 768]>, #const.CastElemType<f16>, #const.CastElemType<!qElemType>, #const.AffineReshape<
        // CHECK-SAME{LITERAL}:  [[0], [0], [1, 2], [3]], [1, 1536, 1, 768]>, #const.Transpose<#NWHC>, #const.AffineReshape<[[0], [0], [0], [1, 2, 3]], [768, 1536, 1, 1]>, #const.MemPermute<#NHWC, #NHWC>]
        // CHECK:       [[CONST2:%.+]] = const.Declare tensor<768x1x1x4xsi32>
        // CHECK:       [[CONST3:%.+]] = const.Declare tensor<768x1536x1x1x!qElemType, {order = #NHWC}> = dense<1> : tensor<3072x768xui8>, [#const.SubView<[0, 0], [1536, 768]>, #const.Reshape<[1, 1, 1536, 768]>, #const.CastElemType<f16>, #const.CastElemType<!qElemType>, #const.AffineReshape<
        // CHECK-SAME{LITERAL}:  [[0], [0], [1, 2], [3]], [1, 1536, 1, 768]>, #const.Transpose<#NWHC>, #const.AffineReshape<[[0], [0], [0], [1, 2, 3]], [768, 1536, 1, 1]>, #const.MemPermute<#NHWC, #NHWC>]
        // CHECK:       [[SLICE0:%.+]] = VPU.Slice [[INPUT_0]] [0, 0, 0, 0] [1, 1536, 128, 4] : tensor<1x3072x128x4xf16, {order = #NHWC}> to tensor<1x1536x128x4xf16, {order = #NHWC}>
        // CHECK:       [[DEQUANTIZE0:%.+]] = VPU.Dequantize([[CONST3]]) {dstElemType = f16, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>} : tensor<768x1536x1x1x!qElemType, {order = #NHWC}> -> tensor<768x1536x1x1xf16, {order = #NHWC}>
        // CHECK:       [[CONVOLUTION0:%.+]] = VPU.NCE.Convolution([[SLICE0]], [[DEQUANTIZE0]], [[CONST2]]) 
        // CHECK:       [[SLICE1:%.+]] = VPU.Slice [[INPUT_0]] [0, 1536, 0, 0] [1, 1536, 128, 4] : tensor<1x3072x128x4xf16, {order = #NHWC}> to tensor<1x1536x128x4xf16, {order = #NHWC}>
        // CHECK:       [[DEQUANTIZE1:%.+]] = VPU.Dequantize([[CONST1]]) {dstElemType = f16, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>} : tensor<768x1536x1x1x!qElemType, {order = #NHWC}> -> tensor<768x1536x1x1xf16, {order = #NHWC}>
...
func.func @SplitOverInputChannelWithF32Output(%arg0: tensor<1x3072x128x4xf16, {order = #NHWC}>, %arg1: tensor<768x3072x1x1xf16, {order = #NHWC}>) -> tensor<1x768x128x4xf32, {order = #NHWC}> {
  %cst = const.Declare tensor<768x1x1x4xsi32> = dense<1> : tensor<768x1x1x4xsi32>
  %0 = VPU.NCE.Convolution(%arg0, %arg1, %cst) {mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, scale = 1.000000e+00 : f64, prelu_alpha = [1.000000e+00], bias = 0.000000e+00 : f64, adder = 0.000000e+00 : f64>, rawFilterShape = [768, 3072, 1, 1], strides = [1, 1], tilingStrategy = [1, 3, 16, 1]} : tensor<1x3072x128x4xf16, {order = #NHWC}>, tensor<768x3072x1x1xf16, {order = #NHWC}>, tensor<768x1x1x4xsi32> -> tensor<1x768x128x4xf32, {order = #NHWC}>
...
func.func @SplitOverInputChannelWithDequantizedAsWeights_excluded(%arg0: tensor<1x3072x128x4xf16, {order = #NHWC}>) -> tensor<1x768x128x4xf16, {order = #NHWC}> {
  ...
  %1 = VPU.NCE.Convolution(%arg0, %cst_0, %cst) {mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, ppe = #VPU.PPEFp<mode = <NOOP>, clamp_low = -3.4028234663852886E+38 : f64, clamp_high = 3.4028234663852886E+38 : f64, prelu_alpha = [1.000000e+00], adder = 0.000000e+00 : f64>, rawFilterShape = [768, 3072, 1, 1], strides = [1, 1], tilingStrategy = [1, 2, 6, 1]} : tensor<1x3072x128x4xf16, {order = #NHWC}>, tensor<768x3072x1x1x!qElemType, {order = #NHWC}>, tensor<768x1x1x4xsi32> -> tensor<1x768x128x4xf16, {order = #NHWC}>  loc(fused<{name = "MatMul_288", type = "MatMul"}>["MatMul_288", "as_convolution"])
