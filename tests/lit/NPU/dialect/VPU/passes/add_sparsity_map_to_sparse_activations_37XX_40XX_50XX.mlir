...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% compilation-mode=DefaultHW" --add-sparsity-map-to-sparse-activations %s | FileCheck %s
// REQUIRES: platform-NPU3720 || platform-NPU4000 || platform-NPU5010

#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>
#NCHW = affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>

// CHECK-LABEL: @addMapSingleLayer
...
func.func @addMapSingleLayer(%arg0: tensor<1x16x16x16xf16, {order = #NHWC}>) -> tensor<1x16x16x16xf16, {order = #NHWC}> {
    ...
    %1 = VPU.NCE.MaxPool(%0, %weights_table) {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        kernel_size = [3, 3],
    ...
    // CHECK:       [[VAL0:%.+]] = VPU.Sparsify([[ARG_0]]) : tensor<1x16x16x16xf16, {order = #NHWC}>
    // CHECK-SAME:      -> !VPU.SparseTensor<data=tensor<1x16x16x16xf16, {order = #NHWC}>, sparsity_map=tensor<1x16x16x16xi1, {order = #NHWC}>>
    // CHECK-DAG:   [[CST:%.+]] = const.Declare tensor<16x1x1x4xsi32, {order = #NCHW}> = dense<10> : tensor<16x1x1x4xsi32>
...
// CHECK-LABEL: @addMapPropagateType
// CHECK-SAME: ([[ARG_0:%[^:]+]]: tensor<1x16x16x32xf16, {order = #NHWC}>)
func.func @addMapPropagateType(%arg0: tensor<1x16x16x32xf16, {order = #NHWC}>) -> (tensor<1x16x16x16xf16, {order = #NHWC}>, tensor<1x16x16x8xf16, {order = #NHWC}>) {
    ...
    %weights_table = const.Declare tensor<16x1x1x4xsi32, {order = #NCHW}> = dense<10> : tensor<16x1x1x4xsi32>
    %2 = VPU.NCE.MaxPool(%1, %weights_table) {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        kernel_size = [3, 3],
        pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
        ppe = #VPU.PPEStub<>,
        strides = [1, 1]
      } -> !VPU.SparseTensor<data=tensor<1x16x16x16xf16, {order = #NHWC}>>
    %3 = VPU.NCE.MaxPool(%2, %weights_table) {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        kernel_size = [3, 3],
        pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
        ppe = #VPU.PPEStub<>,
        strides = [1, 1]
    ...
    %5 = VPU.NCE.MaxPool(%4, %weights_table) {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
        kernel_size = [3, 3],
        pad = #VPU.Padding<left = 1 : i64, right = 1 : i64, top = 1 : i64, bottom = 1 : i64>,
        ppe = #VPU.PPEStub<>,
        strides = [1, 1]
    ...
    // CHECK:       [[VAL0:%.+]] = VPU.Sparsify([[ARG_0]]) : tensor<1x16x16x32xf16, {order = #NHWC}>
...
// CHECK-LABEL: @propagateTypeConcatMixedConsumers
...
func.func @propagateTypeConcatMixedConsumers(%arg0: tensor<1x8x16x16xf16, {order = #NHWC}>, %arg1: tensor<16x1x1x4xsi32>, %arg2: tensor<16x16x1x1xf16, {order = #NHWC}>)
    ...
    // CHECK:       [[VAL0:%.+]] = VPU.Sparsify([[ARG_0]]) : tensor<1x8x16x16xf16, {order = #NHWC}>
...
// CHECK-LABEL: @addMapSingleLayerAndCopy
// CHECK-SAME: ([[ARG_0:%[^:]+]]: tensor<1x16x16x16xf16, {order = #NHWC}>)
