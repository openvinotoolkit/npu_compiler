...
func.func @SplitOverInputChannelWithF32Output(%arg0: tensor<1x3072x128x4xf16, {order = #NHWC}>, %arg1: tensor<768x3072x1x1xf16, {order = #NHWC}>) -> tensor<1x768x128x4xf32, {order = #NHWC}> {
  ...
  // CHECK-SAME:      ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64,
  // CHECK-SAME:                        lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, quant_scale = [4.1999998092651367], fp_prelu_alpha = 1.000000e+00 : f64>
...
func.func @SplitOverInputChannelWithSparsifiedWeights_excluded(%arg0: tensor<1x4304x256x4xf16, {order = #NHWC}>) -> tensor<1x1152x256x4xf16, {order = #NHWC}> {
  ...
  %1 = VPU.NCE.Convolution(%arg0, %0, %cst) rawFilterShape [1152, 4304, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
    mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverHeight>,
    pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64,
                      lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,
    strides = [1, 1], tilingStrategy = [1, 24, 3, 1]}
  : tensor<1x4304x256x4xf16, {order = #NHWC}>, !VPU.SparseTensor<data=tensor<1152x4304x1x1xf16, {order = #NHWC}>, sparsity_map=tensor<1152x1x1x4352xi1>, is_weights, #VPU.SparsityCompression<axis = 0 : i64, numElems = dense<1> : tensor<1152xi64>, alignment = 16 : i64>>, tensor<1152x1x1x4xsi32> -> tensor<1x1152x256x4xf16, {order = #NHWC}>
...
func.func @SplitOverInputChannelWithDequantizedAsWeights_excluded(%arg0: tensor<1x3072x128x4xf16, {order = #NHWC}>) -> tensor<1x768x128x4xf16, {order = #NHWC}> {
  ...
  %1 = VPU.NCE.Convolution(%arg0, %cst_0, %cst) rawFilterShape [768, 3072, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
    mpe_engine = #VPU.MPEEngine37XX<mode = <SCL>>, multiClusterStrategy = #VPU.multi_cluster_strategy<SplitOverKernel>,
    pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    ppe = #VPU.PPEInt<mode = <NOOP>, clamp_low = -2147483648 : i64, clamp_high = 2147483647 : i64,
                      lrelu_mult = 1 : i64, lrelu_shift = 0 : i64, fp_prelu_alpha = 1.000000e+00 : f64>,
    strides = [1, 1], tilingStrategy = [1, 2, 6, 1]}
  : tensor<1x3072x128x4xf16, {order = #NHWC}>, tensor<768x3072x1x1x!qElemType, {order = #NHWC}>, tensor<768x1x1x4xsi32> -> tensor<1x768x128x4xf16, {order = #NHWC}>
