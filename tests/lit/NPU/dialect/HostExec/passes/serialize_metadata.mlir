...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% compilation-mode=HostCompile allow-custom-values=true" --serialize-network-metadata  %s | FileCheck %s
// REQUIRES: platform-NPU4000


#map = affine_map<(d0) -> (-d0 + 720, 44)>
module @Add attributes {HostExec.numSubgraphs = 3 : i64, config.compilationMode = #config.compilation_mode<HostCompile>, config.revisionID = #config.revision_id<REVISION_NONE>} {
...
module @MaxPool attributes {HostExec.numSubgraphs = 1 : i64, config.platform = #config.platform<NPU4000>, config.compilationMode = #config.compilation_mode<HostCompile>, config.revisionID = #config.revision_id<REVISION_NONE>} {
  net.NetworkInfo entryPoint : @main inputsInfo : {
