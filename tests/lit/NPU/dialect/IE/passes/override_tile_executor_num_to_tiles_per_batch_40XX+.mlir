...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% allow-custom-values=true" --override-tile-executor-num="override-to-tiles-per-batch-mode=apply" %s | FileCheck %s
...
module @OverrideToTilesPerBatchForBatchedCase {
    ...
    func.func @MultipleInputSingleOutputDeBatched(%arg0: tensor<3x3x62x62xf32>, %arg1: tensor<3x48x60x60xf32>) -> tensor<3x48x60x60xf32> {
        ...
        %4 = call @MultipleInputSingleOutputDeBatched_Batch1(%2, %3) {debatched = [0, 3]} : (tensor<1x3x62x62xf32>, tensor<1x48x60x60xf32>) -> tensor<1x48x60x60xf32>
        ...
        %7 = call @MultipleInputSingleOutputDeBatched_Batch1(%5, %6) {debatched = [1, 3]} : (tensor<1x3x62x62xf32>, tensor<1x48x60x60xf32>) -> tensor<1x48x60x60xf32>
        ...
        %10 = call @MultipleInputSingleOutputDeBatched_Batch1(%8, %9) {debatched = [2, 3]} : (tensor<1x3x62x62xf32>, tensor<1x48x60x60xf32>) -> tensor<1x48x60x60xf32>
        ...
        %12 = call @MultipleInputSingleOutputDeBatched_Batch1(%0, %1) {debatched = [1, 3]} : (tensor<1x3x62x62xf32>, tensor<1x48x60x60xf32>) -> tensor<1x48x60x60xf32>
        ...
        // CHECK: [[FUNC_0:%.+]] = call @MultipleInputSingleOutputDeBatched_Batch1([[SLICE_00]], [[SLICE_01]]) {available_tiles = 6 : i64, debatched = [0, 3]} :
        ...
        // CHECK: [[FUNC_1:%.+]] = call @MultipleInputSingleOutputDeBatched_Batch1([[SLICE_10]], [[SLICE_11]]) {available_tiles = 6 : i64, debatched = [1, 3]} :
        // CHECK-SAME:  (tensor<1x3x62x62xf32>, tensor<1x48x60x60xf32>) -> tensor<1x48x60x60xf32>
        ...
        // CHECK: [[FUNC_2:%.+]] = call @MultipleInputSingleOutputDeBatched_Batch1([[SLICE_20]], [[SLICE_21]]) {available_tiles = 6 : i64, debatched = [2, 3]} :
        // CHECK-SAME:  (tensor<1x3x62x62xf32>, tensor<1x48x60x60xf32>) -> tensor<1x48x60x60xf32>
