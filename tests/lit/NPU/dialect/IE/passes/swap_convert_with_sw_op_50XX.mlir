...
// CHECK-LABEL: @SwapInterpolate
// CHECK-SAME: ([[INPUT:%.+]]: tensor<1x8x128x128xf16, {order = #NHWC}>)
func.func @SwapInterpolate(%arg0: tensor<1x8x128x128xf16, {order = #NHWC}>) -> tensor<1x2048x1x256xf32> {
  ...
  // CHECK-NEXT:  [[CONVERT:%.+]] = IE.Convert([[INTERPOLATE]]) {dstElemType = f32}
  // CHECK-NEXT:  [[RESHAPE:%.+]] = IE.Reshape([[CONVERT]])
