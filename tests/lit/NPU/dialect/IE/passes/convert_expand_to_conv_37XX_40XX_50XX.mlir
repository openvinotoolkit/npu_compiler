...
func.func @ConvertExpandToConv16Channels(%arg0: tensor<1x3x64x224xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 48, 64, 14]} inputs([[ARG_0]] : tensor<1x3x64x224xf16, {order = #NHWC}>) -> tensor<1x48x64x14xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 64, 224]} inputs([[CONV]] : tensor<1x256x64x14xf16, {order = #NHWC}>) -> tensor<1x16x64x224xf16, {order = #NHWC}>
...
func.func @ConvertExpandToConv4Channels(%arg0: tensor<1x3x64x224xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 4, 64, 224]} inputs([[CONV]] : tensor<1x64x64x14xf16, {order = #NHWC}>) -> tensor<1x4x64x224xf16, {order = #NHWC}>
...
func.func @ConvertExpandToConv16ChannelsPadded(%arg0: tensor<1x3x64x220xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[LINEAR_RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 42240, 1]} inputs([[ARG0]] : tensor<1x3x64x220xf16, {order = #NHWC}>) -> tensor<1x1x42240x1xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 48, 64, 14]} inputs([[EXPAND_INPUT]] : tensor<1x1x43008x1xf16, {order = #NHWC}>) -> tensor<1x48x64x14xf16, {order = #NHWC}>
    ...
    // CHECK:   [[LINEAR_RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 229376, 1]} inputs([[CONV]] : tensor<1x256x64x14xf16, {order = #NHWC}>) -> tensor<1x1x229376x1xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 64, 220]} inputs([[SLICE]] : tensor<1x1x225280x1xf16, {order = #NHWC}>) -> tensor<1x16x64x220xf16, {order = #NHWC}>
...
func.func @ConvertExpandToConv4ChannelsPadded(%arg0: tensor<1x3x64x220xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[LINEAR_RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 57344, 1]} inputs([[CONV]] : tensor<1x64x64x14xf16, {order = #NHWC}>) -> tensor<1x1x57344x1xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 4, 64, 220]} inputs([[SLICE]] : tensor<1x1x56320x1xf16, {order = #NHWC}>) -> tensor<1x4x64x220xf16, {order = #NHWC}>
...
func.func @ConvertExpandAvgPoolToConv16ChannelsPadded(%arg0: tensor<1x3x513x513xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[LINEAR_RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 789507, 1]} inputs([[ARG0]] : tensor<1x3x513x513xf16, {order = #NHWC}>) -> tensor<1x1x789507x1xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 48, 513, 33]} inputs([[EXPAND_INPUT]] : tensor<1x1x812592x1xf16, {order = #NHWC}>) -> tensor<1x48x513x33xf16, {order = #NHWC}>
    ...
    // CHECK:   [[LINEAR_RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 4333824, 1]} inputs([[CONV]] : tensor<1x256x513x33x!qElemType, {order = #NHWC}>) -> tensor<1x1x4333824x1x!qElemType, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 513, 513]} inputs([[SLICE]] : tensor<1x1x4210704x1x!qElemType, {order = #NHWC}>) -> tensor<1x16x513x513x!qElemType, {order = #NHWC}>
...
func.func @ConvertExpandAvgPoolToConv4ChannelsPadded(%arg0: tensor<1x3x513x513xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[LINEAR_RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 1083456, 1]} inputs([[CONV]] : tensor<1x64x513x33x!qElemType, {order = #NHWC}>) -> tensor<1x1x1083456x1x!qElemType, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 4, 513, 513]} inputs([[SLICE]] : tensor<1x1x1052676x1x!qElemType, {order = #NHWC}>) -> tensor<1x4x513x513x!qElemType, {order = #NHWC}>
...
func.func @ConvertExpandQuantizeToConv4ChannelsPadded(%arg0: tensor<1x3x299x299xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[LINEAR_RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 268203, 1]} inputs([[ARG0]] : tensor<1x3x299x299xf16, {order = #NHWC}>) -> tensor<1x1x268203x1xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 48, 299, 19]} inputs([[EXPAND_INPUT]] : tensor<1x1x272688x1xf16, {order = #NHWC}>) -> tensor<1x48x299x19xf16, {order = #NHWC}>
    ...
    // CHECK:   [[LINEAR_RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 272688, 1]} inputs([[CONV]] : tensor<1x48x299x19x!qElemType, {order = #NHWC}>) -> tensor<1x1x272688x1x!qElemType, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 3, 299, 299]} inputs([[SLICE]] : tensor<1x1x268203x1x!qElemType, {order = #NHWC}>) -> tensor<1x3x299x299x!qElemType, {order = #NHWC}>
...
func.func @ConvertExpandQuantize9OutChannelsPadded(%arg0: tensor<1x3x299x299xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[LINEAR_RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 818064, 1]} inputs([[CONV]] : tensor<1x144x299x19x!qElemType, {order = #NHWC}>) -> tensor<1x1x818064x1x!qElemType, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 9, 299, 299]} inputs([[SLICE]] : tensor<1x1x804609x1x!qElemType, {order = #NHWC}>) -> tensor<1x9x299x299x!qElemType, {order = #NHWC}>
...
func.func @ConvertExpandQuantizeSlicePadded(%arg0: tensor<1x3x299x299xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[LINEAR_RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 363584, 1]} inputs([[CONV]] : tensor<1x64x299x19x!qElemType, {order = #NHWC}>) -> tensor<1x1x363584x1x!qElemType, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 4, 299, 299]} inputs([[SLICE]] : tensor<1x1x357604x1x!qElemType, {order = #NHWC}>) -> tensor<1x4x299x299x!qElemType, {order = #NHWC}>
...
func.func @ConvertExpandSkipSliceQuantizeOnHWPadded(%arg0: tensor<1x3x299x299xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[LINEAR_RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 1454336, 1]} inputs([[CONV]] : tensor<1x256x299x19xf16, {order = #NHWC}>) -> tensor<1x1x1454336x1xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 299, 299]} inputs([[SLICE]] : tensor<1x1x1430416x1xf16, {order = #NHWC}>) -> tensor<1x16x299x299xf16, {order = #NHWC}>
...
func.func @ConvertExpandQuantizeNoSlicePadded(%arg0: tensor<1x3x299x299xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[LINEAR_RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 1, 1454336, 1]} inputs([[CONV]] : tensor<1x256x299x19x!qElemType, {order = #NHWC}>) -> tensor<1x1x1454336x1x!qElemType, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 299, 299]} inputs([[SLICE]] : tensor<1x1x1430416x1x!qElemType, {order = #NHWC}>) -> tensor<1x16x299x299x!qElemType, {order = #NHWC}>
...
func.func @ConvertQuantizedExpand(%arg0: tensor<1x3x64x224x!qElemType, {order = #NHWC}>)
    ...
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 48, 64, 14]} inputs([[ARG_0]] : tensor<1x3x64x224x[[Q_ACT]], {order = #NHWC}>) -> tensor<1x48x64x14x[[Q_ACT]], {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 64, 224]} inputs([[CONV]] : tensor<1x256x64x14x[[Q_ACT]], {order = #NHWC}>) -> tensor<1x16x64x224x[[Q_ACT]], {order = #NHWC}>
...
func.func @ConvertLargeExpand(%arg0: tensor<1x3x512x896xf16, {order = #NHWC}>)
    ...
    // CHECK-NOT:   IE.Expand
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 48, 512, 56]} inputs([[ARG_0]] : tensor<1x3x512x896xf16, {order = #NHWC}>) -> tensor<1x48x512x56xf16, {order = #NHWC}>
    // CHECK:   [[EXPAND_WEIGHTS:%.+]] = const.Declare tensor<256x48x1x1xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 512, 896]} inputs([[CONV]] : tensor<1x256x512x56xf16, {order = #NHWC}>) -> tensor<1x16x512x896xf16, {order = #NHWC}>
...
func.func @FuseQuantizeProducer(%arg0: tensor<1x1x19x80xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 19, 5]} inputs([[ARG_0]] : tensor<1x1x19x80xf16, {order = #NHWC}>) -> tensor<1x16x19x5xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 4, 19, 80]} inputs([[CONV]] : tensor<1x64x19x5x[[Q_TYPE]], {order = #NHWC}>) -> tensor<1x4x19x80x[[Q_TYPE]], {order = #NHWC}>
...
func.func @FuseQuantizeWithoutShapeCast(%arg0: tensor<1x1x19x80xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 4, 19, 80]} inputs([[CONV]] : tensor<1x64x19x5x[[Q_ACT_TYPE]], {order = #NHWC}>) -> tensor<1x4x19x80x[[Q_ACT_TYPE]], {order = #NHWC}>
...
func.func @FuseQuantizeWithShapeCastAvgPoolDifferentShapes(%arg0: tensor<1x1x19x160xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 19, 10]} inputs([[ARG_0]] : tensor<1x1x19x160xf16, {order = #NHWC}>) -> tensor<1x16x19x10xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 4, 19, 80]} inputs([[CONV]] : tensor<1x32x19x10x[[Q_TYPE]], {order = #NHWC}>) -> tensor<1x4x19x80x[[Q_TYPE]], {order = #NHWC}>
...
func.func @ConvertExpandToConvWithNon16MultipleWidth(%arg0: tensor<1x1x32x79741xf16, {order = #NHWC}>)
    ...
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 32, 4984]} inputs([[EXPAND]] : tensor<1x1x32x79744xf16, {order = #NHWC}>) -> tensor<1x16x32x4984xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 32, 79744]} inputs([[CONV0]] : tensor<1x256x32x4984xf16, {order = #NHWC}>) -> tensor<1x16x32x79744xf16, {order = #NHWC}>
...
func.func @ConvertExpandToConvWithInputH2CReshapeWithN2(%arg0: tensor<2x7x262144x1xf16, {order = #NHWC}>)
    ...
    // CHECK:     [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 112, 32768, 1]} inputs([[INPUT]] : tensor<2x7x262144x1xf16, {order = #NHWC}>) -> tensor<1x112x32768x1xf16, {order = #NHWC}>
    ...
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [2, 16, 262144, 1]} inputs([[CONV]] : tensor<1x256x32768x1xf16, {order = #NHWC}>) -> tensor<2x16x262144x1xf16, {order = #NHWC}>
...
// CHECK-LABEL: @ConvertExpandToConvWithInputH2CReshape
// CHECK-SAME: [[INPUT:%.+]]: tensor<1x7x262144x1xf16, {order = #NHWC}>
func.func @ConvertExpandToConvWithInputH2CReshape(%arg0: tensor<1x7x262144x1xf16, {order = #NHWC}>)
    -> tensor<1x16x262144x1xf16, {order = #NHWC}> {
    %EXPAND = IE.Expand(%arg0) {
        pads_begin = [0, 0, 0, 0],
        pads_end = [0, 9, 0, 0]
    } : tensor<1x7x262144x1xf16, {order = #NHWC}> -> tensor<1x16x262144x1xf16, {order = #NHWC}>

    return %EXPAND : tensor<1x16x262144x1xf16, {order = #NHWC}>

    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 112, 16384, 1]} inputs([[INPUT]] : tensor<1x7x262144x1xf16, {order = #NHWC}>) -> tensor<1x112x16384x1xf16, {order = #NHWC}>
    ...
    // CHECK-SAME:      pads_end = [0, 0],
    // CHECK-SAME:      strides = [1, 1]
    // CHECK-SAME:  } : tensor<1x112x16384x1xf16, {order = #NHWC}>,
    // CHECK-SAME:      tensor<256x112x1x1xf16, {order = #NHWC}>
    // CHECK-SAME:          -> tensor<1x256x16384x1xf16, {order = #NHWC}>

    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 262144, 1]} inputs([[CONV]] : tensor<1x256x16384x1xf16, {order = #NHWC}>) -> tensor<1x16x262144x1xf16, {order = #NHWC}>

    // CHECK:   return [[RESHAPE_OUTPUT]] : tensor<1x16x262144x1xf16, {order = #NHWC}>
}
...
func.func @ConvertExpandToNHWCLayout(%arg0: tensor<1x8x2048x1xf16>)
    ...
    %EXPAND = IE.Expand(%arg0) {
        pads_begin = [0, 0, 0, 0],
        pads_end = [0, 0, 0, 15]
    } : tensor<1x8x2048x1xf16> -> tensor<1x8x2048x16xf16>
    ...
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 8, 128]} inputs([[PERMUTE_INPUT]] : tensor<1x1x8x2048xf16, {order = #NHWC}>) -> tensor<1x16x8x128xf16, {order = #NHWC}>
    // CHECK:   [[EXPAND_WEIGHTS:%.+]] = const.Declare tensor<256x16x1x1xf16, {order = #NHWC}> = dense<"0x
    // CHECK:   [[CONV:%.+]] = IE.Convolution([[RESHAPE_INPUT]], [[EXPAND_WEIGHTS]]) {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} : tensor<1x16x8x128xf16, {order = #NHWC}>, tensor<256x16x1x1xf16, {order = #NHWC}> -> tensor<1x256x8x128xf16, {order = #NHWC}>
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 8, 2048]} inputs([[CONV]] : tensor<1x256x8x128xf16, {order = #NHWC}>) -> tensor<1x16x8x2048xf16, {order = #NHWC}>
    // CHECK:   [[PERMUTE_OUTPUT:%.+]] = IE.PermuteCast([[RESHAPE_OUTPUT]]) {dst_order = #NCHW, mem_perm = #NCHW} : tensor<1x16x8x2048xf16, {order = #NHWC}> -> tensor<1x8x2048x16xf16>
    // CHECK:   return [[PERMUTE_OUTPUT:%.+]] : tensor<1x8x2048x16xf16>
}
...
// CHECK-LABEL: @ConvertExpandToConvNCHW
// CHECK-SAME: [[ARG_0:%[^:]+]]: tensor<1x256x2048x4xf16>
func.func @ConvertExpandToConvNCHW(%arg0: tensor<1x256x2048x4xf16>)
    -> tensor<1x256x2048x16xf16> {
    %EXPAND = IE.Expand(%arg0) {
        pads_begin = [0, 0, 0, 0],
        pads_end = [0, 0, 0, 12]
    } : tensor<1x256x2048x4xf16> -> tensor<1x256x2048x16xf16>

    return %EXPAND : tensor<1x256x2048x16xf16>

    // CHECK:   [[PERMUTE_INPUT:%.+]] = IE.PermuteCast([[ARG_0]]) {dst_order = #NHWC, mem_perm = #NCHW} : tensor<1x256x2048x4xf16> -> tensor<1x4x256x2048xf16, {order = #NHWC}>
    // CHECK:   [[RESHAPE_INPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 256, 512]} inputs([[PERMUTE_INPUT]] : tensor<1x4x256x2048xf16, {order = #NHWC}>) -> tensor<1x16x256x512xf16, {order = #NHWC}>
    // CHECK:   [[EXPAND_WEIGHTS:%.+]] = const.Declare tensor<64x16x1x1xf16, {order = #NHWC}> = dense<"0x
    // CHECK:   [[CONV:%.+]] = IE.Convolution([[RESHAPE_INPUT]], [[EXPAND_WEIGHTS]]) {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} : tensor<1x16x256x512xf16, {order = #NHWC}>, tensor<64x16x1x1xf16, {order = #NHWC}> -> tensor<1x64x256x512xf16, {order = #NHWC}>
    // CHECK:   [[RESHAPE_OUTPUT:%.+]] = IE.ShapeCast {shape = [1, 16, 256, 2048]} inputs([[CONV]] : tensor<1x64x256x512xf16, {order = #NHWC}>) -> tensor<1x16x256x2048xf16, {order = #NHWC}>
    // CHECK:   [[PERMUTE_OUTPUT:%.+]] = IE.PermuteCast([[RESHAPE_OUTPUT]]) {dst_order = #NCHW, mem_perm = #NCHW} : tensor<1x16x256x2048xf16, {order = #NHWC}> -> tensor<1x256x2048x16xf16>
    // CHECK:   return [[PERMUTE_OUTPUT]] : tensor<1x256x2048x16xf16>
}
