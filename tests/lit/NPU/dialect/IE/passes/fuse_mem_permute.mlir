...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform%" --run-mem-permute-processing-rewriters="rewriter=fuse-mem-permute-set" %s | FileCheck %s
...
// CHECK-LABEL: @FuseConvWithBiasPermuteQuantizeThroughViewOps
// CHECK-SAME:  ([[INPUT:%.+]]: tensor<1x768x32x4xf16, {order = #NHWC}>)
func.func @FuseConvWithBiasPermuteQuantizeThroughViewOps(
        %arg0: tensor<1x768x32x4xf16, {order = #NHWC}>)
        -> tensor<1x128x1x2048xf16, {order = #NHWC}> {
    %filter = const.Declare tensor<2048x768x1x1xf16, {order = #NHWC}>
        = dense<1.000000e+00> : tensor<2048x768x1x1xf16>, [#const.Reorder<#NHWC>]
    %bias = const.Declare tensor<2048x1x1x1xf32>
        = dense<0.000000e+00> : tensor<2048x1x1x1xf32>

    %0 = IE.Convolution(%arg0, %filter, %bias) {
        dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]
    } : tensor<1x768x32x4xf16, {order = #NHWC}>,
        tensor<2048x768x1x1xf16, {order = #NHWC}>,
        tensor<2048x1x1x1xf32>
        -> tensor<1x2048x32x4xf16, {order = #NHWC}>

    %1 = IE.AffineReshape(%0) {
        dim_mapping = [[0], [1], [2], [2, 3]], shape_value = [1, 2048, 128, 1]
    } : tensor<1x2048x32x4xf16, {order = #NHWC}> -> tensor<1x2048x128x1xf16, {order = #NHWC}>

    %2 = IE.PermuteCast(%1) {
        dst_order = #NCHW,
        mem_perm = affine_map<(d0, d1, d2, d3) -> (d1, d3, d0, d2)>
    } : tensor<1x2048x128x1xf16, {order = #NHWC}> -> tensor<128x2048x1x1xf16>

    %3 = IE.AffineReshape(%2) {
        dim_mapping = [[0, 1, 2], [3], [3], [3]], shape_value = [1, 128, 1, 2048]
    } : tensor<128x2048x1x1xf16> -> tensor<1x128x1x2048xf16>

    %4 = IE.PermuteQuantize(%3) {
        dstElemType = f16, dst_order = #NHWC, mem_perm = #NHWC,
        pads_begin = [0, 0, 0, 0], pads_end = [0, 0, 0, 0]
    } : tensor<1x128x1x2048xf16> -> tensor<1x128x1x2048xf16, {order = #NHWC}>

    return %4 : tensor<1x128x1x2048xf16, {order = #NHWC}>

    // CHECK-DAG: [[FILTER:%.+]] = const.Declare tensor<2048x768x1x1xf16, {order = #NHWC}>
    // CHECK-DAG: [[BIAS:%.+]]   = const.Declare tensor<2048x1x1x1xf32>
    // CHECK: [[CONV:%.+]] = IE.Convolution([[INPUT]], [[FILTER]], [[BIAS]])
    // CHECK-SAME: -> tensor<1x2048x32x4xf16>
    // CHECK-NOT: IE.PermuteQuantize
    // CHECK: [[PCAST:%.+]] = IE.PermuteCast([[CONV]]) {dst_order = #NHWC, mem_perm = #NCHW} : tensor<1x2048x32x4xf16> -> tensor<1x4x2048x32xf16, {order = #NHWC}>
    // CHECK: [[SCAST:%.+]] = IE.ShapeCast {shape = [1, 128, 1, 2048]} inputs([[PCAST]] : tensor<1x4x2048x32xf16, {order = #NHWC}>) -> tensor<1x128x1x2048xf16, {order = #NHWC}>
    // CHECK: return [[SCAST]] : tensor<1x128x1x2048xf16, {order = #NHWC}>
}
...
// CHECK-LABEL: @FuseGroupConvPermuteQuantizeThroughViewOps
// CHECK-SAME:  ([[INPUT:%.+]]: tensor<1x2048x16x8xf16, {order = #NHWC}>)
func.func @FuseGroupConvPermuteQuantizeThroughViewOps(
        %arg0: tensor<1x2048x16x8xf16, {order = #NHWC}>)
        -> tensor<1x128x1x2048xf16, {order = #NHWC}> {
    %filter = const.Declare tensor<2048x1x1x1xf16, {order = #NHWC}>
        = dense<1.000000e+00> : tensor<2048x1x1x1xf16, {order = #NHWC}>

    %0 = IE.GroupConvolution(%arg0, %filter) {
        dilations = [1, 1], groups = 2048 : i64,
        pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]
    } : tensor<1x2048x16x8xf16, {order = #NHWC}>,
        tensor<2048x1x1x1xf16, {order = #NHWC}>
        -> tensor<1x2048x16x8xf16, {order = #NHWC}>

    %1 = IE.AffineReshape(%0) {
        dim_mapping = [[0], [1], [2], [2, 3]], shape_value = [1, 2048, 128, 1]
    } : tensor<1x2048x16x8xf16, {order = #NHWC}> -> tensor<1x2048x128x1xf16, {order = #NHWC}>

    %2 = IE.PermuteCast(%1) {
        dst_order = #NCHW,
        mem_perm = affine_map<(d0, d1, d2, d3) -> (d0, d1, d3, d2)>
    } : tensor<1x2048x128x1xf16, {order = #NHWC}> -> tensor<1x128x2048x1xf16>

    %3 = IE.AffineReshape(%2) {
        dim_mapping = [[0], [1, 2], [3], [3]], shape_value = [1, 128, 1, 2048]
    } : tensor<1x128x2048x1xf16> -> tensor<1x128x1x2048xf16>
    ...
    // CHECK-DAG: [[FILTER:%.+]] = const.Declare tensor<2048x1x1x1xf16, {order = #NHWC}>
    // CHECK: [[GCONV:%.+]] = IE.GroupConvolution([[INPUT]], [[FILTER]])
    // CHECK-SAME: -> tensor<1x2048x16x8xf16>
    // CHECK-NOT: IE.PermuteQuantize
    // CHECK: [[PCAST:%.+]] = IE.PermuteCast([[GCONV]]) {dst_order = #NHWC, mem_perm = #NCHW} : tensor<1x2048x16x8xf16> -> tensor<1x8x2048x16xf16, {order = #NHWC}>
    // CHECK: [[SCAST:%.+]] = IE.ShapeCast {shape = [1, 128, 1, 2048]} inputs([[PCAST]] : tensor<1x8x2048x16xf16, {order = #NHWC}>) -> tensor<1x128x1x2048xf16, {order = #NHWC}>
    // CHECK: return [[SCAST]] : tensor<1x128x1x2048xf16, {order = #NHWC}>
}
...
// ShapeCast in the view-ops chain changes the NHWC innermost dim (C: 32->10240),
// which breaks the C_out == W_pq equivalence.  The traversal stops at this ShapeCast
// and the PermuteQuantize must NOT be fused.

// CHECK-LABEL: @DoNotFuseWhenShapeCastChangesInnermostDim
// CHECK-SAME:  ([[INPUT:%.+]]: tensor<1x32x4x2560xf16, {order = #NHWC}>)
func.func @DoNotFuseWhenShapeCastChangesInnermostDim(
        %arg0: tensor<1x32x4x2560xf16, {order = #NHWC}>)
        -> tensor<1x320x32x32xf16, {order = #NHWC}> {
    %filter = const.Declare tensor<32x1x1x1xf16, {order = #NHWC}>
        = dense<1.000000e+00> : tensor<32x1x1x1xf16, {order = #NHWC}>

    %0 = IE.GroupConvolution(%arg0, %filter) {
        dilations = [1, 1], groups = 32 : i64,
        pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]
    } : tensor<1x32x4x2560xf16, {order = #NHWC}>,
        tensor<32x1x1x1xf16, {order = #NHWC}>
        -> tensor<1x32x4x2560xf16, {order = #NHWC}>

    %1 = IE.ShapeCast {shape = [1, 10240, 32, 1]}
        inputs(%0 : tensor<1x32x4x2560xf16, {order = #NHWC}>)
        -> tensor<1x10240x32x1xf16, {order = #NHWC}>

    %2 = IE.PermuteCast(%1) {
        dst_order = #NCHW,
        mem_perm = affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>
    } : tensor<1x10240x32x1xf16, {order = #NHWC}> -> tensor<1x32x1x10240xf16>

    %3 = IE.Reshape(%2) {shape_value = [1, 320, 32, 32]}
        : tensor<1x32x1x10240xf16> -> tensor<1x320x32x32xf16>

    %4 = IE.PermuteQuantize(%3) {
        dstElemType = f16, dst_order = #NHWC, mem_perm = #NHWC,
        pads_begin = [0, 0, 0, 0], pads_end = [0, 0, 0, 0]
    } : tensor<1x320x32x32xf16> -> tensor<1x320x32x32xf16, {order = #NHWC}>

    return %4 : tensor<1x320x32x32xf16, {order = #NHWC}>

    // CHECK-NOT: IE.MemPermute
    // CHECK:     IE.GroupConvolution([[INPUT]], {{%.+}})
    // CHECK:     IE.ShapeCast
...
// CHECK-LABEL: @DoNotFusePermuteQuantizeWithThreeNonTrivialMemDims
// CHECK-SAME:  ([[INPUT:%.+]]: tensor<1x80x1920x4xf16, {order = #NHWC}>)
func.func @DoNotFusePermuteQuantizeWithThreeNonTrivialMemDims(
        %arg0: tensor<1x80x1920x4xf16, {order = #NHWC}>)
        -> tensor<1x120x64x64xf16, {order = #NHWC}> {
    %filter = const.Declare tensor<64x80x1x1xf16, {order = #NHWC}>
        = dense<1.000000e+00> : tensor<64x80x1x1xf16>, [#const.Reorder<#NHWC>]

    %0 = IE.Convolution(%arg0, %filter) {
        dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]
    } : tensor<1x80x1920x4xf16, {order = #NHWC}>,
        tensor<64x80x1x1xf16, {order = #NHWC}>
        -> tensor<1x64x1920x4xf16, {order = #NHWC}>

    %1 = IE.AffineReshape(%0) {
        dim_mapping = [[0], [1], [2], [2, 3]], shape_value = [1, 64, 7680, 1]
    } : tensor<1x64x1920x4xf16, {order = #NHWC}> -> tensor<1x64x7680x1xf16, {order = #NHWC}>

    %2 = IE.PermuteCast(%1) {
        dst_order = #NCHW, mem_perm = #map
    } : tensor<1x64x7680x1xf16, {order = #NHWC}> -> tensor<7680x64x1x1xf16>

    %3 = IE.AffineReshape(%2) {
        dim_mapping = [[0, 1, 2], [3], [3], [3]], shape_value = [1, 120, 64, 64]
    } : tensor<7680x64x1x1xf16> -> tensor<1x120x64x64xf16>

    %4 = IE.PermuteQuantize(%3) {
        dstElemType = f16, dst_order = #NHWC, mem_perm = #NHWC,
        pads_begin = [0, 0, 0, 0], pads_end = [0, 0, 0, 0]
    } : tensor<1x120x64x64xf16> -> tensor<1x120x64x64xf16, {order = #NHWC}>

    return %4 : tensor<1x120x64x64xf16, {order = #NHWC}>

    // CHECK-NOT: IE.MemPermute
    // CHECK:     IE.Convolution([[INPUT]],
