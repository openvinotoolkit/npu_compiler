...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% allow-custom-values=true" --handle-eltwise-with-small-height %s | FileCheck %s
...
// CHECK-LABEL: func.func @NotHandleEltwiseWithUnalignedChannels
// CHECK-SAME:    ([[INPUT0:%arg[0-9]]]: tensor<1x16x3x1370xf16, {order = #NHWC}>
// CHECK-SAME:     [[INPUT1:%arg[0-9]]]: tensor<1x16x3x1370xf16, {order = #NHWC}>)
...
func.func @NotHandleEltwiseWithUnalignedChannels(%arg0: tensor<1x16x3x1370xf16, {order = #NHWC}>, %arg1: tensor<1x16x3x1370xf16, {order = #NHWC}>) -> tensor<1x16x3x1370xf16, {order = #NHWC}> {
    %0 = IE.Add(%arg0, %arg1) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>} : tensor<1x16x3x1370xf16, {order = #NHWC}>, tensor<1x16x3x1370xf16, {order = #NHWC}> -> tensor<1x16x3x1370xf16, {order = #NHWC}>
    return %0 : tensor<1x16x3x1370xf16, {order = #NHWC}>

    // CHECK-NOT: IE.ShapeCast
    // CHECK: [[ADD:%.+]] = IE.Add([[INPUT0]], [[INPUT1]]) {auto_broadcast = #IE.auto_broadcast_type<NUMPY>} : tensor<1x16x3x1370xf16, {order = #NHWC}>, tensor<1x16x3x1370xf16, {order = #NHWC}> -> tensor<1x16x3x1370xf16, {order = #NHWC}>
}
}
