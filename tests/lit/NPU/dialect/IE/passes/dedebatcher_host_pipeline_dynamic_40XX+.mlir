...
net.NetworkInfo entryPoint : @SingleInputSingleOutputDeBatchedTo1 inputsInfo : {
...
func.func @output_shape(%arg0: tensor<?x3x62x62xf32, {bounds = #const.OpaqueI64Elements<[3, 3, 62, 62]> : tensor<4xsi64>}>) -> tensor<4xi64> {
    ...
    %dim_n = tensor.dim %arg0, %c0 : tensor<?x3x62x62xf32, {bounds = #const.OpaqueI64Elements<[3, 3, 62, 62]> : tensor<4xsi64>}>
    %n = arith.index_cast %dim_n : index to i64
    %c = arith.constant 48 : i64
    %h = arith.constant 60 : i64
    %w = arith.constant 60 : i64
    %from_elements = tensor.from_elements %n, %c, %h, %w : tensor<4xi64>
    return %from_elements : tensor<4xi64>
}

func.func nested @SingleInputSingleOutputDeBatchedTo1_Batch1(%arg0: tensor<1x3x62x62xf32>) -> tensor<1x48x60x60xf32> {
...
func.func @SingleInputSingleOutputDeBatchedTo1(%arg0: tensor<?x3x62x62xf32, {bounds = #const.OpaqueI64Elements<[3, 3, 62, 62]> : tensor<4xsi64>}>) -> tensor<?x48x60x60xf32, {bounds = #const.OpaqueI64Elements<[3, 48, 60, 60]> : tensor<4xsi64>}> {
    ...
    // CHECK:   [[FUNC:%.+]] = func.call @SingleInputSingleOutputDeBatchedTo1_Batch[[ANY_SUFFIX:.*]]([[I_SLICE]]) : (tensor<1x3x62x62xf32>) -> tensor<1x48x60x60xf32>
...
net.NetworkInfo entryPoint : @MultipleInputSingleOutputDeBatched inputsInfo : {
    DataInfo "input0" : tensor<3x3x62x62xf32>
    DataInfo "input1" : tensor<3x48x60x60xf32>
} outputsInfo : {
    DataInfo "output" : tensor<3x48x60x60xf32>
}
func.func @output_shape(%arg0: tensor<?x3x62x62xf32, {bounds = #const.OpaqueI64Elements<[3, 3, 62, 62]> : tensor<4xsi64>}>, %arg1: tensor<?x48x60x60xf32, {bounds = #const.OpaqueI64Elements<[3, 48, 60, 60]> : tensor<4xsi64>}>) -> tensor<4xi64> {
    %c = arith.constant 0 : index
    %out1_dim_n = tensor.dim %arg1, %c : tensor<?x48x60x60xf32, {bounds = #const.OpaqueI64Elements<[3, 48, 60, 60]> : tensor<4xsi64>}>
    %out1_n = arith.index_cast %out1_dim_n : index to i64
    %c1 = arith.constant 48 : i64
    %h1 = arith.constant 60 : i64
    %w1 = arith.constant 60 : i64
    %out1_from_elements = tensor.from_elements %out1_n, %c1, %h1, %w1 : tensor<4xi64>
    return %out1_from_elements : tensor<4xi64>
}

func.func nested @MultipleInputSingleOutputDeBatched_Batch1(%arg0: tensor<1x3x62x62xf32>, %arg1: tensor<1x48x60x60xf32>) -> tensor<1x48x60x60xf32> {
    %cst = const.Declare tensor<48x3x3x3xf32> = dense<1.000000e+00> : tensor<48x3x3x3xf32>
    %1 = IE.Convolution(%arg0, %cst) {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} : tensor<1x3x62x62xf32>, tensor<48x3x3x3xf32> -> tensor<1x48x60x60xf32>
    %2 = IE.SoftMax(%1) {axisInd = 1 : i64} : tensor<1x48x60x60xf32> -> tensor<1x48x60x60xf32>
...
func.func @MultipleInputSingleOutputDeBatched(%arg0: tensor<?x3x62x62xf32, {bounds = #const.OpaqueI64Elements<[3, 3, 62, 62]> : tensor<4xsi64>}>, %arg1: tensor<?x48x60x60xf32, {bounds = #const.OpaqueI64Elements<[3, 48, 60, 60]> : tensor<4xsi64>}>) -> tensor<?x48x60x60xf32, {bounds = #const.OpaqueI64Elements<[3, 48, 60, 60]> : tensor<4xsi64>}> {
    ...
    // CHECK:       [[FUNC:%.+]] = func.call @MultipleInputSingleOutputDeBatched_Batch[[ANY_SUFFIX:.*]]([[I_SLICE_0]], [[I_SLICE_1]]) : (tensor<1x3x62x62xf32>, tensor<1x48x60x60xf32>) -> tensor<1x48x60x60xf32>
    // CHECK:       [[O_SLICE:%.+]] = tensor.insert_slice [[FUNC]] into [[LOOP_CARRIED]][[[IND_VAR]], 0, 0, 0] [1, 48, 60, 60] [1, 1, 1, 1] : tensor<1x48x60x60xf32> into tensor<?x48x60x60xf32, [[ANY_MATCH:{.+}]]>
    // CHECK:       scf.yield [[O_SLICE]] : tensor<?x48x60x60xf32, [[ANY_MATCH:{.+}]]>
    // CHECK:   return [[RET]]
}
...
net.NetworkInfo entryPoint : @SingleInputMultipleOutputDeBatched inputsInfo : {
    DataInfo "input" : tensor<3x3x62x62xf32>
} outputsInfo : {
    DataInfo "output0" : tensor<3x48x60x60xf32>
    DataInfo "output1" : tensor<3x48x60x60xf16>
}
func.func @output_shape(%arg0: tensor<?x3x62x62xf32, {bounds = #const.OpaqueI64Elements<[3, 3, 62, 62]> : tensor<4xsi64>}>) -> (tensor<4xi64>, tensor<4xi64>) {
    %c = arith.constant 0 : index
    %out1_dim_n = tensor.dim %arg0, %c : tensor<?x3x62x62xf32, {bounds = #const.OpaqueI64Elements<[3, 3, 62, 62]> : tensor<4xsi64>}>
    %out1_n = arith.index_cast %out1_dim_n : index to i64
    %c1 = arith.constant 48 : i64
    %h1 = arith.constant 60 : i64
    %w1 = arith.constant 60 : i64
...
func.func nested @SingleInputMultipleOutputDeBatched_Batch1(%arg0: tensor<1x3x62x62xf32>) -> (tensor<1x48x60x60xf32>, tensor<1x48x60x60xf16>) {
    %cst = const.Declare tensor<48x3x3x3xf32> = dense<1.000000e+00> : tensor<48x3x3x3xf32>
    %1 = IE.Convolution(%arg0, %cst) {dilations = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], strides = [1, 1]} : tensor<1x3x62x62xf32>, tensor<48x3x3x3xf32> -> tensor<1x48x60x60xf32>
    %2 = IE.SoftMax(%1) {axisInd = 1 : i64} : tensor<1x48x60x60xf32> -> tensor<1x48x60x60xf32>
...
func.func @SingleInputMultipleOutputDeBatched(%arg0: tensor<?x3x62x62xf32, {bounds = #const.OpaqueI64Elements<[3, 3, 62, 62]> : tensor<4xsi64>}>) -> (tensor<?x48x60x60xf32, {bounds = #const.OpaqueI64Elements<[3, 48, 60, 60]> : tensor<4xsi64>}>, tensor<?x48x60x60xf16, {bounds = #const.OpaqueI64Elements<[3, 48, 60, 60]> : tensor<4xsi64>}>) {
    ...
    // CHECK:       [[I_SLICE:%.+]] = tensor.extract_slice [[ARG0]][[[IND_VAR]], 0, 0, 0] [1, 3, 62, 62] [1, 1, 1, 1] : tensor<?x3x62x62xf32, [[ANY_MATCH:{.+}]]> to tensor<1x3x62x62xf32>
    // CHECK:       [[FUNC:%.+]]:2 = func.call @SingleInputMultipleOutputDeBatched_Batch[[ANY_SUFFIX:.*]]([[I_SLICE]]) : (tensor<1x3x62x62xf32>) -> (tensor<1x48x60x60xf32>, tensor<1x48x60x60xf16>)
...
net.NetworkInfo entryPoint : @SingleInputSingleOutputDeBatchedTo2 inputsInfo : {
    DataInfo "input" : tensor<3x3x62x62xf32>
} outputsInfo : {
    DataInfo "output" : tensor<3x48x60x60xf32>
}
...
func.func nested @SingleInputSingleOutputDeBatchedTo2_Batch2(%arg0: tensor<2x3x62x62xf32>) -> tensor<2x48x60x60xf32> {
    %cst = const.Declare tensor<48x3x3x3xf32> = dense<1.000000e+00> : tensor<48x3x3x3xf32>
...
func.func @SingleInputSingleOutputDeBatchedTo2(%arg0: tensor<?x3x62x62xf32, {bounds = #const.OpaqueI64Elements<[3, 3, 62, 62]> : tensor<4xsi64>}>) -> tensor<?x48x60x60xf32, {bounds = #const.OpaqueI64Elements<[3, 48, 60, 60]> : tensor<4xsi64>}> {
    ...
    // CHECK:       [[FUNC:%.+]] = func.call @SingleInputSingleOutputDeBatchedTo2_Batch[[ANY_SUFFIX:.*]]([[I_SLICE]]) : (tensor<2x3x62x62xf32>) -> tensor<2x48x60x60xf32>
