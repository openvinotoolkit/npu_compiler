...
func.func @MemPemutePositioning(%arg0: tensor<1x16x32x64xf16>) -> tensor<1x16x64x32xi8> {
    ...
    // CHECK:   [[POOL:%.*]] = IE.MaxPool([[MEMPERMUTE0]]) {kernel_size = [1, 1], pads_begin = [0, 0], pads_end = [0, 0], post_op = #IE.Clamp<min = 0.000000e+00 : f64, max = 6.000000e+00 : f64>, rounding_type = #IE.rounding_type<FLOOR>, strides = [1, 1]} : tensor<1x16x32x64xf16, {order = #NHWC}> -> tensor<1x16x32x64xf16, {order = #NHWC}>
...
#NCWH = affine_map<(d0, d1, d2, d3) -> (d0, d1, d3, d2)>
// The optimization is done in PropagateMemPermuteThroughEltwise pass
// CHECK-LABEL: @PropagateThroughAddWithDifferentPermutation
// CHECK-SAME:  [[INPUT_0:%.+]]: tensor<1x4x512x512xf16>, [[INPUT_1:%.+]]: tensor<1x512x4x512xf16>
func.func @PropagateThroughAddWithDifferentPermutation(%arg0 : tensor<1x4x512x512xf16>, %arg1 : tensor<1x512x4x512xf16>) -> tensor<1x4x512x512xf16> {
    %LHS_MEM_PERMUTE = IE.MemPermute(%arg0) {dst_order = #NHWC, mem_perm = #NHWC} : tensor<1x4x512x512xf16> -> tensor<1x4x512x512xf16, {order = #NHWC}>

    %RHS_MEM_PERMUTE = IE.MemPermute(%arg1) {dst_order = #NHWC, mem_perm = #NCWH} : tensor<1x512x4x512xf16> -> tensor<1x4x512x512xf16, {order = #NHWC}>

    %ADD = IE.Add(%LHS_MEM_PERMUTE, %RHS_MEM_PERMUTE) {auto_broadcast = #IE.auto_broadcast_type<NONE_OR_EXPLICIT>} : tensor<1x4x512x512xf16, {order = #NHWC}>, tensor<1x4x512x512xf16, {order = #NHWC}> -> tensor<1x4x512x512xf16, {order = #NHWC}>
    ...
    // CHECK:   [[LHS_PERMUTE_CAST:%.+]] = IE.PermuteCast([[INPUT_0]]) {dst_order = #NHWC, mem_perm = #NCHW} : tensor<1x4x512x512xf16> -> tensor<1x512x4x512xf16, {order = #NHWC}>
    // CHECK:   [[RHS_MEM_PERMUTE:%.+]] = IE.MemPermute([[INPUT_1]]) {dst_order = #NHWC, mem_perm = #NHCW} : tensor<1x512x4x512xf16> -> tensor<1x512x4x512xf16, {order = #NHWC}>
    // CHECK:   [[ADD:%.+]] = IE.Add([[LHS_PERMUTE_CAST]], [[RHS_MEM_PERMUTE]]) {auto_broadcast = #IE.auto_broadcast_type<NONE_OR_EXPLICIT>} : tensor<1x512x4x512xf16, {order = #NHWC}>, tensor<1x512x4x512xf16, {order = #NHWC}> -> tensor<1x512x4x512xf16, {order = #NHWC}>
...
// The optimization can be done in both PropagateMemPermuteThroughEltwise and MovePermutePostEltwise
// But MovePermutePostEltwise is running earlier, so in pipeline, is done by MovePermutePostEltwise pass
// CHECK-LABEL: @PropagateThroughAdd
// CHECK-SAME:  [[INPUT_0:%.+]]: tensor<1x4x512x512xf16>, [[INPUT_1:%.+]]: tensor<1x4x512x512xf16> 
func.func @PropagateThroughAdd(%arg0 : tensor<1x4x512x512xf16>, %arg1 : tensor<1x4x512x512xf16>) -> tensor<1x4x512x512xf16> {
    %LHS_MEM_PERMUTE = IE.MemPermute(%arg0) {dst_order = #NHWC, mem_perm = #NHWC} : tensor<1x4x512x512xf16> -> tensor<1x4x512x512xf16, {order = #NHWC}>
    ...
    // CHECK:   [[LHS_PERMUTE_CAST:%.+]] = IE.PermuteCast([[INPUT_0]]) {dst_order = #NHWC, mem_perm = #NCHW} : tensor<1x4x512x512xf16> -> tensor<1x512x4x512xf16, {order = #NHWC}>
    // CHECK:   [[RHS_PERMUTE_CAST:%.+]] = IE.PermuteCast([[INPUT_1]]) {dst_order = #NHWC, mem_perm = #NCHW} : tensor<1x4x512x512xf16> -> tensor<1x512x4x512xf16, {order = #NHWC}>
    // CHECK:   [[ADD:%.+]] = IE.Add([[LHS_PERMUTE_CAST]], [[RHS_PERMUTE_CAST]]) {auto_broadcast = #IE.auto_broadcast_type<NONE_OR_EXPLICIT>} : tensor<1x512x4x512xf16, {order = #NHWC}>, tensor<1x512x4x512xf16, {order = #NHWC}> -> tensor<1x512x4x512xf16, {order = #NHWC}>
    // CHECK:   [[OUT_PERMUTE_CAST:%.+]] = IE.PermuteCast([[ADD]]) {dst_order = #NCHW, mem_perm = #NCHW} : tensor<1x512x4x512xf16, {order = #NHWC}> -> tensor<1x4x512x512xf16>
    // CHECK:   return [[OUT_PERMUTE_CAST]] : tensor<1x4x512x512xf16>
}
