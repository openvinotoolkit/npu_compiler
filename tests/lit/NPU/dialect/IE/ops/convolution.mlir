...
func.func @FuseConvAndBias(%arg0: tensor<1x3x300x300xf32>) -> tensor<1x16x300x300xf32> {
    ...
    // CHECK:       [[VAL0:%.+]] = IE.Convolution([[ARG_0]], [[FILTERS]], [[BIAS]])
...
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<1x3x64x64xf32>)
...
func.func @GroupsToAttr(%arg0: tensor<1x16x300x300xf32>) -> tensor<1x16x300x300xf32> {
    ...
    // CHECK:       [[VAL0:%.+]] = IE.GroupConvolution([[ARG_0]], [[FILTERS]])
    // CHECK-SAME:      dilations = [1, 1]
...
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<1x64x64x157xf32>)
func.func @NotFuseConvAndBias(%arg0: tensor<1x64x64x157xf32>) -> tensor<1x64x64x157xf32> {
    ...
    // CHECK:       [[VAL0:%.+]] = IE.Convolution([[ARG_0]], [[FILTERS]])
    // CHECK-SAME:      dilations = [1, 1]
...
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<1x11x16x16xf32>)
...
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<1x2x128x64xf16, {order = #NHWC}>)
func.func @FuseSliceAndConv(%arg0: tensor<1x2x128x64xf16, {order = #NHWC}>) -> tensor<1x2x128x64xf16, {order = #NHWC}> {
  ...
  // CHECK:       [[RET:%.+]] = IE.Convolution([[ARG_0]], [[FILTERS]]) {
...
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<1x1x128x128xf16, {order = #NHWC}>)
func.func @NotFuseSliceAndConvWhenSliceNotInDimC(%arg0: tensor<1x1x128x128xf16, {order = #NHWC}>) -> tensor<1x2x64x64xf16, {order = #NHWC}> {
  ...
  // CHECK:       [[SLICE_RET:%.+]] = IE.Slice [[ARG_0]]
...
// CHECK-SAME:    ([[ARG_0:%[^:]+]]: tensor<1x17x128x64xf16, {order = #NHWC}>)
