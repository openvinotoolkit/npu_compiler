...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% allow-custom-values=true" --find-wlm-enqueue-barrier-with-pages %s | FileCheck %s
// REQUIRES: platform-NPU4000 || platform-NPU5010

#NHWC = affine_map<(d0, d1, d2, d3) -> (d0, d2, d3, d1)>

func.func @DmaAndDpuGraph() -> memref<1x16x8x32xf16, {order = #NHWC}, [@CMX_NN, 0]> {
    %bar0 = VPURT.ConfigureBarrier<1> <{isStartBarrier, wlmPage = 0 : i64}> -> !VPURT.Barrier
    %bar1 = VPURT.ConfigureBarrier<0> <{wlmPage = 0 : i64}> -> !VPURT.Barrier
    %bar2 = VPURT.ConfigureBarrier<3> <{wlmPage = 1 : i64}> -> !VPURT.Barrier
    %bar3 = VPURT.ConfigureBarrier<2> <{wlmPage = 1 : i64}> -> !VPURT.Barrier
    %bar4 = VPURT.ConfigureBarrier<1> <{wlmPage = 2 : i64}> -> !VPURT.Barrier
    %bar5 = VPURT.ConfigureBarrier<0> <{wlmPage = 2 : i64}> -> !VPURT.Barrier
    %bar6 = VPURT.ConfigureBarrier<3> <{wlmPage = 3 : i64}> -> !VPURT.Barrier
    %bar7 = VPURT.ConfigureBarrier<2> <{isFinalBarrier, wlmPage = 3 : i64}> -> !VPURT.Barrier
    ...
    //  bar<index>(PID)
    //
    // _______      DMA0[0]  <- enqueue = BOOTSTRAP
    //               |
    //              bar0(0)  <- start barrier
    //               |
    //  Page0       DMA1[0]  <- enqueue = BOOTSTRAP
    //               |
    //              bar1(1)  <- to be used as enqueue target for Page1 boundary tasks or Page2 non-boundary tasks
    //               |
    // _______      DMA0[1]  <- enqueue = BOOTSTRAP
    //               |
    //              bar2(2)  <- to be used as enqueue target for Page2 boundary tasks or Page3 non-boundary tasks
    //            /  |
    //  Page1    |  DMA1[1]  <- enqueue = BOOTSTRAP
    //           |   |
    //           |  bar3(3)
    //            \  |
    // _______      DMA0[2]  <- enqueue = bar1
    //               |
    //              bar4(0)
    //               |
    //  Page2       DMA1[2]  <- enqueue = bar1
    //               |
    //              bar5(1)
    //             /      \
    // _______  DMA0[3]   DPU0[0]  <- enqueue = bar2
    //             \      /
    //              bar6(2)
    //               |
    //  Page3       DPU0[1]  <- enqueue = bar2
    //               |
    //              bar7(3)  <- final barrier
    // _______


    // CHECK:   [[BAR0:%.+]] = VPURT.ConfigureBarrier<1> <{isStartBarrier, wlmPage = 0 : i64}> -> !VPURT.Barrier
    // CHECK:   [[BAR1:%.+]] = VPURT.ConfigureBarrier<0> <{wlmPage = 0 : i64}> -> !VPURT.Barrier
    // CHECK:   [[BAR2:%.+]] = VPURT.ConfigureBarrier<3> <{wlmPage = 1 : i64}> -> !VPURT.Barrier
    // CHECK:   [[BAR3:%.+]] = VPURT.ConfigureBarrier<2> <{wlmPage = 1 : i64}> -> !VPURT.Barrier
    // CHECK:   [[BAR4:%.+]] = VPURT.ConfigureBarrier<1> <{wlmPage = 2 : i64}> -> !VPURT.Barrier
    // CHECK:   [[BAR5:%.+]] = VPURT.ConfigureBarrier<0> <{wlmPage = 2 : i64}> -> !VPURT.Barrier
    // CHECK:   [[BAR6:%.+]] = VPURT.ConfigureBarrier<3> <{wlmPage = 3 : i64}> -> !VPURT.Barrier
    // CHECK:   [[BAR7:%.+]] = VPURT.ConfigureBarrier<2> <{isFinalBarrier, wlmPage = 3 : i64}> -> !VPURT.Barrier
    ...
    // CHECK-NEXT: VPUIP.NNDMA <{port = 1 : i64}>
    // CHECK:   VPURT.Task waits([[BAR3]], [[BAR2]] : !VPURT.Barrier, !VPURT.Barrier) updates([[BAR4]] : !VPURT.Barrier) enqueueTarget([[BAR1]] : !VPURT.Barrier) wlmPage(1) {
    // CHECK-NEXT: VPUIP.NNDMA <{port = 0 : i64}>

    // CHECK:   VPURT.Task waits([[BAR4]] : !VPURT.Barrier) updates([[BAR5]] : !VPURT.Barrier) enqueueTarget([[BAR1]] : !VPURT.Barrier) wlmPage(2) {
    // CHECK-NEXT: VPUIP.NNDMA <{port = 1 : i64}>
    // CHECK:   VPURT.Task waits([[BAR5]] : !VPURT.Barrier) updates([[BAR6]] : !VPURT.Barrier) enqueueTarget([[BAR2]] : !VPURT.Barrier) wlmPage(2) {
    // CHECK-NEXT: VPUIP.NNDMA <{port = 0 : i64}>
    // CHECK:   VPURT.Task waits([[BAR5]] : !VPURT.Barrier) updates([[BAR6]] : !VPURT.Barrier) enqueueTarget([[BAR2]] : !VPURT.Barrier) wlmPage(2) {
    // CHECK-NEXT: VPUIP.NCEClusterTask

    // CHECK:   VPURT.Task waits([[BAR6]] : !VPURT.Barrier) updates([[BAR7]] : !VPURT.Barrier) enqueueTarget([[BAR2]] : !VPURT.Barrier) wlmPage(3) {
    // CHECK-NEXT: VPUIP.NCEClusterTask
