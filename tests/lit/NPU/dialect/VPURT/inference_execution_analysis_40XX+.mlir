...
// RUN: vpux-opt --init-compiler="platform=%platform% allow-custom-values=true" --inference-execution-analysis %s | FileCheck %s
...
module @dumpsubgraph attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  ...
  config.Resources 4 of @NCE at 1.700000e+03 MHz {
    // CHECK: config.Resources {activity_factor = {{[0-9]+(\.[0-9]+)?(e[+-][0-9]+)?}} : f64} 4 of @NCE at 1.700000e+03 MHz {
