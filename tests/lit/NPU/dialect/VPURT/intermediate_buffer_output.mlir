...
// RUN: vpux-opt --init-compiler="platform=%platform% allow-custom-values=true" --intermediate-buffer-output="op-index=5 insertion-index=5 buffer-index=4" %s | FileCheck %s
...
module @TestBufferOutput {
  net.NetworkInfo entryPoint : @main inputsInfo : {
    ...
    // CHECK: DataInfo "Multiply_5095/fq_input_0" : tensor<1x224x4x115xui8>
  }
  func.func @main(%arg0: memref<1x3x224x224xf16, @DDR>, %arg1: memref<1x64x56x56xf16, @DDR>) -> memref<1x64x56x56xf16, @DDR> {
  // CHECK: func.func @main([[ARG0:%.+]]: memref<1x3x224x224xf16, @DDR>, [[ARG1:%.+]]: memref<1x224x4x115x!qElemType, {order = #NWCH}, @DDR>) -> memref<1x224x4x115x!qElemType, {order = #NWCH}, @DDR> {
    ...
    %14 = VPURT.DeclareBuffer <CMX_NN> [0] <154560> -> memref<1x224x4x114x!qElemType1, {order = #NWCH}, [@CMX_NN, 0]>
    %15 = VPURT.DeclareBuffer <CMX_NN> [1] <154560> -> memref<1x224x4x115x!qElemType1, {order = #NWCH}, [@CMX_NN, 1]>
    ...
    %31 = VPURT.DeclareBuffer <CMX_NN> [0] <100352> -> memref<1x64x28x56x!qElemType3, {order = #NHWC}, [@CMX_NN, 0]>
    %32 = VPURT.DeclareBuffer <CMX_NN> [1] <100352> -> memref<1x64x28x56x!qElemType3, {order = #NHWC}, [@CMX_NN, 1]>
    ...
    %40 = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %41 = VPURT.DeclareBuffer <CMX_NN> [1] <0> -> memref<1x224x3x115xf16, {order = #NHWC}, [@CMX_NN, 1]>
    %42 = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %43 = VPURT.DeclareBuffer <CMX_NN> [1] <0> -> memref<1x224x3x115xf16, {order = #NHWC}, [@CMX_NN, 1]>
    %44 = VPURT.DeclareBuffer <CMX_NN> [0] <257600> -> memref<64x16x7x7x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>
    %45 = VPURT.DeclareBuffer <CMX_NN> [1] <257600> -> memref<64x16x7x7x!qElemType, {order = #NHWC}, [@CMX_NN, 1]>
    ...
    %47 = VPURT.DeclareBuffer <CMX_NN> [0] <154560> -> memref<1x16x114x224x!qElemType4, {order = #NHWC}, [@CMX_NN, 0]>
    %48 = VPURT.DeclareBuffer <CMX_NN> [1] <154560> -> memref<1x16x115x224x!qElemType4, {order = #NHWC}, [@CMX_NN, 1]>
    ...
    %51 = VPURT.DeclareBuffer <CMX_NN> [0] <268864> -> memref<64x64x1x1x!qElemType5, {order = #NHWC}, [@CMX_NN, 0]>
    %52 = VPURT.DeclareBuffer <CMX_NN> [1] <268864> -> memref<64x64x1x1x!qElemType5, {order = #NHWC}, [@CMX_NN, 1]>
    ...
    %54 = VPURT.DeclareBuffer <CMX_NN> [0] <100352> -> memref<1x64x28x56x!qElemType6, {order = #NHWC}, [@CMX_NN, 0]>
    %55 = VPURT.DeclareBuffer <CMX_NN> [1] <100352> -> memref<1x64x28x56x!qElemType6, {order = #NHWC}, [@CMX_NN, 1]>
    %56 = VPURT.DeclareBuffer <CMX_NN> [0] <100352> -> memref<1x64x28x56x!qElemType6, {order = #NHWC}, [@CMX_NN, 0]>
    %57 = VPURT.DeclareBuffer <CMX_NN> [1] <100352> -> memref<1x64x28x56x!qElemType6, {order = #NHWC}, [@CMX_NN, 1]>
    ...
    VPURT.Task updates(%1 : !VPURT.Barrier) {
      %58 = VPUIP.NNDMA <{port = 0 : i64}> inputs(%cst_0 : memref<64x1x1x160x!qElemType, {order = #NHWC}>) outputs(%16 : !VPUIP.DistributedBuffer<64x1x1x160x!qElemType, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>) -> !VPUIP.DistributedBuffer<64x1x1x160x!qElemType, #NHWC, @CMX_NN, {mode = "DUPLICATED", num_clusters = 2 : i64}>
    }
    VPURT.Task waits(%0 : !VPURT.Barrier) updates(%1 : !VPURT.Barrier) {
      %58 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_permute_quantize, is_superdense, task_type = #VPUIP.nce_task_type<ELTWISE>}> input(%42 : memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%40 : memref<1x224x3x114xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_input(%39 : !VPUIP.DistributedBuffer<1x224x3x224xf16, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>) parent_output(%13 : !VPUIP.DistributedBuffer<1x224x4x224x!qElemType1, #NWCH, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64, equal_memory_and_compute_view}>) outputs(%14 : memref<1x224x4x114x!qElemType1, {order = #NWCH}, [@CMX_NN, 0]>) -> memref<1x224x4x114x!qElemType1, {order = #NWCH}, [@CMX_NN, 0]> variants : {
    ...
    VPURT.Task waits(%0 : !VPURT.Barrier) updates(%1, %2 : !VPURT.Barrier, !VPURT.Barrier) {
      %58 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_permute_quantize, is_superdense, task_type = #VPUIP.nce_task_type<ELTWISE>}> input(%43 : memref<1x224x3x115xf16, {order = #NHWC}, [@CMX_NN, 1]>) weights(%41 : memref<1x224x3x115xf16, {order = #NHWC}, [@CMX_NN, 1]>) parent_input(%39 : !VPUIP.DistributedBuffer<1x224x3x224xf16, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>) parent_output(%13 : !VPUIP.DistributedBuffer<1x224x4x224x!qElemType1, #NWCH, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 1, 2], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64, equal_memory_and_compute_view}>) outputs(%15 : memref<1x224x4x115x!qElemType1, {order = #NWCH}, [@CMX_NN, 1]>) -> memref<1x224x4x115x!qElemType1, {order = #NWCH}, [@CMX_NN, 1]> variants : {
    ...
    VPURT.Task waits(%1 : !VPURT.Barrier) updates(%3 : !VPURT.Barrier) {
      %58 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{cm_sp_pattern = 7 : i64, input_channels_compression, kernel_padding = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 0 : i64>, kernel_size = [7, 7], kernel_strides = [2, 2], task_type = #VPUIP.nce_task_type<CONV>}> input(%47 : memref<1x16x114x224x!qElemType4, {order = #NHWC}, [@CMX_NN, 0]>) weights(%44 : memref<64x16x7x7x!qElemType, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%17 : memref<64x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%46 : !VPUIP.DistributedBuffer<1x16x224x224x!qElemType4, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>) parent_output(%20 : !VPUIP.DistributedBuffer<1x64x112x112x!qElemType2, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) outputs(%23 : memref<1x64x56x112x!qElemType2, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>) -> memref<1x64x56x112x!qElemType2, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]> variants : {
    ...
    VPURT.Task waits(%1 : !VPURT.Barrier) updates(%3 : !VPURT.Barrier) {
      %58 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{cm_sp_pattern = 7 : i64, input_channels_compression, kernel_padding = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 0 : i64, bottom = 2 : i64>, kernel_size = [7, 7], kernel_strides = [2, 2], task_type = #VPUIP.nce_task_type<CONV>}> input(%48 : memref<1x16x115x224x!qElemType4, {order = #NHWC}, [@CMX_NN, 1]>) weights(%45 : memref<64x16x7x7x!qElemType, {order = #NHWC}, [@CMX_NN, 1]>) weight_table(%18 : memref<64x1x1x4xsi32, [@CMX_NN, 1]>) parent_input(%46 : !VPUIP.DistributedBuffer<1x16x224x224x!qElemType4, #NHWC, @CMX_NN, {mode = "OVERLAPPED", num_tiles = [1, 1, 2, 1], kernel = [7, 7], pads = #VPU.Padding<left = 3 : i64, right = 2 : i64, top = 3 : i64, bottom = 2 : i64>, strides = [2, 2], num_clusters = 2 : i64}>) parent_output(%20 : !VPUIP.DistributedBuffer<1x64x112x112x!qElemType2, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) outputs(%24 : memref<1x64x56x112x!qElemType2, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 1]>) -> memref<1x64x56x112x!qElemType2, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 1]> variants : {
    ...
    VPURT.Task waits(%2 : !VPURT.Barrier) updates(%4 : !VPURT.Barrier) {
      %58 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_segmented, kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [1, 1], kernel_strides = [1, 1], task_type = #VPUIP.nce_task_type<CONV>}> input(%26 : memref<1x64x28x56x!qElemType2, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 0]>) weights(%51 : memref<64x64x1x1x!qElemType5, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%49 : memref<64x1x1x4xsi32, [@CMX_NN, 0]>) parent_input(%25 : !VPUIP.DistributedBuffer<1x64x56x56x!qElemType2, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) parent_output(%30 : !VPUIP.DistributedBuffer<1x64x56x56x!qElemType3, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) outputs(%31 : memref<1x64x28x56x!qElemType3, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x64x28x56x!qElemType3, {order = #NHWC}, [@CMX_NN, 0]> variants : {
    ...
    VPURT.Task waits(%2 : !VPURT.Barrier) updates(%4 : !VPURT.Barrier) {
      %58 = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{is_segmented, kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, kernel_size = [1, 1], kernel_strides = [1, 1], task_type = #VPUIP.nce_task_type<CONV>}> input(%27 : memref<1x64x28x56x!qElemType2, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, [@CMX_NN, 1]>) weights(%52 : memref<64x64x1x1x!qElemType5, {order = #NHWC}, [@CMX_NN, 1]>) weight_table(%50 : memref<64x1x1x4xsi32, [@CMX_NN, 1]>) parent_input(%25 : !VPUIP.DistributedBuffer<1x64x56x56x!qElemType2, {order = #NHWC, swizzlingScheme = #VPUIP.SwizzlingSchemeAttr<key = 5 : i64, sizeAlignment = 512 : i64>}, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) parent_output(%30 : !VPUIP.DistributedBuffer<1x64x56x56x!qElemType3, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>) outputs(%32 : memref<1x64x28x56x!qElemType3, {order = #NHWC}, [@CMX_NN, 1]>) -> memref<1x64x28x56x!qElemType3, {order = #NHWC}, [@CMX_NN, 1]> variants : {
  ...
  // CHECK-DAG:   [[OUTPUT:%.+]] = VPURT.DeclareBuffer <NetworkOutput> <0> -> memref<1x224x4x115x!qElemType, {order = #NWCH}, @DDR>
  // CHECK-DAG:   [[TARGET_BUFFER:%.+]] = VPURT.DeclareBuffer <CMX_NN> [1] <154560> -> memref<1x224x4x115x!qElemType, {order = #NWCH}, [@CMX_NN, 1]>

  // Target TaskOp
  // CHECK:       VPURT.Task waits([[BAR0]] : !VPURT.Barrier) updates([[BAR1]] : !VPURT.Barrier) <{taskIndex = [5]}>
  // CHECK-NEXT:     outputs([[TARGET_BUFFER]] : memref<1x224x4x115x!qElemType, {order = #NWCH}, [@CMX_NN, 1]>)

  // Target insertion point and DMA out
  // CHECK:       VPURT.Task waits([[BAR1]] : !VPURT.Barrier) updates([[BAR2]] : !VPURT.Barrier)
  // CHECK:          VPUIP.NNDMA
  // CHECK-SAME:        inputs([[TARGET_BUFFER]] : memref<1x224x4x115x!qElemType, {order = #NWCH}, [@CMX_NN, 1]>
  // CHECK-SAME:        outputs([[OUTPUT]] :  memref<1x224x4x115x!qElemType, {order = #NWCH}, @DDR>

  // CHECK-NOT:   VPURT.Task

  // Updated return
  // CHECK:   return [[ARG1]] : memref<1x224x4x115x!qElemType, {order = #NWCH}, @DDR>
}
