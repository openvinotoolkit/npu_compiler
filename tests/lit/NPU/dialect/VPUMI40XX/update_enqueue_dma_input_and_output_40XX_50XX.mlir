...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% allow-custom-values=true" --update-enqueue-dma-input-and-output %s | FileCheck %s
...
module @UpdateEnqueueDMAOpsDPU attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  ...
  func.func @main(%arg0: memref<1x16x16x16xf16, @DDR>, %arg1: memref<1x16x14x14xf16, @DDR>) -> memref<1x16x14x14xf16, @DDR> {
    ...
    %19 = VPUMI40XX.DPUInvariant <{clean_after = 2 : ui64, is_permute_quantize, mpe_frequent_mode = #VPU.mpe_mode<CUBOID_16x16>, nce_task_type = #VPUIP.nce_task_type<ELTWISE>, start_after = 3 : ui64, wlmPage = 0 : i64}>
    ...
    kernel_strides = [1, 1], mpe_frequent_mode = #VPU.mpe_mode<CUBOID_16x16>, nce_task_type = #VPUIP.nce_task_type<CONV>, start_after = 4 : ui64, wlmPage = 0 : i64}>
    taskLocation(%16 : !VPURegMapped.Index<0:0:1>) previousTask(%19 : !VPURegMapped.Index<0:0:0>) input(%7 : memref<1x16x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) weights(%9 : memref<16x16x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>)
    ...
    %21 = VPUMI40XX.DPUVariant taskLocation(%17 : !VPURegMapped.Index<0:0:0>) calls(%19 : <0:0:0>) weights(%6 : memref<1x16x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) <{end = [15, 15, 15], inEnd = [15, 15, 15], inStart = [0, 0, 0], wlmPage = 0, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, nce_task_type = #VPUIP.nce_task_type<ELTWISE>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, start = [0, 0, 0]}> -> <0:0:0>
    %22 = VPUMI40XX.DPUVariant taskLocation(%18 : !VPURegMapped.Index<0:0:1>) previousTask(%21 : !VPURegMapped.Index<0:0:0>) calls(%20 : <0:0:1>) weights(%9 : memref<16x16x3x3xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%8 : memref<16x1x1x4xsi32, [@CMX_NN, 0]>) {taskLinkAttrName = #VPURegMapped.IndexType<<0:0:0>>} <{end = [13, 13, 15], inEnd = [15, 15, 15], inStart = [0, 0, 0], wlmPage = 0, mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, nce_task_type = #VPUIP.nce_task_type<CONV>, pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>, start = [0, 0, 0]}> -> <0:0:1>
...
module @UpdateEnqueueDMAOpsSHV attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  ...
  func.func @main(%arg0: memref<1x1000x1x1xf16, @DDR>, %arg1: memref<1x1000x1x1xf16, @DDR>) -> memref<1x1000x1x1xf16, @DDR> {
    ...
    %18 = VPUMI40XX.KernelParams <{dynamicInputShapesSize = array<i32>, dynamicOutputShapesSize = array<i32>}> inputs(%10 : memref<1x3x32x62xf16, {order = #NHWC, strides = [11532, 1, 186, 3]}, [@CMX_NN, 0]>) outputs(%11 : memref<1x3x32x62xf16, {order = #NHWC, strides = [11532, 1, 186, 3]}, [@CMX_NN, 0]>) kernel_type("softmax") kernel_params([]) -> !VPURegMapped.Index<0:0:0>
    %19 = VPUMI40XX.KernelParams <{dynamicInputShapesSize = array<i32>, dynamicOutputShapesSize = array<i32>}> inputs(%12 : memref<1x3x30x62xf16, {order = #NHWC, strides = [11532, 1, 186, 3]}, [@CMX_NN, 0]>) outputs(%13 : memref<1x3x30x62xf16, {order = #NHWC, strides = [11532, 1, 186, 3]}, [@CMX_NN, 0]>) kernel_type("softmax") kernel_params([]) -> !VPURegMapped.Index<0:0:1>
    ...
    %37 = VPUMI40XX.NNDMA {taskLinkAttrName = #VPURegMapped.IndexType<<0:0:0>>} <{is_critical, is_out_of_order, port = 0 : i64, wlmPage = -1 : i64}> inputs(%32 : memref<2x40xui8>) outputs(%33 : memref<2x40xui8, [@CMX_NN, 0]>) previousDMA(%36 : !VPURegMapped.Index<0:0:0>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<0:0:1>

    %38 = VPUMI40XX.NNDMA {taskLinkAttrName = #VPURegMapped.IndexType<<0:0:1>> }<{is_critical, is_out_of_order, port = 0 : i64, wlmPage = -1 : i64}> inputs(%34 : memref<2x96xui8>) outputs(%35 : memref<2x96xui8, [@CMX_NN, 0]>) previousDMA(%37 : !VPURegMapped.Index<0:0:1>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<0:0:2>

    %39 = VPUMI40XX.NNDMA {taskLinkAttrName = #VPURegMapped.IndexType<<0:0:2>>} <{dma_descriptor = #VPUIP.DMADescriptorAttr<numPlanes = 0 : i64, len = 0 : i64, srcWidth = 0 : i64, srcStride = 0 : i64, srcPlaneStride = 0 : i64, dstWidth = 0 : i64, dstStride = 0 : i64, dstPlaneStride = 0 : i64>, port = 0 : i64, wlmPage = 0 : i64}> inputs(%3 : memref<0x0x0x0xi32, @DDR>) outputs(%4 : memref<0x0x0x0xi32, @DDR>) previousDMA(%38 : !VPURegMapped.Index<0:0:2>) updates(%20 : !VPURegMapped.Index<0:0:0>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<0:0:3>

    %40 = VPUMI40XX.NNDMA {taskLinkAttrName = #VPURegMapped.IndexType<<0:0:3>>} <{allow_different_in_out_shapes, dma_descriptor = #VPUIP.DMADescriptorAttr<numPlanes = 3 : i64, len = 7688 : i64, srcWidth = 7688 : i64, srcStride = 2 : i64, srcPlaneStride = 7688 : i64, dstWidth = 2 : i64, dstStride = 6 : i64, dstPlaneStride = 2 : i64>, is_out_of_order, port = 0 : i64, wlmPage = 0 : i64}> inputs(%1 : memref<3x3844xf16, @DDR>) outputs(%6 : memref<3844x3xf16, [@CMX_NN, 0]>) previousDMA(%39 : !VPURegMapped.Index<0:0:3>) waits(%20 : !VPURegMapped.Index<0:0:0>) updates(%21 : !VPURegMapped.Index<0:0:1>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<0:0:4>
    ...
    %42 = VPUMI40XX.NNDMA {taskLinkAttrName = #VPURegMapped.IndexType<<0:1:0>>} <{port = 0 : i64, wlmPage = 0 : i64}> inputs(%8 : memref<1x3x62x62xf16, [@CMX_NN, 0]>) outputs(%2 : memref<1x3x62x62xf16, @DDR>) previousDMA(%41 : !VPURegMapped.Index<0:1:0>) waits(%22 : !VPURegMapped.Index<0:0:2>) updates(%23 : !VPURegMapped.Index<0:0:3>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) dma_transaction(#VPUMI40XX.NNDMATransaction<inputType = memref<1x3x62x62xf16, [@CMX_NN, 0]>, outputType = memref<1x3x62x62xf16, @DDR>>) -> !VPURegMapped.Index<0:1:1>
