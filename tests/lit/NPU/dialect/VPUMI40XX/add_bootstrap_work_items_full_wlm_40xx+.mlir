...
// RUN: vpux-opt --split-input-file --init-compiler="platform=%platform% allow-custom-values=true" --add-bootstrap-work-items="workload-management-mode=FWLM_V1_PAGES" %s | FileCheck %s
...
module {
  ...
  func.func @twoDmaPortsOneTile() {
    ...
    %17 = VPUMI40XX.NNDMA <{port = 1 : i64}> taskLocation(%T10 : !VPURegMapped.Index<1:0:0>) inputs(%6 : memref<1x16x16x16xf16, {order = #NHWC}, @DDR>) outputs(%9 : memref<1x16x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) updates(%11 : !VPURegMapped.Index<0:0:0>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<1:0:0>
    %18 = VPUMI40XX.NNDMA <{port = 1 : i64}> taskLocation(%T11 : !VPURegMapped.Index<1:0:1>) inputs(%6 : memref<1x16x16x16xf16, {order = #NHWC}, @DDR>) outputs(%9 : memref<1x16x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) previousDMA(%17 : !VPURegMapped.Index<1:0:0>) waits(%11 : !VPURegMapped.Index<0:0:0>) updates(%12 : !VPURegMapped.Index<0:0:1>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<1:0:1>
    %19 = VPUMI40XX.NNDMA <{port = 1 : i64}> taskLocation(%T12 : !VPURegMapped.Index<1:0:2>) inputs(%6 : memref<1x16x16x16xf16, {order = #NHWC}, @DDR>) outputs(%10 : memref<1x16x16x16xf16, {order = #NHWC}, [@CMX_NN, 1]>) previousDMA(%18 : !VPURegMapped.Index<1:0:1>) updates(%11 : !VPURegMapped.Index<0:0:0>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<1:0:2>
    %20 = VPUMI40XX.NNDMA <{port = 1 : i64}> taskLocation(%T13 : !VPURegMapped.Index<1:0:3>) inputs(%6 : memref<1x16x16x16xf16, {order = #NHWC}, @DDR>) outputs(%10 : memref<1x16x16x16xf16, {order = #NHWC}, [@CMX_NN, 1]>) previousDMA(%19 : !VPURegMapped.Index<1:0:2>) waits(%11 : !VPURegMapped.Index<0:0:0>) updates(%12 : !VPURegMapped.Index<0:0:1>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<1:0:3>

    %21 = VPUMI40XX.NNDMA <{port = 0 : i64}> taskLocation(%4 : !VPURegMapped.Index<0:1:0>) inputs(%9 : memref<1x16x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) outputs(%7 : memref<1x16x16x16xf16, {order = #NHWC}, @DDR>) waits(%12 : !VPURegMapped.Index<0:0:1>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<0:1:0>
    %22 = VPUMI40XX.NNDMA <{port = 0 : i64}> taskLocation(%5 : !VPURegMapped.Index<0:1:1>) inputs(%10 : memref<1x16x16x16xf16, {order = #NHWC}, [@CMX_NN, 1]>) outputs(%8 : memref<1x16x16x16xf16, {order = #NHWC}, @DDR>) previousDMA(%21: !VPURegMapped.Index<0:1:0>) waits(%12 : !VPURegMapped.Index<0:0:1>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<0:1:1>

    %23 = VPUMI40XX.NNDMA <{port = 1 : i64}> taskLocation(%T14 : !VPURegMapped.Index<1:1:0>) inputs(%9 : memref<1x16x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) outputs(%7 : memref<1x16x16x16xf16, {order = #NHWC}, @DDR>) waits(%12 : !VPURegMapped.Index<0:0:1>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<1:1:0>
    %24 = VPUMI40XX.NNDMA <{port = 1 : i64}> taskLocation(%T15 : !VPURegMapped.Index<1:1:1>) inputs(%10 : memref<1x16x16x16xf16, {order = #NHWC}, [@CMX_NN, 1]>) outputs(%8 : memref<1x16x16x16xf16, {order = #NHWC}, @DDR>) previousDMA(%23 : !VPURegMapped.Index<1:1:0>) waits(%12 : !VPURegMapped.Index<0:0:1>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>) -> !VPURegMapped.Index<1:1:1>
