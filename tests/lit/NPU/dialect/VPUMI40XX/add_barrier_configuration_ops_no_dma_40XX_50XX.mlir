...
// RUN: vpux-opt --split-input-file --init-compiler="vpu-arch=%arch% allow-custom-values=true" --add-barrier-configuration-ops="workload-management-barrier-programming-mode=NO_BARRIER_DMAS_SCHEDULED" %s | FileCheck %s
...
module @Convolution attributes {config.compilationMode = #config.compilation_mode<DefaultHW>, config.revisionID = #config.revision_id<REVISION_NONE>} {
  ...
  // CHECK-NOT: const.Declare memref<64xui32>

  // CHECK-NOT: VPURT.DeclareBuffer <Register> <788594688> -> memref<64xui32, @Register>
  // CHECK-NOT: VPUMI40XX.NNDMA {dma_descriptor = #VPUIP.DMADescriptorAttr<numPlanes = 0 : i64, len = 256 : i64, srcWidth = 256 : i64, srcStride = 0 : i64, srcPlaneStride = 0 : i64, dstWidth = 16 : i64, dstStride = 32 : i64, dstPlaneStride = 0 : i64>, port = 0 : i64}

  // CHECK: {workloadManagementBarrierProgrammingMode = #VPURegMapped.workload_management_barrier_programming_mode<NO_BARRIER_DMAS_SCHEDULED>}
