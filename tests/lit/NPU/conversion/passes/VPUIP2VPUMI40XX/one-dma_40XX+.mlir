...
module @OneDMAWithoutAttributes {
  ...
  func.func @main(%arg0: memref<1x2x3x4xf16, @DDR>, %arg1: memref<1x2x3x4xf16, @DDR>) -> memref<1x2x3x4xf16, @DDR> {
    ...
    // CHECK:       [[VAL0:%.+]] = VPUMI40XX.NNDMA <{port = 0 : i64}> inputs([[ARG_0]] : memref<1x2x3x4xf16, @DDR>) outputs([[ARG_1]] : memref<1x2x3x4xf16, @DDR>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>){{.+}}dma_transaction(#VPUMI40XX.NNDMATransaction<inputType = memref<1x2x3x4xf16, @DDR>, outputType = memref<1x2x3x4xf16, @DDR>>){{.+}}-> !VPURegMapped.Index<0:0:0>
...
module @OneDMAWithAttributes {
  ...
  func.func @main(%arg0: memref<1x2x3x4xf16, @DDR>, %arg1: memref<1x2x3x4xf16, @DDR>) -> memref<1x2x3x4xf16, @DDR> {
    ...
    // CHECK:       [[VAL0:%.+]] = VPUMI40XX.NNDMA <{is_critical, is_out_of_order, port = 0 : i64}> inputs([[ARG_0]] : memref<1x2x3x4xf16, @DDR>) outputs([[ARG_1]] : memref<1x2x3x4xf16, @DDR>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>){{.+}}dma_transaction(#VPUMI40XX.NNDMATransaction<inputType = memref<1x2x3x4xf16, @DDR>, outputType = memref<1x2x3x4xf16, @DDR>>){{.+}}-> !VPURegMapped.Index<0:0:0>
...
func.func @doubleDMAWithDistributedBuffer() {
    %cst_3 = const.Declare memref<1x1x1x2688xf16> = dense<0> : tensor<48x1x1x4xsi32>, [#const.FuseWeights<tensor<1x1x1x2688xf16>, weightsTable = <dense<0> : tensor<48x1x1x4xsi32>, [#const.RelocateWeightsTable<weightsPtr=[768], sparsityPtr=3840 : i64, offsets=[0], weightsTableSize=768 : i64, weightsElemBitSize=16 : i64, weightsCompression=#VPUIP.SparsityCompressionAttr<axis = 0 : i64, numElems = dense<27> : tensor<48xi64>, alignment = 16 : i64>, channelOffset=0 : i64, originalOC=0 : i64>]>, weights = <dense<0.0> : tensor<48x16x3x3xf16, {order = #NHWC}>, [#const.Sparsify<false>]>, sparsity = <dense<0.0> : tensor<48x16x3x3xf16, {order = #NHWC}>, [#const.GetSparsityMap]>>]
    // CHECK: [[INPUT:%.+]] = const.Declare memref<[[INPUT_TYPE:.+]]> =
...
module @OneConvertDMA {
  net.NetworkInfo entryPoint : @main inputsInfo : {
    DataInfo "input_0" : tensor<1x3x15x28xf32>
  } outputsInfo : {
    DataInfo "output_0" : tensor<1x3x15x28xf16>
  }
  // CHECK-LABEL: func.func @main
  // CHECK-SAME:    [[ARG_0:%[^:]+]]: memref<1x3x15x28xf32, @DDR>
  // CHECK-SAME:    [[ARG_1:%[^:]+]]: memref<1x3x15x28xf16, @DDR>
  func.func @main(%arg0: memref<1x3x15x28xf32, @DDR>, %arg1: memref<1x3x15x28xf16, @DDR>) -> memref<1x3x15x28xf16, @DDR> {
    VPURT.Task {
      %0 = VPUIP.ConvertDMA <{port = 0 : i64}> inputs(%arg0 : memref<1x3x15x28xf32, @DDR>) outputs(%arg1 : memref<1x3x15x28xf16, @DDR>) -> memref<1x3x15x28xf16, @DDR>
    }
    // CHECK:       VPUMI40XX.NNDMA <{port = 0 : i64}> inputs([[ARG_0]] : memref<1x3x15x28xf32, @DDR>) outputs([[ARG_1]] : memref<1x3x15x28xf16, @DDR>) start_after(0) clean_after(0) acceleration_mode(<DISABLE>){{.+}}dma_transaction(#VPUMI40XX.NNDMATransaction<inputType = memref<1x3x15x28xf32, @DDR>, outputType = memref<1x3x15x28xf16, @DDR>>){{.+}}-> !VPURegMapped.Index<0:0:0>

    return %arg1 : memref<1x3x15x28xf16, @DDR>
  }
}
