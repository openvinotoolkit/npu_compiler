...
module @Module0 {
  ...
  func.func @EnqueueDma(%arg0: memref<1x16x3x6xf16>, %arg1: memref<1x16x3x6xf16>) -> memref<1x16x3x6xf16> {
    ...
    %dpu_in = VPURT.DeclareBuffer <CMX_NN> [0] <9216> -> memref<1x64x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %dpu_out = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x64x9x8xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %dpu_par_in = VPURT.DeclareBuffer <CMX_NN> [0] <9216> -> memref<1x64x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %dpu_par_out = VPURT.DeclareBuffer <CMX_NN> [0] <0> -> memref<1x64x9x8xf16, {order = #NHWC}, [@CMX_NN, 0]>
    %dpu_wt = VPURT.DeclareBuffer <CMX_NN> [0] <42000> -> memref<64x1x1x4xsi32, {order = #NHWC}, [@CMX_NN, 0]>
    ...
    VPURT.Task waits(%bar0 : !VPURT.Barrier) updates(%bar1 : !VPURT.Barrier) {
      %dpu = VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{kernel_padding = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 1 : i64, bottom = 1 : i64>, kernel_size = [2, 2], kernel_strides = [2, 2], task_type = #VPUIP.nce_task_type<MAXPOOL>}> input(%dpu_in : memref<1x64x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) weight_table(%dpu_wt : memref<64x1x1x4xsi32, {order = #NHWC}, [@CMX_NN, 0]>) parent_input(%dpu_par_in : memref<1x64x16x16xf16, {order = #NHWC}, [@CMX_NN, 0]>) parent_output(%dpu_par_out : memref<1x64x9x8xf16, {order = #NHWC}, [@CMX_NN, 0]>) outputs(%dpu_out : memref<1x64x9x8xf16, {order = #NHWC}, [@CMX_NN, 0]>) -> memref<1x64x9x8xf16, {order = #NHWC}, [@CMX_NN, 0]> variants : {
