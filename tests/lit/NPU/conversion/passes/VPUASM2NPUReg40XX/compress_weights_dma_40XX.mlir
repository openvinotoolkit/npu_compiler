...
// RUN: vpux-opt --split-input-file --platform=%platform% --convert-VPUASM-to-NPUReg40XX %s | FileCheck %s
...
module @mainModule attributes {config.platform = #config.platform<NPU4000>} {
  ...
  func.func @dma_compressed_constant() {
    ELF.Main {
      ...
      ELF.CreateLogicalSection @program.DMA.cmx.0.1 aligned(64) secType(SHT_PROGBITS) secFlags("SHF_NONE") secLocation(<DDR>) {
      ...
      ELF.CreateSection @buffer.Constant.0.constant aligned(64) secType(SHT_PROGBITS) secFlags(SHF_ALLOC) secLocation(<Constant>) {
      ...
      ELF.CreateLogicalSection @buffer.CMX_NN.0 aligned(64) secType(SHT_PROGBITS) secFlags("SHF_NONE") secLocation(<DDR>) {
