...
// CHECK:       module @VPU.SW  {
// CHECK:           func.func nested @builtin_SoftMax(memref<*xf16>, memref<*xf16>, i64, i64)
...
// CHECK: func.func @NCEConv([[ARG0:%.+]]: memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>, [[ARG1:%.+]]: memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>) -> memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN> {
func.func @NCEConv(%arg0 : tensor<1x32x16x16xf16, {mem_space = @CMX_NN, order = #NHWC}>) -> tensor<1x64x14x14xf16, {mem_space = @CMX_NN, order = #NHWC}> {
    ...
    %out = VPU.NCE.Convolution(%arg0, %weights, %weights_table) rawFilterShape [64, 32, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
    ...
    // CHECK-DAG:       [[WEIGHTS:%.+]] = const.Declare memref<64x32x3x3xf16, {order = #NHWC}, @CMX_NN> = dense<1.000000e+00>
    ...
    // CHECK:       [[OUT_BUF:%.+]] = memref.alloc() : memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>
    ...
    // CHECK-SAME:      -> memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN> variants :  {
    ...
    // CHECK:       [[RESULT:%.+]] = VPUIP.Copy inputs([[OUT]] : memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>) outputs([[ARG1]] : memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>)
    // CHECK-SAME:      -> memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>
    // CHECK:       return [[RESULT]] : memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>
}
}
// -----
...
// CHECK:       func.func @SparseNCEConv([[ARG0:%.+]]: memref<1x32x16x16xf16, {order = #NHWC}>,
// CHECK-SAME:                      [[ARG1:%.+]]: memref<1x32x16x16xi1, {order = #NHWC}>,
// CHECK-SAME:                      [[ARG2:%.+]]: !VPUIP.SparseBuffer<data=memref<1x64x14x14xf16, {order = #NHWC}>,
// CHECK-SAME:                                                        sparsity_map=memref<1x64x14x14xi1, {order = #NHWC}>>)
// CHECK-SAME:       -> !VPUIP.SparseBuffer<data=memref<1x64x14x14xf16, {order = #NHWC}>, sparsity_map=memref<1x64x14x14xi1, {order = #NHWC}>> {
func.func @SparseNCEConv(%arg0 : tensor<1x32x16x16xf16, {order = #NHWC}>, %arg1 : tensor<1x32x16x16xi1, {order = #NHWC}>)
    ...
    %output_sparse_cmx = VPU.NCE.Convolution(%input_sparse_cmx, %weights_sparse_cmx, %weights_table_cmx) rawFilterShape [64, 32, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    ...
    // CHECK-DAG:       [[CST_WEIGHTS:%.+]] = const.Declare memref<64x32x3x3xf16, {order = #NHWC}> = dense<1.000000e+00>
    ...
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<1x32x16x16xf16, {order = #NHWC}>,
    // CHECK-SAME:                             sparsity_map=memref<1x32x16x16xi1, {order = #NHWC}>>
    // CHECK:       [[IN_DATA_CMX:%.+]] = memref.alloc() : memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>
    // CHECK:       [[IN_SM_CMX:%.+]] = memref.alloc() : memref<1x32x16x16xi1, {order = #NHWC}, @CMX_NN>
    // CHECK:       [[IN_SPARSE_CMX:%.+]] = VPUIP.GroupSparseBuffer([[IN_DATA_CMX]], [[IN_SM_CMX]])
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>,
    // CHECK-SAME:                             sparsity_map=memref<1x32x16x16xi1, {order = #NHWC}, @CMX_NN>>
    ...
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<64x32x3x3xf16, {order = #NHWC}>,
    // CHECK-SAME:                             sparsity_map=memref<64x1x1x384xi1>, is_weights>
    // CHECK:       [[WEIGHTS_DATA_CMX:%.+]] = memref.alloc() : memref<64x32x3x3xf16, {order = #NHWC}, @CMX_NN>
    ...
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<64x32x3x3xf16, {order = #NHWC}, @CMX_NN>,
    ...
    // CHECK:       [[OUT_BUF:%.+]] = memref.alloc() : memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>
    // CHECK:       [[OUT_SM_BUF:%.+]] = memref.alloc() : memref<1x64x14x14xi1, {order = #NHWC}, @CMX_NN>

    // CHECK:       [[INPUT:%.+]], [[INPUT_SM:%.+]] = VPUIP.UngroupSparseBuffer([[IN_SPARSE]])
    // CHECK-SAME:      -> memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>, memref<1x32x16x16xi1, {order = #NHWC}, @CMX_NN>
    // CHECK:       [[WEIGHTS:%.+]], [[WEIGHTS_SM:%.+]] = VPUIP.UngroupSparseBuffer([[WEIGHTS_SPARSE]])
    // CHECK-SAME:      -> memref<64x32x3x3xf16, {order = #NHWC}, @CMX_NN>, memref<64x1x1x384xi1, @CMX_NN>
    ...
    // CHECK-SAME:      -> memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>, memref<1x64x14x14xi1, {order = #NHWC}, @CMX_NN> variants :  {
    // CHECK:               DPUTask {mpe_mode = #VPU.mpe_mode<VECTOR_FP16>, outEnd = [15, 15, 31], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
    // CHECK:           } PPE :  {
    // CHECK:           }

    // CHECK:       [[OUT_SPARSE_CMX:%.+]] = VPUIP.GroupSparseBuffer([[OUT]]#0, [[OUT]]#1)
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>,
    // CHECK-SAME:                             sparsity_map=memref<1x64x14x14xi1, {order = #NHWC}, @CMX_NN>>

    // CHECK:       [[OUT_DATA_DDR:%.+]] = memref.alloc() : memref<1x64x14x14xf16, {order = #NHWC}>
    // CHECK:       [[OUT_SM_DDR:%.+]] = memref.alloc() : memref<1x64x14x14xi1, {order = #NHWC}>
    // CHECK:       [[OUT_SPARSE_DDR:%.+]] = VPUIP.GroupSparseBuffer([[OUT_DATA_DDR]], [[OUT_SM_DDR]])
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<1x64x14x14xf16, {order = #NHWC}>,
    // CHECK-SAME:                             sparsity_map=memref<1x64x14x14xi1, {order = #NHWC}>>
    ...
    // CHECK:       return [[RESULT]] : !VPUIP.SparseBuffer<data=memref<1x64x14x14xf16, {order = #NHWC}>, sparsity_map=memref<1x64x14x14xi1, {order = #NHWC}>>
}
}
...
// CHECK:       func.func @SparseNCEConvSOH([[ARG0:%.+]]: memref<1x32x16x16xf16, {order = #NHWC}, @DDR>,
// CHECK-SAME:                         [[ARG1:%.+]]: memref<1x32x16x16xi1, {order = #NHWC}, @DDR>,
// CHECK-SAME:                         [[ARG2:%.+]]: !VPUIP.SparseBuffer<data=memref<1x64x14x14xf16, {order = #NHWC}, @DDR>,
// CHECK-SAME:                                                           sparsity_map=memref<1x64x14x14xi1, {order = #NHWC}, @DDR>>)
// CHECK-SAME:       -> !VPUIP.SparseBuffer<data=memref<1x64x14x14xf16, {order = #NHWC}, @DDR>, sparsity_map=memref<1x64x14x14xi1, {order = #NHWC}, @DDR>> {
func.func @SparseNCEConvSOH(%arg0 : !Input_DDR, %arg1 : !InputSM_DDR) -> !VPU.SparseTensor<data=!Output_DDR, sparsity_map=!OutputSM_DDR> {
    ...
    %output_sparse_cmx = VPU.NCE.Convolution(%input_sparse_cmx, %weights_sparse_cmx, %weights_table_sparse_cmx) rawFilterShape [64, 32, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
                ppe = #VPU.PPEStub<>,
                pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    ...
    // CHECK-SAME:      : tensor<64x32x3x3xf16>, [#const.Reorder<#NHWC>, #const.GetSparsityMap]
    // CHECK-DAG:       [[CST_WEIGHTS:%.+]] = const.Declare memref<64x32x3x3xf16, {order = #NHWC}, @DDR> = dense<1.000000e+00>
    // CHECK-SAME:      : tensor<64x32x3x3xf16>, [#const.Reorder<#NHWC>, #const.Sparsify<false>]

    // CHECK:       [[INPUT_SPARSE_DDR:%.+]] = VPUIP.GroupSparseBuffer([[ARG0]], [[ARG1]])
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<1x32x16x16xf16, {order = #NHWC}, @DDR>,
    // CHECK-SAME:                             sparsity_map=memref<1x32x16x16xi1, {order = #NHWC}, @DDR>>
    // CHECK:       [[WEIGHTS_SPARSE_DDR:%.+]] = VPUIP.GroupSparseBuffer([[CST_WEIGHTS]], [[CST_WEIGHTS_SM]]) <{is_weights}>
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<64x32x3x3xf16, {order = #NHWC}, @DDR>,
    ...
    // CHECK:       [[INPUT_SPARSE_DIST_CMX:%.+]] = VPUIP.Copy
    // CHECK-SAME:          inputs([[INPUT_SPARSE_DDR]] : !VPUIP.SparseBuffer<data=memref<1x32x16x16xf16, {order = #NHWC}, @DDR>, sparsity_map=memref<1x32x16x16xi1, {order = #NHWC}, @DDR>>)
    // CHECK-SAME:          outputs([[INPUT_SPARSE_DIST]] : !VPUIP.SparseBuffer<data=!VPUIP.DistributedBuffer<1x32x16x16xf16, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>, sparsity_map=!VPUIP.DistributedBuffer<1x32x16x16xi1, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>>)
    ...
    // CHECK:       [[WEIGHTS_SPARSE_DIST_CMX:%.+]] = VPUIP.Copy
    // CHECK-SAME:          inputs([[WEIGHTS_SPARSE_DDR]]
    // CHECK-SAME:          outputs([[WEIGHTS_SPARSE_DIST]]
    ...
    // CHECK:       [[WEIGHTS_TABLE:%.+]] = VPUIP.Copy
    // CHECK-SAME:          inputs([[CST_WEIGHTS_TABLE]]
    // CHECK-SAME:          outputs([[WEIGHTS_TABLE_DIST]]
    ...
    // CHECK-SAME:              parent_input_sparsity_map([[INPUT_SM]]
    // CHECK-SAME:              parent_output([[OUTPUT_DATA]]
    // CHECK-SAME:              parent_output_sparsity_map([[OUTPUT_SM]]
    // CHECK-SAME:              outputs([[OUTPUT_DATA]]
    // CHECK-SAME:              output_sparsity_map([[OUTPUT_SM]]
    // CHECK-SAME:          -> !VPUIP.DistributedBuffer<1x64x14x14xf16, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}>,
    // CHECK-SAME:          !VPUIP.DistributedBuffer<1x64x14x14xi1, #NHWC, @CMX_NN, {mode = "SEGMENTED", num_tiles = [1, 1, 2, 1], num_clusters = 2 : i64}> variants : {
    // CHECK:                   DPUTask {mpe_mode = #VPU.mpe_mode<VECTOR_FP16>, outEnd = [15, 15, 31], outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}
    // CHECK:               } PPE :  {
    // CHECK:               }
    ...
    // CHECK:       [[OUTPUT_BUF_DDR:%.+]] = memref.alloc() : memref<1x64x14x14xf16, {order = #NHWC}, @DDR>
    // CHECK:       [[OUTPUT_SM_BUF_DDR:%.+]] = memref.alloc() : memref<1x64x14x14xi1, {order = #NHWC}, @DDR>
    // CHECK:       [[OUTPUT_BUF_SPARSE:%.+]] = VPUIP.GroupSparseBuffer([[OUTPUT_BUF_DDR]], [[OUTPUT_SM_BUF_DDR]])
    // CHECK:       [[OUTPUT_SPARSE_DDR:%.+]] = VPUIP.Copy
    // CHECK-SAME:          inputs([[CONV_OUTPUT_SPARSE]]
    // CHECK-SAME:          outputs([[OUTPUT_BUF_SPARSE]]
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<1x64x14x14xf16, {order = #NHWC}, @DDR>, sparsity_map=memref<1x64x14x14xi1, {order = #NHWC}, @DDR>>

    // CHECK:       [[RESULT:%.+]] = VPUIP.Copy inputs([[OUTPUT_SPARSE_DDR]]
    // CHECK-SAME:                              outputs([[ARG2]]
    // CHECK:       return [[RESULT]] : !VPUIP.SparseBuffer<data=memref<1x64x14x14xf16, {order = #NHWC}, @DDR>, sparsity_map=memref<1x64x14x14xi1, {order = #NHWC}, @DDR>>
}
}
// -----
...
// CHECK-SAME:    [[ARG_0:%[^:]+]]: memref<1x32x16x16xf16, {order = #NHWC}>
// CHECK-SAME:    [[ARG_1:%[^:]+]]: memref<1x32x16x16xi1, {order = #NHWC}>
// CHECK-SAME:    [[ARG_2:%[^:]+]]: memref<1x64x14x14xf16, {order = #NHWC}>
func.func @SparseNCEConvSETable(%arg0 : tensor<1x32x16x16xf16, {order = #NHWC}>, %arg1 : tensor<1x32x16x16xi1, {order = #NHWC}>)
    ...
    %output_cmx = VPU.NCE.Convolution(%input_sparse_cmx, %weights_cmx, %weights_table_cmx) rawFilterShape [64, 32, 3, 3] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
            ppe = #VPU.PPEStub<>,
            pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
    ...
    // CHECK-DAG:       [[WEIGHTS:%.+]] = const.Declare memref<64x32x3x3xf16, {order = #NHWC}> = dense<1.000000e+00> : tensor<64x32x3x3xf16>, [#const.Reorder<#NHWC>]
    // CHECK:       [[SE_TABLE:%.+]] = VPUIP.StorageElementTable
    // CHECK-SAME:      {dataElemType = f16, dataShape = [1, 32, 16, 16], seDepth = 1 : i64, seSize = [32]} -> memref<1x1x16x16xi32, {order = #NHWC}>
    // CHECK:       [[GROUP_SPARSE_BUFFER:%.+]] = VPUIP.GroupSparseBuffer([[ARG_0]], [[ARG_1]], [[SE_TABLE]])
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<1x32x16x16xf16, {order = #NHWC}>,
    // CHECK-SAME:                             sparsity_map=memref<1x32x16x16xi1, {order = #NHWC}>,
    // CHECK-SAME:                             storage_element_table=memref<1x1x16x16xi32, {order = #NHWC}>>

    // CHECK:       [[DATA_ALLOC:%.+]] = memref.alloc() : memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>
    // CHECK:       [[SPARSITY_MAP_ALLOC:%.+]] = memref.alloc() : memref<1x32x16x16xi1, {order = #NHWC}, @CMX_NN>
    // CHECK:       [[SE_TABLE_ALLOC:%.+]] = memref.alloc() : memref<1x1x16x16xi32, {order = #NHWC}, @CMX_NN>
    ...
    // CHECK-SAME:      data=memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>,
    // CHECK-SAME:      sparsity_map=memref<1x32x16x16xi1, {order = #NHWC}, @CMX_NN>,
    // CHECK-SAME:      storage_element_table=memref<1x1x16x16xi32, {order = #NHWC}, @CMX_NN>>

    // CHECK:       [[IN_SPARSE:%.+]] = VPUIP.Copy
    // CHECK-SAME:      inputs([[GROUP_SPARSE_BUFFER]] : !VPUIP.SparseBuffer<data=memref<1x32x16x16xf16, {order = #NHWC}>,
    // CHECK-SAME:                                                           sparsity_map=memref<1x32x16x16xi1, {order = #NHWC}>,
    // CHECK-SAME:                                                           storage_element_table=memref<1x1x16x16xi32, {order = #NHWC}>>)
    // CHECK-SAME:      outputs([[GROUP_SP_BUFF_ALLOC]] : !VPUIP.SparseBuffer<data=memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>,
    // CHECK-SAME:                                                            sparsity_map=memref<1x32x16x16xi1, {order = #NHWC}, @CMX_NN>,
    // CHECK-SAME:                                                            storage_element_table=memref<1x1x16x16xi32, {order = #NHWC}, @CMX_NN>>)
    // CHECK-SAME:      -> !VPUIP.SparseBuffer<data=memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>,
    // CHECK-SAME:                             sparsity_map=memref<1x32x16x16xi1, {order = #NHWC}, @CMX_NN>,
    ...
    // CHECK:       [[WEIGHTS_ALLOC:%.+]] = memref.alloc() : memref<64x32x3x3xf16, {order = #NHWC}, @CMX_NN>
    // CHECK:       [[WEIGHTS_CMX:%.+]] = VPUIP.Copy inputs([[WEIGHTS]] : memref<64x32x3x3xf16, {order = #NHWC}>)
    // CHECK-SAME:      outputs([[WEIGHTS_ALLOC]] : memref<64x32x3x3xf16, {order = #NHWC}, @CMX_NN>) -> memref<64x32x3x3xf16, {order = #NHWC}, @CMX_NN>
    ...
    // CHECK:       [[OUT_NCE_CLUSTER:%.+]] = memref.alloc() : memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>
    // CHECK:       [[INPUT:%.+]], [[INPUT_SM:%.+]],  [[INPUT_SE:%.+]] = VPUIP.UngroupSparseBuffer([[IN_SPARSE]])
    // CHECK-SAME:      -> memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>, memref<1x32x16x16xi1, {order = #NHWC}, @CMX_NN>, memref<1x1x16x16xi32, {order = #NHWC}, @CMX_NN>
    ...
    // CHECK-SAME:      task_type = #VPUIP.nce_task_type<CONV>}
    // CHECK-SAME:      input([[INPUT]] : memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>)
    // CHECK-SAME:      input_sparsity_map([[INPUT_SM]] : memref<1x32x16x16xi1, {order = #NHWC}, @CMX_NN>)
    // CHECK-SAME:      input_storage_element_table([[INPUT_SE]] : memref<1x1x16x16xi32, {order = #NHWC}, @CMX_NN>)
    // CHECK-SAME:      weights([[WEIGHTS_CMX]] : memref<64x32x3x3xf16, {order = #NHWC}, @CMX_NN>) weight_table([[WT_CMX]] : memref<64x1x1x4xsi32, @CMX_NN>)
    // CHECK-SAME:      parent_input([[INPUT]] : memref<1x32x16x16xf16, {order = #NHWC}, @CMX_NN>)
    // CHECK-SAME:      parent_input_sparsity_map([[INPUT_SM]] : memref<1x32x16x16xi1, {order = #NHWC}, @CMX_NN>)
    // CHECK-SAME:      parent_input_storage_element_table([[INPUT_SE]] : memref<1x1x16x16xi32, {order = #NHWC}, @CMX_NN>)
    // CHECK-SAME:      parent_output([[OUT_NCE_CLUSTER]] : memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>)
    // CHECK-SAME:      outputs([[OUT_NCE_CLUSTER]] : memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>) -> memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN> variants : {
    // CHECK:           DPUTask {mpe_mode = #VPU.mpe_mode<VECTOR_FP16>, outEnd = [15, 15, 31], outStart = [0, 0, 0],
    // CHECK-SAME:          pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}

    // CHECK:       [[RESULT_ALLOC:%.+]] = memref.alloc() : memref<1x64x14x14xf16, {order = #NHWC}>
    // CHECK:       [[RESULT:%.+]] = VPUIP.Copy inputs([[RESULT_CMX]] : memref<1x64x14x14xf16, {order = #NHWC}, @CMX_NN>)
    // CHECK-SAME:      outputs([[RESULT_ALLOC]] : memref<1x64x14x14xf16, {order = #NHWC}>) -> memref<1x64x14x14xf16, {order = #NHWC}>
    // CHECK:       [[RESULT_OUT:%.+]] = VPUIP.Copy inputs([[RESULT]] : memref<1x64x14x14xf16, {order = #NHWC}>)
    // CHECK-SAME:      outputs([[ARG_2]] : memref<1x64x14x14xf16, {order = #NHWC}>) -> memref<1x64x14x14xf16, {order = #NHWC}>

    // CHECK:       return [[RESULT_OUT]] : memref<1x64x14x14xf16, {order = #NHWC}>
}
}
...
module {
  net.NetworkInfo entryPoint : @NCEMatMul inputsInfo : {
    DataInfo "Parameter" : tensor<64x1x32x35x1xf16>
  } outputsInfo : {
    DataInfo "Result" : tensor<64x1x64x35x1xf16>
  }
// CHECK: func.func @NCEMatMul([[ARG0:%.+]]: memref<64x1x32x35x1xf16, {order = #GNHWC}, @CMX_NN>, [[ARG1:%.+]]: memref<64x1x64x35x1xf16, {order = #GNHWC}, @CMX_NN>) -> memref<64x1x64x35x1xf16, {order = #GNHWC}, @CMX_NN> {
func.func @NCEMatMul(%arg0 : tensor<64x1x32x35x1xf16, {mem_space = @CMX_NN, order = #GNHWC}>) -> tensor<64x1x64x35x1xf16, {mem_space = @CMX_NN, order = #GNHWC}> {
    %weights = const.Declare tensor<64x64x32x1x1xf16, {mem_space = @CMX_NN, order = #GNHWC}> = dense<1.000000e+00> : tensor<64x64x32x1x1xf16, {mem_space = @CMX_NN}>, [#const.Reorder<#GNHWC>]
    %weights_table = const.Declare tensor<64x64x1x1x4xsi32, {mem_space = @CMX_NN}> = dense<1> : tensor<64x64x1x1x4xsi32, {mem_space = @CMX_NN}>

    %1 = VPU.NCE.MatMul(%arg0, %weights, %weights_table) rawFilterShape [64, 64, 32, 1, 1] {resultSegmentSizes = array<i32: 1, 0, 0, 0>,
                ppe = #VPU.PPEStub<>,
                pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>,
                 strides = [1, 1]
    ...
    // CHECK:       [[WEIGHTS:%.+]] = const.Declare memref<64x64x32x1x1xf16, {order = #GNHWC}, @CMX_NN> = dense<1.000000e+00> : tensor<64x64x32x1x1xf16,
    ...
    // CHECK:       [[OUTPUT_BUF:%.+]] = memref.alloc() : memref<64x1x64x35x1xf16, {order = #GNHWC}, @CMX_NN>
    ...
    // CHECK-SAME:  task_type = #VPUIP.nce_task_type<CONV>}> input([[ARG0]] : memref<64x1x32x35x1xf16, {order = #GNHWC}, @CMX_NN>)
    // CHECK-SAME:  weights([[WEIGHTS]] : memref<64x64x32x1x1xf16, {order = #GNHWC}, @CMX_NN>) weight_table([[WEIGHT_TABLE]] : memref<64x64x1x1x4xsi32, @CMX_NN>)
    // CHECK-SAME:  parent_input([[ARG0]] : memref<64x1x32x35x1xf16, {order = #GNHWC}, @CMX_NN>) parent_output([[OUTPUT_BUF]] : memref<64x1x64x35x1xf16, {order = #GNHWC}, @CMX_NN>)
    // CHECK-SAME:  outputs([[OUTPUT_BUF]] : memref<64x1x64x35x1xf16, {order = #GNHWC}, @CMX_NN>) -> memref<64x1x64x35x1xf16, {order = #GNHWC}, @CMX_NN> variants : {

    // CHECK:       DPUTask {inEnd = [0, 34, 31], inStart = [0, 0, 0], mpe_mode = #VPU.mpe_mode<CUBOID_16x16>, outEnd = [0, 34, 63],
    // CHECK-SAME:  outStart = [0, 0, 0], pad = #VPU.Padding<left = 0 : i64, right = 0 : i64, top = 0 : i64, bottom = 0 : i64>}

    // CHECK:       [[RESULT:%.+]] = VPUIP.Copy inputs([[OUT]] : memref<64x1x64x35x1xf16, {order = #GNHWC}, @CMX_NN>)
    // CHECK-SAME:  outputs([[ARG1]] : memref<64x1x64x35x1xf16, {order = #GNHWC}, @CMX_NN>) -> memref<64x1x64x35x1xf16, {order = #GNHWC}, @CMX_NN>
    // CHECK:       return [[RESULT]] : memref<64x1x64x35x1xf16, {order = #GNHWC}, @CMX_NN>
