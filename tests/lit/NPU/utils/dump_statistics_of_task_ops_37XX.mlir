...
module @dual_tile attributes {config.compilationMode = #config.compilation_mode<DefaultHW>} {
  ...
  func.func @main(
    ...
    VPURT.Task updates(%inputs_ready : !VPURT.Barrier) {
      VPUIP.NNDMA <{port = 0}>
        inputs(%weights_constant : memref<16x4x8x16x!qtype, {order = #NHWC}, @DDR>)
        outputs(%weights0 : memref<16x4x8x16x!qtype, {order = #NHWC}, [@CMX_NN, 0]>)
        -> memref<16x4x8x16x!qtype, {order = #NHWC}, [@CMX_NN, 0]>
    }
    ...
    VPURT.Task updates(%inputs_ready : !VPURT.Barrier) {
      VPUIP.NNDMA <{port = 0}>
        inputs(%weights_constant : memref<16x4x8x16x!qtype, {order = #NHWC}, @DDR>)
        outputs(%weights1 : memref<16x4x8x16x!qtype, {order = #NHWC}, [@CMX_NN, 1]>)
        -> memref<16x4x8x16x!qtype, {order = #NHWC}, [@CMX_NN, 1]>
    }
    ...
    VPURT.Task waits(%inputs_ready : !VPURT.Barrier) updates(%conv_complete : !VPURT.Barrier) {
      VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{
        ...
        weights(%weights0 : memref<16x4x8x16x!qtype, {order = #NHWC}, [@CMX_NN, 0]>)
    ...
    VPURT.Task waits(%inputs_ready : !VPURT.Barrier) updates(%conv_complete : !VPURT.Barrier) {
      VPUIP.NCEClusterTask {resultSegmentSizes = array<i32: 1, 0, 0, 0, 0, 0>} <{
        ...
        weights(%weights1 : memref<16x4x8x16x!qtype, {order = #NHWC}, [@CMX_NN, 1]>)
