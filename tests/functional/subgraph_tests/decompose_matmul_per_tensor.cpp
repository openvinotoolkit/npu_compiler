...
class MatMulWithAsymZeroPointPerTensor : public VpuOv2LayerTest, public testing::WithParamInterface<MatMulParams> {
public:
    ...
    void SetUp() override {
        ...
        function = std::make_shared<ov::Model>(results, ov::ParameterVector{param}, "MatMulDecomposition");
    }
};

TEST_P(MatMulWithAsymZeroPointPerTensor, NPU3720_HW) {
    setDefaultHardwareMode();
    configuration[ov::intel_npu::compilation_mode_params.name()] =
            "enable-matmul-mixed-precision-decomposition=true matmul-mixed-precision-decomposition-ratio=0.5";
    run(Platform::NPU3720);
}

TEST_P(MatMulWithAsymZeroPointPerTensor, NPU4000_HW) {
    setDefaultHardwareMode();
    configuration[ov::intel_npu::compilation_mode_params.name()] =
            "enable-matmul-mixed-precision-decomposition=true matmul-mixed-precision-decomposition-ratio=0.5";
...
TEST_P(MatMulWithAsymZeroPointPerTensor, NPU5010_HW) {
    setDefaultHardwareMode();
    configuration[ov::intel_npu::compilation_mode_params.name()] =
            "enable-matmul-mixed-precision-decomposition=true matmul-mixed-precision-decomposition-ratio=0.5";
...
const std::vector<MatMulParams> allParams = {
        ...
        // MatMulParams{64, 49, 128, 384, 128, false, true, 200.f, 0.001f},
        // MatMulParams{1, 32, 256, 256, 64, false, false, 120.f, 1.f},
        // MatMulParams{32, 64, 256, 256, 128, false, false, 30.f, 1.f}};
        MatMulParams{1, 1, 2048, 2048, 512, false, false, 30.f, 0.01f}};

INSTANTIATE_TEST_SUITE_P(precommit_MatMulDecomposition, MatMulWithAsymZeroPointPerTensor,
                         ::testing::ValuesIn(allParams), MatMulWithAsymZeroPointPerTensor::getTestCaseName);
