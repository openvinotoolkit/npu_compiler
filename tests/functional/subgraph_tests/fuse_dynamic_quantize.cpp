...
class FuseDQSignedTestCommon : public VpuOv2LayerTest, public testing::WithParamInterface<DQDecompositionParams> {
public:
    static std::string getTestCaseName(testing::TestParamInfo<DQDecompositionParams> obj) {
        const std::string sep = "_";
        std::ostringstream result;
    ...
    std::shared_ptr<ov::Node> find_min_value(const ov::Output<ov::Node>& input) {
        ...
        const auto& reduce_axes =
                std::make_shared<ov::op::v4::Range>(zero_node, input_rank_as_scalar, one_node, ov::element::i64);
        const auto& input_min = std::make_shared<ov::op::v1::ReduceMin>(input, reduce_axes);
        const auto& zero_node_f32 = ov::op::v0::Constant::create(ov::element::f32, ov::Shape{1}, {0});
        return std::make_shared<ov::op::v1::Minimum>(zero_node_f32, input_min);
    }
    ...
    std::shared_ptr<ov::Node> find_max_value(const ov::Output<ov::Node>& input) {
        ...
        const auto& input_max = std::make_shared<ov::op::v1::ReduceMax>(input, reduce_axes);
        const auto& zero_node_f32 = ov::op::v0::Constant::create(ov::element::f32, ov::Shape{1}, {0});
        return std::make_shared<ov::op::v1::Maximum>(zero_node_f32, input_max);
    }

    std::shared_ptr<ov::Node> quantize_linear_signed(ov::Output<ov::Node> x, ov::Output<ov::Node> x_span,
                                                     ov::Output<ov::Node> quant_range_span,
                                                     ov::Output<ov::Node> y_zero_point) {
        const auto& x_scaled = std::make_shared<ov::op::v1::Divide>(
                std::make_shared<ov::op::v1::Multiply>(x, quant_range_span), x_span);
        ...
        const auto& y_zero_point_f32 = std::make_shared<ov::op::v0::Convert>(y_zero_point, ov::element::f32);
        const auto& result_shifted = std::make_shared<ov::op::v1::Add>(x_rounded, y_zero_point_f32);
        const auto& result_clamped = std::make_shared<ov::op::v0::Clamp>(result_shifted, -127, 127);

        return std::make_shared<ov::op::v0::Convert>(result_clamped, ov::element::i8);
    }

    ov::OutputVector dynamic_quantize_linear_signed(const ov::ParameterVector& inputs) {
        const auto& x = inputs[0];
        // quantization range for signed int8: [-127, 127]
        const auto& quant_range_min = ov::op::v0::Constant::create(ov::element::f32, ov::Shape{}, {-127});
        ...
        const auto& intermediate_zero_point =
                std::make_shared<ov::op::v5::Round>(std::make_shared<ov::op::v1::Divide>(x_min_shifted, y_scale),
                                                    ov::op::v5::Round::RoundMode::HALF_TO_EVEN);
        const auto& y_zero_point = std::make_shared<ov::op::v0::Convert>(
                std::make_shared<ov::op::v0::Clamp>(intermediate_zero_point, -127, 127), ov::element::i8);

        const auto& y = quantize_linear_signed(x, x_span, quant_range_span, y_zero_point);
        return {y, y_scale, y_zero_point};
    }
    ...
    std::shared_ptr<ov::Model> init_subgraph(ov::Shape& input_shape, const ov::element::Type input_precision) {
        ...
        const auto output = dynamic_quantize_linear_signed(params);
        return std::make_shared<ov::Model>(output, params, "DQSignedDecomposition");
    }
...
TEST_P(FuseDQSignedTestCommon, NPU3720_HW) {
    setDefaultHardwareMode();
    run(Platform::NPU3720);
}

TEST_P(FuseDQSignedTestCommon, NPU4000_HW) {
    setDefaultHardwareMode();
    run(Platform::NPU4000);
}

TEST_P(FuseDQSignedTestCommon, NPU5010_HW) {
    setDefaultHardwareMode();
    run(Platform::NPU5010);
}
TEST_P(FuseDQSignedTestCommon, NPU5020_HW) {
    setDefaultHardwareMode();
    run(Platform::NPU5020);
}
...
INSTANTIATE_TEST_SUITE_P(precommit_FuseDQSigned, FuseDQSignedTestCommon,
                         ::testing::Combine(::testing::ValuesIn(input_shapes_basic),
                                            ::testing::ValuesIn(input_precisions)),
                         FuseDQSignedTestCommon::getTestCaseName);

INSTANTIATE_TEST_SUITE_P(smoke_FuseDQSigned, FuseDQSignedTestCommon,
                         ::testing::Combine(::testing::ValuesIn(input_shapes), ::testing::ValuesIn(input_precisions)),
                         FuseDQSignedTestCommon::getTestCaseName);
