 ...
 * @param systemSettings Input current system setting
 ...
 */
bool categoryRuleEnabler(const std::string& category, const std::vector<std::string>& systemSettings,
    ...
    FOREACH_CHILD(enableRule, enableRules, category.c_str()) {
        ...
        for (auto& systemSetting : systemSettings) {
            // Perform logical XOR to invert condition
            if (!(categoryRuleString == systemSetting) != !invert) {
                conditionFlag = true;
            }
        }
    }
...
std::vector<std::string> disabledTestPatterns() {
    ...
    static const auto skipRegistry = []() {
        ...
        try {
            const auto& filePath = ov::test::utils::VpuTestEnvConfig::getInstance().OV_NPU_TESTS_SKIP_CONFIG_FILE;
